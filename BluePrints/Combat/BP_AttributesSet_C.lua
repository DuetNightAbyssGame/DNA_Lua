...
-- function BP_AttributesSet_C:ReceiveBeginPlay()
...
function BP_AttributesSet_C:OnRep_Exp()
    self:AddDelayFrameFunc(
        function()
            ...
            if not IsStandAlone(Owner) and IsAuthority(Owner) then return end
...
function BP_AttributesSet_C:AddSpAnim(TargetEid)
    --DebugPrint("@zyh执行了一次ClientAddSpAnim")
...
function BP_AttributesSet_C:OnAttrChanged_Lua(AttrName, OldValue, NewValue)
    -- 传过来的Fname会转为小写，因此注册函数也应该小写，举例: OnAttrChanged_Lua_hp
...
function BP_AttributesSet_C:SetAttr(AttrName, Value)
    ...
    -- 如果属性类型为数字，带Value；如果是string则只触发函数
    if type(Value) == "number" then
