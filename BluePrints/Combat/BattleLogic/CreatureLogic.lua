...
function Component:TargetFilterAndSetEndHitLocationOnFiringRay(SkillCreatureData, BornLocation, Direction)
	...
	local HitOthers = RayCreatureConfig.HitOthers or {0, 0, 0}
	...
	if RayCreatureConfig.Radius then
		...
		UE4.UKismetSystemLibrary.SphereTraceMulti(Source, BornLocation + Direction * RayCreatureConfig.Radius, RayTargetLocation, RayCreatureConfig.Radius,
				ETraceTypeQuery.TraceCreatureHit, false, nil, Const.IsShowRayCreature and 2 or 0, HitResults, true)
	else
		UE4.UKismetSystemLibrary.LineTraceMulti(Source, BornLocation, HitLocation,
				ETraceTypeQuery.TraceCreatureHit, false, nil, Const.IsShowRayCreature and 2 or 0, HitResults, true)
	end
	...
	for Index = 1, HitResults:Length() do
		...
		if HitActor.Eid and EidMap[HitActor.Eid] then
			goto continue
		end
		...
		if IsTarget then
			...
			if HitActor:IsEnemy(Source) and HitOthers[1] == 1 then
				IsStopAddHitList = true
				HitLocation = FinalLocation
			elseif HitActor:IsFriend(Source) and HitOthers[2] == 1 then
				IsStopAddHitList = true
				HitLocation = FinalLocation
			end
			RayHitLocation = FinalLocation
		elseif not IsTarget then
			if HitOthers[3] == 1 then
...
function Component:ExecuteRaySkillEffect(SkillCreatureData, OtherActor, RayHitLocation, HitComponent)
	...
	if CreatureConfig.DamageSource and CreatureConfig.DamageSource == 1 then
