...
function Component:RealExecuteOneEffect_Lua(EffectStruct, Index)
	...
	local Effect = SkillUtils.GrowProxyBySkillLevel('SkillEffects', EffectId, EffectStruct.SkillLevelInfo.SkillLevel, EffectInfo.TaskEffects[Index], {Source = EffectStruct.Source})
		...
		for _, Eid in pairs(Eids) do
			...
			if ParamentsTable.IsSceneCameraShake and Target then
				...
				if IsValid(TargetRootSource) and TargetRootSource:IsMainPlayer() then
					...
					UGameplayStatics.PlayWorldCameraShake(TargetRootSource, ShakeClass, TargetLocation,
							0, ParamentsTable.OuterRadius or 0, ParamentsTable.Falloff or 1)
				end
			else
...
function Component:Effect_GatherTargets(EffectStruct, ParamentsTable)
	...
	if EffectStruct.CreatureInfo.CreatureEid ~= 0 then
...
function Component:Effect_SetToCondemnLoc(EffectStruct, ParamentsTable)
	...
	--Source:GetMovementComponent():ForceClientUpdate()
end
...
function Component:Effect_SaveLoc(EffectStruct, ParamentsTable)
	...
	if TargetType == "Target" and FirstTargetEid then
		local Target = self:GetEntity(FirstTargetEid)
		if not Target then
			return
		end
		TargetLocation = Target:K2_GetActorLocation()
	elseif TargetType == "StaticCreator" and Source.CreatorId then
		local GameState = UE4.UGameplayStatics.GetGameState(self)
		if not GameState then return end
		local StaticCreator = GameState.StaticCreatorMap:Find(Source.CreatorId)
		if not StaticCreator then return end
...
function Component:Effect_SetSelfLoc(EffectStruct, ParamentsTable)
	...
	if TargetCenterLoc == "Target" and FirstTargetEid then
		local Target = self:GetEntity(FirstTargetEid)
		if not Target then 
			return 
		end
		TargetLocation = Target:K2_GetActorLocation()
	elseif TargetCenterLoc == "StaticCreator" and Source.CreatorId then
		local GameState = UE4.UGameplayStatics.GetGameState(self)
		...
		TargetLocation = StaticCreator:K2_GetActorLocation()
	elseif TargetCenterLoc == "SavedLoc" then
	...
	elseif TargetCenterLoc == "BattlePoint" then
...
function Component:CallBackSkillCreature(CallBackCreature, TargetEid, TargetSocket, TargetSocketLocation, MinDistance, IsFollowing, EventDistance, SetSpeed, TargetWeapon)
	...
	if SkillCreatureConfig.RefreshTargetRule and SkillCreatureConfig.RefreshTargetRule == 2 then
...
function Component:Effect_CallBackSkillCreature(EffectStruct, ParamentsTable)
	...
	local TargetSocketLocation = ParamentsTable.TargetSocketLocation or {0, 0, 0}
	...
	if BeCallBacked and EffectStruct.CreatureInfo.CreatureEid ~= 0 then
...
function Component:Effect_CameraUnlock(EffectStruct, ParamentsTable)
	local GameMode = UE4.UGameplayStatics.GetGameMode(self)
	if not ParamentsTable or not ParamentsTable.CantLockTime or not EffectStruct.Source then
		return
	end
...
function Component:Effect_SetCameraPitch(EffectStruct, ParametersTable)
	--DebugPrint("@zyh使用 Effect_SetCameraPitch")
	local GameMode = UE4.UGameplayStatics.GetGameMode(self)
	for _, Player in pairs(GameMode:GetAllPlayer()) do
...
function Component:Effect_EnablePitchInput(EffectStruct, ParametersTable)
	--DebugPrint("@zyh使用 Effect_EnablePitchInput")
	local GameMode = UE4.UGameplayStatics.GetGameMode(self)
	for _, Player in pairs(GameMode:GetAllPlayer()) do
...
function Component:Effect_CreateDanmaku(EffectStruct, ParamentsTable)
	...
	local bInTransformValid = EffectStruct.SourceComponentEid == -1
...
---@param EffectStruct SkillEffectStruct
function Component:Effect_StartRecovery(EffectStruct, ParamentsTable)
	...
	local Params = {
		Speed = ParamentsTable.Speed, 	-- 复活速度每秒，负数立即复活
		RecoveryEffectCreature = ParamentsTable.RecoveryEffectCreature	-- 复活时创建特效创生物，挂在被复活对象的root上
	}
...
function Component:Effect_EndRecovery(EffectStruct, ParamentsTable)
	...
	if not Source or not Source.RecoverTargets then return end
...
function Component:Effect_Disarm(EffectStruct, ParamentsTable)
	...
	for _, Eid in ipairs(HitTargets) do
		...
		if not ChangeWeaponParams or not ChangeWeaponParams.Disarm or (ChangeWeaponParams.Disarm ~= 0 and not WeaponList[ChangeWeaponParams.Disarm]) then
			goto continue
		end
		if Target:GetCannotBeDisarmed() then
			goto continue
		end
		...
		if (not WeaponObject or (WeaponObject and not (WeaponObject.WeaponId == WeaponList[ChangeWeaponParams.Disarm]))) then
...
function Component:Effect_ClearHitTargets(EffectStruct, ParamentsTablex)
	...
	if not CreatureInfo.CreatureEid ~= 0 then
		return
	end
...
function Component:Effect_ReplaceBulletFXID(EffectStruct, ParamentsTablex)
	...
	if CreatureInfo.CreatureEid == 0 then
		return
	end
	local Creature = self:GetEntity(CreatureInfo.CreatureEid)
	if not Creature then
