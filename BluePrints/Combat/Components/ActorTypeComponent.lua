...
function Component:IsMonster()
    -- 所有继承MonsterCharacter类的：怪物、魅影、被召唤的怪物
...
function Component:IsRealMonster()
    -- 真正的怪物，不包含被召唤的怪物（无论是玩家召唤的还是怪物召唤的）
    if not self.GameplayTagsInitSuccess then
        return nil
    end
    if self._IsRealMonster_Type == nil then
        self._IsRealMonster_Type = (self:GetObjType() == EObjType.MonsterCharacter and not self:HasAnyTags_Table(self, {"Player.Summon", "Mon.Summon"}, false))
    end
...
function Component:IsAIControlled()
    if self._IsAIControlled_Type == nil then
        self._IsAIControlled_Type = (self:GetObjType() == EObjType.MonsterCharacter or 
            self:GetObjType() == EObjType.NpcCharacter or
                self:GetObjType() == EObjType.PhantomCharacter or self:GetObjType() == EObjType.PetNpcCharacter)
    end
...
function Component:IsSummonMonster()
    ...
    if self.IsSummon == nil then
        self.IsSummon = self:IsMonster() and self:HasAnyTags_Table(self, {"Player.Summon", "Mon.Summon"}, false)
...
function Component:IsCaptureMonster()
    ...
    if self.IsCapture == nil then 
        self.IsCapture = self:HasAnyTags_Table(self, {"Mon.Capture",}, false)
...
function Component:IsTreasureMonster()
    ...
    if self.IsTreasure == nil then 
        self.IsTreasure = self:HasAnyTags_Table(self, {"Mon.Treasure",}, false)
...
function Component:IsJailerMonster()
    ...
    if self.IsJailer == nil then 
        self.IsJailer = self:HasAnyTags_Table(self, {"Mon.Elite.Jailer",}, false)
