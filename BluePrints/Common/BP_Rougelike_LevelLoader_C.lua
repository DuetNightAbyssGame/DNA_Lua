...
---@type BP_Rougelike_LevelLoader_C
...
function M:ReceiveBeginPlay()
    ...
    self.StepId=0--已加载房间的轮数
    self.RoomId2StreamLevel={}
    self.StreamLevelNavMeshCheck={}
    self.ID2DesignStreamingLevel={}
    self.LoadingFlag=false--单个关卡加载标志
    self.NextRoomReady=false--下一轮所有房间加载完毕
    -- self.StartPointClass=LoadClass('/Game/BluePrints/Common/Level/BP_StartPoint.BP_StartPoint_C')
    ...
    if not IsDedicatedServer(self) then
        ...
        -- self.LoadingUI:AddQuene(self.loadProgress * 0.9 * 100)
    end

    -- self:EnterRoom(202,temp)
    -- self:AddTimer(20,self.TestTimer,true)
    -- local avatar=GWorld:GetAvatar()
    -- if avatar then
    --     self:EnterRoom(avatar:GetCurrentRoomId(),avatar:GetCurrentRandomRooms())
    -- end
...
function M:EnterRoom(RoomId,NextRoomId)
    ...
    if not self.LoadingComplete then--加载初始关卡
        self:LoadStreamLevelByRoomId(self.CurrentRoomId,0)
    else
        local IsBossRoom=DataMgr.RougeLikeRoom[self.CurrentRoomId].RoomType==5
        if IsBossRoom then
...
function M:LoadNextLevel(NextRoomId)
    ...
    if self.LastRoomId then--先卸载上个关卡
    ...
    if self.NextRoomId:Length() >0 then--卸载这一轮未进入的其他关卡
        for i=1, self.NextRoomId:Length() do
            ...
            if roomId~=self.CurrentRoomId then
                ...
                UKismetSystemLibrary.Delay(self, 0.5)--暂定做个延迟，可能后面需要检测卸载的关卡状态
            end
        end
    end

    if NextRoomId then--加载下一轮关卡
...
function M:CheckLevelLoaded(RoomId)--协程访问streamleve的对象好像有些情况不会报trace，要多小心
    self:PrintRougeLikeLevelLoader('CheckLevelLoaded:'..RoomId)
    if not self.LoadingComplete then
        self.LoadingUI:AddQuene(0.5 * 0.9 * 100)
    end
    while true do 
        local levelCheck=true
        for i=1,#self.RoomId2StreamLevel[RoomId] do--这里用pairs会导致直接中断？？？
            local streamLevel=self.RoomId2StreamLevel[RoomId][i]
            if streamLevel and i ~= 2 and i<=3 then
            ...
            elseif i>3 then
                if #streamLevel>0 then
                    if not streamLevel[1]:GetLoadedLevel() or not streamLevel[1]:GetLoadedLevel().bIsVisible then
                        levelCheck=false
                        break
                    else
                    end
    ...
    if not self.LoadingComplete then--初始关卡
        self.LoadingUI:AddQuene(1 * 0.9 * 100)
...
function M:SetPlayerTrans()
    ...
    for _,startPoint in pairs(startPoints:ToTable()) do
        if currentId== self:GetDesignActorLevelName(startPoint) then
            startPoint:SetPlayerTrans()--Todo:后面正式接进后有avatar应该用InitSetPlayerTrans
...
function M:CreateBattleMap()
    ...
    if MiniMap then
        ...
        local levelTransform=self.LevelID2GridFrame:FindRef(self.CurrentRoomId):GetTransform()--self.RoomId2StreamLevel[self.CurrentRoomId][3].LevelTransform
        data[2]={X=levelTransform.Translation.X,Y=levelTransform.Translation.Y}
