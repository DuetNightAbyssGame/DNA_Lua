...
function M:Initialize(RegionDataMgr)
	...
	-- {
	-- 		RandomCreatorId -> int
	-- 	    RegionDataType ->ERegionDataType
	-- 	    {
	-- 			WorldRegionEid -> String
	-- 			LevelName ->String
	-- 			SubRegionId -> int
	-- 			RandomTableId -> int
	-- 			RandomIdxInRule-> int
...
-- function M:GetRegionEntityDataUnitId(Index)
...
-- 		return DataMgr.RandomCreator[Info.RandomRuleId].RandomInfos[Info.RandomTableId].UnitId
-- 	elseif Info.CreatorId and Info.CreatorId > 0 then
-- 		local Creator = GameState.StaticCreatorMap:FindRef(Info.CreatorId)
-- 		if Creator then
-- 			return Creator.UnitId
-- 		end
-- 	end
-- 	return Info.UnitId
-- end
...
-- function M:CompleteRegionData(Data)
-- 	local DeepCopiedInfo = CommonUtils.DeepCopy(Data)
-- 	if DeepCopiedInfo.Completed then
-- 		return DeepCopiedInfo
-- 	end
-- 	if DeepCopiedInfo.RandomRuleId and DeepCopiedInfo.RandomRuleId > 0 then
-- 		self:CompleteRandomCreatorData(DeepCopiedInfo)
-- 	elseif DeepCopiedInfo.CreatorId and DeepCopiedInfo.CreatorId > 0 then
-- 		self:CompleteStaticCreatorData(DeepCopiedInfo)
-- 	end
-- 	if not DeepCopiedInfo.UnitType or not DeepCopiedInfo.UnitId then
-- 		DebugPrint(1)
-- 	end
-- 	if DeepCopiedInfo.Type == 3 then
-- 		if Data.BornLocation then
-- 			DeepCopiedInfo.Loc = FVector(Data.BornLocation.X, Data.BornLocation.Y, Data.BornLocation.Z)
-- 		else
-- 			DeepCopiedInfo.Loc = FVector(0, 0, 0)
-- 		end
-- 		DeepCopiedInfo.BornPos = DeepCopiedInfo.Loc
-- 	end
-- 	return DeepCopiedInfo
-- end
--标记：已有转C++版本
	...
	if self:CheckUnitDataNeedStorage(DataTable.RegionDataType) then
		...
		if DataTable.Type ~= 2 then
			if not bIsFromServer then
				DataTable.UnitType = DataTable.UnitType or (Info.Creator and Info.Creator.UnitType)
				DataTable.UnitId = DataTable.UnitId or (Info.Creator and Info.Creator.UnitId)
...
-- 	self.RegionDataMgr:OnRegionDataFill_Log(WorldRegionEid, Eid, LevelName)
-- end
...
function M:UploadRandomCreatorData(RandomRuleId)
	...
	local CopyData = CommonUtils.DeepCopy(self.RandomCreatorDatas[RandomRuleId])--1.0临时处理深拷贝+手动剔除数据，1.1应迭代掉随机点的异步情况
...
function M:RemoveQuestChainData(QuestChainId, DestroyReason)
	if not QuestChainId or not self.QuestChainId2Data[QuestChainId] or not self.RegionDataMgr then
		return
	end
	local DataToRemove = {}
	...
	for _, WorldRegionEid in pairs(DataToRemove) do
		...
		-- QuestChainData = {} -- 顺便清掉self.RegionData里的那一份
	end
...
function M:RemoveDynamicQuestData(DynamicQuestId, DestroyReason)
	if not DynamicQuestId or not self.DynamicQuestId2Data[DynamicQuestId] or not self.RegionDataMgr then
		return
	end
	local GameState = UE4.UGameplayStatics.GetGameState(GWorld.GameInstance)
...
function M:RemoveSpecialQuestData(SpecialQuestId, DestroyReason)
	if not SpecialQuestId or not self.SpecialQuestId2Data[SpecialQuestId] or not self.RegionDataMgr then
		return
	end
	local GameState = UE4.UGameplayStatics.GetGameState(GWorld.GameInstance)
...
function M:CheckQuestDataExist(QuestChainId, NewData)--主要是检查NPC重复创建，所以只检查静态数据
	if self.QuestChainId2Data[QuestChainId] then
		for _, Data in pairs(self.QuestChainId2Data[QuestChainId]) do
			if NewData.CreatorId ~= nil and NewData.CreatorId == Data.CreatorId and NewData.UnitId == Data.UnitId
			 and NewData.UnitType == Data.UnitType and NewData.WorldRegionEid ~= Data.WorldRegionEid then
				return true
			end
		end
	end
...
function M:UpdateStateByTable(DataTableIndex,NewState)
	...
	if Info then
		...
		-- DebugPrint("M:UpdateStateByTable SomethingDiff = ",SomethingDiff,Info.WorldRegionEid)
	else
...
function M:MarkRegionDataDead(LuaTableIndex)
	...
	local Avatar = GWorld:GetAvatar()
	if not self:CheckUnitDataNeedStorage(Info.RegionDataType) or not Avatar or not Avatar:IsInBigWorld() then
		return
	end
	-- if Avatar then
...
function M:FillStaticCreatorDataNew(Index, Info)
	...
	self.RegionDataMgr:OnRegionDataFill_Log(DataTable.WorldRegionEid, DataTable.Eid, DataTable.LevelName)
	-- Info.IntParams:Add("SubRegionId", DataTable.SubRegionId)--填充下FillRegionDataNew需要的参数，减少调lua次数
	-- Info.NameParams:Add("WorldRegionEid", DataTable.WorldRegionEid)
...
function M:FillRandomCreatorData(DataTable, Info)
	...
	-- DataTable.RarelyId = GameMode.RandomActorManager:GetCreatorRegionDataRarelyId(DataTable.RandomRuleId, DataTable.RandomCreatorId)
...
function M:FillRandomCreatorDataNew(Index, Info)
	...
	local GameMode = UE4.UGameplayStatics.GetGameMode(GWorld.GameInstance)
	local WorldLoader = GameMode:GetLevelLoader()
	-- self:FillRandomCreatorData(self.RegionData[Index], Info)
	...
	self.RegionDataMgr:OnRegionDataFill_Log(DataTable.WorldRegionEid, DataTable.Eid, DataTable.LevelName)
	-- Info.IntParams:Add("SubRegionId", DataTable.SubRegionId)
	-- Info.NameParams:Add("WorldRegionEid", DataTable.WorldRegionEid)
	return DataTable.SubRegionId, DataTable.WorldRegionEid
end
...
function M:FillCommonDataNew(Index, Info)
	...
	local GameMode = UE4.UGameplayStatics.GetGameMode(GWorld.GameInstance)
	local WorldLoader = GameMode:GetLevelLoader()
	-- self:FillCommonData(self.RegionData[Index], Info)
...
function M:GetAllRegionDataByUnitType(UnitType)
	...
	for _, Data in pairs(self.RegionData) do
		-- PrintCrack('Data.UnitType',Data.UnitType,UnitType)
		-- PrintTable(Data)
