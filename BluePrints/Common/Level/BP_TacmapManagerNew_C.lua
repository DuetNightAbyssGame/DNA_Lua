...
--- @param mode string "Level","Player","Defence"
function M:GetSpawnPoints(info)
    local preset_targets,mode,unit_spawn_radius_min,unit_spawn_radius_max,b_enable_random, cull_unreachable = info.PresetTargets,
        info.Mode,info.UnitSpawnRadiusMin,info.UnitSpawnRadiusMax,info.RandomSpawn,info.FilterReachable
    
    -- DebugPrint("GetSpawnPoints Request:",preset_targets,mode,unit_spawn_radius_min or -1,unit_spawn_radius_max or -1,b_enable_random)
    -- DebugPrintTable(preset_targets,3)
    ...
    -- DebugPrint("GetSpawnPoints Result:",spawn_points)
    -- DebugPrintTable(spawn_points,3)
    return spawn_points
end
...
function M:GenerateSabotagePoints()
    ...
    for i=1,sabotage_datas:Length() do
        ...
        table.insert(sabotage_points,{Loc=UE4.FVector(loc.X,loc.Y,loc.Z),Id=sabotage_data.Id})
    end
...
-- function M:IsLevelLoadedByID(level_id)
--     -- DebugPrint("IsLevelLoadedByID",level_id,self.CachedLevelLoader,self.CachedLevelLoader.ID2ArtStreamingLevel,
--     -- self.CachedLevelLoader.ID2ArtStreamingLevel[level_id])
--     return self.CachedLevelLoader.ID2ArtStreamingLevel[level_id]:IsLevelLoaded()
-- end
