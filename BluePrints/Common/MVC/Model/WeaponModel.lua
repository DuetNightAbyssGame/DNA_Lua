...
local _ConsumeWeaponRedCount= {}  --解锁和未满级/未满熔炼的武器
-- 更新指定WeaponId的所有武器的红点计数
    ...
    if weaponUuids then
        for Uuid, weapon in pairs(weaponUuids) do
            ...
            if weapon then
                ...
                if weapon.Status == 0 and weapon.Level < weapon.MaxLevel and weapon.GradeLevel < MaxGradeLevel then
...
local AddWeaponToMap = function(Weapon)
    ...
    if(Weapon:HasTag(CommonConst.WeaponType.MeleeWeapon))then
        _WeaponMap[Weapon.Uuid] = {WeaponTag = CommonConst.WeaponType.MeleeWeapon,WeaponId = Weapon.WeaponId}
        ...
        _WeaponMap[Weapon.Uuid] = {WeaponTag = CommonConst.WeaponType.RangedWeapon,WeaponId = Weapon.WeaponId}
...
local RemoveWeaponFromMap = function(Uuid)
    ...
    if WeaponId and _WeaponId2Uuid[WeaponId] then
        ...
        if next(_WeaponId2Uuid[WeaponId]) == nil then
...
local MappingReward = function(self,Avatar)
    local Strings
    local WeaponId
...
function M:GetWeaponCount(WeaponId)
    return _WeaponCount[CommonConst.WeaponType.MeleeWeapon][WeaponId]
         or _WeaponCount[CommonConst.WeaponType.RangedWeapon][WeaponId]
         or 0
end

function M:GetConsumeWeaponCount(Uuid)
    local count = _ConsumeWeaponRedCount[CommonConst.WeaponType.MeleeWeapon][Uuid] 
    or _ConsumeWeaponRedCount[CommonConst.WeaponType.RangedWeapon][Uuid]
...
function M:OnWeaponDeleted(Uuid)
    ...
    if(WeaponTag)then
        ArmoryUtils:SetItemReddotRead({ItemType = CommonConst.DataType.Weapon,Tag = WeaponTag, Uuid = Uuid},
                                            true,true,true)
