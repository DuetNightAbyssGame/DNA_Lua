...
function BP_StartAndEndPoint_C:DebugPrint(...)
	...
	else DebugPrint("StartAndEndPoint EndPoint", ...) end
end

function BP_StartAndEndPoint_C:ReceiveBeginPlay()
	self:DebugPrint("ReceiveBeginPlay")

	self.PointArray = self:K2_GetComponentsByClass(LoadClass(
		'/Game/BluePrints/Item/Mechanism/Seat/BP_SeatEnterComponent.BP_SeatEnterComponent'
	):StaticClass())
	self.PointList = {}
...
-- 策划希望玩家随机取初始点，且不能重复选取
function BP_StartAndEndPoint_C:ShufflePointList()
	-- 通常来讲self.PointList会是个array而非map，先这样吧
...
function BP_StartAndEndPoint_C:SetPlayerTrans()
	...
	if URuntimeCommonFunctionLibrary.IsWorldCompositionEnabled(self) then
		...
		if not WorldCompositionSubSystem then return end
		local Transform = self:GetTransform(0)
		WorldCompositionSubSystem:RequestAsyncTravel(Character, Transform, {self, self.OpenPlayerPositionSync}, true, true)
		return
	end
...
function BP_StartAndEndPoint_C:InitSetPlayerTrans()
	...
	for i = 0, GameMode:GetTargetPlayerNum() -1 do
		...
		-- Controller:SetControlRotation(Controller.TargetBornRotator)
...
function BP_StartAndEndPoint_C:OpenPlayerPositionSync()
	...
	local GameMode = UE4.UGameplayStatics.GetGameMode(self)
	if not Avatar or not IsValid(GameMode) then return end
	local WorldLoader = GameMode:GetLevelLoader()
	if not IsValid(WorldLoader) or not WorldLoader.IsWorldLoader then return end
...
function BP_StartAndEndPoint_C:SetEnteredPlayerTrans(PlayerController)
	...
	--local PlayerArray = GWorld.GameInstance:GetCurrentGameMode().EMGameState.PlayerArray
	--if PlayerArray:Length() <= 1 then
	--	self:InitSetPlayerTrans()
	--	return
	--end
	--
	--local FirstPlayerState = GWorld.GameInstance:GetCurrentGameMode().EMGameState.PlayerArray[1]
	--local FirstPlayerController = FirstPlayerState:GetOwner():Cast(UE4.ASinglePlayerController)
	--local FirstPlayerCharacter = FirstPlayerController:GetMyPawn()
	--if not FirstPlayerCharacter then
	--	pcall(function ()
	--		local Character = UE4.UGameplayStatics.GetPlayerCharacter(self, 0)
	--		print(_G.LogTag, "Debug Info First PlayerCharacter", Character)
	--		print(_G.LogTag, "Debug Info First PlayerCharacter PlayerIndex", Character:GetOwner().PlayerState.PlayerId)
	--		print(_G.LogTag, "Debug Info First FirstPlayerState", FirstPlayerState.PlayerId)
	--	end)
	--	self:InitSetPlayerTrans()
	--	return
	--end
	--
	--PlayerController.TargetBornLocation = FirstPlayerCharacter.LastSafeLocation
	--PlayerController.TargetBornRotator = FirstPlayerCharacter:K2_GetActorRotation()
...
function BP_StartAndEndPoint_C:RealSetNewEnteredPlayerTrans(AvatarEidStr)
	...
	-- PlayerController:SetControlRotation(PlayerController.TargetBornRotator)
...
-- function BP_StartAndEndPoint_C:GetTransformWithBattleCharTag(i, BattleCharTag)
-- 	local Transform = self:GetTransform(i)
-- 	if i == 0 then
-- 		local Offset = FVector(0, 0, 0)
-- 		if BattleCharTag == "Loli" then
-- 			Offset = FVector(-10, 60, 55)
-- 		elseif BattleCharTag == "Girl" then
-- 			Offset = FVector(-10, 60, 30)
-- 		elseif BattleCharTag == "Man" then
-- 			Offset = FVector(-10, 60, 0)
-- 		elseif BattleCharTag == "Woman" then
-- 			Offset = FVector(-10, 60, 15)
-- 		elseif BattleCharTag == "Boy" then
-- 			Offset = FVector(-10, 60, 25)
-- 		end
-- 		Transform = FTransform(Transform.Rotation, Transform.Translation + Offset, Transform.Scale3D)
-- 	end
-- 	return Transform
-- end
