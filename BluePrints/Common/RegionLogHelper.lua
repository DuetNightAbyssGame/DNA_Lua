...
function M:Init()
	-- {worldRegionEid \ Eid, {list of : {OperateType, WorldRegionEid, Eid, Time, LevelName, Message} }
	self.RegionLog = {}
end
...
function M:_Log(WorldRegionEid, Eid, OperateType, LevelName, Message)
	...
	if WorldRegionEid then
		...
		table.insert(self.RegionLog[WorldRegionEid], {OperateType, WorldRegionEid, TmpEid, Time, LevelName, Message})
	end
	...
	if Eid then
		...
		table.insert(self.RegionLog[Eid], {OperateType, WorldRegionEid, Eid, Time, LevelName, Message})
	end
end
