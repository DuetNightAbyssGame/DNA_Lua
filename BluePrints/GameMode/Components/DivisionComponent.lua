...
function DivisionComponent:InitMonsterSpawnDivisions(ContentInfos)
    ...
    DebugPrint("InitMonsterSpawnDivisions, DungoenId = "..tostring(self.DungeonId)..", DivSize = "
        ..tostring(self.MonsterSpawnDivSize)..", InfoLength = "..tostring(ContentInfos:Num()))
end
...
-- function DivisionComponent:GetAroundKeys(DivSize, Location)
-- 	local Off = {-1, 0, 1}
-- 	local i, j, Cnt = 1, 0, #Off
...
-- 		local ArdX = IndX + Off[i]
--         local ArdY = IndY + Off[j]
--         return ArdX .. ", " .. ArdY
-- 	end
-- end
...
-- function DivisionComponent:GetAroundKeys(Location)
--     local Off = {-1, 0, 1}
--     local i, j, Cnt = 1, 0, #Off
...
-- function DivisionComponent:GetCurrentDivision(Divisions, DivSize, Location)
--     local Val = nil
--     local Iter, Tab, Key = pairs(
--         Divisions[self:GetCurrentKey(DivSize, Location)]
--     )
--     if Tab == nil then
--         return function()
--             return
...
-- function DivisionComponent:GetAroundDivisions(Divisions, DivSize, Location)
--     local NextKey = self:GetAroundKeys(DivSize, Location)
--     local K, Iter, Tab, Key, Val = nil, nil, nil, nil, nil
...
--             end
--         end
--         Iter, Tab, Key = pairs(Divisions[K])
--     end
