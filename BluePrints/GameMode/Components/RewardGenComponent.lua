...
function Component:AddCacheBattleReward(RewardIds, Reason, Transform, ExtraInfo)
    ...
    self.CacheBattleRewards[self.CacheRearIndex] = {RewardIds, Reason, Transform, ExtraInfo}
...
function Component:AddCacheGenUnitReward(UnitId, Reason, Transform, ExtraInfo, Callback)
    ...
    if not ExtraInfo.Avatar then
        -- @SnowMoon 如果没有击杀者信息，那么这个奖励不知道来源，按照普通奖励发放即可，如果后续有需求有自己的来源标识再做相应的处理即可
...
function Component:AddCacheLogicUnitReward(UnitId, Reason, Transform, ExtraInfo, Callback)
    ...
    ExtraInfo.Avatar = nil -- 不需要传递给逻辑服
    ...
    AvatarCache.UpValues[#AvatarCache.UpValues + 1] = {Reason, Transform, ExtraInfo, Callback}
    AvatarCache.Rewards[#AvatarCache.Rewards + 1] = {UnitId, Reason, ExtraInfo, Location}
...
function Component:TriggerGenerateRewardForMonsterDeath(UnitId,Transform,UniqueSign,KillerEid,bKilledByPlayer,WeaponType,
    Level,IsSummonMonster,MonEid,DamageCauserLocation, UniqueId, CreatorType, CreatorId, RelationSpawn)
    ...
    if Const.bServerMonsterDead and self:CheckServerDungeonEnable() then
        ...
        -- DebugPrint("Mon Die ExtraInfo.Avatar", ExtraInfo.Avatar, "hhh", AvatarEid, "hhh", not AvatarEid)
