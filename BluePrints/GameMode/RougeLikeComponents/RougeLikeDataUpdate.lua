...
function Component:RefreshRougeInfo(Info, ExtraInfo)
    -- PrintTable({GetRougeInfoFromAvatar=1})
    ...
    if not Info then
        Info = self.Avatar.RougeLikeManagerInfo
    end
    PrintTable(Info, 5)
    self.AppendInfo = {}
    ...
    -- todo @zyh 迭代优化，放到具体的奖励相关字段中去处理，把self.AppendInfo迭代掉
    --if not NoNeedTriggerGetAward then
    --    self:OnUpdateAward(self.AppendInfo)
    --end
end
...
function Component:SetRandomBlessings(_RandomBlessings)
    ...
    for i=1, #_RandomBlessings do
        ...
        table.insert(UpdateInfo.AwardsId, {ItemId = _RandomBlessings[i]})
    end
...
function Component:SetRandomTreasures(_RandomTreasures)
    ...
    for i=1, #_RandomTreasures do
        ...
        table.insert(UpdateInfo.AwardsId, {ItemId =  _RandomTreasures[i]})
    end
    if #UpdateInfo.AwardsId > 0 and not self.NoNeedTriggerGetAward then
        self:OnUpdateAward(UpdateInfo)
    end
...
local function UpdateCharInfo()
    ...
    EventManager:FireEvent(EventID.OnSwitchRole, Avatar.CurrentChar) -- 增补祝福/宝物/天赋之后刷新角色面板
end
...
function Component:SetBlessing_Remove(_Blessings)
    ...
    for i=1, #_Blessings do
        ...
        table.insert(UpdateInfo.AwardsId, {ItemId = _Blessings[i]})
    end
    EventManager:FireEvent(EventID.OnRougeDealEventReward)
    if #UpdateInfo.AwardsId > 0 then
        self:OnUpdateAward(UpdateInfo)
...
function Component:SetBlessing_Update(_Blessings)
    ...
    for k, v in pairs(_Blessings) do
        ...
        if not ExistAward then
            self:SetBlessing_Add({[k] = v})
        else
            local OldLevel = ExistAward.Level
            local NewLevel = v.Level or FRougeAwardInfo.Level
            if NewLevel ~= OldLevel then
                -- 等级更新
                self:UpgradeModById("Blessing", k, OldLevel - 1, NewLevel - 1) -- Mod的等级从0开始 但祝福宝物等级从1开始 需要-1
                table.insert(UpdateInfo.AwardsId, {ItemId = k, OldLevel = OldLevel, NewLevel = NewLevel})
            end

            self:ResetRougeLikeAwardInfo(ExistAward) -- 取默认值
...
function Component:SetTreasure_Remove(_Treasures)
    ...
    for i=1, #_Treasures do
        ...
        table.insert(UpdateInfo.AwardsId, {ItemId = _Treasures[i]})
    end
    ...
    for k,v in pairs(_Talent) do
        if self.Talents:Find(k) then
            if self.Talents:Find(k) ~= v then
                self:UpgradeModById("Talent", k, self.Talents:Find(k), v - 1) -- Mod的等级从0开始 但天赋等级从1开始 需要-1
            end
        else
            -- Mod的等级从0开始 但天赋等级从1开始 需要-1
...
function Component:ResortRougeInfo(Info)
    ...
    for k, v in pairs(Info) do
        if string.find(k, "Blessing") or string.find(k, "Treasure") or string.find(k, "Talent") then
            if string.find(k, "Remove") then -- 先表现失去Award，后表现其他类型
                SortInfo[2][k] = v
            else
                SortInfo[3][k] = v
            end
