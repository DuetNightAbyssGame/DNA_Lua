...
function WBP_Battle_Char_C:Initialize(Initializer)
	...
	self.CharHpPercent = 1.0         -- 实际血条百分比
	self.CharShieldPercent = 1.0     -- 实际护盾百分比
	self.HelpCharHpPercent = 1.0     -- 辅助血条百分比
	self.HelpCharShieldPercent = 1.0 -- 辅助护盾百分比
    self.TickTime = 0.1              -- 扣血的Tick时间
    self.AnimTime = 0.5              -- 扣血实际时间
    self.FixTime = 1.0               -- 单独扣血的修正时间
    self.NowTime = nil
    self.Hp = 0.0                    -- 角色血量
    self.Shield = 0.0                -- 角色护盾
    self.MaxHp = 0.0                 -- 角色最大血量
    self.MaxShield=0.0               -- 角色最大护盾值
    self.OverShield = 0.0            -- 角色超载护盾值
    --self.MaxOverShield = 0.0         -- 角色最大超载护盾值
    self.IsLowHealth = false         -- 是否进入过低血量
    self.IsOverShield = false        -- 是否拥有过超载护盾
    self.LastHp= 0.0
    self.LastShield=0.0
    ...
    self.RoleId=nil
    self.NowEid = nil
    self.IsShowDeductBlood = false   -- 血条是否有延时扣血效果
    self.IsShowDeductShield = false  -- 护盾条是否有延时效果
end
...
function WBP_Battle_Char_C:InitChar()
    ...
    --self.MaxOverShield = self.Owner:GetAttr("MaxOverShield")
    if self.MaxShield == 0 and self.MaxHp== 0 then
        return
    end
...
-- function WBP_Battle_Char_C:InitHUDScale()
--     local HUDScale = EMCache:Get("HUDScale")
--     if HUDScale == nil then
--         HUDScale = Const.DefaultHUDScale
--         EMCache:Set("HUDScale", HUDScale)
--     end
--     UIManager(self):AdjustBattleHUDScale(HUDScale/Const.DefaultHUDScale)
-- end
...
function WBP_Battle_Char_C:ResetShieldBarSize()
    ...
    self.ShieldBarLength = BarSize1.X   --护盾的原长度
    self.ShieldBarHeight = BarSize2.Y   --护盾的原高度
...
function WBP_Battle_Char_C:OnUpdateCharLevelAndExp()
    ...
    --local NowExp = self.Owner:GetAttr("Exp")
    --local MaxExp = DataMgr.LevelUp[NowLevel].CharLevelMaxExp
    --local ExpPercent = math.min(math.max(NowExp/MaxExp,0), 1)
    --self.Bar_Exp:SetPercent(ExpPercent)
end
...
function WBP_Battle_Char_C:UpdateState()
    ...
    self:SetBloodNum()
    --self:UpdateBloodEffect()
    --local IsRealReduceBlood = self.HpReduce > 0
    ...
    if (self.MaxShield <= 0) then
        ...
        if self.ShieldReduce<0 then
            ...
            self.FixTime = self.CharShieldPercent < ShieldPercent and 1.0 or 0.0
            self.Char_Shield_Bar:SetSize(FVector2D(self.ShieldBarLength*self.CharShieldPercent,self.ShieldBarHeight))
            if self.Shield > 0.00001 then
                self:UpdateShieldEffect()
...
function WBP_Battle_Char_C:SetShieldNum(DelayShield)
    ...
    if( self.MaxHp>0 and self.MaxShield<=0.0001 and self.OverShield<=0.0001) then
    ...
    if self.OverShield > 0 then
        ...
        --self.Num_Shield:SetColorAndOpacity(UE4.UUIFunctionLibrary.GetSlateColorByRGBA(0.337164, 0.002428, 0.814847, 1.0))
        if DelayShield then
        ...
        --self.Num_Shield:SetColorAndOpacity(UE4.UUIFunctionLibrary.GetSlateColorByRGBA(0.879623, 0.423268, 0.059511, 1.0))
        if DelayShield then
...
function WBP_Battle_Char_C:RecoveryShield()
    local TickTime = 0.033            -- 护盾恢复的Tick时间
    local AnimTime = 1.0              -- 护盾恢复实际时间
    local ShieldRecoveryFunc = function()
...
function WBP_Battle_Char_C:DecreaseShield()
    ...
    if not self.IsShowDeductShield then return end

    local WrapShieldFunc = function()
        local NowTime = UE4.UGameplayStatics.GetTimeSeconds(self)
        local PassedTime = NowTime - self.NowTime
        if (PassedTime >= self.AnimTime+1.0 ) then --or PassedTime <= 1.000
            self:RemoveTimer("RealReduceShield")
            self.HelpCharShieldPercent = self.CharShieldPercent
            self.Char_Shield_Deduct:SetSize(FVector2D(self.ShieldBarLength*self.HelpCharShieldPercent,self.ShieldBarHeight))
            --self:UpdateShieldEffect()
...
function WBP_Battle_Char_C:DecreaseBlood()
    ...
    if not self.IsShowDeductBlood then return end
...
function WBP_Battle_Char_C:CheckChangeRole()
    ...
    if self.RoleId~=nil and self.RoleId~=NewRoleId and NewRoleId ~=0 then
        self:GetCharacter(Player)
    end
end
...
--function WBP_Battle_Char_C:CharDie(Target_Eid)
--    local GameInstance = UE4.UGameplayStatics.GetGameInstance(self)
--    local UIManger = GameInstance:GetGameUIManager()
--    if self.Owner.Eid == Target_Eid and self.Owner:IsMainPlayer() then
--        DebugPrint("----jzn---CharDie-----",self.Owner,self.NowEid,Target_Eid)
--        local UIBattleMain = UIManger:GetUI("BattleMain")
--        UIBattleMain.Battle_Resurgence_PC:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
--        UIBattleMain.Battle_Resurgence_PC:InitResurgenceUI(self.Owner)
--        --UIManger:UnLoadUI("BattleMain")
--        --local UICharResurgence = UIManger:GetUI("CharResurgence")
--        --UICharResurgence:InitResurgenceUI(self.Owner)
--        --UIManger:LoadUI(UIConst.CHARRESURGENCE,"CharResurgence", UIConst.ZORDER_FOR_NORMAL,self.Owner)
--    end
--end
