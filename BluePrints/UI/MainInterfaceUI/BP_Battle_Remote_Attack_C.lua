...
--function BP_Battle_Remote_Attack_C:Initialize(Initializer)
--end

--function BP_Battle_Remote_Attack_C:PreConstruct(IsDesignTime)
--end
...
function BP_Battle_Remote_Attack_C:Construct()
    ...
    --self.timerHandle=UE4.UKismetSystemLibrary.K2_SetTimerDelegate({self, self.UpdateOverheat}, 0.1, true, 0)
end
...
function BP_Battle_Remote_Attack_C:Tick(MyGeometry, InDeltaTime)
    ...
    if realValue~=MaxHotValue or MaxHotValue==0 then
        self.Bg:SetVisibility(UE4.ESlateVisibility.Collapsed)
    else
...
function BP_Battle_Remote_Attack_C:AfterClickFire(IsPress)
    if (self.WBP_PlayerSkill == nil or self.WBP_PlayerSkill.Owner == nil) then
        return
    end
