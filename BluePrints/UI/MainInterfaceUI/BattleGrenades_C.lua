...
--function BattleGrenades_C:Initialize(Initializer)
--end

--function BattleGrenades_C:PreConstruct(IsDesignTime)
--end

-- function BattleGrenades_C:Construct()
-- end
...
function BattleGrenades_C:Tick(MyGeometry, InDeltaTime)
    ...
    if not (screenLocation.X>=0 and screenLocation.X<=self.ScreenCenter.X*2 
    and screenLocation.Y>=0 and screenLocation.Y<=self.ScreenCenter.Y*2 and inScreen) then
        local centerTransLocation=screenLocation-self.ScreenCenter
        if centerTransLocation.X==0 then
