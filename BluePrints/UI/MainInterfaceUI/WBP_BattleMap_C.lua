...
--function WBP_BattleMap_C:Initialize(Initializer)
--end

--function WBP_BattleMap_C:PreConstruct(IsDesignTime)
--end
...
function WBP_BattleMap_C:Construct()
    ...
    self.DoorIcons={}
    -- self.MapCachePoint={}
    -- self.MapCurrentPoint={}
    -- self.BuildingFloor2Map={}
    -- self.BuildingDefaultFloor={}
    -- self.BuildingName2Map={}
    -- self.LevelId2BuildingMap={}
    -- self.BuildingFloor2Z={}
    -- self.LastFloorId=nil
    -- self.LastFloorLevelId=nil
    ...
    -- self.Text_Direction:SetText(GText("UI_Radar_Dire_North"))
    ...
    -- self:AddTimer(0.1, self.DelayAddTouchLayer)
    self:OnChangeKeyBoardSet()
    -- self:PlayAnim("OpenAnimation",self:GetAnimationByName("OpenAnimation"):GetEndTime(),1,UE4.EUMGSequencePlayMode.Reverse)
    ...
    if self.levelLoader then
        ...
        -- if self.levelLoader.shortname=='Prologue/Main/prologue' then--临时屏蔽下序章
        --     self.Panel_Zoom:SetVisibility(ESlateVisibility.Collapsed)
        -- end
        self.IsPrologue=self.levelLoader.shortname=='Prologue/Main/prologue'--序章特殊处理
        ...
        -- self:UpdateLevelFloor(self.levelLoader:GetLevelIdByLocation(self.Player:K2_GetActorLocation()))
    end
    self:InitFloorBox()
    self:InitPointPool()
    ...
    if self.bNewMaterial then
        ...
        for i = 1, self.MapMaterialNum do
            if self["MapPanel"..i] then
                ...
                if i < self.MapMaterialNum then--1~4地图，5门
    ...
    -- key = asset name, vale = {Widget,FunctionName}
    ...
    -- self.DoorArrowPanel:SetVisibility(ESlateVisibility.Collapsed)
    -- self.EnemyPanel:SetVisibility(ESlateVisibility.Collapsed)--11.30
    self.DropPanel:SetVisibility(ESlateVisibility.Collapsed)
    self.DropArrowPanel:SetVisibility(ESlateVisibility.Collapsed)
    -- self.GuidePanel:SetVisibility(ESlateVisibility.Collapsed)
    -- self.GuideArrowPanel:SetVisibility(ESlateVisibility.Collapsed)
    -- self.PhantomPanel:SetVisibility(ESlateVisibility.Collapsed)
end
...
function WBP_BattleMap_C:InitMapWidth()
    ...
    self.TraceArrowRange = (1 - 32 / self.MapWidth.X) / 2--指引点图标默认64
...
function WBP_BattleMap_C:ChangeEvent()
    if self.IsOpen then
        ...
        -- self.Panel_Root.Slot:SetAnchors(FAnchors(0.5,0.5))
        -- self.Panel_Root.Slot:SetPosition(FVector2D(0,0))
        -- self.Panel_Root.Slot:SetSize(FVector2D(0,0))
        -- self.Overridden.ChangeEvent(self,Panel)
...
function WBP_BattleMap_C:DelayAddTouchLayer()
    ...
    self:AddStaticSubTouchItem("BattleMapLayer", self.MapPanel, {Move=self.TouchMapMove, MultiMove=self.TouchMapMultiMove})
end

-- function WBP_BattleMap_C:Tick(MyGeometry, InDeltaTime)
--     self.Overridden.Tick(self,MyGeometry,InDeltaTime)
    -- if not IsValid(self.Player) then
    --     self.Player=UE4.UGameplayStatics.GetPlayerCharacter(self,0)
    -- end
    -- local location=self.Player.CurrentLocation
    -- local rotation=self.Player.CurrentRotation
    -- local cameraRot=self.Player.CurrentCameraRotation
    -- self.panelLoc:Set(location.X,location.Y)
    -- FVector2D.Mul(self.panelLoc, -self.Scale)
    -- FVector2D.Mul(self.panelLoc, self.MapPanel.RenderTransform.Scale)
    -- --local panelLoc=-UE4.FVector2D()*self.Scale*self.MapPanel.RenderTransform.Scale--以地图panel为准，子图标计算时再考虑两边panel不同的缩放
    -- self.MapPanel:SetRenderTranslation(self.panelLoc)
    -- self.PointPanel:SetRenderTranslation(self.panelLoc)
    -- self.RangePanel:SetRenderTranslation(self.panelLoc)
    -- self.FloorPanel:SetRenderTranslation(self.panelLoc)
    -- self.Gamer:SetRenderTransformAngle(rotation.Yaw+90)
    -- self.Direction:SetRenderTransformAngle(cameraRot.Yaw+90)
    -- self:TickGuidePoint()
    -- self:AppearMap()
    -- self:TickFloor()
-- end

-- function WBP_BattleMap_C:AppearMap()
--     --没有到过的地图不显示
--     for _,map in pairs(self.MapArray) do
--         -- local levelLocation = map.RenderTransform.Translation
--         -- local levelSize = map:GetDesiredSize()
--         -- local playerLocation = self.Player:K2_GetActorLocation() * self.Scale
--         -- if (playerLocation.X < levelLocation.X + levelSize.X * 0.5 and playerLocation.Y < levelLocation.Y + levelSize.Y * 0.5 and playerLocation.X > levelLocation.X - levelSize.X * 0.5 and playerLocation.Y > levelLocation.Y - levelSize.Y * 0.5) then
--         --     map.Map:GetDynamicMaterial():SetScalarParameterValue("Appear", 1)
--         -- end

--         if self.MapCachePoint[map] then
--             self.MapCurrentPoint[map][1]:Set(self.MapCachePoint[map][1].X,self.MapCachePoint[map][1].Y)
--             self.MapCurrentPoint[map][2]:Set(self.MapCachePoint[map][2].X,self.MapCachePoint[map][2].Y)
--             FVector2D.Add(self.MapCurrentPoint[map][1],self.panelLoc)
--             FVector2D.Add(self.MapCurrentPoint[map][2],self.panelLoc)
--             if self.MapCurrentPoint[map][1].X>0 and self.MapCurrentPoint[map][1].Y>0 and self.MapCurrentPoint[map][2].X<0 and self.MapCurrentPoint[map][2].Y<0 then
--                 map.Map:GetDynamicMaterial():SetScalarParameterValue("Appear", 1)
--                 self.MapCachePoint[map] =nil
--                 self.MapCurrentPoint[map]=nil
--             end
--         end
--     end
-- end
...
function WBP_BattleMap_C:CreateSingleBattleMap(id,data)
    ...
    if texture then
        ...
        -- self.MapCachePoint[map]={(levelLocation+size*0.5)*self.MapPanel.RenderTransform.Scale,(levelLocation-size*0.5)*self.MapPanel.RenderTransform.Scale}
        -- self.MapCurrentPoint[map]={FVector2D(0,0),FVector2D(0,0)}
        if self.LevelId2BuildingMap:FindRef(id) then
...
function WBP_BattleMap_C:CreateRougelikeBattleMap(Id,Data)
    ...
    if texture then
        local map=self:NewMap()--关卡位置原因，肉鸽先不用材质
        map.Map:SetBrushFromTexture(texture,true)
        local size=FVector2D(texture:Blueprint_GetSizeX(), texture:Blueprint_GetSizeY())
        map.Map:SetBrushSize(size)
        local levelLocation= FVector2D(Data[2].X, Data[2].Y) * self.Scale
...
function WBP_BattleMap_C:OnClickRealOpen()
    ...
    if GameState and (GameState:IsInRegion() or (not GameState:IsInRegion() and self.levelLoader.IsWorldLoader)) then
    ...
    -- self:PlayAnim("OpenAnimation")
    self:ChangeEvent()
    self.BG:SetVisibility(UE4.ESlateVisibility.Hidden)
    -- local duration=self:GetAnimationByName("OpenAnimation"):GetEndTime()
...
function WBP_BattleMap_C:OnClickClose()
    ...
    -- self.RetainerBox_101:SetRenderScale(self.InitBoxScale)
    self.MapPanel:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    self.Btn_Close:SetVisibility(UE4.ESlateVisibility.Collapsed)
    -- local delayTime=self:GetAnimationByName("OpenAnimation"):GetEndTime()
    -- self:PlayAnim("OpenAnimation",0,1,UE4.EUMGSequencePlayMode.Reverse)
    self:ChangeEvent()
...
function WBP_BattleMap_C:OnKeyboardClick()
    ...
    if GameState and GameState:IsInRegion() then
        -- self.Player:OpenMap()
        local Avatar = GWorld:GetAvatar()
        if not Avatar or not Avatar:IsRealInBigWorld() or Avatar:IsInHardBoss() then
            return
        end
        ...
        if self.IsOpen then
            ...
            if self.levelLoader.IsWorldLoader then
...
function WBP_BattleMap_C:NewPointArrow()
    ...
    if not Arrow then
        Arrow = self.Overridden.NewPointArrow(self)
    end
...
function WBP_BattleMap_C:UpdateGuideIcon(SceneManager, GuideName, OpType, TargetEid, TargetActor,
        TargetActorLocation, ConfigData, IsNeedArrow, IsGuideFollowActor, IsNeedLookUpEntity)
    ...
    if OpType == "Delete" then
        ...
        if ConfigData.GuideVisibility then --这个配置了说明GuideIconBPPath只用于设置GuideIconAni的图标
            return
        end
        local MiniMapGuidePNG = ConfigData.GuideIconBPPath
        if GuideAni == "Pet" then
            local GameMode = UE4.UGameplayStatics.GetGameMode(GWorld.GameInstance)
            if GameMode and GameMode:IsInDungeon() == false and ConfigData.GuideIconBPPath2 then
                MiniMapGuidePNG = ConfigData.GuideIconBPPath2
            end
        end
        
        -- if MiniMapGuidePNG == nil then       -- 这个删了试试，不然 GuideAni == "Destroy" 里面就走不进去了
                                                -- 即，破坏玩法的三个精英怪虽然配置了MiniMapGuidePNG，但是后续需要动态替换为真正的路径
            if TargetEid ~= nil and GuideAni == "Destroy" then
                -- local Content = SceneManager:GetABCText(SceneManager.SabotageABCMap, TargetEid, 26)
                -- MiniMapGuidePNG = SceneManager:GetSabotageABCIconPath(Content)
                local Info = {
                ...
                self.SabotageABCGuideCache[GuideName] = Info        -- 回调传参不方便，只好用个Map来暂存一下需要的信息
            ...
            elseif TargetEid ~= nil and GuideAni == "Excavation" then
                --local Content = SceneManager:GetABCText(SceneManager.ExcavationABCMap, TargetEid, 6)
                if not IsValid(TargetActor) then return end
                if not TargetActor.GuideOrderIndex then return end
            ...
            -- else             -- 这里的return该干掉了，否则其他的正常图标都加载不出来. 若MiniMapGuidePNG真为空，下面会拦住
            --     return 
            end
        -- end

        -- local icon = LoadObject(MiniMapGuidePNG)
        if not MiniMapGuidePNG then
            return
        end

    --     if not self.GuidePoint[GuideName] then 
    --         self.GuidePoint[GuideName]=self:NewPoint()
    --    end
    --    self.GuideData[GuideName]={TargetActor,TargetActorLocation, ConfigData, TargetEid,IsNeedArrow, IsGuideFollowActor, IsNeedLookUpEntity, FVector2D(0,0)}
    --    self:SetImageWithTextureOrSprite(self.GuidePoint[GuideName].Icon,icon)
        if self.bNewMaterial then
            ...
            -- self:SetImageWithTextureOrSprite(self.GuidePoint:FindRef(GuideName).Icon,icon)
            self:AddTrack(GuideName)
        end
    end
end
...
function WBP_BattleMap_C:SetSabotageABCGuideIcon_Callback(Eids, UnitIds)
    -- 只要有一个回调触发，便会更新三个有ABC的怪物小地图指引图片
...
function WBP_BattleMap_C:CheckIsNeedCalEveryFrameByActor(guideData)
    if (guideData[1] == nil or not UE4.UKismetSystemLibrary.IsValid(guideData[1])) then
        return false
    end
    return guideData[6] or guideData[2] == nil
end
...
function WBP_BattleMap_C:OnChangeKeyBoardSet()
    ...
    self.HudKeyShow = platformName == 'Windows' and not IsTemple and IsGamepad
    if self.Common_Key_Hud_PC and self.HudKeyShow then
...
function WBP_BattleMap_C:ShowDropInMinimap(PickupBase,IsShow)
    ...
    -- local icon=LoadObject(ItemInfo.GuideIconBPPath)
    -- if not icon then
    --     return
    -- end
--     if not self.GuidePoint[PickupBase.Eid] then 
--         self.GuidePoint[PickupBase.Eid]=self:NewPoint()
--    end
--    self.GuideData[PickupBase.Eid]={PickupBase,PickupBase:K2_GetActorLocation(), ItemInfo, PickupBase.Eid,true, true, true}
--    self:SetImageWithTextureOrSprite(self.GuidePoint[PickupBase.Eid].Icon,icon)
    if self.bNewMaterial then
        if not self.GuideIconMap:Find(GuideName) then
...
function WBP_BattleMap_C:ShowRangedIconInMinimap(Eid, IsShow, ImagePath, Radius)
    ...
    guideData.IsGuideFollowActor = true
    guideData.IsNeedLookUpEntity = false--暂时按必须有actor处理
    guideData.NeedTrack=false
    if self.WildMap then
...
function WBP_BattleMap_C:TickFloor()
    if not self.Player or self.Player.CurrentLevelId:Num()==0 or self.Player:IsCharacterInAir() then
        return
    end
    ...
    if self.LevelId2BuildingMap[levelId] then
        ...
        for floorId,map in pairs(self.LevelId2BuildingMap[levelId]) do
            if self.BuildingFloor2Z[map] then
                if playerZ>self.BuildingFloor2Z[map] and (currentFloorId==nil or currentZ<self.BuildingFloor2Z[map]) then
        ...
        for floorId,map in pairs(mapArray) do
            if id == levelId then
                if floorId == currentFloorId then
                    ...
                    if map.CurrentAni~=map.Out then
                        map:StopAllAnimations()
...
function WBP_BattleMap_C:AddFloorBox(FloorBox)
    ...
    self:AddAppearMap(map.Map,levelLocation,size)
    map:SetVisibility(ESlateVisibility.Collapsed)
    -- if not self.BuildingFloor2Map[FloorBox.BuildingFloor] then
    --     self.BuildingFloor2Map[FloorBox.BuildingFloor]={}
    -- end
    -- table.insert(self.BuildingFloor2Map[FloorBox.BuildingFloor],map)
    -- if not self.BuildingName2Map[FloorBox.BulidingName] then
    --     self.BuildingName2Map[FloorBox.BulidingName]={}
    -- end
    -- table.insert(self.BuildingName2Map[FloorBox.BulidingName],map)
    local levelId=self.levelLoader:GetDesignActorLevelName(FloorBox)
    -- if FloorBox.IsDefaultFloor then
    --     self.BuildingDefaultFloor[levelId]=map
    -- end
    -- if not self.LevelId2BuildingMap[levelId] then 
    --     self.LevelId2BuildingMap[levelId]={}
    -- end
    self:AddBuildingMap(levelId,FloorBox.BuildingFloor,map,FloorBox:k2_GetActorLocation().Z)
    -- PrintCrack(levelId,FloorBox.BuildingFloor,BuildingStruct.Floor2Z:Num())
    -- self.LevelId2BuildingMap[levelId][FloorBox.BuildingFloor]=map
    -- self.BuildingFloor2Z[map]=FloorBox:k2_GetActorLocation().Z
    if self.LevelId2Map[levelId] then
...
function WBP_BattleMap_C:InitPhantomConfigData(GuideName,GuideData,UnitId)
    ...
    if GuideData.PlayerIndex == 0 then
        ...
        if DataMgr.BattleChar[UnitId] == nil or DataMgr.BattleChar[UnitId].GuideIconImg == nil then
            return
        end
        ...
        -- phantom.Icon.Img_Avatar:SetBrushResourceObject(IconImage)
    elseif GuideData.PlayerIndex > 0 then
        phantom.Icon.PhantomEid = GuideData.GuideTargetEid
        phantom.Icon.PhantomActor = GuideData.GuideTarget
        phantom.Icon.PlayerIndex = GuideData.PlayerIndex
        -- phantom.Icon.Panel_Bg:SetVisibility(ESlateVisibility.Collapsed)
...
function WBP_BattleMap_C:InitHostageImage(GuideName)
    ...
    -- local IconImage = LoadObject('/Game/UI/Texture/Dynamic/Atlas/GuidePoint/T_Gp_Rescue_HostageA.T_Gp_Rescue_HostageA')
    -- phantom.Icon:SetBrushResourceObject(IconImage)
end
...
function WBP_BattleMap_C:SetPhantomGuideStateByEvent(Eid, State, PrevState)
    ...
    -- local TextTure = LoadObject(Path)
    -- if TextTure then
    --     local GuidePoint = self.GuidePoint:FindRef(tostring(Eid))
    --     if GuidePoint then
    --         GuidePoint.Icon.Img_Avatar:SetBrushResourceObject(TextTure)
    --     end
    -- end
end
...
function WBP_BattleMap_C:InOrOutEdgeState(bIn)
    ...
    if bIn then
        ...
        -- self.VX_EdgeWarnings:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
    else
...
function WBP_BattleMap_C:OnTextureLoadFinish(Object)
    ...
    if FuncName == "SetImageWithTextureOrSprite" and Widget.Icon then
        self:SetImageWithTextureOrSprite(Widget.Icon, Object)
    elseif FuncName == "SetBrushResourceObject" and Widget.Icon then
...
function WBP_BattleMap_C:LoadTextureAsync(Path,Widget,FuncName,AssetName)
    ...
    if not Path or not Widget or not FuncName then return end
    ...
    UE.UResourceLibrary.LoadObjectAsync(self,Path,{self,WBP_BattleMap_C.OnTextureLoadFinish})
...
function WBP_BattleMap_C:AddDoorToMinimap(Door)
    ...
    -- DebugPrint('this door.state is:',Door.door_state,'door.next_id is:',Door.next_id)
    if self.bNewMaterial then
        local Obj = NewObject(UBattleMapLevelObj.StaticClass(), self)
        Obj.ImagePath = Path
        ...
        if not LevelConnect then
            LevelConnect = FBattleMapLevelConnect()
        end
        ...
        if Door.OtherLevelId ~= "-1" and Door.OtherLevelId ~= "" then
            ...
            if not LevelConnect1 then
                LevelConnect1 = FBattleMapLevelConnect()
            end
    ...
    -- map.Icon:SetBrushFromTexture(texture,true)
...
function WBP_BattleMap_C:UpdateDoorIcon(AutoDoor)
    ...
    if self.bNewMaterial then
        ...
        if self.DoorIcons[AutoDoor] then
