...
local GMObjectUtils = {}
--以content为源数据，创建并返回一个class类的GM指令对象
...
function GMObjectUtils.GetCharCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "button",callback = "ChangeRole",close_gm = true}
    local Result = {}
...
function GMObjectUtils.GetDynCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "button",callback = "ForceStartDynQuest",close_gm = true}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
...
function GMObjectUtils.GetWeaponCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "button",callback = "ChangeWeapon",close_gm = true}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
...
function GMObjectUtils.GetResourceCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "edit",callback = "AddResource"}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
...
function GMObjectUtils.GetHotKeyCmdObjs()
    ...
    for key, value in pairs(KeyboardConfig) do
        local content = {mode = "text", parameters = {GText(value.ActionNameText or ""),value.Key}}
        local obj = GMObjectUtils.NewCommandObject(content)
        table.insert(Result,obj)
    end
    return Result
...
function GMObjectUtils.GetMonsterCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "edit",callback = "CreateMonster",close_gm = true}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
...
function GMObjectUtils.GetCMSCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "edit",callback = "CreateMechanismSummon",close_gm = true}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
        obj.Text = id .. " " .. GText(Data[id].UnitName or "")
...
function GMObjectUtils.GetCPCmdObjs()
    ...
    table.sort(Array)
    local content = {mode = "edit",callback = "CreatePhantom",close_gm = true}
    local Result = {}
    for _,id in ipairs(Array) do
        local obj = GMObjectUtils.NewCommandObject(content)
        obj.Text = id .. " " .. GText(Data[id].UnitName or "")
...
function GMObjectUtils.GetFriendCmdObjs()
    ...
    table.sort(Uids)
    local content = {mode = "button", callback = "CanSendFriendGift", close_gm = false}
