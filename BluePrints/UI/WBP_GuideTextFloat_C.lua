...
function WBP_GuideTextFloat_C:ShowDungeonGuide(TextIndex, Duration)
    ...
    if not WarningHint then
        WarningHint = UIManager:LoadUI(UIConst.WARNINGHINT, UIConst.WarningHintName, UIConst.ZORDER_FOR_SECONDARY_POPUP)
    end
...
function WBP_GuideTextFloat_C:Close()
    ...
    -- if(self.GuideTextFloatList) then 
    --     self.GuideTextFloatList:close() 
    --     self.GuideTextFloatList = nil
    -- end
...
function WBP_GuideTextFloat_C:PlayOutAnim(IsExecuteFinish)
    ...
    if self:IsAnimationPlaying(self.Complete) or self:IsAnimationPlaying(self.GuideText_Out) then
     ...
     if IsExecuteFinish or IsExecuteFinish == nil then
    ...
    if(self.LastTime > 0) then
        --self:RemoveTimer("UpdateTextContent")
        self.LastTime = 0
    end
    if(self.NowState) then
        -- EventManager:FireEvent(EventID.HideSkillAnim, self.NowState)
    end
end
...
function WBP_GuideTextFloat_C:UpdateTextContent(Messgae)
    ...
    -- if(self.HasGuideTextFloatListSetTitle == nil) then
    --     self.GuideTextFloatList = UIManager(self):GetUIObj(UE4.UUIStateAsyncActionBase.GetGuideTextFloatListKey())
    --     if(self.GuideTextFloatList) then self.GuideTextFloatList:SetTitle(TitleContent) end
    --     self.HasGuideTextFloatListSetTitle = true
    -- end
    ...
    -- if(self.LastTime > 0) then
    --     self:AddTimer(0.1, self.UpdateTextContent, false, 0, "UpdateTextContent",nil,Messgae)
    -- end
end
...
function WBP_GuideTextFloat_C:CheckSpecialPadkey(ActionName)
    if ActionName == "R" or ActionName == "L" or ActionName == "DpadV" then
        return true
    end
    return false
end
...
function WBP_GuideTextFloat_C:GetSpecialPadkey(ActionName)
    if ActionName == "R" then
        return "R", "GamepadKey"
    elseif ActionName == "L" then
        return "L", "GamepadKey"
    elseif ActionName == "DpadV" then
...
function WBP_GuideTextFloat_C:SetCommonKey(KeyInfo, TableIndex)
    ...
    elseif  ActionName == "W" or ActionName == "A" or ActionName == "S" or ActionName == "D" then  --特殊处理
    ...
    -- if KeyName ~= '' then
    --     local RealKey = CommonUtils:GetKeyText(KeyName)
    --     if RealKey ~= '' then
    --         Key = RealKey
    --     end
    -- end
    ...
    if ActionName == "ControlMove" then
        if self.CurrentInputDevice[1] == "GamepadKey" then
            ...
            Widget:CreateCommonKey({KeyInfoList = {{Type = "Text",Text = "W",}},})
            Widget1:CreateCommonKey({KeyInfoList = {{Type = "Text",Text = "A",}},})
            Widget2:CreateCommonKey({KeyInfoList = {{Type = "Text",Text = "S",}},})
            Widget3:CreateCommonKey({KeyInfoList = {{Type = "Text",Text = "D",}},})
            self["Key_"..UIIndex]:AddChild(Widget)
...
function WBP_GuideTextFloat_C:AnalyzeExecuteLogic(LogicStr)
    ...
    for i,v in pairs(self.ExecuteLogics) do
        self:ListenForInputAction(v, UE4.EInputEvent.IE_Pressed, true, {self, self["Execute"..v.."Logic"]})
    end
...
function WBP_GuideTextFloat_C:LogicExecuteFinish()
    ...
    if self.NowState == self.ExecuteLogics[#self.ExecuteLogics] then
        ...
        -- EventManager:FireEvent(EventID.ShowSkillAnim, self.NowState)
    end
end
...
function WBP_GuideTextFloat_C:ReleaseSkillCombLogic()
    -- if not (string.match(self.NowState, "Skill")) then
    --     return
    -- end
    if self.NowState ~= "Skill1" then
        return
...
function WBP_GuideTextFloat_C:SetInfoAboutGuide_Enum(Actions, IsForceTouch, IsTimeDilation, ForbidSuccessAnim, IsNotForceRelease)
    if self.LastTime < 0 and (IsForceTouch or IsTimeDilation) and #Actions == 0 then
        print(_G.LogTag, "Error: WBP_GuideTextFloat_C 强制输入/时间膨胀, 持续时间为-1, 但是没有需要进行的操作, UI不会关闭, 玩家无法操作/时间一直膨胀, 表现错误")
        self:DeleteGuideMessage()
        return
    end
    --self:SetInputGameOnly(true)
        ...
        if ActionName then
            ...
            if self.IsForceTouch then
                if ActionName == "OpenMenu" then
                    self:ListenForInputAction(ActionName, UE4.EInputEvent.IE_Pressed, false, {self, self["Execute"..ActionName.."Logic"]})
                elseif string.match(ActionName, "Skill") then
                    self:ListenForInputAction("GamepadUseSkill", UE4.EInputEvent.IE_Pressed, true, {self, self.PressSkillCombLogic})
                    self:ListenForInputAction("GamepadUseSkill", UE4.EInputEvent.IE_Released, false, {self, self.ReleaseSkillCombLogic})
                    self:ListenForInputAction(ActionName, UE4.EInputEvent.IE_Pressed, true, {self, self["Execute"..ActionName.."Logic"]})
                    self:ListenForInputAction(ActionName, UE4.EInputEvent.IE_Released, true, {self, self["Release"..ActionName.."Logic"]})
                else
                    self:ListenForInputAction(ActionName, UE4.EInputEvent.IE_Pressed, true, {self, self["Execute"..ActionName.."Logic"]})
                end
            else
                if ActionName == "BulletJump" then
                    -- self:ListenForInputAction("Jump", UE4.EInputEvent.IE_Pressed, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                    self:ListenForInputAction("Jump", UE4.EInputEvent.IE_Repeat, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                    self:ListenForInputAction("Jump", UE4.EInputEvent.IE_Released, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                elseif ActionName == "HeavyAttack" then
                    self:ListenForInputAction("Attack", UE4.EInputEvent.IE_Pressed, false, {self, self["Wait"..ActionName.."PressLogicExecute"]})
                    self:ListenForInputAction("Attack", UE4.EInputEvent.IE_Released, false, {self, self["Wait"..ActionName.."ReleaseLogicExecute"]})
                elseif ActionName == "FireInAir" then
                    self:ListenForInputAction("Fire", UE4.EInputEvent.IE_Pressed, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                elseif ActionName == "ContinuousShooting" then
                    self:ListenForInputAction("Fire", UE4.EInputEvent.IE_Pressed, false, {self, self["Wait"..ActionName.."PressLogicExecute"]})
                    self:ListenForInputAction("Fire", UE4.EInputEvent.IE_Released, false, {self, self["Wait"..ActionName.."ReleaseLogicExecute"]})
                elseif ActionName == "ControlMove" then
                ...
                elseif ActionName == "FallAttack" then
                    self:ListenForInputAction("Attack", UE4.EInputEvent.IE_Pressed, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                elseif ActionName == "SlideAttack" then
                    self:ListenForInputAction("Attack", UE4.EInputEvent.IE_Released, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                elseif ActionName == "OpenMenu" then
                    self:ListenForInputAction("OpenMenu", UE4.EInputEvent.IE_Released, false, {self, self["Wait"..ActionName.."LogicExecute"]})
                else
                    self:ListenForInputAction(ActionName, UE4.EInputEvent.IE_Pressed, false, {self, function() self:LogicExecuteFinish() end})
                end
            end
        end
    end
...
function WBP_GuideTextFloat_C:WaitBulletJumpLogicExecute()
    ...
    local PlayerCharacter = UE4.UGameplayStatics.GetPlayerCharacter(self,0)
    if PlayerCharacter:GetCharacterTag() == "BulletJump" then
        self:LogicExecuteFinish()
    end
end
...
function WBP_GuideTextFloat_C:WaitHeavyAttackPressLogicExecute()
    ...
    self:AddTimer(0.2, function()self:LogicExecuteFinish()end, false, 0, "TryHeavyAttack")
end
...
function WBP_GuideTextFloat_C:WaitContinuousShootingPressLogicExecute()
    ...
    self:AddTimer(0.2, function()self:LogicExecuteFinish()end, false, 0, "TryContinuousShooting")
end
