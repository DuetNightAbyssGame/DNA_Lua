...
function M:UpdateOwner(Hook, HookComp, PlayerActor)
    ...
    -- if not self:IsListeningForInputAction("Jump") then
    --     self:ListenForInputAction("Jump", EInputEvent.IE_Released, true, {self, self.ReleasedSpace})
    -- end
    self.Hook = Hook
...
function M:RefreshKeyLoc(FXLocComp)
    ...
    local CurrentOffsetOnDoor, LocLerpFinished, IndicatorAngle, TargetDistance,
        ...
        UUIFunctionLibrary.LerpAndProjectWorldToScreenInEllipse(
            Controller, Loc, Loc, 3,
            self.ScreenLocation, self.CenterPos, self.OvalSize, self.BoardSize, false,
            0, self.CurrentOffsetOnDoor, self.OffsetLerpInterval, false, 0, 0, 0, false)
