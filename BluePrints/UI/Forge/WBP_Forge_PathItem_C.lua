...
function M:OnItemMouseButtonUp()
    local CurInputDeviceType = UIUtils.UtilsGetCurrentInputType()
    if CurInputDeviceType == UE4.ECommonInputType.Gamepad then -- 手柄端A键触发铸造
...
function M:UpdateView(Content)
    Content.OnMouseButtonUpEvents = {Obj = self, Callback = self.OnItemMouseButtonUp}
    -- 如果处于图鉴模式，移除一些参数
...
function M:SetUpLineVisible(IsShow)
    if IsShow then 
        ...
        if self.Line01_Connect:GetVisibility() == UE4.ESlateVisibility.SelfHitTestInvisible then 
...
function M:IsEmpty()
    return not self.Item.Id or self.Item.Id == 0
end
...
function M:OnAnimationFinished(InAnimation)
    if InAnimation == self.ChildConnect and self.ShowDownLine == false then 
        self.Line02_Connect:SetVisibility(UE4.ESlateVisibility.Collapsed)
    elseif InAnimation == self.ParentConnect and self.ShowUpLine == false then 
        self.Line01_Connect:SetVisibility(UE4.ESlateVisibility.Collapsed)
