...
function WBP_Forging_Compendium_Path_C:OnLoaded(DraftId)
    ...
    self.ControllerFSM = ControllerFSM.New(self, {
        [CompemdiumPathStates.Normal] = {
            OnEnter = function() self:OnEnterState_Normal() end,
        },
        [CompemdiumPathStates.FocusRight] = {
            OnEnter = function() self:OnEnterState_FocusRight() end,
        },
    })
...
function WBP_Forging_Compendium_Path_C:Init(DraftId)
    ...
    self.PathTree.OnClosedCallback = {self, self.HandleCloseDraftPathView}
    self.PathTree.OnFocusToDetailsView = {self, self.OnFocusToDetailsView}
    self.PathTree.OnFocusToPathView = {self, self.RefocusToPathView}
...
function WBP_Forging_Compendium_Path_C:HandleCloseDraftPathView()
    self.PathTree.HasPreInit = false    -- 确保下次初始化时走完全部流程
    self:Close()
end
