...
---@class WBP_Forging_DialogBatch_PC_C : WBP_Forging_DialogBatch_C
...
function WBP_Forging_DialogBatch_PC_C:InitView()
    ...
    Content.OnMenuOpenChangedEvents = {Obj = self, Callback = self.OnMenuOpenChange}
    ...
    -- DebugPrint("MaxCanProduceNum: " .. tostring(self.ForgeItemContent.MaxCanProduceNum))
end
    ...
    for Index = 1, 4 do 
        ...
        if ResInfo then 
            ...
            Widget:Init({
                ...
                OnMenuOpenChangedEvents = {Obj = self, Callback = self.OnMenuOpenChange}
            })
...
function WBP_Forging_DialogBatch_PC_C:OnProduceValueChange(NewValue)
    ...
    -- self:BroadcastDialogEvent("UpdateDialogCostTip", NewValue)
...
function WBP_Forging_DialogBatch_PC_C:OnContentKeyDown(MyGeometry, InKeyEvent)
    ...
    elseif InKeyName == Const.GamepadFaceButtonBottom then 
        if self.ControllerFSM:Current() == FSMState.CheckItem then 
            self.ControllerFSM:Enter(FSMState.ShowTip)
        end
    end
