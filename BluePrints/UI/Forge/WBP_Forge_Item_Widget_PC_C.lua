...
--- @type Forging_Item_Widget_PC_C
local WBP_ForgeItemWidget = Class({"BluePrints.Common.TimerMgr", "BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_ForgeItemWidget:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    elseif InKeyName == Const.GamepadFaceButtonLeft then 
        if self.Content.State == ForgeConst.DraftState.InProgress then 
...
function WBP_ForgeItemWidget:OnFocusReceived(MyGeometry, InFocusEvent)
    if UIUtils.UtilsGetCurrentInputType() == UE4.ECommonInputType.Gamepad then
        if self.Content.OnReceiveFocus then 
...
function WBP_ForgeItemWidget:OnFocusLost(InFocusEvent)
    if UIUtils.UtilsGetCurrentInputType() == UE4.ECommonInputType.Gamepad then
        if self.Content.OnLostFocus then 
...
function WBP_ForgeItemWidget:EnableGamepadBlock(Value)
    if not self.BtnGamepadBlock then return end
    if Value then 
...
function WBP_ForgeItemWidget:OnBtnStartClicked()
    if self.WidgetCurrentState == ForgeConst.DraftState.NotStarted and not (self.Content.IsFoundryEnough and self.Content.IsResourceEnough) then 
        ...
        if self.EventStartProduce then 
...
function WBP_ForgeItemWidget:InitializeView()
    ...
    Content.OnMouseButtonUpEvents = {Obj = self, Callback = self.OnItemWidgetClicked}
...
function WBP_ForgeItemWidget:InitNotStartedView()
    -- DebugPrint("Tianyi@ On Widget State Changed: " .. self.WidgetCurrentState .. " TO " .. self.Content.State)
    ...
    for Index = 1, 4 do 
        ...
        if ResInfo then 
            ...
            Widget:Init({
                ...
                OnMenuOpenChangedEvents = {Obj = self,Callback = self.OnMenuOpenChanged,},
                HandleMouseDown = true
            })
...
function WBP_ForgeItemWidget:UpdatePercent()
    ...
    -- DebugPrint("Tianyi CurrentTime, StartTime: ", CurrentTime, self.Content.StartTime)
    ...
    if RestTime > 0 then 
        ...
        if not self.ErrorRestTimeMap[DraftId] then 
            ...
            if Battle then
                local ErrorStr = string.format("Forge@ RestTime <= 0, CurrentTime: %d, DurationTime: %d, RestTime: %d, osTime: %d", 
                    CurrentTime, DurationTime, RestTime, os.time())
                local DataModel = self.Content.GetDataModel()
                if DataModel then
                    local DraftInfo = DataModel:CheckState(DraftId)
                    if DraftInfo then
                        ErrorStr = ErrorStr .. string.format(", State: %d, StartTime: %d, DoingNum: %d, CompleteNum: %d", 
                        DraftInfo.State, DraftInfo.StartTime, DraftInfo.DraftDoingNum, DraftInfo.DraftCompleteNum)
                    end
                end
