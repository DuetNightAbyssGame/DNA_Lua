...
---@class WBP_ForgeMain : WBP_Forging_Main_P_C
...
function WBP_ForgeMain_C:OnLoaded(...)
    ...
    self.KeyboardKeyConfig = {
        [ForgeConst.BottomKeyTypes.BottomKey_Keyboard_Space] = {
            KeyInfoList = {
                {Type="Text", Text="Space", Owner=self, ClickCallback = self.TryBatchComplete},
            },
            Desc=GText("UI_Mail_Recieveall")
        },
        [ForgeConst.BottomKeyTypes.BottomKey_Keyboard_Esc] = {
            KeyInfoList = {
                {Type="Text", Text="Esc", Owner=self, ClickCallback = self.OnClose},
            },
            Desc=GText("UI_BACK")
        },
    }
    ...
    self.GamepadKeyCofig = {
        ...
        [ForgeConst.BottomKeyTypes.BottomKey_ShowItem] = {
            GamePadInfoList = {
                {Type="Img",ImgShortPath=UIConst.GamePadImgKey.SpecialLeft, Owner=self}
            },
        ...
        [ForgeConst.BottomKeyTypes.BottomKey_SetTarget] = {
            GamePadInfoList = {
                {Type="Img",ImgShortPath=UIConst.GamePadImgKey.FaceButtonLeft, Owner=self}
            },
    ...
    -- 初始化通用排序按钮
    self.Sort:Init(self, {GText("UI_Select_Kind"), GText("UI_Select_Unique")}, CommonConst.DESC, {
    ...
    if not self.RefreshTimer then
        self.RefreshTimer = self:AddTimer(0.5, 
        function() 
...
function WBP_ForgeMain_C:OnEnterState_NormalPage_FocusItem()
    self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_ShowItem, ForgeConst.BottomKeyTypes.BottomKey_Back})
end

-- 聚焦到一个条目的图标
function WBP_ForgeMain_C:OnEnterState_NormalPage_ShowItem()
    if self.CurrentGamepadSelectedItem then
        self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_ShowDetails, ForgeConst.BottomKeyTypes.BottomKey_Back})
...
function WBP_ForgeMain_C:OnEnterState_NormalPage_FocusSort()
    ...
    self.Controller:SetVisibility(UE4.ESlateVisibility.HitTestInvisible)
    self.Btn_Receive:SetGamePadIconVisible(false)
    self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_Confirm, ForgeConst.BottomKeyTypes.BottomKey_Back})
end
...
function WBP_ForgeMain_C:OnEnterState_NormalPage_NoFocus()
    self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_Back})
...
function WBP_ForgeMain_C:OnForgeItemReceiveFocus(Widget)
    if self.CurrentGamepadSelectedItem == Widget.Content then return end
    if self.CurrentGamepadSelectedItem then 
        local CurrentGamepadSelectedEntry = self:GetEntryFromItem(self.CurrentGamepadSelectedItem)
        if CurrentGamepadSelectedEntry then 
...
function WBP_ForgeMain_C:HandleGamepadSetTarget()
    if not self.CurrentGamepadSelectedItem then return end 
    local CurrentState = self.ControllerFSM:Current()
    ...
    if CurrentState == ForgeConst.ControllerFSMStates.NormalPage_FocusItem then 
        if self.CurrentGamepadSelectedItem.State == ForgeConst.DraftState.NotStarted then 
            if self.CurrentGamepadSelectedItem.IsSetTarget then 
                self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_UnSetTarget, ForgeConst.BottomKeyTypes.BottomKey_ShowItem, ForgeConst.BottomKeyTypes.BottomKey_Back})
            else
                self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_SetTarget, ForgeConst.BottomKeyTypes.BottomKey_ShowItem, ForgeConst.BottomKeyTypes.BottomKey_Back})
            end
        else 
            self:UpdateGamepadBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_ShowItem, ForgeConst.BottomKeyTypes.BottomKey_Back})
        end
...
function WBP_ForgeMain_C:ReceiveEnterState(StackAction)
    ...
    -- 如果此前处于详细查看材料的模式，重置一下状态
    if self.ControllerFSM then
        local Current = self.ControllerFSM:Current()
        if Current == ForgeConst.ControllerFSMStates.NormalPage_ShowItem 
            or Current == ForgeConst.ControllerFSMStates.NormalPage_FocusCompendium then
            self.ControllerFSM:Enter(ForgeConst.ControllerFSMStates.NormalPage_FocusItem)
        end
    end
end
...
function WBP_ForgeMain_C:RefreshSingleItemData(ForgeItemContent)
    if ForgeItemContent then 
        ...
        if not DraftInfo 
            or (TabType == ForgeConst.TabType.Producing and DraftInfo.State == ForgeConst.DraftState.NotStarted) 
            or (DraftInfo.State == ForgeConst.DraftState.NotStarted and DraftInfo.Count <= 0 and not DraftInfo.IsInfinity) then
                -- 设计稿已经用完了, 或者位于铸造中的页面，且完成了铸造
...
function WBP_ForgeMain_C:TickRefreshItemsView(WidgetState)
    if self.IsShowingDraftPathView then 
        ...
        for i = 1, Length do 
            ...
            if ForgeItemWidget.Content.IsEmptyWidget then break end
...
function WBP_ForgeMain_C:UpdateBatchCompleteButton()
    ...
    for _, Item in pairs(Items) do 
        ...
        if DraftInfo and DraftInfo.State == ForgeConst.DraftState.Complete then
            ...
            if UIUtils.IsKeyboardInput() then
                self:UpdateKeyboardBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_Keyboard_Space, ForgeConst.BottomKeyTypes.BottomKey_Keyboard_Esc})
            end
            return 
        end
    end
    ...
    self.Btn_Receive:ForbidBtn(true)
    if UIUtils.IsKeyboardInput() then
        self:UpdateKeyboardBottomKeyInfo({ForgeConst.BottomKeyTypes.BottomKey_Keyboard_Esc})
    end
end
...
function WBP_ForgeMain_C:InitKeySetting()
    ...
    self.KeyDownEvent[self.SwitchTab_LeftKey] = function() self.Tab:TabToLeft() end
    self.KeyDownEvent[self.SwitchTab_RightKey] = function() self.Tab:TabToRight() end
    self.KeyDownEvent[self.SwitchSubTab_LeftKey] = function() self.SubTab_List:TabToLeft() end
    self.KeyDownEvent[self.SwitchSubTab_RightKey] = function() self.SubTab_List:TabToRight() end
end
...
function WBP_ForgeMain_C:ShowDraftPathView(DraftId)
    ...
    self.Forging_Path_PC.OnClosedCallback = {self, self.HandleCloseDraftPathView}
    self.Forging_Path_PC.OnDetailsViewBtnStartClickedCallback = {self, self.OnDraftBtnStartClicked}
    self.Forging_Path_PC.OnDetailsViewBtnCancelClickedCallback = {self, self.OnDraftBtnCancelClicked}
    self.Forging_Path_PC.OnFocusToDetailsView = {self, self.OnFocusToPathDetailsView}
    self.Forging_Path_PC.OnFocusToPathView = {self, self.OnFocusToPathView}
...
function WBP_ForgeMain_C:Handle_PreviewKeyDownOnGamePad(InKeyName)
    ...
    elseif CurrentState == ForgeConst.ControllerFSMStates.NormalPage_ShowItem then 
        if InKeyName == Const.GamepadDPadUp or InKeyName == Const.GamepadDPadDown then   -- 禁止上下导航 
            IsEventHandled = true
        end
    end
...
function WBP_ForgeMain_C:Handle_KeyDownOnGamePad(InKeyName)
    ...
    elseif  CurrentState == ForgeConst.ControllerFSMStates.NormalPage_FocusItem or 
            CurrentState == ForgeConst.ControllerFSMStates.NormalPage_NoFocus then 
    ...
    elseif CurrentState == ForgeConst.ControllerFSMStates.NormalPage_FocusSort then 
        ...
        elseif InKeyName == Const.GamepadRightThumbstick then
            IsEventHandled = true   -- 禁止右摇杆聚焦到Tab上
        end
    ...
    elseif CurrentState == ForgeConst.ControllerFSMStates.PathPage_Normal then 
...
function WBP_ForgeMain_C:UpdateSubTabReddot(TabIdx, SubTabIdx)
    ...
    -- 如果当前二级Tab不存在，直接返回
    if self.TabIndex ~= TabIdx or not self.TabIdx2SubTabType[TabIdx] or not self.TabIdx2SubTabType[TabIdx][SubTabIdx] then 
        return
    end
...
function WBP_ForgeMain_C:InitTabContent()
    ...
    for TabIdx = 1, #DataMgr.ForgeTab do 
        ...
        table.insert(AllTabInfo, {Text=GText(TabData.TabName),TabId = TabIdx,IconPath = TabData.Icon})
    end
...
function WBP_ForgeMain_C:GetMaxItemNumPerPanel()
    ...
    if self.ForgeContent:GetNumItems() > 0 then  
        local ForgeItemWidgets = self.ForgeContent:GetDisplayedEntryWidgets()
        if ForgeItemWidgets:Length() ~= 0 then 
...
function WBP_ForgeMain_C:AddSingleItemToContent(DraftInfo, IsEmpty)
    ...
    if IsEmpty then 
        ...
        Obj.OnReceiveFocus = function(Widget)
            self:OnForgeItemReceiveFocus(Widget)
        end
        Obj.OnLostFocus = function(Widget)
            self:OnForgeItemLostFocus(Widget)
        end
        Obj.OnNavigateUp = function(Widget)
            self:OnForgeItemNavigateUp(Widget)
        end 
        Obj.OnNavigateDown = function(Widget)
            self:OnForgeItemNavigateDown(Widget)
        end
        Obj.OnMenuOpenChanged = function(bIsOpen)
            self:OnMenuOpenChanged(bIsOpen)
        end
        Obj.OnItemWidgetClicked = function(Widget)
...
function WBP_ForgeMain_C:BeginAddForgeContentItem(DraftInfos)
    ...
    if #DraftInfos <= 0 then return end
...
function WBP_ForgeMain_C:OnForgeContentEntryInit(Item, Widget)    
    Widget.EventStartProduce = function() self:OnDraftBtnStartClicked(Item.ID) end
    Widget.EventCancelProduce = function() self:OnDraftBtnCancelClicked(Item.ID) end
    Widget.EventShowPath = function() self:OnDraftShowPath(Item.ID) end
...
function WBP_ForgeMain_C:OnDraftBtnStartClicked(DraftId)
    if self.DisableBtnStartClicked then return end
...
------------------------ 跳转定位 -------------------------
-- 导航到目标Tab，不会触发分帧加载, 不会触发手柄自动聚焦
function WBP_ForgeMain_C:NavigateToTab(TabIndex, SubTabIndex)
    if self.Tab.CurrentTab == TabIndex then return end
    self:SetNotDelayAddListItem(true)
...
function WBP_ForgeMain_C:ShowStartBatchProduceConfirmWindow(DraftId, co)
    ...
    CommonDialogParams.RightCallbackFunction = function(_, PackageData)
...
function WBP_ForgeMain_C:HandleShowSecPasswordWindow(DraftId, Count, CostItemList)
    ...
    if not HasLockedMod then return true end
...
function WBP_ForgeMain_C:ShowAccerateProduceConfirmWindow(DraftId)
    ...
    local AccelerateCostType = DataMgr.GlobalConstant["AccelerateCostType"].ConstantValue -- 加速设计稿消耗的货币类型（Resource ID）
...
function WBP_ForgeMain_C:OnClose()
    if self:IsAnimationPlaying(self.In) then return end
...
function WBP_ForgeMain_C:OnModFilterChanged(SelectedItems, ItemDatas)
    self.CommonFilterData = {
        FilterSelectedItems = SelectedItems,
        FilterItemDatas = ItemDatas,
    }
...
function WBP_ForgeMain_C:OnConvertClicked()
    ...
    -- local Avatar = GWorld:GetAvatar()
    -- if not Avatar:CheckUIUnlocked("HardBoss") then
    --     UIManager(self):ShowUITip(UIConst.Tip_CommonToast, GText(DataMgr.UIUnlockRule.HardBoss.UIUnlockDesc))
    --     return
    -- end
