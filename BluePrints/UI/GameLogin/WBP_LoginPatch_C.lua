...
function M:Construct()
  ...
  --  GameMode.AssetDownloadCompleteDelegate:Add(self, self.OnAssetDownloadComplete)
...
---@param UpdateEvent EUpdateEvent
...
function M:ShowOptionPatchPopUI(OptionalAssetsSize)
    self:ShowPatchPopUI(string.format(GText("UI_PATCH_OPTIONALPATCH"), string.format("%.2f", OptionalAssetsSize / 1024 / 1024) .. "MB"), true, true,
    function()
        ---@type AHotUpdateGameMode
        local GameMode = UGameplayStatics.GetGameMode(self)
...
function M:ShowDownloadBasepakUI()
    self:ShowPatchPopUI(GText("UI_PATCH_NEWPAK"), true, false,
    function()
...
function M:OnPatchFailed(UpdateEvent)
    if UpdateEvent == EUpdateEvent.GetRemoteVersionFailed then
        self:ShowPatchPopUI(GText("UI_PATCH_GETREMOTEVERSIONFAILED"), true, true, 
        function()
            ---@type AHotUpdateGameMode
            local GameMode = UGameplayStatics.GetGameMode(self)
    ...
    elseif UpdateEvent == EUpdateEvent.GetRemotePakListFailed then
        self:ShowPatchPopUI(GText("UI_PATCH_GETREMOTEPAKLISTFAILED"), true, true,
        function()
            ---@type AHotUpdateGameMode
            local GameMode = UGameplayStatics.GetGameMode(self)
    ...
    elseif UpdateEvent == EUpdateEvent.DownloadFailed then
        self:ShowPatchPopUI(GText("UI_PATCH_DOWNLOADFAILED"), true, true,
        function()
            ---@type AHotUpdateGameMode
            local GameMode = UGameplayStatics.GetGameMode(self)
