...
---@note 通用道具格子
---@type Common_Item_subsize_PC_C|TimerMgr
local WBP_CommonItemSubsize_C = Class({"BluePrints.Common.TimerMgr", "BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_CommonItemSubsize_C:Construct()
    ...
    --self:BindToAnimationFinished(self.Click,{self,self.OnClickAnimFinished})
    self:BindToAnimationFinished(self.Selected_Loop,{self,self.OnSelectedLoopAnimFinished})
    self:BindToAnimationFinished(self.UnHover,{self,self.OnUnHoverAnimFinished})
...
function WBP_CommonItemSubsize_C:Destruct()
    ...
    --self:UnbindFromAnimationFinished(self.Click,{self,self.OnClickAnimFinished})
    self:UnbindFromAnimationFinished(self.Selected_Loop,{self,self.OnSelectedLoopAnimFinished})
    self:UnbindFromAnimationFinished(self.UnHover,{self,self.OnUnHoverAnimFinished})
    self.ItemDetails_MenuAnchor.ItemDetailsMenuAnchor.OnMenuOpenChanged:Remove(self,self.OnMenuOpenChanged)
...
function WBP_CommonItemSubsize_C:OnListItemObjectSet(Content)
    ...
    if (Content.StuffType ~= nil and Content.StuffType ~= "EmptyGrid") then
        self:SetStuffStyleByStateTag(Content)
    end
    self.Content = Content
    self:InitVariables(Content)
    Content.UI = self
   -- DebugPrint("WBP_CommonItemSubsize_C::OnListItemObjectSet::Normal")
...
function WBP_CommonItemSubsize_C:ShowAddIcon(IsShow)
    if(IsShow)then
        ...
        self:BindToAnimationFinished(self.Empty_Click,{self,self.OnEmpty_ClickFinished})
    else
...
function WBP_CommonItemSubsize_C:RefreshStuffNodeStyle(Content)
    ...
    if (self.StuffType == "Mod") then
        ...
        -- self.GradeLevel_Sell:SetVisibility(UE4.ESlateVisibility.Collapsed)
...
---@type Content Common_Item_subsize_PC_Content_C
...
function WBP_CommonItemSubsize_C:UpdateRewardItem(Content)
    ...
    -- self:SetGradeLevel(1)
    self:SetIcon(Content.Icon)
    -- self:SetName(GText(Content.StuffName))
    self:SetCount(Content.Count)
    self:SetRarity(Content.Rarity)
end
...
function WBP_CommonItemSubsize_C:UpdateModItem(Content)
    ...
    local ModDataInfo, ModPolarity, ModCost,Mod = nil, nil, nil, nil
    ...
    UResourceLibrary.LoadObjectAsync(self, ModDataInfo.Icon, {self,function(_,Icon)
        if not Icon then
    ...
    if (Content.StuffType == "Mod") then
        self:SetIsEquipped(Content.IsEquipped)
        --- fix 2024 10.31 背包迭代，要求Mod不显示同卡，原来名字的地方改成显示数量
...
--------设置MOD!!!!强化等级，函数名SetStars是历史原因----------
function WBP_CommonItemSubsize_C:SetStars(CurrentLevel, bUsePlus)
    if bUsePlus == nil then bUsePlus = true end
...
function WBP_CommonItemSubsize_C:SetStarsInBagSell(Content, bUsePlus)
    ...
    if (ModDataInfo == nil or self.StarWidgetInSell == nil) then
        return
    end
...
-- function WBP_CommonItemSubsize_C:SetStar_Sell(CurrentLevel)
--     if not CurrentLevel or CurrentLevel <= 0 then
--         self.Star_Sell:SetVisibility(UE4.ESlateVisibility.Collapsed)
--         return
--     end
--     if(not self.StarWidget)then
--         self.StarWidget = Utils.UIManager(self):CreateWidget('/Game/UI/UI_PC/Common/Item_Subsize_Widget/Common_Item_Subsize_Star_PC.Common_Item_Subsize_Star_PC',false)
--         self.Star_Sell:ClearChildren()
--         self.Star_Sell:AddChild(self.StarWidget)
--     end
--     self:UpdateStarWidget(CurrentLevel,true)
--     self.Star_Sell:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
-- end

---设置是否选中
---@param bPlayClick boolean 是否播放选中的动画,默认值true,true则调用该接口会播放一次Click, false则将动画加速10000倍直接到Click的最后一帧
function WBP_CommonItemSubsize_C:SetIsSelected(IsSelected, bPlayClick)
    if bPlayClick == nil then bPlayClick = true end
...
function WBP_CommonItemSubsize_C:SetRarity(Rarity)
    if not Rarity or Rarity < 1 or Rarity > 5 then
        ...
        if (self.NameWidget and self.Name:GetVisibility() == UE4.ESlateVisibility.SelfHitTestInvisible) 
        or (self.LevelWidget and self.Level:GetVisibility() == UE4.ESlateVisibility.SelfHitTestInvisible)
        or (self.NumWidget and self.Num:GetVisibility() == UE4.ESlateVisibility.SelfHitTestInvisible) then
            self.VerticalBox_NameLevelNum:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        ...
        --self.Img_Line:SetBrushResourceObject(LoadObject('/Game/UI/UI_PNG/Common/Deco/Deco_Quality_' .. UIConst.RarityColorName[6] .. '.Deco_Quality_' .. UIConst.RarityColorName[6]))
        return
    end
    self.VerticalBox_NameLevelNum:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    self.Spacer_Switch:SetVisibility(UE4.ESlateVisibility.Collapsed)
...
function WBP_CommonItemSubsize_C:InitAnimationVariables()
    self.InteractionAnimations = {  [self.Normal] = "Normal",
                                    [self.hover] = "hover",
...
function WBP_CommonItemSubsize_C:OnAnimationFinished(Animation)
    ...
    if(self.InteractionAnimations[Animation])then
        if(not self:IsAnimationPlaying(self.CurAnim))then
...
function WBP_CommonItemSubsize_C:OnHoveredChanged(bHovered)
    --DebugPrint("WBP_CommonItemSubsize_C::OnHoveredChanged::hover",bHovered)
    self.bHovered = bHovered
    if(self.IsSelected)then
        return
    end
    if(bHovered)then
        if(self.CurAnim == self.hover)then return end
...
function WBP_CommonItemSubsize_C:OnPressedChanged(bPressed)
    --DebugPrint("WBP_CommonItemSubsize_C::OnPressedChanged",bPressed)
...
function WBP_CommonItemSubsize_C:OnSelectedChanged(bSelected, bPlayClick)
    --DebugPrint("WBP_CommonItemSubsize_C::OnSelectedChanged",bSelected, bPlayClick)
    if(bSelected)then
        if(bPlayClick)then
...
-- function WBP_CommonItemSubsize_C:OnClickAnimFinished()
--     if(self.IsSelected)then
--         self:TryPlayAnimation(self.Selected_Loop,0,0,EUMGSequencePlayMode.Forward,1,true)
--     end
-- end
...
function WBP_CommonItemSubsize_C:OnMouseEnter(MyGeometry,MouseEvent)
    if not self.bInteractive then return end
...
function WBP_CommonItemSubsize_C:OnMenuOpenChanged(IsOpened)
    ...
    if(self.ItemDetails_MenuAnchor.bAllowHover) then return end
    if(IsOpened)then
...
function WBP_CommonItemSubsize_C:OnTouchStarted(MyGeometry, InTouchEvent)
    CommonUtils:CloseGuideTouchIfExist(self) -- todo 临时处理
    if not self.bInteractive then return UE4.UWidgetBlueprintLibrary.Unhandled() end
    if(self._OnMouseEnter) then
        self._OnMouseEnter(self, self.Content)
    end
...
---@field self.Parent table CreateDragWidget 执行时的接收对象
...
function WBP_CommonItemSubsize_C:OnDragLeave(PointerEvent, Operation)
    ...
    if Operation.Tag ~= "WBP_CommonItemSubsize_C" then return end
...
function WBP_CommonItemSubsize_C:ShowContentWarning(Text)
    if not self.Conflict then return end
    if not Text then
...
function WBP_CommonItemSubsize_C:ShowModStar(Mod)
    if not self.List_ModStar or not self.ModStar then return end
...
function WBP_CommonItemSubsize_C:SetGetItemInfo(Rarity,Count)
    -- local Name = Content.UnitName
    -- self.Name_GetWidget = UIManager(self):CreateWidget('/Game/UI/UI_PC/Common/Item_Subsize_Widget/Common_Item_Subsize_Name_PC.Common_Item_Subsize_Name_PC')
    -- self.Name_Get:ClearChildren()
    -- self.Name_Get:AddChild(self.Name_GetWidget)
    -- self.Name_GetWidget.Name:SetText(Name)
    self.Name:SetVisibility(ESlateVisibility.Collapsed)
    -- self.Name_Get:SetVisibility(ESlateVisibility.Visible)
    self:SetCount(Count)
