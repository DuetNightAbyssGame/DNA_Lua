...
function M:SetTargetNew()
    ...
    if self:IsWeaponSkin() then
        if AvatarUtils:GetTargetDataStatistics(Avatar,"WeaponSkin",self.TargetId) == 1 then
            self.New_Skin:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        end
    else
        if AvatarUtils:GetTargetDataStatistics(Avatar,"Weapon",self.TargetId) == 1 then
...
function M:PlayInAnim()
    ...
    -- if self.CurShowIndex == CommonConst.GachaOneResult and not self:IsWeaponSkin() then
    --     self:PlayAnimation(self.In_Start)
    -- end
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if InKeyName == "Escape" and not self.bSpaceBarSkip then
