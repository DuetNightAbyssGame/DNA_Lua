...
---@type WBP_Com_SortDown_C
local M = Class("BluePrints.UI.BP_UIState_C")


---@param SortBy_List  array 排序依据数组
---@param FocusKeyName string 监听手柄按钮的按键 如LS、RS、X、Y……
---@param Parent UWidget 父UI，用于退出当前焦点时返回父焦点，需实现BP_GetDesiredFocusTarget接口
---相关接口详见https://herogames.feishu.cn/wiki/ZQu0w9aGOiVuVukq3IFcolTXnUf 代码接口部分
...
function M:Construct()
    ...
    --self.Btn_Filter_List:SetNavigationRuleCustom(self.Button_List_Rule, {self, self.NavigatePos})
...
function M:OnBtn_Filter_List_Released()
    -- DebugPrint("OnBtn_Filter_List_Released")
    -- self:SetFocus()
end
    ...
    if(self.SortBy_List)then
        if(self.IsListViewOpened)then
            ...
            self:AddDelayFrameFunc(function()
                ...
                --self.Btn_Filter_List:SetNavigationRuleExplicit(self.Button_List_Rule, self.List)
            end, 3)
        end
    else
        self.Filter_List:SetVisibility(UIConst.VisibilityOp["Collapsed"])
...
function M:OnListClosed()
    if(self.SortBy_List)then
        if(self.IsListViewOpened)then
            ...
            self.IsListViewOpened = false
            --self.Btn_Filter_List:SetNavigationRuleBase(self.Button_List_Rule, EUINavigationRule.Stop)
        end
    else
        self.Filter_List:SetVisibility(UIConst.VisibilityOp["Collapsed"])
    end
...
function M:OnFocusLost(InFocusEvent)
    -- DebugPrint("OnFocusLost", self:HasFocusedDescendants())
...
function M:UpdateGamePadFocus()
    ...
    if not self.bAddedToFocusPath then return end
    if self.IsListViewOpened then
        if (self.SelectedItem and self.SelectedItem.Entry) then
            self.List:BP_NavigateToItem(self.SelectedItem)
            return
    ...
    self.List:BP_CancelScrollIntoView()--OnKeyDown在刷新输入设备的后面触发，BP_NavigateToItem设置焦点会在下一帧,因此位置可能不正确,先取消下焦点
    self.Btn_Filter_List:SetFocus()
end
