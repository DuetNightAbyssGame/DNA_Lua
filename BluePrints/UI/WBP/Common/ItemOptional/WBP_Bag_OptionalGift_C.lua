...
function M:InitAllOptionalItemsInfo()
    ...
    for Index, ItemInfo in ipairs(self.OptionalItemsList) do
        ...
        Content.OnMenuOpenChangedEvents = {Obj = self, Callback = self.OnMenuOpenChangedEvents}
        -- Content.PackCount = ItemInfo.PackCount
        self.List_Gift:AddItem(Content)
        if not self.CurrentChooseInfo then
            self.CurrentChooseInfo = {ResourceId = ItemInfo.ResourceId, OptionalId = ItemInfo.OptionalId, ConsumeCount = 1}
...
function M:ChangeChooseClickCallback(bSelectState, Count)
    ...
    -- end
    -- self.CurrentChooseInfo.ConsumeCount = Count
    ...
    --     -- self:BroadcastDialogEvent("UpdateDialogTipText", { Tips = {string.format(GText("UI_Consumable_Effect"), GText(ResourceName), 
    --     --                             self.CurrentChooseInfo.ChooseName)}, DialogItemIndex = 1, bShowTip = true})
    -- else
    --     -- 取消选中
    --     self.CurrentChooseWidget = nil
...
function M:OnContentKeyDown(MyGeometry, InKeyEvent)
    ...
    if not IsEventHandled then
        ...
        elseif (InKeyName == UIConst.GamePadKey.FaceButtonRight) then
            if self.GamePadMode == "CheckItems" then
                self:SetFocus()
    ...
    --     DebugPrint("ayff test LeftTrigger")
