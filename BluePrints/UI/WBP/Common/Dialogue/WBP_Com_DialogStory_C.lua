...
function M:Initialize(Initializer)
    ...
    self.ConfigData = nil
    self.OpenKey = "Tab"
    self.IsCanCloseByHotKey = nil     -- 是否可以通过热键关闭
...
function M:OnLoaded(...)
    ...
    -- self:ListenForInputAction("ItemDetail", UE4.EInputEvent.IE_Pressed, true, {self, self.Close})
end
...
function M:FillWithData()
    ...
    elseif (self.ItemType == "DetectiveAnswer") then
        ...
        if (DetectiveAnswerConfigData ~= nil) then
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        if ((InKeyName == self.OpenKey and self.IsCanCloseByHotKey) or InKeyName == "Escape") then
            self:PlayOutAnim()
        end
    end
