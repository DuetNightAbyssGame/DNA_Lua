---@type WBP_BattlePass_PurchaseDialog_C
...
function M:OnContentKeyDown(MyGeometry, InKeyEvent)
    ...
    local IsEventHandled = nil
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        IsEventHandled = self:OnGamePadDown(InKeyName)
    end
    if IsEventHandled then return true end
...
function M:OnGamePadUp(InKeyName)
    ...
    elseif InKeyName == Const.GamepadFaceButtonDown then
        ...
        if self.FocusState == STATE_GAMEPAD_FOCUS.FOCUS_DIALOG then
            ...
            --     self.OKCallback(self.OKCallbackObj, {["BattlePass"] = self:PackageData()})
