...
--- @class MonthCardController :Controller
...
function M:TryPurchaseMonthCard()
    if self.WaitPayTag then return end
    local MonthCard = MonthCardModel:GetNowMonthCard()
    ...
    Avatar:MonthlyCardRequestPay(function(ret, OrderId, CallbackUrl, GoodsId)
        DebugPrint("MonthCard RequestPay", MonthCard.CardID, GoodsId, 
            ret, OrderId, "PurchaseOrder: ", PurchaseOrder)
        ...
        DebugPrint("MonthCard HeroUSDKSubsystem():HeroSDKPay", MonthCard.CardID, PayGoods, GameRoleInfo, 
            OrderId, "PurchaseOrder: ", PurchaseOrder)
        ...
        Avatar:ListenPayFinish(OrderId, function(Result, OrderId, GoodsId, ShopItems)
            ...
            self:FinishPurchase(PurchaseOrder)
            DebugPrint("MonthCard OnPayFinish", MonthCard.CardID, PayGoods, GameRoleInfo, OrderId, 
                GoodsId, Result, "PurchaseOrder: ", PurchaseOrder)
...
function M:FinishPurchase(PurchaseOrder)
    if PurchaseOrder ~= self.PurchaseOrder then return end
    self.WaitPayTag = false
end
...
local function MergeReward(MergeReward, TotalReward)
    local RewardCount = 0
    if not MergeReward then return RewardCount end
...
function M:TryPopUpCacheReward()
    ...
    self:RealDisplayPurchaseRewards(PurchaseReward,
    function()
        self:TryDisplayMonthCardPop(DailyReward)
    end)
end
...
function M:TryDisplayMonthCardPop(DailyReward)
    ...
    --     DailyReward = MonthCardModel.DailyRewardCache
    -- end
    if not DailyReward then return end
    local Reward = {}
    local RewardType = DataMgr.RewardType
