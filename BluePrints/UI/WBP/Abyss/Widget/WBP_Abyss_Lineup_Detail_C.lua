...
function WBP_Abyss_Lineup_Detail:ClearAllSlots(bInit)
    ...
    -- 用于判断是否有槽位被清空
    local ret = false
    local NewSelectedSlot = nil
    for _, Slot in pairs(self.Slots) do
        if Slot:Clear() then
            ret = true
            --若清空时角色的武器槽正处于被选中状态，则转而选中该角色(仅限阵容配置界面)
...
local SlotOrder = {[1] = "Char", [2] = "Melee", [3] = "Ranged",
                        [4] = "Phantom1", [5] = "PhantomWeapon1", 
                        [6] = "Phantom2", [7] = "PhantomWeapon2"}
...
function WBP_Abyss_Lineup_Detail:OnModBtnClicked()
    ...
    for EName, SlotName in ipairs(SlotOrder) do
        ...
        if Slot and not Slot.IsEmpty then
            ...
            if SlotName == "Phantom1" or SlotName == "Phantom2" then
            ...
            if SlotName == "PhantomWeapon1" or SlotName == "PhantomWeapon2" then
    ...
    if ModUI then
        if Tag == "Melee" or Tag == "Ranged" then
            Tag = Tag.."Weapon"
        end
