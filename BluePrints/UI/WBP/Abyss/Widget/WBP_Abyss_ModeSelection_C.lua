...
local WBP_Abyss_ModeSelection_C = Class({"BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_Abyss_ModeSelection_C:Init(Root, Index, AbyssId, IsLocked, FirstSelectedAbyssId)
    ...
    -- if AbyssType == 1 then
    --     IconPath = "Texture2D'/Game/UI/Texture/Static/Atlas/Abyss/T_Abyss_IconBasic.T_Abyss_IconBasic'"
    -- elseif AbyssType == 2 then
    --     IconPath = "Texture2D'/Game/UI/Texture/Static/Atlas/Abyss/T_Abyss_IconRotation.T_Abyss_IconRotation'"
    -- elseif AbyssType == 3 then
    --     IconPath = "Texture2D'/Game/UI/Texture/Static/Atlas/Abyss/T_Abyss_IconEndless.T_Abyss_IconEndless'"
    -- end
    -- if IconPath then
        -- local BrushSizeX = self.Icon.Brush.ImageSize.X
        -- local BrushSizeY = self.Icon.Brush.ImageSize.Y
        -- local image = LoadObject(IconPath)
        -- if image then
        --     self.Icon:SetBrushFromTexture(image,true)
        --     self.Icon:SetBrushSize(FVector2D(BrushSizeX, BrushSizeY))
        -- end
        -- local IconImgMat = self.Icon:GetDynamicMaterial()
        -- IconImgMat:SetTextureParameterValue("Mask", self["Icon_"..AbyssType])
        -- local BGImgMat = self.BG:GetDynamicMaterial()
        -- BGImgMat:SetTextureParameterValue("MainTex", self["BG_"..AbyssType])
...
function WBP_Abyss_ModeSelection_C:InitNavigation()
    self.Btn_Click:SetNavigationRuleCustom(EUINavigation.Up,{self,self.TryChangeSelectedTab})
    self.Btn_Click:SetNavigationRuleCustom(EUINavigation.Down,{self,self.TryChangeSelectedTab})
    self.Btn_Click:SetNavigationRuleCustom(EUINavigation.Right,{self.Root,self.Root.FocusToFirstMission})
end
