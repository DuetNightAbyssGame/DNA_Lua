...
local WBP_Abyss_Lineup_List_C = Class({"BluePrints.UI.BP_EMUserWidget_C","BluePrints.Common.TimerMgr"})
...
function WBP_Abyss_Lineup_List_C:Construct()
    ...
    self:AddLSFocusTarget(nil, {self.Sort})
    self:InitKeyInfo()
...
function WBP_Abyss_Lineup_List_C:OnListKeyUp(MyGeometry, InKeyEvent)
    ...
    -- DebugPrint("lhr1@WBP_Abyss_Lineup_List_C:OnKeyUp", IsHandled)
    return IsHandled
end
...
function WBP_Abyss_Lineup_List_C:Init(Parent,Params)
    ...
    -- if(self.AllFilterContentObj)then
    --     self.Tab_Sub:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    --     self:OnFilterListItemClicked(self.AllFilterContentObj)
    -- else
    --     self.FilteredContents = self.AllItemContents
    --     self.Tab_Sub:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- end
...
--由于最后一个Item右侧可能是空Item，无法自动导航到右侧Tab栏，需手动设置
