...
---@type WBP_Activity_FeinaEvent_LevelSelect_P_C
...
function M:OnLoaded(...)
    ...
    self.LevelTable = {self.Level01, self.Level02, self.Level03, self.Level04}
    ...
    self.Reward.Key_Controller:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Y", Owner = self} }})
    ...
    if IsOpen then
        if self.EventMainConfig.PermanenEventTime and self.EventMainConfig.PermanenEventTime >= TimeUtils.NowTime() then
            ...
            if not IsPassDungeon1 then
                ...
                if FeinaEventConfig.DungeonId[2] then
