...
--
require "UnLua"
---@type WBP_Activity_FeinaEvent_LevelDetail_P_C
...
function M:Construct()
    ...
    if self.Controller_L then
        self.Controller_L:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "LB", Owner = self} }})
        self.Controller_R:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "RB", Owner = self} }})
        self.Controller_Tab_L:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "LT", Owner = self} }})
        self.Controller_Tab_R:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "RT", Owner = self} }})
        self.Controller_Reward:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "LS", Owner = self} }})
        self.FeinaEventBtn.Key_GamePad:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "X", Owner = self} }})
    end
...
function M:GoToDungeon()
    ...
    Avatar:EnterEventDungeon(function() self.RootPanel:DirectlyClose() end, DungeonId, nil, EventId)
...
function M:OnBtnClick1()
    if #self.DungeonId == 1 or self.Tab01.IsClick then
        return 
    end
    self:ShowDungeon1()
...
function M:OnBtnClick2()
    if #self.DungeonId == 1 or self.Tab02.IsClick then
        return
    end
...
function M:InitLevelDetail(DungeonId, IsPassDungeon1, IsPassDungeon2, Index)
    ...
    self.Panel_L:SetVisibility(Index == 1 and UE4.ESlateVisibility.Collapsed or UE4.ESlateVisibility.SelfHitTestInvisible)
    self.Panel_R:SetVisibility(Index == 4 and UE4.ESlateVisibility.Collapsed or UE4.ESlateVisibility.SelfHitTestInvisible)
    self:ShowDungeon1()
...
function M:InitLevelDetailTap()
    self.Panel_Tab:SetVisibility(#self.DungeonId == 1 and UE4.ESlateVisibility.Collapsed or UE4.ESlateVisibility.SelfHitTestInvisible)
    self.Tab01.IsClick = true
    self.Tab01.Btn_Click:SetVisibility(UE4.ESlateVisibility.Collapsed)
    self.Tab01:PlayAnimation(self.Tab01.Click)
    if not self.IsPassDungeon1 then
