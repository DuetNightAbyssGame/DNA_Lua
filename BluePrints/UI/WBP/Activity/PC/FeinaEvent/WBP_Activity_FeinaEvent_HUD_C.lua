...
---@type WBP_Activity_FeinaEvent_Hud_P_C
...
function M:InitUIInfo(Name, IsInUIMode, EventList, ...)
    ...
    self:ListenForInputAction("ChargeBullet", EInputEvent.IE_Pressed, false, {self, self.ChangeColor})
...
function M:SetGamepadIcons()
    ...
    self.Controller_Switch:CreateSubKeyDesc({
        KeyInfoList = {
            {Type = "Img", ImgShortPath = ReloadGamepadList[1], Owner=self},
            {Type = "Img", ImgShortPath = ReloadGamepadList[2], Owner=self},
        },
        Type = "Add",
        Desc = GText("FeinaEvent_DungeonColor_Switch"),
    })
    ...
    self.Controller_Shoot:CreateCommonKey({
        KeyInfoList = {
            {Type = "Img", ImgShortPath = FireGamepadList[1], Owner=self},
        },
        Desc = GText("UI_EventHud_Shoot"),
    })
end
