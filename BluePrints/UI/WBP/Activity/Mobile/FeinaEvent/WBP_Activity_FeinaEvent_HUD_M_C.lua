...
---@type WBP_Activity_FeinaEvent_Hud_M_C
...
function M:Construct()
    ...
    self.PassiveEffectId = 18011 -- 固定写死
    self.IsLoading = true
    self.CollectLevel = 0
end
...
function M:OnCollectFeinaStar(StarNum)
    ...
    for Index, Target in ipairs(self.CollectTargets) do
        if Target <= StarNum and Index > self.CollectLevel then
...
function M:InitButtons()
    ...
    if self.Btn_Shoot then
        ...
        self:AddMovedSubTouchItem("FeinaRangedAtk", self.Btn_Shoot.Joystick, nil, {Down=self.Btn_Shoot.ButtonFireDown, Move=self.Btn_Shoot.ButtonFireMove, Up=self.Btn_Shoot.ButtonFireUp},
                {Type="Circle", Param={Radius=self.Btn_Shoot.CircleLimitArea, NeedReset=false}, TouchTimes=-1, NeedResetPos=true})
