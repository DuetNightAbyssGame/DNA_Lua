...
function ActivityFortRewardModel:SetRewardParams(Type)
    ...
    for TabIndex,Info in ipairs(self.LevelsInfo) do
        ...
        Data.ReceiveAllParam = {RewardModel = self, Type = Info.DungeonId}
...
function ActivityFortRewardModel.GetRewards(RewardItem,Content)
    ...
    if Avatar then
        local Callback = function (Ret,Rewards)
            ...
            --Content.ConfigData.ReceiveParm.RewardModel:RefreshReddotInfo(Content.ConfigData.Type, Content.ConfigData.Index)
    ...
    if Avatar then
        ...
        if RewardsGotInfo then
            ...
            for Index,Score in pairs(LevelInfo) do
                if not RewardsGotInfo[Score] and MaxScore >= Score then
                    return true
                end
            end
        end
...
-- function ActivityFortRewardModel:RefreshReddotInfo(Type,Index)
...
--         if next(CacheDetail[Type]) == nil then
--             CacheDetail[Type] = nil
--         end
--         ReddotManager.DecreaseLeafNodeCount("PaotaiEventReward")
--     end
-- end
