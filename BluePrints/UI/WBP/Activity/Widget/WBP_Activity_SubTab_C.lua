...
function M:OnListItemObjectSet(Content)
    ...
    ActivityReddotHelper.AddReddotListenByTabId(self.TabId, 
        {Obj=self, Func = function(self, Count, RdType,Name)
            if Count == 0 then
    ...
    -- 设置往右的导航规则
    self:SetNavigationRuleCustom(EUINavigation.Right, {self,self.OnSubTabNavigationRight})
...
function M:InitListItemView(Content)
    ...
    UE.UResourceLibrary.LoadObjectAsync(self, Content.BgPath, {self, function(self, IconObj)
...
function M:Btn_Press()
    ...
    if self.bMobile then
        self:BindToAnimationFinished(self.Press, {self, function()
            if (self.IsTabSelected) then return end
            self:PlayAnimation(self.Normal)
        end})
    end
end
