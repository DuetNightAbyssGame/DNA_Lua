...
function M:ResetInfo()
    -- self.IsInGamePadViewMode = false
end
...
function M:RefreshItemDetails(WalnutInfo)
    ...
    if (WalnutConfigData) then
        ...
        -- self.Text_Name:SetText(GText(WalnutTypeConfigData.Name)..NumberOrderString)
...
function M:RefreshIconAsset(bIsAsync, WalnutTypeConfigData, WalnutId)
    ...
    if (bIsAsync) then
        UE.UResourceLibrary.LoadObjectAsync(self, WalnutTypeConfigData.Icon, {self, function(self, IconObj)
                        if(IconObj)then
                            self.Icon_Walnut:SetBrushResourceObject(IconObj)
                        end
                    end})
        --IconDynaMaterial:SetTextureParameterValue("IconMap",LoadObject(Icon))
        -- self.Walnut_Number:InitWalnutNumber(WalnutId)
        -- self.HB_Number:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    else
...
function M:GenerateRewardShowInfo(WalnutConfigData, Rarity, Index, ItemWidget)
    ...
    if (RewardConfigData ~= nil) then
        local RewardData = {Id=RewardId, Icon=RewardConfigData.Icon, Type=RewardType, Count=RewarCount, Rarity=RewardConfigData.Rarity}
        ...
        for index, TargetWidget in ipairs(self.AllMethodSubWidgetList) do
            if (TargetWidget) then
                ...
                if (AllCanNavigateAccessCount == 1) then
                    -- 只有一个可以聚焦的并且跳转获取途径
                    TargetWidget:SetNavigationRuleCustom(EUINavigation.Up, {self, self.DoCustomNavigationWithMethod})
