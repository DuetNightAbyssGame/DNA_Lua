...
function M:Initialize(Initializer)
    ...
    self.ItemId2GridIndex = {}                 -- ItemId对应的GridIndex
...
function M:RefreshBaseInfo()
    ...
    -- 这里处理self.WidgetSwitcher_State的状态
    ...
    -- self.List_Item.BP_OnItemClicked:Add(self, self.OnListSelectStuffClicked)
    -- 按钮音效相关
...
function M:TryToSaleWalnut()
    ...
    for ItemUuid, ItemData in pairs(self.NeedDealWithStuffData) do
        ...
        -- StuffServerData = PlayerAvatar.Resources[math.tointeger(ItemUuid)]
        -- StuffServerData = PlayerAvatar.Walnuts[math.tointeger(ItemUuid)]
        if (StuffServerData ~= nil) then
            -- StuffServerData.Id = StuffServerData.Id or StuffServerData.ResourceId
...
function M:FillWithStuffSellData(StuffData)
    ...
    if self.NeedDealWithStuffData[StuffData.Uuid] ~= nil then
        ...
        StuffData.GridIndex = self.CurStuffCountInList + 1
        StuffData.StateTagInfo = {Name="InSelectList", ExtraData={AddStuffCount, StuffData.StuffCount, StuffData.Price, StuffData.CoinId, self.RemoveWalnutItemInList}}
        ...
        -- StuffObj.OnMouseButtonUpEvents = {Obj = self, Callback = self.OnListSelectStuffClicked}
...
function M:RefreshCoinInfo()
    ...
    for k, v in pairs(self.AllTypeCoinInfo) do
        if (v > 0) then
            ...
            local RewardObject = {}
            RewardObject.Id = k
            RewardObject.Icon = ItemUtils.GetItemIconPath(k, "Resource")    -- 临时
            RewardObject.ParentWidget = self
            RewardObject.ItemType = "Resource"                              -- 临时
            RewardObject.Rarity = RewardInfo.Rarity or 1
            RewardObject.IsShowDetails = true
            RewardObject.UIName = "WalnutSelectToList"
...
function M:OnInputStuffNum(NewValue, OldValue)
    if (self.CurSelectStuffContentInList == nil) then return end
...
function M:SliderChangeCallback(Value)
    ...
    -- self.ParentWidget:RefreshItemInfo(Value, self.CurSelectStuffContentInList.StuffId or self.CurSelectStuffContentInList.Id)
    self:RefreshCoinInfo(StuffCoinId)
...
function M:OnClickToMinusStuff()
    ...
    -- self.ParentWidget:RefreshItemInfo(Num, StuffId)
    self:RefreshCoinInfo(StuffCoinId)
...
-- function M:OnUpdateWalnutItemByAction(OpAction, ErrCode, ...)
    ...
    -- if (OpAction == "WalnutBulkSale") then
...
--             UIUtils.ShowGetItemPageAndOpenBagIfNeeded(nil, nil, nil, AllRewards, false, self.PlayOutAnim, self, true)
--         end)
--     else
--         UIUtils.ShowGetItemPageAndOpenBagIfNeeded(nil, nil, nil, AllRewards, false, self.PlayOutAnim, self, true)
--     end
