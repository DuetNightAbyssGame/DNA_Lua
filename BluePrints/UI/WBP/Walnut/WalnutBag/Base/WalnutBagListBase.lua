...
function M:SetupListContent(ItemIndex, ItemInfo, Content)
    ...
    Content.Id = ItemId
    -- Content.Icon = TabConfigData.Icon
...
function M:OnListFillWith(FromSrc)
    ...
    for Index, ItemInfo in ipairs(self.ListDatas) do
        if FromSrc == "ShowHaveWalnutOnly" and ItemInfo.Count<= 0 then
            goto continue
        end 
        local Content = NewObject(UIUtils.GetCommonItemContentClass())
...
function M:RefreshListItem(Data, bForceAll)
    if bForceAll == nil then bForceAll = true end
...
function M:RefreshWalnutSaleItemSelect(StuffData)
    if self.BagSellState then
        if (self.DesireSaleWalnutObjList[StuffData.Id] ~= nil) then
            local SellPageMainUI = UIManager(self):GetUI("WalnutSelectToList")
            if (SellPageMainUI) then
                -- self.DesireSaleWalnutObjList[WalnutId].StateTagInfo.ExtraData[1] = self.DesireSaleWalnutObjList[WalnutId].StateTagInfo.ExtraData[1] + AddNum
