...
function WBP_Play_HardBoss_Root_C:RefreshOtherInfo()
    --self.Btn_Enter:SetKeyPanelVisibility(UE4.ESlateVisibility.Collapsed)
...
function WBP_Play_HardBoss_Root_C:InitListBossInfo()
    ...
    local function FillWrapBoxFunc()
        ...
        if ScrollBoxSize.X ==0 or EntrySize.X == 0 then
            return false
        end
        local XCount = math.floor(ScrollBoxSize.X / (EntrySize.X)) + 1
        if Index > XCount then
            return false
        end
...
function WBP_Play_HardBoss_Root_C:PlayBGInAnimation()
    if self.BossBackGround and self.BossBackGround.WBP_AreaCoop_LoadingBG then
        ...
        self.BossBackGround.WBP_AreaCoop_LoadingBG:BindToAnimationFinished(self.BossBackGround.WBP_AreaCoop_LoadingBG.In, {self, function()
    ...
    if Avatar then
        for DifficultyIndex = #BossInfo.DifficultyId, 1, -1 do
            ...
            if Avatar:CheckHardBossCondition(DifficultyId) then
                if (not self.HaveFreeChance) and Avatar.HardBoss:GetPassCount(DifficultyId) == 0 then
                    self.HaveFreeChance = true
                end
                if HighestDifficultyId == BossInfo.DifficultyId[1] then
                    if (not self.DifficultyIndexCache and DifficultyIndex > 1 and Avatar.HardBoss:GetPassCount(DifficultyId) == 0) or
                    (self.DifficultyIndexCache and self.DifficultyIndexCache < DifficultyIndex and Avatar.HardBoss:GetPassCount(DifficultyId) == 0) then
                        self.IsUpdate = true
...
function WBP_Play_HardBoss_Root_C:InitSelectTab()
	local BottomKeyInfo = {
        ...
        {
            KeyInfoList = {{Type = "Text", Text = "Esc", ClickCallback = self.Root.OnClickBack, Owner = self.Root}},
    ...
    if UIUtils.UtilsGetCurrentInputType() == ECommonInputType.Gamepad then
        ...
        -- self.Root.ComTab.Left_GamePad:SetVisibility(UE4.ESlateVisibility.Collapsed)
        -- self.Root.ComTab.Right_GamePad:SetVisibility(UE4.ESlateVisibility.Collapsed)
        -- self.Root.ComTab.WBP_Com_Tab_ResourceBar.KeyImg_GamePad:SetVisibility(UE4.ESlateVisibility.Collapsed)
...
function WBP_Play_HardBoss_Root_C:InitNormalTab()
    ...
    if UIUtils.UtilsGetCurrentInputType() == ECommonInputType.Gamepad then
        ...
        -- self.Root.ComTab.Left_GamePad:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        -- self.Root.ComTab.Right_GamePad:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        -- self.Root.ComTab.WBP_Com_Tab_ResourceBar.KeyImg_GamePad:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
