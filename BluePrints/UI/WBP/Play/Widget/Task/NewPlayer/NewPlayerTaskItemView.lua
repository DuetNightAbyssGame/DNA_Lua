    ...
    if (QuestRewards) then
        ...
        for _, ItemInfo in pairs(RewardList) do
            ...
            if ItemInfo.Type == "Char" then
                RewardObject.OnMouseButtonUpEvents = {Obj = self, Callback = function()
                    local WorldContext = GWorld.GameInstance
                    UIManager(WorldContext):LoadUINew('ArmoryDetail',{PreviewCharIds = {ItemId},         
...
function M:IsGoToSystem()
    return self.WS_Text:GetActiveWidgetIndex() == 0 and self.Switch_Btn:GetActiveWidgetIndex() == 0
end
