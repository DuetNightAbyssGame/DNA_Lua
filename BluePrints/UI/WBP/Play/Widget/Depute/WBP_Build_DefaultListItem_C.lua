...
---@type WBP_Build_DefaultListItem_C
...
function M:InitItemContent()
    ...
    local SquadIcons = {
        ...
        { Widget = self.Phantom01, DataTable = DataMgr.Char,   Id = self.SquadInfo.Phantom1Id,       SetMethod = "SetCharIcon",   EmptyMethod = "SetEmptyIcon",    Type = "Phantom",IsMiniIcon = true },
        { Widget = self.Phantom02, DataTable = DataMgr.Char,   Id = self.SquadInfo.Phantom2Id,       SetMethod = "SetCharIcon",   EmptyMethod = "SetEmptyIcon",    Type = "Phantom",IsMiniIcon = true },
        { Widget = self.Weapon01,  DataTable = DataMgr.Weapon, Id = self.SquadInfo.PhantomWeapon1Id, SetMethod = "SetWeaponIcon", EmptyMethod = "SetEmptyIcon",    Type = "Weapon" },
        { Widget = self.Weapon02,  DataTable = DataMgr.Weapon, Id = self.SquadInfo.PhantomWeapon2Id, SetMethod = "SetWeaponIcon", EmptyMethod = "SetEmptyIcon",    Type = "Weapon" },
    }
    ...
    if self.SquadInfo.Index == 0 then
        return
    end

    if not self.SquadInfo.PhantomWeapon1Id or not self.SquadInfo.PhantomWeapon2Id or not self.SquadInfo.PetId then
        local Info = {}
...
function M:SetIsSelected(IsSelected, IsAutoSelected)
    -- 若选择状态未变，或者是自动选择的情况，则不处理
    if self.IsSelected == IsSelected or IsAutoSelected then return end
...
function M:ShowMissingComponentHint()
    if not self.SquadInfo then
        ...
        --UIManager(self):LoadUINew("SquadMainUINew", nil, true)
...
function M:UpdatKeyDisplay(FocusTypeName)
    ...
    if FocusTypeName == "NotSelected" then
        local BottomKeyInfo = {
            {
                GamePadInfoList = {
