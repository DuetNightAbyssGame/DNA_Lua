...
---@type WBP_Entertainment_TopicDetail_P_C
...
function M:OnGamePadKeyDown(MyGeometry, InKeyEvent)
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if self.HandleKeyDown then return false end
...
function M:OnGamePadKeyUp(MyGeometry, InKeyEvent)
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if not self.HandleKeyDown then return false end
    if InKeyName == Const.GamepadFaceButtonUp then
...
--- 没从UIState_C继承，只是同名，所以和其他正常继承的行为可能不同
