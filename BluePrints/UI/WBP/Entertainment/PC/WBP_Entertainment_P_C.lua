...
function M:UpdateGamePadBottomInfo()
	...
	if not self.GameInputModeSubsystem:GetCurrentInputType() == ECommonInputType.Gamepad then return end
	if not self.Tab.ConfigData then return end
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
	...
	--if (not self:HasAnyUserFocus()) and (not self:HasFocusedDescendants()) then return end
...
function M:OnUpdateUIStyleByInputTypeChange(CurInputDevice, CurGamepadName)
	...
	if (CurInputDevice == ECommonInputType.Gamepad) then
		...
		--self.Key_Controller01:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
		...
		--self.Key_Controller01:SetVisibility(UIConst.VisibilityOp.Collapsed)
...
function M:OnPreviewKeyDown(MyGeometry, InKeyEvent)
	...
	if self.State == EEntertainmentState.Topic then
		if self.GetBadge:IsPanelOpened() then
			-- if self.GetBadge:OnGamePadKeyPreviewDown(MyGeometry, InKeyEvent) then
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
	...
	--- 全局返回键
	if self.HandleKeyDown then return UIUtils.Unhandled end
	...
	if self.State == EEntertainmentState.SwitchCharacter then
		if self.SwitchCharacter:OnGamePadKeyDown(MyGeometry, InKeyEvent) then
			return UIUtils.Handled
		end
	end
