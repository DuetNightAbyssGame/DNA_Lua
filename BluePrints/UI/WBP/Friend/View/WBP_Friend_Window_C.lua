...
---@type WBP_Friend_Window_C|WBP_Friend_ListBase|TimerMgr
local M = Class({
...
function M:Construct()
    ...
    local Filters = {"UI_Friend_RequestTime",}
    local CommonFunc = function(bAnimation) 
        self:RefreshList(bAnimation) 
    end
    local AgreeAddFunc = function(bAnimation)
        self:RefreshList(bAnimation)
        self:AddTimer(0.35, function()
            if (self.ListDatas == nil or #self.ListDatas == 0) then
                self:SetFocus()
    ...
    FriendController:RegisterEvent(self, function(self, Reason, ...)
        ...
        if not SwitchFunc then return end
        SwitchFunc(...)
    end)
    --界面打开音频
...
function M:InitContent(Params, PopupData, Owner)
    ...
    if Params.ButtonBarName then
        local Filters = {"UI_Friend_OnlineTime", "UI_LEVEL_SELECT"}
        self.Owner:GetButtonBar().WBP_Com_Sort:Init(self.Owner ,Filters, self.SortType, {OnGetBackFocusWidget = function()
    ...
    self:ShowPlayerInfoBtn(#self.ListDatas ~= 0)
...
function M:_SetupBottom()
    ...
    elseif self.Type == FriendCommon.FriendDialogType.FriendRequest then
        self.Switcher_Content:SetActiveWidgetIndex(1)
        if #self.ListDatas == 0 then return end
        -- self.Panel_Button:SetVisibility(UIConst.VisibilityOp.Visible)
        self.Owner:GetButtonBar():SetVisibility(UIConst.VisibilityOp.Visible)
        self.Switcher_Content:SetActiveWidgetIndex(0)
    end
end
...
function M:OnPreviewKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        if (InKeyName == "Gamepad_DPad_Left") then
