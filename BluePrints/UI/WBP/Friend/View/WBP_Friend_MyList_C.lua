...
---@type WBP_Friend_MyList_C|WBP_Friend_ListBase
local M = Class( {
...
function M:Construct()
    ...
    self.WBP_Com_FilterSort:Init(self,Filters, CommonConst.DESC, {OnGetBackFocusWidget = function()
    ...
    local RefreshListOnly = function(bAnimation) 
        self:RefreshList(bAnimation) 
    end
    local RefreshOneItemByUid = function(Uid) 
    ...
    self.EventSwitch = {
        ...
        [FriendCommon.EventId.RefreshFriend] =  function(ErrCode) 
            if not self.bRefreshing then return end
        ...
        [FriendCommon.EventId.UpdateOneFriend] = function(Uid)
            if self.bRefreshing then return end
    ...
    TeamController:RegisterEvent(self, function(self, EventId, ...)
        --DebugPrint(DebugTag, LXYTag, "FriendMyList, TeamController EventId: ", EventId)
