...
function Component:Construct()
    ...
    self.CharOrderByAttrNames = {"Level", "Rarity", "SortPriority", "UnitId"}
    ...
    -- 武器tag、显示名称
    self.MeleeFilterTags, self.MeleeFilterNames, self.RangedFilterTags, self.RangedFilterNames =
...
function Component:SetReceivedEvent(IsReceive)
    ...
    -- self.ComponentReceivedEvent["InitSubUIKeySetting"] = IsReceive
...
function Component:ExpandList(IsListExpanded)
    if (IsListExpanded) then
        ...
        -- 若要移植该组件CurMainTab，需要设置self.CurMainTab.Name
        if (self[self.CurMainTab.Name .. "FilterTags"]) then
            self.Filters = self:CreateFilters(self[self.CurMainTab.Name .. "FilterTags"],
                self[self.CurMainTab.Name .. "FilterNames"], self[self.CurMainTab.Name .. "FilterIcons"],
...
function Component:SortItemContents(InOutContentArray, SortByIdx, SortType)
    local FirtContent = self[self.CurMainTab.Name .. "Main_CurContent"] or
                            self[self.CurMainTab.Name .. "Main_CmpContent"]
...
function Component:FilterItemContents(InContentArray, FilterIdxes)
    ...
    elseif (TabName == ArmoryUtils.ArmoryMainTabNames.Melee or TabName == ArmoryUtils.ArmoryMainTabNames.Ranged or
        TabName == ArmoryUtils.ArmoryMainTabNames.Weapon) then
