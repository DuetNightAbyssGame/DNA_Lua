...
function M:InitUIInfo(Name, IsInUIMode, EventList, ...)
    ...
    --保底修复套装信息内容不显示问题,保证播放Auto_In在初始化流程的最后执行
    self:AddTimer(0.05,function()
        if not self then return end
        if not self.Auto_In then return end
...
function M:InitSuitVariables()
    ...
    for _,v in pairs(self.BlessingGroupData) do
        ...
        if not SuitCount then
            SuitCount = 0
        end
        if SuitCount > MaxSuitCount then
...
function M:InitRougeBtn()
    ...
    if self.Btn_Bag then
        ...
        self.Btn_Bag.Key_Bag:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "Menu"}} })
    end
    -- 左右查看套装
    if self.AwardType == "Blessing" then
        ...
        if self.Left_GamePad then
            self.Left_GamePad:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "LB"}} })
            self.Right_GamePad:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "RB"}} })
        end
    end
    -- 选择按钮
    ...
    self.Btn_GiveUp.Key_Gamepad:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "X"}} })
    -- 切换简述按钮
    self.Text_SortDesc:SetText(GText("RLBlessing_SimpleDesc_Switch"))
    if self.Key_Gamepad_Desc then
        self.Key_Gamepad_Desc:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "View"}} })
    end
...
function M:InitRefreshButton(AwardType)
    ...
    if not GWorld.RougeLikeManager then return end
    ...
    self.Btn_Refresh.Key_GamePad:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "Y"}} })
end
...
function M:ShowNextAward()
    ...
    for i = 1,3 do
        ...
        if InfoList[i] then
            self[self.AwardType.."Widgets"][i]:OnLoaded({AwardType = self.AwardType, AwardId = InfoList[i].ItemId}, self)
        else
...
function M:SelectItem(AwardId, SelectWidget)
    if self.AwardType == "Blessing" then -- 针对套装专门做的逻辑
...
function M:OnRefreshAward()
    if not self.Btn_Refresh then return end
    self.CurrentTime = UE4.UGameplayStatics.GetRealTimeSeconds(self)
    ...
    local Avatar = GWorld:GetAvatar()
    local Currency = Avatar.Resources:QueryResourceCount(self.CoinId)
    if self.ResetCost > Currency then
...
function M:PlayItemsUnSelectAnimations(AwardId)
    for i = 1,3 do
        if self[self.AwardType.."Widgets"][i].AwardId ~= AwardId then
            if self[self.AwardType.."Widgets"][i].IsSelected then
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    elseif InKeyName == Const.GamepadLeftThumbstick then--去套装详情
        if self.AwardType=="Blessing" then
            local TopChildWidget = self.ScrollBox_Suit:GetChildAt(0)
            if TopChildWidget then
                TopChildWidget:SetFocus()
            ...
            if self.CurrentSelectItem and self.CurrentSelectItem.Rouge_SuitSign and self.CurrentSelectItem.Rouge_SuitSign.Com_BtnQa then
                if self.CurrentSelectItem.Rouge_SuitSign:IsVisible() then
    ...
    elseif InKeyName == Const.GamepadFaceButtonRight then--从套装详情返回3选1
        if self.bInSuitScroll then
             if self.AwardType=="Blessing" then
...
function M:OnAnalogValueChanged(MyGeometry,InAnalogInputEvent)
    ...
    if InKeyName == "Gamepad_RightY" or InKeyName == "Gamepad_LeftY" then
...
function M:InitGamepadView()
    ...
    -- 初始点
    if not UIUtils.HasAnyFocus(self) then return end
    if self.CurrentSelectItem then
        self.CurrentSelectItem.Button_Select:SetFocus()
    else
...
function M:RealClose()
    ...
    if CommonUtils.GetDeviceTypeByPlatformName(self) == "Mobile" then  -- 通知系统入口播收入背包的动画
...
function M:CheckIfSupportAward()
    return GWorld.RougeLikeManager.RoomIndex == 1 and GWorld.RougeLikeManager.PassRooms:Num() == 0
end
