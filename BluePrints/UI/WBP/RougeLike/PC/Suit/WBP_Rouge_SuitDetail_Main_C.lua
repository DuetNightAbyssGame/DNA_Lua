...
-- 第一个参数传选了哪个套装
-- 第二个参数传哪个套装被预加了，没有就不传（比如肉鸽商城选中了某个）
...
function M:InitKey()
    ...
    if CommonUtils.GetDeviceTypeByPlatformName() == "Mobile" then
        ...
        self.Key_L:CreateCommonKey({KeyInfoList = {{Type = "Img", ImgShortPath = "LB"}}})
        self.Key_R:CreateCommonKey({KeyInfoList = {{Type = "Img", ImgShortPath = "RB"}}})
...
function M:UpdateSuitInfo(SuitId, Index)
    ...
    self.Text_SuitDesc:SetText(GText(self.BlessingGroupData[SuitId].GroupEffectDesc)) -- 后续读表
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if InKeyName == "A" or InKeyName == Const.GamepadLeftShoulder then
        self:TabToLeft()
    elseif InKeyName == "D" or InKeyName == Const.GamepadRightShoulder then
        self:TabToRight()
