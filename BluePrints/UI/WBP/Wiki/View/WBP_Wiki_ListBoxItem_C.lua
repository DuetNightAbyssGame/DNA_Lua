...
---@type WBP_Encyclopedia_ListBox_C
...
function M:UpdateEntryList()
    if not self.List_Box then return end
...
function M:InitNavigationRules()
    self:SetNavigationRuleCustom(EUINavigation.Down, {self, self.SetDownTarget})
    self:SetNavigationRuleCustom(EUINavigation.Up, {self, self.SetUpTarget})
end

function M:SetDownTarget()
    if self.List_Box:GetVisibility() == UIConst.VisibilityOp.SelfHitTestInvisible and
       self.List_Box:GetChildrenCount() > 0 then
...
function M:SetUpTarget()
    ...
    if ListBoxItem.List_Box:GetVisibility() == UIConst.VisibilityOp.SelfHitTestInvisible and
       ListBoxItem.List_Box:GetChildrenCount() > 0 then
