...
function M:OnHoverButton()
    ...
    if Parent.CurInputDeviceType == ECommonInputType.Gamepad then
        if Parent.CurrentReasoningState == 1 or Parent.CurrentReasoningState == 4 then
            self:OnClickButton()
        end
        if Parent.CurrentReasoningState == 2 and Parent.CurrentChosenContent and not Parent.IsCommitAnswerMultiSelect then
            self:OnClickButton()
        end
    end
end
