...
---@type WBP_Pet_Capture_Progress_C
...
function M:StartProgressAnimation(PetOwner, Probability, IsSuccess, UniqueId)
    ...
    Probability = Probability ~= 1 and Probability or 10
    ...
    self.Growth2 = (IsSuccess and (not Growth3) or Growth3 == 0) and 100 or (DataMgr.CaptureProgress[CaptureProgressId].Growth2 + math.random(-5, 4) + self.Growth1)
    if Growth3 then
...
function M:CaptureSuccess()
    ...
    local IsPremium = PetId and DataMgr.Pet[PetId].Premium == 1 or false
