...
---@type WBP_Camera_Conceal_M_C
local M = Class("BluePrints.UI.WBP.Camera.WBP_Camera_ComBtn_Base_C")

function M:Construct()
    M.Super.Construct(self)

    self:BindToAnimationFinished(self.Warning, {self, self.OnWarningAnimationFinished})
end

function M:Destruct()
    self:UnbindFromAnimationStarted(self.Warning, {self, self.OnWarningAnimationFinished})
end
