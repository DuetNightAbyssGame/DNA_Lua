...
local EDetectTargetMethods = {ProjectToScreen = 1,LineTrace = 2,ProjectToScreenAndLineTrace = 3}

---@type WBP_Carmera_P_C
...
function M:Construct()
    ...
    self.CharType = {Char = 1, Player = 2, NPC = 4, Monster = 8, Pet = 16, All = 31}
    self.bHasFoundTargets = false
    self.IsShotTargetSucceeded = false
    self.CloseCallback = nil  -- 相机关闭的回调
...
function M:InitConstValue()
    ...
    local GetDataValue = function(DataRow)
...
function M:InitUIInfo(Name, IsInUIMode, EventList, Params)
    ...
    if self.InitParams ~= nil then
        ...
        -- if self.Btn_Pause:GetChecked() ~= CurGamePause then
    ...
    if self.Camera then
        ...
        UE4.UKismetSystemLibrary.SphereTraceMulti(
            self.Camera,
            self.Camera:K2_GetActorLocation(),
            self.Camera:K2_GetActorLocation(),
            self.CameraMoveMaxRang,
		    UE4.ETraceTypeQuery.TraceOverlap,
            false,
            nil,
        ...
        for _, value in pairs(HitResults) do
            if value.Actor ~= nil and value.Actor.IsCharacter ~= nil and value.Actor:IsCharacter() then
                if not table.findValue(self.NeedUpdateLODCharacter, value.Actor) then
...
function M:SetInitParams(Params)
    ...
    if Params ~= nil and Params.bForceGamePause ~= nil then
...
function M:InitTaskInfo()
    ...
    if (self.InitParams) and (self.InitParams.EventId)  -- 有的拍照活动不显示任务面板
        and (self.InitParams.EventId == CameraGameUtils.GetEventId()) then
        if self.TaskBar then self.TaskBar:SetVisibility(UIConst.VisibilityOp.Collapsed) end
        return
    end
...
function M:Tick(MyGeometry, InDeltaTime)
    if(self.CameraManager)then
        ...
        if Dis2D < self.PlayerCapsuleRadius * self.PlayerCapsuleRadius
            and CameraLocation.Z < Position.Z + self.PlayerCapsuleHalfHeight
            and CameraLocation.Z > Position.Z - self.PlayerCapsuleHalfHeight  then
            if(not self.Player.bHidden)then
                self.Player:SetActorHideTag("CameraScreenshotTick", true)
            end
        else
            if(self.Player.bHidden and (self.CharType.Char & self.CurCharHiddenState == 0 ))then
    ...
    if UE4.UGameplayStatics.IsGamePaused(self) then
        if self.NeedUpdateLODCharacter and #self.NeedUpdateLODCharacter > 0 and self.bNeedUpdateLODCharacterOnce then
            self.NeedPauseNextFrameMeshes = {}
...
local GetTargetLoc = function(Actor,TargetsLoc)
...
function M:TryFindTargets()
    ...
    local TargetsLoc = {}
    if(EDetectTargetMethods.ProjectToScreen & self.DetectTargetMethod ~= 0)then
        ...
        for key, value in pairs(self.TargetActors) do
            ...
            if(ScreenPos.X > LeftTop.X and ScreenPos.Y > LeftTop.Y
                and ScreenPos.X < LeftTop.X + Size.X and ScreenPos.Y < LeftTop.Y + Size.Y)then
    ...
    if(ContinueFindTarget and EDetectTargetMethods.LineTrace &  self.DetectTargetMethod ~= 0)then
        if(self.CameraManager)then
            local CameraLocation = self.CameraManager:GetCameraLocation()
            for key, value in pairs(self.TargetActors) do
...
function M:CreateCamera()
    ...
    self.Camera = self:GetWorld():SpawnActor(LoadClass('/Game/AssetDesign/Camera/BP_ScreenshotCamera.BP_ScreenshotCamera_C'),
                                            self.OriginalCameraTransform,
                                            UE4.ESpawnActorCollisionHandlingMethod.AdjustIfPossibleButAlwaysSpawn,
                                            nil, nil, nil)
...
function M:OnHideBtnClickedImp(HiddenButtonType, CharacterType)
    ...
    self[HiddenButtonType]:SetHiddenState(CharacterType & self.CurCharHiddenState ~= 0)
    self:UpdateCheckBox()
end
...
function M:HideChar(HiddenState)
    if(self.CharType.Char & HiddenState ~= self.CharType.Char & self.CurCharHiddenState)then
        --角色隐藏状态改变时
        local bHide = self.CharType.Char & HiddenState ~= 0
...
function M:HidePlayer(HiddenState)
    if(self.CharType.Player & HiddenState ~= self.CharType.Player & self.CurCharHiddenState)then
        local bHide = self.CharType.Player & HiddenState ~= 0
        local GameState = UE4.UGameplayStatics.GetGameState(self)
...
function M:HideNPC(HiddenState)
    if(self.CharType.NPC & HiddenState ~= self.CharType.NPC & self.CurCharHiddenState)then
        local bHide = self.CharType.NPC & HiddenState ~= 0
...
function M:HideMonster(HiddenState)
    if(self.CharType.Monster & HiddenState ~= self.CharType.Monster & self.CurCharHiddenState)then
        local bHide = self.CharType.Monster & HiddenState ~= 0
        local GameInstance = GWorld.GameInstance
        local GameState = UE4.UGameplayStatics.GetGameState(GameInstance)
        if(not GameState)then
            return
...
function M:HidePet(HiddenState)
    if(self.CharType.Pet & HiddenState ~= self.CharType.Pet & self.CurCharHiddenState)then
        local bHide = self.CharType.Pet & HiddenState ~= 0
...
function M:ResetCamera0()
    ...
    if(self.InitParams.bUseStartPos)then
        ...
        UKismetSystemLibrary.SphereTraceSingle(self,self.TraceStartLocation,Position,self.CameraProbeSize,
        UE4.ETraceTypeQuery.Camera,false,self.ActorsToIgnore,0,HitResult,true)
...
function M:ResetCamera1()
    ...
    UKismetSystemLibrary.SphereTraceSingle(self,StartLoc,DesiredLoc,self.CameraProbeSize,
    UE4.ETraceTypeQuery.Camera,false,self.ActorsToIgnore,0,HitResult,true)
    if(HitResult.bBlockingHit)then
        self.Camera:SetLocation(HitResult.Location)
    else
...
function M:ResetFocalLength()
    ...
    if(self.FocalLengthSlider)then
        ...
        self.FocalLengthSlider:InitSliderValue(self.Camera:GetActiveCamera().CurrentFocalLength,
                                                MinFocalLength,MaxFocalLength, self.SliderClickInterval)
    end
end
...
function M:Screenshot()
    ...
    if(self.IsShotTargetSucceeded)then
        if (not self.InitParams.IsAprilFoolsDayActivity) then -- 愚人节活动不弹出提示
    ...
    if ScreenResolution then DebugPrint("ScreenResolution get success") end
    ...
    -- --固定基准分辨率
    -- local DesktopResolution = {X = 1920,Y = 1080} 
    -- local DesktopProportion = DesktopResolution.X / DesktopResolution.Y
    -- 计算窗口的长宽比
    local AspectRatio = ScreenResolution.X / ScreenResolution.Y

    -- 确定短边为1080，然后根据长宽比计算长边的分辨率
    local Height = 1080
    local Width = math.floor(Height * AspectRatio)
    -- 如果计算出的宽度小于高度，则说明实际的短边是宽度，需要调整
    ...
    if(self.bNeedCaptureActors)then
        -- if(self.BP_SetSceneCaptureShowOnlyActors)then
        --     --测试代码
        --     local Actors = TArray(AActor)
        --     Actors:Add(UGameplayStatics.GetPlayerCharacter(self,0))
        --     self:BP_SetSceneCaptureShowOnlyActors(Actors)
        -- end
...
function M:BP_OnScreenshotFinished(Image)
    ...
    EventManager:FireEvent(EventID.OnScreenshotToken)  --拍照事件
    --相机数据埋点
...
function M:ShowScreenshotWidget(Image)
    ...
    if(IsValid(self.ScreenshotWidget))then
        ...
        self.ScreenshotWidget:SetFocus()
        -- self.GameInputModeSubsystem:SetTargetUIFocusWidget(self.ScreenshotWidget.Btn_Yes_1)
    end
end
...
function M:Destruct()
    ...
    if self.CloseCallback then
        local OnUnloadUI = function(_self,UIName)
            if(UIName == self.WidgetName)then
                EventManager:RemoveEvent(EventID.UnLoadUI, _self)
                self.CloseCallback({IsSucceeded = self.IsShotTargetSucceeded})
            end
        end
		EventManager:AddEvent(EventID.UnLoadUI, self,OnUnloadUI)
    end

    -- 通过 GM 触发 STLNode 进入相机拍照，退出时无法正常执行到 Clear() 函数，在这里手动设置一次 GamePause(false)
