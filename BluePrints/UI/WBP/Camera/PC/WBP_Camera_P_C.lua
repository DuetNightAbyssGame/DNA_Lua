...
function M:InitKeySetting()
    ...
    -- self.KeyDownEvent[self.LeftMouseButton] = self.OnLeftMouseButtonDown
...
function M:Construct()
    ...
    self.MidMouseBtn_Key:CreateCommonKey({
        KeyInfoList = {
            {Type = "Text", ImgShortPath = "Mouse_Button", Owner = self}
        },
        SoundFunc = function() end,
        Desc = GText("UI_Dye_Zoom")
    })
    self.HideUI_Key:CreateCommonKey({
        KeyInfoList = {
            {Type = "Text", Text = self.HideUIKey, ClickCallback = self.OnHideUIKeyDown, Owner = self}
        },
        SoundFunc = function() end,
    ...
    self.HideRole_Key:CreateCommonKey({
        KeyInfoList = {
            {Type = "Text", Text = self.HideCharacterKey, ClickCallback = self.OnHideCharacterKeyDown, Owner = self}
        },
        SoundFunc = function() end,
    ...
    self.Restore_Key:CreateCommonKey({
        KeyInfoList = {
            {Type = "Text", Text = self.ResetCameraKey, ClickCallback = self.OnResetCameraKeyDown, Owner = self}
        },
        SoundFunc = function() end,
    ...
    self.Close_Key:CreateCommonKey({
        KeyInfoList = {
            {Type = "Text", Text = "Esc", ClickCallback = self.OnBackKeyDown, Owner = self}
        },
        SoundFunc = function() end,
        Desc = GText("UI_BACK"),
    })
    ...
    --     KeyInfoList = {
    --         {Type = "Text", Text = self.CameraRollLeftKey, ClickCallback = self.OnCameraRollLeftClicked, Owner = self}
    --     },
    --     SoundFunc = function() end,
    ...
    --     KeyInfoList = {
    --         {Type = "Text", Text = self.CameraRollRightKey, ClickCallback = self.OnCameraRollRightClicked, Owner = self}
    --     },
    --     SoundFunc = function() end,
    ...
    self.Key_Rotation:CreateSubKeyDesc({
        KeyInfoList = {
            { Type = "Text", Text = self.CameraRollLeftKey, Owner = self },
            { Type = "Text", Text = self.CameraRollRightKey, Owner = self }
        },
        Type = "Or",
        SoundFunc = function() end,
    ...
    --     KeyInfoList = {
    --         {Type = "Img", ImgShortPath = "Left", Owner = self}
    --     },
    --     SoundFunc = function() end,
    --     Desc = GText("UI_CameraSystem_CameraLeftLean")
    -- })
    ...
    self.Key_Controller_View:CreateCommonKey({
        KeyInfoList = {
            {Type = "Img", ImgShortPath = "R", Owner = self}
        },
        SoundFunc = function() end,
    ...
    self.Key_Controller_HideRole:CreateCommonKey({
        KeyInfoList = {
            { Type = "Img", ImgShortPath = "View", Owner = self }
        },
        SoundFunc = function() end,
        Desc = GText("UI_CameraSystem_HideModel")
    })
    ...
    self.Key_Controller_Pause:CreateCommonKey({
        KeyInfoList = {
            { Type = "Img", ImgShortPath = "RS", Owner = self }
        },
        SoundFunc = function() end,
    })
    ...
    self:BindToAnimationFinished(self.Menu_Out, {self, self.OnMenu_OutFinished})
    self:UnbindAllFromAnimationFinished(self.Menu_Out)
    self:BindToAnimationFinished(self.MenuPart_Out, {self, self.OnMenuPart_OutFinished})
    ...
    for key, value in pairs(self.WidgetHideTags) do
        self["Set"..key .."WidgetVisibility"](self,value == 0)
    end
    self.WidgetHideAnims = {
        ["Top"] = {["In"] = self.Hide_In,["Out"] = self.Hide_Out},
        ["Bottom"] = {["In"] = self.MenuPart_In,["Out"] = self.MenuPart_Out},
        -- ["ESC"] = {["In"] = self.ESC_In,["Out"] = self.ESC_Out},
        ["Menu"] = {["In"] = self.Menu_In,["Out"] = self.Menu_Out},
    }
    self.OperationStack = {}
...
function M:OnMouseButtonDown(MyGeometry, MouseEvent)
    ...
    --UWidgetBlueprintLibrary.LockMouse(Handle, self)
...
function M:OnMouseMove(MyGeometry, InKeyEvent)
    ...
    if(self.IsDragging)then
        ...
        --鼠标tick频率与控件tick频率不一致，如果上一次的检查结果没被消耗掉则保留
        self.RotateCameraByPointer = self.RotateCameraByPointer or CursorDelta.X ~= 0 or CursorDelta.Y ~= 0
    end
    return Unhandle
end
...
function M:OnRepeatKeyDown(MyGeometry, InKeyEvent)
    ...
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if GamepadName ~= "PS" or (Const.GamepadDPadUp ~= InKeyName and Const.GamepadDPadDown ~= InKeyName and Const.GamepadDPadLeft ~= InKeyName and Const.GamepadDPadRight ~= InKeyName) then
        return UIUtils.Unhandled
    end
...
function M:Tick(MyGeometry, InDeltaTime)
    ...
    if self.FocalLengthKeyPressedTime ~= nil and self.FocalLengthKeyPressedTime >= 0.5 then
    ...
    elseif (self.FocalLengthSub or self.FocalLengthAdd) and self.FocalLengthKeyPressedTime ~= nil and self.FocalLengthKeyPressedTime < 0.5 then
        self.FocalLengthKeyPressedTime = self.FocalLengthKeyPressedTime + InDeltaTime
    end

    if(self.bCameraMoveLeftKeyDown
    or self.bCameraMoveRightKeyDown
    or self.bCameraMoveUpKeyDown
    or self.bCameraMoveDownKeyDown
    or self.bCameraMovedByJoyStick
    or self.RotateCameraByPointer
    ...
    if(self.RotateCameraByPointer and (self.RotPitch ~= 0 or self.RotYaw ~=0))then
...
function M:OnCloseHideModel()
    -- DebugPrint(TXTTag, "OnCloseHideModel")
    self:ToggleShowHideCharacterWidget()
end
...
function M:SetMenuHideExcludeBottom()
    ...
    self:UnbindFromAnimationFinished(self.WidgetHideAnims["Menu"].Out, {self, self.SetMenuHideExcludeBottom})
end

function M:AddWidgetHideTag(Tag)
    if(self.WidgetHideTags[Tag] == 0)then
    ...
    if Tag == "Menu" and self.WidgetHideTags["Menu"] > 0 then
        self:BindToAnimationFinished(self.WidgetHideAnims[Tag].Out, {self, self.SetMenuHideExcludeBottom})
    end
end
...
function M:OnGlobalGamePause()
    if self.WS_Pause:GetActiveWidgetIndex() == 0 then
