...
function M:Construct()
    self.PressAnims = {
        Left = {Anim = self.A_Press},
        Right = {Anim = self.D_Press},
        Up = {Anim = self.W_Press},
        Down = {Anim = self.S_Press},
    }
end
...
function M:SetPressed(DirectionStr,bPressed)
    ...
    if(bPressed)then
        ...
        if(self:IsAnimationPlaying(self.PressAnims[DirectionStr].Anim))then
...
function M:OnUpdateUIStyleByInputTypeChange(CurInputType, CurGamepadName)
    if ( CurInputType == ECommonInputType.Gamepad ) then
        ...
        if self.WS_Type:GetActiveWidgetIndex() ~= 1 then
