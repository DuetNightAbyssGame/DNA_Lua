...
---@type WBP_MiniGame_ShuifaMain_P_C
...
function M:InitListenEvent()
    -- self:BindToAnimationFinished(self.Fail_Out, {self, function() self:Close() end})
    -- self:BindToAnimationFinished(self.Success_Out, {self, function() self:Close() end})
end
...
function M:InitWidghtPC()
    ...
    -- self.Key_Back_GamePad:AddExecuteLogic(self, self.GameFailed)
...
function M:CountDown()
    ...
    if self.Time <= 3 then
        ...
        -- self.ShuiFaDial.MiniGame_Time.Text_Time:SetColorAndOpacity(UE4.UUIFunctionLibrary.GetSlateColorByRGBA(1,0,0,0.7))
    end
    if self.Time <= 0 then
        self:GameOver(false)
        return
