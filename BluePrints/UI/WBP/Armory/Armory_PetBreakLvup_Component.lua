...
function Component:InitEntryItems()
    ...
    for i = 1,4 do
        if self["EntryItem_"..i] then
            self:BindToAnimationFinished(self["EntryItem_"..i].UnLock, {self,function ()
...
function Component:RefreshListComp()
    if self.CurrentMode == "BreakLevelUp" then
        ...
        for UniqueId,Pet in pairs(self._Avatar.Pets) do
            ...
            if not bConsumePet then goto continue end
    ...
    if self.CurrentMode == "BreakLevelUp" then
        ...
        if not self.BreakLvupSortFunc then
            ...
            self.BreakLvupSortFunc[2] =  function(P1, P2)
                if P1.Type == P2.Type then
                    if P1.Level and P2.Level and P1.Level ~= P2.Level then return self:ComparePetLevel(P1, P2, self.SortType) end
                    if P1.Rarity ~= P2.Rarity then return CommonUtils:Compare(P1.Rarity, P2.Rarity, self.SortType) end
                else
                    if P1.Type == CommonConst.DataType.Resource then return true end
                    if P2.Type == CommonConst.DataType.Resource then return false end
...
function Component:DestructComp()
    -- if(self.EnhanceWidget)then
    --     self.EnhanceWidget:BindEvents(nil,nil)
    -- end
end
...
function Component:UpdateEntryInfos(Pet)
    ...
    if(Pet:IsResourcePet())then
        ...
        if(Pet:IsPremium())then
            EntryItemWidgets = {self.EntryItem_1,self.EntryItem_3,self.EntryItem_4,self.EntryItem_2}
        else
            EntryItemWidgets = {self.EntryItem_1,self.EntryItem_3,self.EntryItem_2}
...
function Component:CheckAndPlayEntryItemLock()
    if not self.bHasLockEntryItem then return end
    self:BlockAllUIInput(false)
    self.GameInputModeSubsystem:SetNavigateWidgetOpacity(1)
    self:SetFocus()
    --处理槽位解锁
    local BreakConf = self.Target:BreakData()
    local EntryIndex = {1,3,2}
    ...
    if self.Target.BreakNum >= 3 and (self.Target.IsLock ~= nil and self.Target.IsLock == false) then
        self:BlockAllUIInputWithDelayUnblock(5)
...
function Component:BlockAllUIInputWithDelayUnblock(DelayTime)
    ...
    self:AddTimer(TargetTime, function()
        if not self then return end
        if not self:IsAllUIInputBlocked() then return end
