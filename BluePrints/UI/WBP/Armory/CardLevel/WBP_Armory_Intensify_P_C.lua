...
---@type WBP_Armory_Intensify_P_C|BP_UIState_C
...
function M:InitUIInfo(Name, IsInUIMode, EventList, ...)
    ...
    if(ArmoryMain and ArmoryMain.ActorController)then
        ...
        elseif(User == CommonConst.ArmoryType.Pet)then
            ...
            ArmoryMain.ActorController:SetArmoryCameraTag(CameraTag1,"LevelUp",nil,User)--宠物升级
        end
    end
    self:SetFocus()
end
...
function M:InitTab(TitleName)
    ...
    self.BottomKeyInfo = {
        {GamePadInfoList =  {{Type="Img", ImgShortPath="RS"}}, Desc = GText("UI_CTL_Remove")},
        {GamePadInfoList =  {{Type="Img", ImgShortPath="A"}}, Desc = GText("UI_Controller_Check")},
        {GamePadInfoList =  {{Type="Img", ImgShortPath="A"}}, Desc = GText("UI_RougeLike_BlessingConfirm")},
        {GamePadInfoList =  {{Type="Img", ImgShortPath="A"}}, Desc = GText("UI_CTL_Add/Remove")},
        {
            KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=BackCb,Owner=self}},
            GamePadInfoList = {{Type="Img", ImgShortPath="B", ClickCallback=BackCb,Owner=self}},
            Desc=GText("UI_BACK")
        },
    }
    local TabKeyInfo = {}
...
function M:ShowRemoveBtn(bShow)
    if not self.Tab_Intensify then return end
...
function M:OnExpandList(bListExpand, bRefreshList, bSkip)
    if self.bListExpand == bListExpand and not bRefreshList then return end
    ...
    if bListExpand then
        ...
        -- 更新勾选状态
        -- if self.OnChosenItemChanged then
    ...
    if(bShow)then
        if(self.ItemDetailsContent ~= Content)then
            ...
            if Content.LockType ~= nil then
                Content.LockedButtonClickCallBack = function(SetLockFunc)
        ...
        -- self:OnUpdateUIStyleByInputTypeChange(ECommonInputType.MouseAndKeyboard)
...
function M:IsContentLocked(Content)
    return Content.LockType and Content.LockType ~= 0
end
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if self.OnKeyDownComp then
        if self:OnKeyDownComp(MyGeometry, InKeyName) then
            IsHandled = true
            IsHandleGamepadButtonRight = true
        end
    end
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        if self.bItemDetailsShowed and self.ItemDetailsWidget then
            if self.ItemDetailsWidget:OnGamePadDown(InKeyName) then
                IsHandled = true
            end
        end
    end
...
function M:OnKeyUp(MyGeometry, InKeyEvent)
    ...
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if self.OnKeyUpComp then
        if self:OnKeyUpComp(MyGeometry, InKeyName) then
            IsHandled = true
        end
    end
    if IsHandled then
