...
function M:Construct()
    self:BindToAnimationFinished(self.LevelUp_In,{self,self.OnBreakLevelUpAnimFinished})
...
function M:OnBreakLevelUpSuccess(NewEnhanceLevel)
    ...
    if not self.OnBreakLevelUpSuccessCallback then return end
...
function M:UpdataAttrListView(Attrs,ComparedAttrs)
    ...
    for i, Attr in ipairs(self.Attrs) do
        if Attr.IsDispatchTag then
            ...
            if(UnlockLevel)then
                if(self.Target.EnhanceLevel >= UnlockLevel)then
                    Obj.Unlocked = true
                else
                    Obj.Unlocked = false
                end
