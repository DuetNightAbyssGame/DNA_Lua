...
function M:UpdateWidgetData(Params)
    ...
    if self.NowLevel then
        ...
        if bMax == nil and self.MaxLevel then
    ...
    if self.MaxLevel then
        if self.PreviewLevel then
            self.Btn_Add:BindEventOnClicked(self, function()
                if self.PreviewLevel >=self.MaxLevel then return end
    ...
    if self.MinLevel then
        if self.PreviewLevel then
            self.Btn_Minus:BindEventOnClicked(self, function()
                if self.PreviewLevel <= self.MinLevel then return end
...
function M:CallBtnClickCallback(InPreviewLevel)
    if not self.OnBtnClickCallback then return end
    local Attrs, ComparedAttrs,Desc,bMax,OutPreviewLevel = 
        self.OnBtnClickCallback(self.Parent, InPreviewLevel)
