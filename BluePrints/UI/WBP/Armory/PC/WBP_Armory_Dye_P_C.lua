...
---@type WBP_Armory_Dye_P_C|WBP_Armory_Dye_Base_C
local M = Class("BluePrints.UI.WBP.Armory.WBP_Armory_Dye_Base_C")
local FSM = require("Blueprints.UI.FocusStateMachine")

local FocusAreas = {NoramlDyeTab = "NoramlDyeTab",ColorWidget = "ColorWidget",Tips = "Tips",
                    ConsumItem = "ConsumItem", Resource = "Resource", Coin = "Coin",
                    FunctionBtn = "FunctionBtn",Plan = "Plan", Self = "Self"}
...
function M:Construct()
    ...
    self.FSM = FSM:New(self,{StateNames = FocusAreas,
...
function M:InitKeySetting()
    ...
    self.Tab_KeyInfoList = {
        KeyInfoList = {
            {Type="Text", Text=CommonUtils:GetKeyText("Tab"),
            --ClickCallback=self.OnContrastKeyDown,
    ...
    self.HideUI_KeyInfoList = {
        KeyInfoList = {
            {Type="Text", Text = CommonUtils:GetKeyText("U"), ClickCallback=self.OnUKeyDown, Owner=self},
        },
    ...
    self.Back_KeyInfoList = {
        KeyInfoList = {
            {Type="Text", Text = self.EscapeKey, ClickCallback=self.OnBackKeyDown, Owner=self},
        },
        GamePadInfoList =  {
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    if(self.IsGamepadInput)then
        ...
        if(not IsStateValid or IsValid(State.Widget) or not UIUtils.HasAnyFocus(State.Widget)
         or (self.bSelfHidden and State.Name == FocusAreas.Self))then
...
function M:OnSubTabLeftKeyDown()
    -- self.SubTab_Dye:TabToLeft()
    -- return self:GetDesiredFocusWidget()
end

function M:OnSubTabRightKeyDown()
    -- self.SubTab_Dye:TabToRight()
    -- return self:GetDesiredFocusWidget()
end
...
function M:UpdateKeyEvents()
    ...
    if(self.bSelfHidden)then
        ...
        if(CurrentFocusedArea == FocusAreas.NoramlDyeTab
        or CurrentFocusedArea == FocusAreas.ColorWidget
        or CurrentFocusedArea == FocusAreas.Resource)then
            self.RepeatKeyDownEvents[self.LeftTriggerKey] = self.OnCameraScrollBackwardKeyDown
            self.RepeatKeyDownEvents[self.RightTriggerKey] = self.OnCameraScrollForwardKeyDown
        ...
        -- elseif(CurrentFocusedArea == FocusAreas.ConsumItem)then
        -- elseif(CurrentFocusedArea == FocusAreas.Coin)then
        -- elseif(CurrentFocusedArea == FocusAreas.FunctionBtn)then
        -- elseif(CurrentFocusedArea == FocusAreas.Tips)then
...
function M:OnGamepadApplyKeyDown()
    if(self.WidgetSwitcher_Preview:GetActiveWidgetIndex() == 0)then
        if(self.Btn_Done.IsForbidden)then
            self:OnForbiddenDoneBtnClicked()
        else
            self:OnDoneBtnClicked()
        end
    elseif(self.WidgetSwitcher_Preview:GetActiveWidgetIndex() == 1)then
...
function M:OnFocusReceived(MyGeometry, InFocusEvent)
    if(self.bSelfHidden)then
        self.FSM:Push({Name = FocusAreas.Self, Widget = self})
    else
...
function M:OnColorContentCreated(Content)
    Content.Owner = self
    Content.OnAddedToFocusPath = function(_self,_Content)
        if(_Content.ColorId ~= self.DefaultColorId)then
            if(_Content.ListContent)then
        ...
        self.FSM:Push({Name = FocusAreas.ColorWidget, Content = _Content,Widget = _Content.Widget})
    end
    Content.OnRemovedFromFocusPath = function(_self,_Content)
    end
end
...
function M:OnNormalColorContentCreated(Content)
    ...
    Content.OnNavigateDown = function()
        local Idx = self.EMList_Normal:GetIndexForItem(Content.ListContent)
...
function M:OnNormalColorListContentCreated(Content)
    Content.OnResourceAddedToFocusPath = function(_self,ResourceWidget)
        self.FSM:Push({Name = FocusAreas.Resource,Content = Content,Widget = ResourceWidget})
    end
    Content.OnResourceRemovedFromFocusPath = function()
    end
    Content.OnResourceNavigateUp = function()
    ...
    Content.OnResourceNavigateDown = function()
        local Idx = self.EMList_Normal:GetIndexForItem(Content)
        local NextItem = self.EMList_Normal:GetItemAt(Idx + 1)
        if(NextItem)then
    ...
    Content.OnMenuOpenChanged = function(_self,IsOpen,_Content)
        if(IsOpen)then
            self.FSM:Push({Name = FocusAreas.Tips,Content = Content})
        end
    end
end
...
function M:OnNoramlDyeTabContentCreated(Content)
    Content.OnAddedToFocusPath = function(_self,_Content)
        self.FSM:Push({Name = FocusAreas.NoramlDyeTab,Content = Content,Widget = Content.Widget})
    end
    Content.OnRemovedFromFocusPath = function(_self,_Content)
    end
    Content.OnHovered = function(_self,_Content)
        if(self.IsGamepadInput)then
...
function M:OnFunctionBtnAddedToFocusPath(Widget)
    self.FSM:Push({Name = FocusAreas.FunctionBtn,Widget = Widget})
end
...
function M:OnConsumItemContentCreated(Content)
    Content.OnAddedToFocusPathEvent = {
        Obj = self,Callback = self.OnConsumItemAddedToFocusPath,Params = Content
    }
    Content.OnRemovedFromFocusPathEvent = {
        Obj = self,Callback = self.OnConsumItemRemovedFromFocusPath,Params = Content
    }
end

function M:OnConsumItemAddedToFocusPath(Content)
    self.FSM:Push({Name = FocusAreas.ConsumItem,Content = Content,Widget = Content.SelfWidget})
end
...
function M:OnResourceBarAddedToFocusPath(Widget)
    self.FSM:Push({Name = FocusAreas.Coin,Widget = Widget})
end
...
function M:OnModifyPlanParams(Params)
    ...
    Params.OnAddedToFocusPath = function()
        self.FSM:Push({Name = FocusAreas.Plan,Widget = self.Plan_Dye})
    end
...
function M:BackToColorList(BackState)
    ...
    if(self.CurrentTabIdx == self.NormalColorTabIdx)then
        ...
        if(ColorContent == self.SpecialDefaultContent)then
            return self.ColorLump_Default_Sp
        end
    end
...
function M:OnNormalDyeTabInit(...)
    ...
    -- for index, value in ipairs(AllContents) do
    --     value.Pre = AllContents[index - 1]
    --     value.Next = AllContents[index + 1]
    -- end
end
...
function M:InitNavigationRules()
    if(self.CurrentTabIdx == self.NormalColorTabIdx)then
        ...
        self.List_Tab:SetNavigationRuleCustom(EUINavigation.Left, {self,function()
            self.FSM:Pop()
            ...
            if(ColorContent == DefaultColorContent)then
                ...
                if(ColorContent and ColorContent.ListContent and self.EMList_Normal:GetIndexForItem(ColorContent.ListContent) >= 0)then
        ...
        self.ColorLump_Default:SetNavigationRuleCustom(EUINavigation.Down, {self,function()
            local Content = self.EMList_Normal:GetItemAt(0)
            return self:NavigateToNormalColorList(Content)
        end})
    else
        ...
        self.ColorLump_Default_Sp:SetNavigationRuleCustom(EUINavigation.Down, {self,function()
