...
---@type WBP_Armory_CharSkillDetail_P_C
local M = Class({"BluePrints.UI.WBP.Armory.WBP_Armory_CharSkillDetail_Base_C"})

--function M:Initialize(Initializer)
--end
local FocusStates = {CharSkill = 1, SkillAttr = 2, Resource = 3}
...
function M:Construct()
    ...
    self.Key_Consume:CreateCommonKey({KeyInfoList = {{Type = "Img",ImgShortPath = "LS"}}})
    self.Keys = {self.Key_1,self.Key_2,self.Key_3}
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    if(self.IsInFocusPath and self.IsGamepadInput or self.IsHyperLinkTipOpened)then
        ...
        elseif(self.CurrentFocusState == FocusStates.SkillAttr)then
            if(IsValid(self.SkillAttrWidget))then
    ...
    if(self.IsGamepadInput)then
        ...
        if(self.CurrentFocusState == FocusStates.Resource)then
            ...
            if(self.IsItemMenuOpened)then
                ...
                table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "A"}},Desc = GText("UI_Controller_Check")})
            end
        elseif(self.CurrentFocusState == FocusStates.SkillAttr)then
            table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "A"}},Desc = GText("UI_CTL_Squad_Expand")})
        end
        if(self.WidgetSwitcher_Page:GetActiveWidgetIndex() == 0)then
            if(self.CurrentNodeInfo.Skill)then
                if(not self.IsGamepadTermTipShowed and self.DisplayedHyperLinkTers and #self.DisplayedHyperLinkTers >0 )then
                    table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "X"}},Desc = GText("UI_CTL_Explain")})
                end
            end
        else
            if(self.ShouldShowBtnCheck and self.CurrentNodeInfo.NodeIdx == 1)then
                table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "X"}},Desc = GText("UI_CTL_NextLevel")})
            end
            if(self:GetAnyFocusableSkillAttrWidget() and self.CurrentFocusState ~= FocusStates.SkillAttr)then
                table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "RS"}},Desc = GText("UI_CTL_CheckProperty")})
            end
        end
        if(bHideBackKeyInfo)then
            ...
            table.insert(KeyInfoLists,{KeyInfoList = {{Type = "Img",ImgShortPath = "B"}},Desc = GText("UI_BACK")})
...
function M:ModifyCharSkillInitParams(Params)
    ...
    Params.OnTreeNodeWidgetAddedToFocusPath = function(self,NodeInfo)
        if(self.IsGamepadInput)then
            if(NodeInfo and NodeInfo.UI)then
                NodeInfo.UI:OnBtnClicked()
            end
        end
    end

    Params.OnTreeNodeWidgetRemovedFromFocusPath = function(self,NodeInfo)
    end
end
...
function M:OnResourceContentCreated(Content)
    if(Content)then
        Content.OnAddedToFocusPathEvent = {Obj = self,Callback = function()
            self.CurrentFocusState = FocusStates.Resource
            self:OnFocusChanged()
        end}
        Content.OnRemovedFromFocusPathEvent = {Obj = self,Callback = function()
            self.CurrentFocusState = nil
        end}
        Content.OnMenuOpenChangedEvents = {Obj = self,Callback = function(_self,IsOpen)
...
function M:OnTraingCloseKeydown()
    ...
    if GameMode ~= nil and GameState ~= nil and (GameState.GameModeType == "Training" or GameState.GameModeType == "Trial") then
        self:OnCloseBtnClicked()
    end
end

function M:OnTabLeftKeyDown()
    if(self.IsNexLevelInfoShowed or not self.CurrentNodeInfo or not self.ScrollBox_Attr:HasAnyChildren())then
        return
    end
    local IsSkillAttrFocused = self.CurrentFocusState == FocusStates.SkillAttr
    self.Tab_SkillDetail:TabToLeft()
    if(self.IsGamepadInput)then
        if(IsSkillAttrFocused)then
            return UWidgetBlueprintLibrary.SetUserFocus(UWidgetBlueprintLibrary.Handled(),self.CurrentNodeInfo.UI),true
        end
    end
end
