...
function M:OnBtn_PreviewClicked()
    ...
    local Params = {
        ...
        TabConfigData = {
           Tabs = {
                { Text = GText("UI_Armory_Pet_Positive"), TabId = 1},
                { Text = GText("UI_Armory_Pet_Passive"), TabId = 2}
            }
        },
    }
...
function M:Init(Params)
    ...
    self.Parent.ActorController:BindEvent(self, {OnPlayPetFresnel = self.OnPlayPetFresnel, OnCharModelChanged = self.OnCharModelChanged})
...
function M:UpdatePetInfos(Pet)
    ...
    if Pet.Level >= RealMaxLevel then Percent = 1 end
...
function M:UpdateSkillInfos(Pet)
    ...
    if(Data.SupportSkillId)then
        ...
        Obj.ExtraLevel = Pet:GetSkillLevelUp()
