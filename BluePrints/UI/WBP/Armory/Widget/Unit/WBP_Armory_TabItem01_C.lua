...
function M:SetCharSkinPreViewItemIsLocked(CharSkinPreViewLockType)
    if not CharSkinPreViewLockType then return end
...
function M:SetPetStar(Content)
    ...
    if(not BreakNum or BreakNum == 0)then
        self.Panel_PetStar:SetVisibility(UIConst.VisibilityOp.Collapsed)
        return
    end
    self.Panel_PetStar:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    -- 直接访问子蓝图 PetStar_1 到 PetStar_3
    local totalStars = 3  -- 总共有3个星星
    -- 遍历所有星星
    for i = 1, totalStars do
        local starWidget = self["PetStar_" .. i]  -- 获取对应名称的星星控件
        if starWidget then
            -- 根据索引和breaknum设置ActiveWidgetIndex
            if (i - 1) < BreakNum then
                -- 小于breaknum的星星设置为1（激活）
