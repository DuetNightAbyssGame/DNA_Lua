...
function M:Init(Level,MaxLevel,ExpPercent,StarNumber,BreakLevelUpData,Type,Tag,IsPreviewMode)
    ...
    if (not self.IsPreviewMode) and (Tag == "Char" or Tag == "Ranged" or Tag == "Melee") then
        self.CurLevel = Level
...
function M:MakeBreakRewardData()
    ...
    local Params={}
    local ConfigData={}
    ConfigData.ReceiveAllCallBack=function(_,ReceiveAllParm)
    ...
    for _, BreakData in pairs(self.BreakLevelUpData) do
        if BreakData.CollectReward then
            ...
            local ReceiveParm = table.concat({self.Type, "Break", ".", StrTypeId, ".", BreakNum})
            ...
            Item.CanReceive = (StoredReward ~= nil)
            Item.RewardsGot = (not Item.CanReceive) and (CollectedReward)
            Item.ReceiveCallBack = function(_,ReceiveAllParm)
...
function M:GetReward(Content)
    ...
    if Avatar and #ReceiveParm > 0 then
        ...
        if DialogWidget then DialogWidget:BlockAllUIInput(true) end
...
function M:RefreshRewardDialog(DialogWidget, Rewards)
    ...
    for i = 1, DialogWidget.List_Item:GetNumItems() do
        ...
        if Item then
            ...
            local CanReceive = (StoredReward ~= nil)
