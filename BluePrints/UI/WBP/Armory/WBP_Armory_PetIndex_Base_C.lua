...
---@type WBP_Armory_PetIndex_Base_C
...
function M:InitComp(SelectedPetEntryID)
    ...
    self.PetEntryAchive.Pet_Sort:Init(self, {GText("UI_RARITY_NAME"),GText("Pet_Affix_Type")}, CommonConst.DESC,Params)
...
function M:LockPetEntry(IsChecked,LockBagPet)
    ...
    Avatar:PetEntryLock(self.SelectedContent.Id, IsChecked,LockBagPet,Cb)  -- 锁定词条
end

function M:OnLockStateChanged(IsChecked)
    print("OnLockStateChanged", IsChecked)
    if not IsChecked then
        if not self.SelectedContent.IsLocked then return end
        ...
        if self.SelectedContent.IsLocked then return end
    end
...
-- function M:FindFirstOwnedEntry() --功能取消
--     for index, value in ipairs(self.ContentArray) do
--         if(value.IsOwned)then
--             return value
--         end
--     end
-- end
...
function M:SelectItem(Content)
    ...
    -- self.PetEntryAchive:Init(Content)
    ...
    -- self:AddTimer(0.1,function()
    --     if not Content.SelfWidget:HasAnyUserFocus() then
...
function M:UpdateLockState(Content)
    if(Content.IsLocked)then
        ...
        if self.PetEntryAchive.Switch_Lock:GetChecked() then
            self.PetEntryAchive.Switch_Lock:SetChecked(false,true)
        end
    end
end
...
function M:ReallyOnFocusReceived(MyGeometry, InFocusEvent)
    -- if not self.PetEntryAchive:HasAnyUserFocus() then
    --     local FirstItem = self.PetEntryAchive.List_Item:GetItemAt(0)
    --     self:AddTimer(0.1,function()
...
function M:ReallyRefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    if (IsGamepad) then
        if not self.PetEntryAchive.List_Item:HasAnyUserFocus() and not self.PetEntryAchive.List_Item:HasFocusedDescendants() then
