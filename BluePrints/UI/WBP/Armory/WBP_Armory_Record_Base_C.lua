...
function M:Init(Params)
    ...
    if(self.IsPreviewMode)then
        ...
        self:UpdataReddotInfo(self.Target.CharId)--此处在切换角色时会被覆盖
        self.WidgetSwitcher_State:SetActiveWidgetIndex(0)
        self.Btn_ShowPic:SetVisibility(UIConst.VisibilityOp.Collapsed)
    end
...
function M:UpdataReddotInfo(CharId)
    ...
    local Callback=function(self,Count)
        if Count>0 then
...
function M:UpdateRecordListView(Target)
    ...
    local RecordTable = {
        ...
        },
        {
            ...
            GDate("Date_MD", {Year=year, Month=month, Day=day}, CommonConst.SystemLanguage),
        },
        {
    ...
    if(CharData and CharData.DispatchTag)then
        ...
        for index, value in ipairs(DispatchTags) do
            ...
            if(CharDispatchTagData)then
                ...
                if(UnlockLevel)then
                    if(Target.EnhanceLevel >= UnlockLevel)then
                        Obj.Unlocked = true
                    else
...
function M:OnBtnVoiceClicked()
    ...
    --     -- UIManager(self):LoadUI(UIConst.CHARRECORD_MOBILE, "CharRecord",self.Parent:GetZOrder(),self,self.Target,ArmoryUtils.FilesTabType[1],false)
    -- else
    --     -- UIManager(self):LoadUI(UIConst.CHARRECORD_PC, "CharRecord",self.Parent:GetZOrder(),self,self.Target,ArmoryUtils.FilesTabType[1],false)
    -- end
end
...
function M:OnBtnRecordClicked()
    ...
    -- if(CommonUtils.GetDeviceTypeByPlatformName(self) == "Mobile")then
        -- UIManager(self):LoadUI(UIConst.CHARRECORD_MOBILE, "CharRecord",self.Parent:GetZOrder(),self,self.Target,ArmoryUtils.FilesTabType[2],false)
    -- else
        -- UIManager(self):LoadUI(UIConst.CHARRECORD_PC,"CharRecord",self.Parent:GetZOrder(),self,self.Target,ArmoryUtils.FilesTabType[2],false)
    -- end
end
