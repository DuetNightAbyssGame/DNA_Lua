...
function M:PetMain_Init()
    ...
    if(#self.ResourcePetItemContentsArray > 0)then
        self.Tab_PetAlive:Init({Parent = self,TabIdx = self.CurPetTabIdx,OnClicked = self.PetMain_OnPetTabClicked})
...
function M:PetMain_InitSubUI()
    self:DefaultInitSubUI({Target = self.ComparedPet,CurPet = self.CurrentPet,bHideDeployBtn = self.Params.bHideDeployBtn})
...
function M:PetMain_SelectRoleListItem(Content)
    ...
    self:CreateAndSelectSubTab({Pet = self.ComparedPet})
...
function M:PetMain_SortItemContents()
    if(self.PetItemContentsArray == self.ResourcePetItemContentsArray)then
        ArmoryUtils:SortItemContents(self.PetItemContentsArray,{"_PetEntryRarity","_PetEntryId","SortPriority","UnitId"},CommonConst.DESC)
    else
        ArmoryUtils:SortItemContents(self.PetItemContentsArray,{"BreakNum","Level","Rarity","SortPriority","UnitId"},CommonConst.DESC,self.PetMain_CurContent)
    end
end

local AddContent = function(self,Pet)
...
local RemoveContent = function(self,UniqueId)
...
function M:PetMain_CreateItemContents()
    ...
    local InsertLogic = function(Pet)
    ...
    if(self.CurPetTabIdx == 1 and #self.UseablePetItemContentsArray == 0)then
        self.CurPetTabIdx = 2
    end
end
...
function M:OnPetEntryUp(ErrCode,UniqueId,EntryIndex,ConsumePetUniqueIds)
    ...
    if Content.IsResourcePet then
        Content.PetEntry = {Pet.Entry and Pet.Entry[1]}
...
function M:PetMain_ReceiveEnterState()
    self:ResetPetData()
    -- self:PetMain_CreateItemContents()
    -- self:PetMain_SelectPetTab(self.CurPetTabIdx,true)
