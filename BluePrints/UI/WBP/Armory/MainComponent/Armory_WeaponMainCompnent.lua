...
function M:Construct()
    self.AllWeaponMainTabNames = {ArmoryUtils.ArmoryMainTabNames.Melee,ArmoryUtils.ArmoryMainTabNames.Ranged,ArmoryUtils.ArmoryMainTabNames.Weapon}
    --注册各种武器的同名函数
    ...
    local WeaponTag = {CommonConst.ArmoryTag.Melee,CommonConst.ArmoryTag.Ranged}
...
function M:WeaponMain_JumpToSubPage(SubPageName)
    ...
    if(SubPageName == ArmoryUtils.ArmorySubPageName.Mod)then
...
function M:WeaponMain_PreprocessContents(Type,PreprocessParams)
    ...
    if(self.WeaponTag == CommonConst.ArmoryTag.UWeapon)then
        ...
        self[self.CurContentName] = {Type = CommonConst.ArmoryType.Weapon,Tag = CommonConst.ArmoryTag.UWeapon,
                                    Avatar = _Avatar,AvatarPrime = AvatarPrime,AvatarMax = AvatarMax}
...
function M:WeaponMain_UpdatAttributeButton()
    ...
    if(string.find(self.CurMainTab.Name,ArmoryUtils.ArmoryMainTabNames.UWeapon))then
        ...
        if(self.CurrentChar == self.ComparedChar)then
            ...
            self.CurrentSubUI:SetOpenModParams({CharUuid = self.ComparedChar.Uuid})
        end
    else
        ...
        if(self[self.ComparedWeaponName].Level >= TargetTable[self[self.ComparedWeaponName].WeaponId].WeaponMaxLevel)then
            ...
            if(self[self.ComparedWeaponName].Level >= MaxLevel)then
        ...
        if(self.CurrentChar == self.ComparedChar)then
            if(self[self.CurContentName] == self[self.CmpContentName])then
                ...
                self.CurrentSubUI:SetOpenModParams({[self[self.CmpContentName].Tag.."Uuid"] = self[self.CmpContentName].Uuid})
            end
        else
            self.CurrentSubUI:SetOpenModParams({[self[self.CmpContentName].Tag.."Uuid"] = self[self.CmpContentName].Uuid})
        end
...
function M:WeaponMain_SelectRoleListItem(Content)
    ...
    self:CreateAndSelectSubTab({Weapon = self[self.ComparedWeaponName]})
...
function M:UpdateWeaponCardLevel()
    for _, value in ipairs(self.SubTabs) do
        if(value.Name == ArmoryUtils.ArmorySubTabNames.Grade)then
            ...
            value.IsMaxLevel = value.Number >= MaxGradeLevel
...
local GetMainTabNameByWeaponTag = function(self,WeaponTag)
    local MainTabName
...
local AddContentCommon = function(self,Obj,MainTabName)
...
local AddContent = function(self,Weapon,Content)
...
local AddUnownedContent = function(self,Weapon,Content)
    local WeaponTag = Weapon:HasTag("Melee") and "Melee" or "Ranged"
    local MainTabName = GetMainTabNameByWeaponTag(self,WeaponTag)
    if(self[MainTabName.."ItemContentsMap"] == nil)then
        return
...
local UnownedContent2Content = function(self,UnownedContent,Weapon)
    local WeaponTag = Weapon:HasTag("Melee") and "Melee" or "Ranged"
...
local RemoveContent = function(self,WeaponUuid)
    local RemoveContentByTag = function(Tag)
...
function M:WeaponMain_CreateItemContents()
    ...
    if(self.WeaponTag == "Weapon")then
        --近战、远程混合在一个Tab
        local WeaponTags = {}
        local InsertLogic = function(Weapon)
            local WeaponTag = Weapon:HasTag("Melee") and "Melee" or "Ranged"
            ...
            try({
                exec = function()
                    ...
                    for WeaponId, value in pairs(DataMgr.Weapon) do
                        if(not value.IsNotOpen and not OwnedWeapons[WeaponId] and ShouldDisplayWeapon(WeaponId)
                        and CommonUtils.IsCurrentVersionRealease(CommonConst.DataType.Weapon,WeaponId))then
                            local DummyAvatar = ArmoryUtils:CreateNewDummyAvatar(ArmoryUtils.PreviewTargetStates.Prime,{WeaponIds = {WeaponId}})
                            ...
                            local DummyAvatarMax = ArmoryUtils:CreateNewDummyAvatar(ArmoryUtils.PreviewTargetStates.Max,{WeaponIds = {WeaponId}})
...
function M:WeaponMain_SortItemContents()
    ...
    ArmoryUtils:SortItemContents(self.WeaponItemContentsArray,
                                {"Level","Rarity","SortPriority","UnitId"},
                                CommonConst.DESC,
                                self.CurContentName and self[self.CurContentName],
                                ArmoryUtils.IsOwnedCmpFunc)
end
...
function M:WeaponMain_OnBtnIntensifyClicked()
    ...
    if(Level < RealMaxLevel)then
        local Params = {
                        --Title = GText(self[self.ComparedWeaponName][self.Type.."Name"]),
        ...
        if(NextBreakLevelInfo)then
            if(Level < NextBreakLevelInfo.WeaponBreakLevel)then
                ...
                -- UIManager(self):ShowCommonPopupUI(100029,Params,self)
                ...
                -- UIManager(self):ShowCommonPopupUI(100030,Params,self)
                UIManager(self):LoadUI(UIConst.LoadInConfig, UIConfig.UIName,self:GetZOrder(),CommonConst.ArmoryType.Weapon,self[self.ComparedWeaponName],self,Params)
            end
        else
            Params.BehaviourType = "LevelUp"
...
function M:WeaponMain_OnBtnReplaceClicked()
    ...
    if(Weapon.AssisterId and Weapon.AssisterId ~= 0)then
        local func = function()
            self:BlockAllUIInput(false)
        end
        local OnPopupConfirm = function()
            self:BlockAllUIInput(true)
            Avatar:SwitchWeapon(self.WeaponTag,Uuid)
        end
        local Params = {
            ShortText = string.format(  GText(DataMgr.CommonPopupUIContext[100044].PopoverText),
                                        GText(Weapon.WeaponName),
                                        GText(Avatar.Resources[Weapon.AssisterId].ResourceName)),
...
function M:ResetWeaponData()
    ...
    if(self.WeaponTag == CommonConst.ArmoryTag.UWeapon)then
        ...
        if(self.CharMain_CmpContent and self.CharMain_CmpContent.Avatar)then
            Avatar = self.CharMain_CmpContent.Avatar or Avatar
        end
        self[self.CurrentWeaponName]= Avatar.UWeapons[UWeaponUuid]
        self[self.ComparedWeaponName] = self[self.CurrentWeaponName]
    else
...
function M:WeaponMain_InitKeySetting(KeyDownEvents,KeyUpEvents,BottomKeyInfo)
    ...
    self:AddKeyEvents(KeyDownEvents,
