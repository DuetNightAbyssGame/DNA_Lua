...
function M:BattleWheelMain_ReceiveEnterState()
    ...
    if(IsValid(self.CurrentSubUI))then
        if(self.CurrentSubUI:IsPhantomWeaponMenuOpened())then
...
function M:BattleWheelMain_Init()
    ...
    self:DefaultInitSubUI({ToBattleWheelIndex = ToBattleWheelIndex,bHideWheelPlan = self.Params.bHideWheelPlan})
