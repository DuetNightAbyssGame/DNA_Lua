...
function M:CharMain_OnArmoryTargetStateChanged(NewAvatar)
    ...
    if(self.CurMainTab.Name == ArmoryUtils.ArmoryMainTabNames.Char)then
        ...
        self:CreateMainTabs({Char = self.ComparedChar,Avatar = CmpContent.Avatar})
...
function M:CharMain_InitSubUI()
    ...
    local Params = {CharUuid = Target.Uuid,
                    Target = Target,
...
function M:CharMain_UpdatAttributeButton()
    ...
    if(self.ComparedChar.Level >= DataMgr.Char[self.ComparedChar.CharId].CharMaxLevel)then
        ...
        if(self.ComparedChar.Level >= MaxLevel)then
...
function M:CharMain_OnRoleListItemClicked(Content)
    ...
    local Avatar = ArmoryUtils:GetAvatar()
    self.ComparedChar = Avatar.Chars[Content.Uuid] or Content.Target
    self:CreateMainTabs({Char = self.ComparedChar,Avatar = Content.Avatar})
    local CurMainTabIdx = self.MainTabName2Idx[self.CurMainTab.Name]
    if(self.MainTabs[CurMainTabIdx] and self.MainTabs[CurMainTabIdx].Name  == self.CurMainTab.Name)then
        self.CurMainTab = self.MainTabs[CurMainTabIdx]
        self.CurMainTab.IsOn = true
...
function M:CharMain_SelectRoleListItem(Content)
    ...
    -- SetItemReddotRead只清除NewChar，还需要检查7命New状态
    local ReadNew = true
    if not Content.Upgradeable and Content.IsOwned then
        ...
        if UltraNode and UltraNode.Count > 0 then
            ...
            if CacheDetail and CacheDetail[Content.UnitId] == 1 then
                Content.IsNew = true
...
function M:UpdateCharCardLevel()
    for _, value in ipairs(self.SubTabs) do
        if(value.Name == ArmoryUtils.ArmorySubTabNames.Grade)then
            ...
            if HasUltra then
                value.IsMaxLevel = (self.ComparedChar.GradeLevel >= MaxGradeLevel) and self.ComparedChar:IsExtraGradeLevelUnlocked()
            else
...
local AddContent = function(self,Char)
...
local AddUnownedContent = function(self,Char)
...
local UnownedContent2Content = function(self,UnownedContent,Char)
...
function M:CharMain_CreateItemContents()
    ...
    local InsertLogic = function(Char)
        ...
        try({
            exec = function()
                ...
                for CharId, value in pairs(DataMgr.Char) do
                    if(not value.IsNotOpen and ExcludeCharId ~= CharId and not OwnedChars[CharId]
                    and CommonUtils.IsCurrentVersionRealease(CommonConst.DataType.Char,CharId))then
                        ...
                        local DummyAvatarMax = ArmoryUtils:CreateNewDummyAvatar(ArmoryUtils.PreviewTargetStates.Max,{CharIds = {CharId}})
...
function M:CharMain_SortItemContents()
    ...
    local AttrNames = {"Level","SortPriority","UnitId"}
    ArmoryUtils:SortItemContents(self.CharItemContentsArray,
                AttrNames,
                                CommonConst.DESC,
                                self.CharMain_CurContent,
                                function(a,b)
...
function M:CharMain_OnBtnReplaceClicked()
    if(not self.ComparedChar or not self.ComparedChar.Uuid or self.ComparedChar == self.CurrentChar)then
        return
    end
...
function M:_UpdateSkillUpgradeReddot(Avatar,CharUuid)
    ...
    if not CharUuid then CharUuid = self.ComparedChar.Uuid end
    ...
    for _, CharSkill in pairs(Char.Skills) do
        ...
        if not Ret.CanLevelUp then
            ArmoryUtils:SetItemReddotRead({Type=ContentType, CharUuid=CharUuid,SkillId=CharSkill.SkillId},false,true)
        end
    end
end
...
function M:ResetCharData()
    ...
    self.ComparedChar = Avatar.Chars[self.ComparedChar.Uuid] or (self.CharMain_CmpContent and self.CharMain_CmpContent.Target)
    if(self.CurrentChar)then
...
function M:OnCharSkillLevelUp(Ret, SkillId, CurrentLevel)
    self:ResetCharData()
    local Avatar = ArmoryUtils:GetAvatar()
    ArmoryUtils:SetItemReddotRead({Type=CommonConst.DataType.Char..Const.Skill, CharUuid=self.ComparedChar.Uuid,SkillId=SkillId},false,true)
...
function M:OnCharExtraGradeLevelUp(Ret, CharUuid)
    ...
    -- 第7级解锁后，将该角色的PromoteChar缓存标记为已读（Count-1）
...
function M:OnCharExtraGradeItemClick(CharUuid)
    ...
    if(Char and Content)then
        ...
        ArmoryUtils:SetItemReddotRead({ItemType = CommonConst.DataType.Char,Uuid = CharUuid},true)
    end
end
...
function M:CharMain_InitKeySetting(KeyDownEvents,KeyUpEvents,BottomKeyInfo)
    ...
    if(self.CurSubTab.Name == ArmoryUtils.ArmorySubTabNames.Attribute)then
        if(not self.AttributeButtonStyleParams[1].ForbidBtn and not self.IsPreviewMode and ArmoryUtils:GetCharByUuid(self.ComparedChar.Uuid) ~= nil)then
