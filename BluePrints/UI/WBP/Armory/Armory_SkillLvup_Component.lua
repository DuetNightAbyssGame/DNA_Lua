...
function Component:PreLevel()
    -- if(self.ComparedLevel - 1 <= self.Level)then
    --     return
    -- end
    -- self:UpdateSkillLevelUpInfo(self.Level,self.ComparedLevel - 1)
end
...
function Component:UpdateSkillLevelUpInfo(Level,ComparedLevel)
    ...
    -- self.Params.Funds = {}
    -- for index, value in ipairs(self.ResourceUse) do
    --     if(value.Count > 0)then
    --         table.insert(self.Params.Funds,{FundId = value.ResourceId,FundNeed = value.Count})
    --     end
    -- end
    -- self:BroadcastDialogEvent("UpdateFunds",self.Params)
...
function Component:UpdateSkillAttrView(Level, ComparedLevel)
    ...
    for i, Attr in ipairs(CurLevelAttrTable) do
        if(Attr.Value ~= ComparedLevelAttrTable[i].Value)then
            table.insert(self.Attrs,{AttrName = Attr.Desc,
                                            AttrValue = Attr.Value,
                                            AttrDesc = Attr.Hint
                                            })
            table.insert(self.ComparedAttrs,{AttrValue = ComparedLevelAttrTable[i].Value,Delta=1}) --技能升级应该都是正面的，Delte直接给一个正数，之后有迭代再改
        end
    end
