...
---@type WBP_Armory_BattleMenu_Base_C
...
function M:Construct()
    ...
    for Idx, value in pairs(self.BattleWheelWidgets) do
        ...
        value.WheelIdx = Idx
        value:PreInitByParam({BehaviourType = UIConst.BATTLE_MENU_BEHAVIOUR_TYPE.ARMOURY_BATTLE_MENU})
        ...
        value.InstallSoundFunc = function(Content)
    ...
    self:BindToAnimationFinished(self.Auto_In, {self,self.OnInAnimFinished})

    local OnWheelBtnClicked = function(Btn)
...
function M:EnableWheelWidgetPointerDetection(WheelWidget,bEnable)
    ...
    -- WheelWidget.bIsFocusable = bEnable
end
...
function M:SetSlotItem(_SlotIdx,Content)
    if(_SlotIdx == nil)then return end
...
function M:ExchangedItem(bSuccessful,_FromIdx,_ToIdx)
    if(_FromIdx == nil or _ToIdx == nil)then return end
...
function M:ModifyWheelAnimationVariable(Params)
    ...
    self.PositionLerpEnd = {
        [1] = {[2] = 0,[3] = -self.DesiredPositionX},
        [2] = {[1] = self.DesiredPositionX,[3] = -self.DesiredPositionX},
        [3] = {[2] = 0,[1] = self.DesiredPositionX},
    }
end
...
function M:CreateWheelAnimationVariable()
    ...
    self.ScrollDirection2WhellIdx = {
        [self.EWheelScrollDirection.Left] = {{1,3},{2,1},{3,2}},
        [self.EWheelScrollDirection.Right] = {{3,1},{2,3},{1,2}},
    }
    ...
    self.Curves_Position = {
        [1] = {
            [2] = {Curve = self.Curve_BattleMenuPosition_Left2Middle,Dir = 1},
            [3] = {Curve = self.Curve_BattleMenuPosition_Left2Right,Dir = 1},
        },
        [2] = {
            [1] = {Curve = self.Curve_BattleMenuPosition_Left2Middle,Dir = -1},
            [3] = {Curve = self.Curve_BattleMenuPosition_Left2Middle,Dir = -1},
        },
        [3] = {
            [1] = {Curve = self.Curve_BattleMenuPosition_Left2Right,Dir = -1},
            [2] = {Curve = self.Curve_BattleMenuPosition_Left2Middle,Dir = -1},
        }
    }
    self.Curves_Scale = {
        [1] = {
            [2] = {Curve = self.Curve_BattleMenuScale_Left2Middle},
        },
        [2] = {
            [1] = {Curve = self.Curve_BattleMenuScale_Left2Middle,bReverse = -1},
            [3] = {Curve = self.Curve_BattleMenuScale_Left2Middle,bReverse = -1},
        },
        [3] = {
            [2] = {Curve = self.Curve_BattleMenuScale_Left2Middle},
        }
    ...
    self.Curves_RenderOpacity = {
        [1] = {
            [2] = {Curve = self.Curve_BattleMenuRenderOpacity_Left2Middle},
        },
        [2] = {
            [1] = {Curve = self.Curve_BattleMenuRenderOpacity_Left2Middle,bReverse = -1},
            [3] = {Curve = self.Curve_BattleMenuRenderOpacity_Left2Middle,bReverse = -1},
        },
        [3] = {
            [2] = {Curve = self.Curve_BattleMenuRenderOpacity_Left2Middle},
        }
...
function M:Tick(MyGeometry, InDeltaTime)
    ...
    local Position,Scale,RendreOpacity
    local WheelIdx1,WheelIdx2
        ...
        EditTextConfig = {
            ...
            OnCheckStringSensitive = function(_,bSuccess,Name)
                if(bSuccess)then
                    ...
                    --DebugPrint("sssssssssFail",self,ComDialogInputInfo.Text)
                end
            end
        },
    }, self)
...
local AddContent = function(self,Resource,CharId2Char)
    ...
    if(Data and string.match(Data.Type, "BattleItem")) then
        ...
        Content.OnMouseButtonDownEarly = function(Widget, Content, MouseEvent)
            return self:OnRightMouseButtonDown(Content, MouseEvent)
        end
        Content.OnMouseEnterEvent = {Callback = function(_Content)
...
function M:NewItemContent(ServerData)
    ...
    if(Data.Type == "InfiniteBattleItem")then
        ...
        elseif(Obj.ResourceSType == "MountItem")then
            ...
            Obj.ItemName = GText("INFINITY_SYMBOL") --用于列表的显示数量
...
function M:IsContentHasWeapon(Content)
    if(Content and Content.IsPhantom)then
        ...
        if(WeaponUuid == nil or WeaponUuid == "")then
            return false
        else
            return true
        end
...
function M:InitFilters()
    self.FiterContents = self.FiterContents or {{Owner = self,OnClicked = self.OnFilterWidgetClicked,IsSelected = true}}
    ...
    for index, value in ipairs(FiterWidgets) do
        if(index <= #Data)then
            self.FiterContents[index] = self.FiterContents[index] or {Owner = self,OnClicked = self.OnFilterWidgetClicked,IsSelected = false}
...
function M:SortContents(InOutContents)
    --排序
    -- local SortBy, SortType = self.SortList:GetSortInfos()
    local SortBy, SortType = 1,CommonConst.DESC
    local SortByAttrNames = {self.OrderByAttrNames[SortBy]}
    for index, value in ipairs(self.OrderByAttrNames) do
        if(index ~= SortBy)then
            table.insert(SortByAttrNames,value)
        end
    end
    ArmoryUtils:SortItemContents(InOutContents,SortByAttrNames,SortType,nil,
                                function(a,b)
...
function M:OnListItemSelectionChanged(Content,IsSelected)
    -- if(self.CurSelectedSlotId)then
    --     self:HideItemDetail()
    -- end
...
function M:OpenPhantomWeaponMenu(Content)
    self.PhantomWeaponMenu = UIManager(self):LoadUI(nil, "ArmoryBattleMenuWeaponConfig",
                                                    self.Parent:GetZOrder(),
                                                    {
                                                        Parent = self.Parent,
...
function M:SetItemDetailContent(Content,WheelIdx,SlotIdx)
    ...
    if(self.LastItemDetailWheelIdx)then
        if(WheelIdx ~= self.LastItemDetailWheelIdx or SlotIdx ~= self.LastItemDetailSlotIdx)then
...
function M:UpdateItemDetails(Content)
    if(Content == nil or Content == self.NoneContent)then
...
function M:TryEquipContent(Content,WheelIdx,WheelSlotIdx,SuccessCallback)
    ...
    if(self:IsContentInWheelAndFull(Content))then
        ...
        Avatar:ChangeBattleWheel(self.CurrentWheelIndex,
                                ServerSlotIdx,
                                Content.UnitId)
    end
end

--gm GWorld.GameInstance:GetGameUIManager():GetUIObj("ArmoryMain").CurrentSubUI:TestFunc()
function M:TestFunc()
    local Avatar = GWorld:GetAvatar()
...
function M:OnWheelSlotDroped(Content,DropedSlot)
    ...
    if(ContentInWheel)then
        if(Content.WheelIdx and Content.WheelIdx ~= self.CurWheelWidget.WheelIdx)then
            ...
            self.Parent:BlockAllUIInput(true)
            Avatar:ExchangeBattleWheel(self.CurrentWheelIndex,
                                        FormSlotIdx,
                                        self:WheelSlotIdx2ServerSlotIdx(DropedSlot))
            return
        else
            if(self:IsContentInWheelAndFull(ContentInWheel))then
...
function M:OnWheelSlotItemExchanged(FromContent,ToContent,FromSlotIdx,ToSlotIdx)
    ...
    local Avatar = GWorld:GetAvatar()
    Avatar:ExchangeBattleWheel(self.CurrentWheelIndex,
                            self:WheelSlotIdx2ServerSlotIdx(FromSlotIdx),
                            self:WheelSlotIdx2ServerSlotIdx(ToSlotIdx))
end
...
function M:OnWheelItemExchanged(Ret,WheelIndex, SlotId, TargetSlotId)
    ...
    self:ResetWheelSlotItems()
    self:SetItemDetailContent(self.ItemtDetailContent)
    -- local FromWheelIdx,FromSlotIdx = self:CalcWheelIdxAndSlotIdx(SlotId)
    -- local ToWheelIdx,ToIdx = self:CalcWheelIdxAndSlotIdx(TargetSlotId)
    -- if(FromWheelIdx == ToWheelIdx)then
    --     if(self.CurSelectedSlotId == FromSlotIdx or self.CurSelectedSlotId == ToIdx)then
    --         self:HideItemDetail()
    --     end
    -- else
    --     self:HideItemDetail()
