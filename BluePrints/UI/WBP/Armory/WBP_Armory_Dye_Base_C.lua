...
function M:Construct()
    ...
    rawset(self,"AllPlanNames",{"UI_Squad_Appearance_TITLE1","UI_Squad_Appearance_TITLE2","UI_Squad_Appearance_TITLE3"})
    ...
    self.Btn_Hide:BindEvents(self,{OnClicked = self.OnHideUIKeyDown,
    ...
    self.Btn_Compare:BindEvents(self,{OnPressed = self.OnContrastKeyDown,
                                    OnReleased = self.OnContrastKeyUp,
                                    OnAddedToFocusPath = self.OnFunctionBtnAddedToFocusPath,
                                    OnRemovedFromFocusPath = self.OnFunctionBtnRemovedFromFocusPath})
    self.Btn_Compare.Text_Name:SetText(GText("UI_Dye_Compare"))
    self.Btn_Delete:BindEvents(self,{OnClicked = self.OnDeleteBtnClicked,
                                    OnAddedToFocusPath = self.OnFunctionBtnAddedToFocusPath,
                                    OnRemovedFromFocusPath = self.OnFunctionBtnRemovedFromFocusPath})
    self.Btn_Delete.Text_Name:SetText(GText("UI_RegionMap_Delete"))
    self.Btn_Import:BindEvents(self,{OnClicked = self.OnImportBtnClicked,
                                    OnAddedToFocusPath = self.OnFunctionBtnAddedToFocusPath,
                                    OnRemovedFromFocusPath = self.OnFunctionBtnRemovedFromFocusPath})
    self.Btn_Import.Text_Name:SetText(GText("UI_Dye_Input_Title"))
    self.Btn_Mounted:BindEvents(self,{OnClicked = self.OnMountedBtnClicked,
                                    OnAddedToFocusPath = self.OnFunctionBtnAddedToFocusPath,
                                    OnRemovedFromFocusPath = self.OnFunctionBtnRemovedFromFocusPath})
...
function M:OnImportBtnClicked()
	local Params = {}
	Params.TextLenMax = 200 -- 设置最大文本长度
	Params.MultilineType = 1 -- 单行输入
	Params.HintText = GText("UI_Dye_Input_Content") -- 提示文本
    Params.UseGenaral = true
	Params.RightCallbackFunction = function(self2, Data)
		local shareCode = Data.ComDialogInput.Text -- 正确获取输入框文本
		if shareCode and shareCode ~= "" then
            ...
            if (dyePlanInfo.SkinId ~= self.SkinId)
                or (dyePlanInfo.SkinType == "Char" and self.SkinType ~= CommonConst.DataType.Skin)
                or (dyePlanInfo.SkinType == "Hair" and self.SkinType ~= CommonConst.DataType.Hair) then
                local TargetName
...
function M:InitUIInfo(Name, IsInUIMode, EventList, Params)
    ...
    if self.SkinType == CommonConst.DataType.Hair then
        ...
        self.HairId = Params.HairId --暂时用不到
...
function M:OnActorCreated(WeaponActor)
    ...
    elseif self.Type == CommonConst.ArmoryType.Weapon and self.ArmoryPlayer and not self.bStandaloneWeapon then --手持武器，有ArmoryPlayer
        self.ActorController:SetArmoryCameraTag("Weapon","0","Dye")
    elseif self.Type == CommonConst.ArmoryType.Weapon and (not self.ArmoryPlayer or self.bStandaloneWeapon) then --独立武器,无ArmoryPlayer
        --不做任何操作，模型和相机的参数继承自上一个界面(皮肤界面等)
    else
...
function M:InitUI()
    ...
    --初始化本地缓存数据
    -- 根据类型选择不同的标识符：角色和武器使用Uuid，坐骑使用MountId
    local CacheKey = nil
...
function M:ApplyColorsToNormalDyeTabs(TargetColors)
    ...
    for Idx,DyeTabItem in pairs(AllDyeTabItems) do
        ...
        local ColorContent
        if tonumber(TargetColors[Idx]) ~= -1 then
...
function M:SelectDyePlan(PlanIndex,TabIdx)
    ...
    elseif(self.Type == CommonConst.ArmoryType.Mount)then
        ...
        -- self.SubTab_Dye:Init({
        --     Tabs = {{
        --                 Text = GText("UI_Armory_Dye_Common"),
        --             },
        --             {
        --                 Text = GText("UI_Armory_Dye_Special"),
        --             },},
        --     LeftGamePadKey = "LeftShoulder",
        --     RightGamePadKey = "RightShoulder",
        --     })
        -- self.SubTab_Dye:BindEventOnTabSelected(nil,nil)
        -- self.SubTab_Dye:SelectTab(TabIdx)
        -- self.SubTab_Dye:BindEventOnTabSelected(self,self.OnDyeingTypeTabClicked)
        self.Panel_SubTab:SetVisibility(UIConst.VisibilityOp.Collapsed)
    end
    self:UpdateItemConsume()
end
...
function M:SwitchColorPlan(NewPlanIndex)
    ...
    local CallServerSwitchPlan = function()
...
function M:UpdatePreviewOrNotUI()
    ...
    local ProcessUIVisibility = function(Target,InTargetVisiblity)
...
function M:ShareCurrentDraft()
    -- if(self.SpecialCurrentContent and self.SpecialComparedContent)then
    --     if(self.SpecialCurrentContent.ColorId ~= self.SpecialComparedContent.ColorId)then
    --         UIManager(self):ShowUITip(UIConst.Tip_CommonToast,GText("UI_Dye_Output_Temporary"))
    --         return
    --     end
    -- end
    -- for index, value in pairs(self.NormalComparedContents) do
    --     if(value.ColorId ~= self.NormalCurrentContents[index].ColorId)then
    --         UIManager(self):ShowUITip(UIConst.Tip_CommonToast,GText("UI_Dye_Output_Temporary"))
    --         return
    --     end
    -- end
...
function M:ImportDraft(DraftInfo)
    if self.IsPreviewMode and self.OpenPreviewDyeFromChat then --从聊天频道打开
        if not self:HasRealTargetAndSkin() then return end
    ...
    elseif not self.IsPreviewMode then --从军械库打开
        self:SaveCurrentDraft()
        self:UpdatePreviewOrNotUI()
    end
end
...
function M:CreateDefaultSkin(SkinId)
    ...
    for i = 1, self.ColorPartCount, 1 do
...
function M:CreateNormalColorContents()
    ...
    for key, value in pairs(SwatchData) do
        ...
        UKismetMathLibrary.LinearColor_SetFromSRGB(Obj.Color,
            FColor(Color[1] or 0,Color[2] or 0,Color[3] or 0))
    ...
    for index, value in ipairs(self.ColorContents) do
        if(index % self.ColorPerRow == 1)then
...
function M:ResetNormalCurrentContens()
    ...
    local Colors = self.CurrentSkin:GetColors(self.CurrentPlan)
    for Idx = 1, self.ColorPartCount, 1 do
...
function M:CreateNormalDefaultColor()
    ...
    if(self.Type == CommonConst.ArmoryType.Char)then
        if(self.SkinType == CommonConst.DataType.Hair)then
            self.DefaultColors = {self.ArmoryPlayer.CharacterFashion:GetHiarDefaultColors()}
            return
        end
        self.DefaultColors = {self:GetCharDefaultColorsFromDataTable(self.ArmoryPlayer)}
        self.DefaultFresnel = {[8] = self.DefaultColors[#self.DefaultColors]}
    ...
    elseif(self.Type == CommonConst.ArmoryType.Mount)then
        self.DefaultColors = {self:GetMountDefaultColorsFromDataTable(self.ArmoryPlayer.CurMount)}
    else
        self.DefaultColors = {self:GetWeaponDefaultColorsFromDataTable(self.UsingWeapon)}
    end
end
...
function M:OnNormalDyeTabInitialized(Content,Widget)
    if(IsValid(Widget))then
        ...
        --Widget:SetColor(self.NormalCurrentContents[Content.Idx].ResourceId and self.NormalCurrentContents[Content.Idx].Color)
    end
end
...
function M:NormalColorHoverLogic(Content)
    ...
    if(self.CurrentSkin:GetWeaponSpecialColor(self.CurrentPlan) ~= self.DefaultColorId
        and not next(self.NormalComparedContents)
        and self.SpecialDefaultContent)then
    ...
    if(self.CurNormalDyeTab.Idx == 9)then
        self.ActorController:ChangeSkinWeaponVFXColor(Content.Color)
    end
end
...
function M:OnNormalColorListItemUnhovered()
    ...
    if(not next(self.NormalComparedContents)
        and self.CurrentSkin:GetWeaponSpecialColor(self.CurrentPlan) ~= self.DefaultColorId
        and self.SpecialCurrentContent)then
...
function M:CreateSpecialColorContents()
    ...
    self.SpecialComparedContent = nil
    self.SpecialCurrentContent = self.SpecialColorContentsMap[self.CurrentSkin:GetWeaponSpecialColor(self.CurrentPlan)]
                                 or self.SpecialDefaultContent
end
...
function M:UpdateItemConsume()
    if(self.CurrentTabIdx == self.NormalColorTabIdx)then
        ...
        self:UpdateItemConsumeManual({self.SpecialCurrentContent},{self.SpecialComparedContent})
    end
end
    ...
    if(not CanApplyColors)then
        ...
        for ResourceId, value in pairs(UseItems_Map) do
            if(value.NotEnoughCount > 0)then
                local Obj = {ItemId = ResourceId,Count = value.NotEnoughCount, ItemType = CommonConst.ItemType.Resource,}
            ...
            if(self.ResourceId2CommonDyeIds[NotEnoughResource.ItemId])then
                for CommonDyeId, _ in pairs(self.ResourceId2CommonDyeIds[NotEnoughResource.ItemId]) do
                    ...
                    if(not CommonDyeObj)then
                        ...
                        CommonDyeObj = {Count = CommonDyeResource.Count,
                                        NeedCount = 0,
                                        Icon = Data.Icon,
                                        Id = CommonDyeId,
                                        Rarity = Data.Rarity,
        ...
        if(TotalNotEnoughResourceCount == 0)then
            for index, value in ipairs(NewUseItems_Array) do
                table.insert(self.CommonDyeResource,{ItemId = value.Id,Count = value.NeedCount, ItemType = CommonConst.ItemType.Resource,})
            end
...
function M:UpdateConsumItemWidgets(Contents)
    if(#Contents == 0)then
        ...
        if #self.ColorItemContents == 0 and (#Contents == 1)then
            self:PlayAnimation(self.In_2)
        else
        ...
        if(ChildrenNum - 1 > ContentNum)then
            --要留一个Widget用于下次获取Class
            local ShouldKeepCount = ContentNum + 1
            if ShouldKeepCount % ItemPerRow ~= 0 then
                ShouldKeepCount = ShouldKeepCount + ItemPerRow - ShouldKeepCount % ItemPerRow
            end
            if(ShouldKeepCount == 3 and ContentNum == 1)then
                ShouldKeepCount = 2
            end
        ...
        elseif(ChildrenNum - 1 < ContentNum)then
            for i = ChildrenNum,ContentNum,1 do
                self.WB_Consume:AddChild(UIManager:CreateWidget(WidgetClass, false))
            end
            if ((ContentNum + 1) % ItemPerRow ~= 0) then
                for i=1,ItemPerRow - (ContentNum + 1) % ItemPerRow,1 do
                    self.WB_Consume:AddChild(UIManager:CreateWidget(WidgetClass, false))
                end
            end
        end
        AllChildren = self.WB_Consume:GetAllChildren():ToTable()
        for i = 2,#AllChildren,1 do
...
function M:OnDoneBtnClicked()
    ...
    for _, Item in pairs(UseItems) do
        if(Item.Id)then
            table.insert(ItemList,{ItemId = Item.Id,
            ItemType = CommonConst.ItemType.Resource,
    ...
    if(#ItemList > 0 )then
        if(self.bNeedCommonDye)then
            ...
            Params.ShortTextParams = {ItemList[1].ItemNeed}
...
function M:OnResourcesChanged(ResourceId)
    if(self.CurrentTabIdx == self.NormalColorTabIdx)then
        ...
        if(self.SpecialComparedContent)then
            if(self.SpecialComparedContent.ResourceId == ResourceId)then
                self:UpdateItemConsume()
            end
        end
    end
...
function M:RecoverActorColor()
    ...
    if(self.Type == CommonConst.ArmoryType.Char)then
        ...
        if(self.CurrentSkin:GetWeaponSpecialColor(self.CurrentPlan) == self.DefaultColorId)then
            self:ChangeSpecialColor(self.SpecialCurrentContent)
            self:ChangeToNormalCurrentColors()
        else
            self:ChangeToNormalCurrentColors()
