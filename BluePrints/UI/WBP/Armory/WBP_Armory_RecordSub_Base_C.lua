...
function M:OnRecordSubTabSelected(TabWidget)
    ...
    for Idx, Data in ipairs(Datas) do
        ...
        Obj.IsLocked = false
        --@todo 解锁状态不应该归红点管，后面有解锁需求在业务里拓展吧
        --Obj.IsLocked =  Data.CharDataTargetType == nil or ConditionUtils.CheckCondition(Avatar,Data.CharDataTargetType,false)
...
function M:BP_GetDesiredFocusTarget()
    --- 接入通用下拉框的时候写的默认聚焦控件，使得通用下拉框能够返回上一次选中的控件
    --- 后续当前界面接入手柄的时候可以根据需要修改
    local SelectedContent = self.SelectedContent
    local UI = SelectedContent and SelectedContent.UI
    if not UI then
        UI = self.ScrollBox:GetChildAt(0)
    end
    if UI then
