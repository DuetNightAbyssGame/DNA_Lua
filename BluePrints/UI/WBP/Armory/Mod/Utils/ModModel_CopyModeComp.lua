...
function Component:CalcImportSlotsList(Target, Mod, bIgnorePolarity)
    ...
    for SlotId, Slot in pairs(Target.ModSuits[Target.ModSuitIndex]) do
        ...
        if not self:IsSpecificSlotByMod(Mod, SlotId) then 
            goto continue4
        end
        if not bIgnorePolarity and Mod.Polarity ~= CommonConst.NonePolarity then
            if Slot.Polarity ~= CommonConst.NonePolarity and Slot.Polarity ~= Mod.Polarity then
                goto continue4
            end
        end 
    ...
    local SortFunc = function(Slot1, Slot2)
        if Mod.Polarity == Slot1.Polarity and Mod.Polarity == Slot2.Polarity then
            return Slot1.SlotId < Slot2.SlotId
        end
        if Mod.Polarity == Slot1.Polarity then return true end
        if Mod.Polarity == Slot2.Polarity then return false end
        if Mod.Polarity ~= CommonConst.NonePolarity then
            ...
            if CommonConst.NonePolarity == Slot1.Polarity then return true end
            if CommonConst.NonePolarity == Slot2.Polarity then return false end
        end
        return Slot1.SlotId < Slot2.SlotId
    end
...
function Component:CalcImportModList()
    ...
    local SortFunc = function(ImportMod1, ImportMod2)
        if Tag == "Char" and ImportMod1.SlotId == 9 then return true end
        if Tag == "Char" and ImportMod2.SlotId == 9 then return false end
...
function Component:IsModSuitInfoMsg(InMsgStr)
    ...
    if not SuitInfoCopyed then return false end
...
function Component:GenerateModSuitInfoMsg()
    ...
    if Table.ModsInfo then
        ...
        if bEmptySuit then return nil end
    end
...
function Component:CreateDummyAvatarForCopyMode(ModSuitInfo, SenderName)
    ...
    if TargetType == CommonConst.DataType.Char then
        ArmoryUtils._CreateDummyAvatarCustom(self.DummyAvatar_CopyMode, {
            CharInfos ={{
                RoleId = TargetId, Exp = 0, Level = Level,
                GradeLevel = GradeLevel, EnhanceLevel = EnhanceLevel,
                ModData = ModData, SlotData = SlotData, ModSuitIndex = 1
            }}
        })
    ...
    elseif TargetType == CommonConst.DataType.Weapon then
        ArmoryUtils._CreateDummyAvatarCustom(self.DummyAvatar_CopyMode, {
            WeaponInfos ={{
                WeaponId = TargetId, Exp = 0, Level = Level,
                GradeLevel = GradeLevel, EnhanceLevel = EnhanceLevel,
                ModData = ModData, SlotData = SlotData, ModSuitIndex = 1
            }}
        })
...
function Component:_DumpModData(ModSuitInfo)
    ...
    for SlotId, ModInfo in ipairs(ModSuitInfo.ModsInfo) do
        ...
        local OneSlotData = {
            SlotId= SlotId, Polarity = SlotPolarity,
...
function Component:CopyCodeToModSuitInfo(Code)
    ...
    if Pos<= string.len(Code) then
        ...
        if TargetTypeCode == "C" then
            TargetInfo[1] = CommonConst.DataType.Char
        elseif TargetTypeCode == "W" then
            TargetInfo[1] = CommonConst.DataType.Weapon
        elseif TargetTypeCode == "U" then
