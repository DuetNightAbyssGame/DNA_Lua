...
---@type WBP_Armory_Mod_Replace_C|BP_UIState_C
...
function M:CheckIsAnySlotDirty()
    local bDirty = ModModel.PolarityEditModeData:GetDirtyCount() ~=0
...
function M:SetCostNum()
    ...
    self.bEnough = ResourceCount>0 and ResourceCount>=Cost
...
function M:SetUpWhenSelectSlot(SelectedStuff)
    ...
    local Polarity = SlotUIData:GetPolarity() == -1 and 0 or SlotUIData:GetPolarity()
    self.List_Polarity:SetSelectedIndex(Polarity)
end

function M:_GetResourceIds()
    local TargetType = ModModel:GetTarget():GetTypeName()
    self.ResourceIds = TargetType=="Char" and DataMgr.ResourceSType2Resource.Polarization 
                        or DataMgr.ResourceSType2Resource.WeaponPolarization
    return self.ResourceIds
end

function M:GetResourceId()
    return self:_GetResourceIds()[1] ---@todo 目前只消耗一个道具，等之后有多消耗再迭代吧
end
...
function M:OnApplyPolarityEdit()
    ...
    if ModModel.PolarityEditModeData:IsAnySlotOverCost() then
        ...
        for i=#ModModel.PolarityEditModeData.DirtySlots, 1, -1 do
            ...
            if SlotUIData.bOverCost == false then break end
            local Mod = SlotUIData:GetMod()
            table.insert(ModNames, string.format("<H>%s</>",GText(Mod:Data().Name)))
        end
        Params.ShortTextParams = {table.concat(ModNames, "|")}
...
function M:SetUpButtonWidget()
    ...
    local Params = {
        ...
        --KeyIconName ---@todo 接手柄端时需要填写这个参数
        Owner = self,
        Numerator = 0,
    }
