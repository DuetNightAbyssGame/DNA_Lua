...
function Component:PreLevel()
    if(self.ComparedLevel - 1 <= self.Level or self.PreLevelTimes == 0)then
        return
    end
...
function Component:OnBtnAddMaxClicked()
    ...
    repeat
        ...
        if(Res.CanUpgrade)then
            ...
            if(ComparedExp < ComparedMaxExp)then
                self.PreLevelTimes = self.PreLevelTimes - 1
                self.NextLevelTimes = self.NextLevelTimes + 1
            end
        end
...
function Component:InitContent(Params, PopupData, Owner)
    ...
    if(self.Owner)then
        ...
        self.Owner.RightBtnClickedCallback = self.TryToLevelUp
...
function Component:OnLvUpResourcesChanged(ResourceId)
    if self.CurrentMode ~= "LevelUp" then return end
...
function Component:InitLvUpView(LevelUpParams)
    ...
    if(not Avatar or not self.Target or not self.Type)then
        -- return
    end
    self.MaxLevel = 1
...
function Component:UpdateLevelUpInfo(Level,ComparedLevel)
    ...
    local ComparedExp = 0
    local Res = nil
    if(ComparedLevel <= Level + 1 or self.PreLevelTimes == 0)then
    ...
    if(ComparedLevel <= Level)then
        ...
        if(self.CanUpgrade and ComparedExp >= ComparedMaxExp and ComparedLevel + 1 <= self.MaxLevel)then
            --经验溢出
            ...
            elseif(ComparedLevel - 1 > Level)then
                --向后跳级
                while CanUpgrade and _ExpConsum == Res.ExpConsume and SkipToLevel > Level do
                    SkipToLevel = SkipToLevel - 1
                    Res = UpgradeUtils.CalcCharOrWeaponLevelUp(self.Target,self.Type,Level,SkipToLevel)
                    CanUpgrade = Res.CanUpgrade
                end
    ...
    --更新道具数量
    if (Res) then 
        table.insert(Res.ResourceUse, {ResourceId=self.CashId, Count=self.CashConsume})
...
function Component:UpdateLvUpResourceItems(ResourceUse)
    if not ResourceUse or #ResourceUse == 0 then return end
    -- 分离有效和无效资源
    ...
    for _, Resource in ipairs(validResources) do
        ...
        if IsValid(itemWidget) then
            -- itemWidget.Switch_Type:SetActiveWidgetIndex(0)
            ...
            if resourceConfig then
                ...
                -- itemWidget.Text_Switch:SetActiveWidgetIndex(1)
    ...
    for _, Resource in ipairs(emptyResources) do
        ...
        if IsValid(itemWidget) then
            itemWidget:Init({})
            -- itemWidget.Switch_Type:SetActiveWidgetIndex(1)
            itemWidget:SetVisibility(UIConst.VisibilityOp.Visible)
        end
        currentIndex = currentIndex + 1
    end
...
function Component:UpdataLevelUpAttr(Level,ComparedLevel)
    ...
    if(BaseATK and ATKLevelGrow)then
        ...
        table.insert(Attrs,{Key = "ATK",Value = BaseATK * LevelUpData[ATKLevelGrow]})
        table.insert(ComparedAttrs,{Key = "ATK",Value = BaseATK * ComparedLevelUpData[ATKLevelGrow]})
    end
    ...
    for i, Attr in ipairs(Attrs) do
        if(Attr.Value ~= ComparedAttrs[i].Value)then
            ...
            table.insert(self.Attrs,{AttrName = Data.Name,
                                            AttrValue = CommonUtils.AttrValueToString(Data,Attr.Value),
                                            AttrDesc = GText(Data.AttrDesc)})
            table.insert(self.ComparedAttrs,{AttrName = Data.Name,AttrValue = CommonUtils.AttrValueToString(Data, ComparedAttrs[i].Value or 0),Delta=(ComparedAttrs[i].Value or 0)-Attr.Value})
        end
    end
...
function Component:OnTargetLevelUp(Ret,Uuid, Level,NewLevel)
    ...
    if Ret == ErrorCode.RET_SUCCESS then
        ...
        if Avatar and Avatar[self.Type.."s"][Uuid] and Level == self.Level and NewLevel == Avatar[self.Type.."s"][Uuid].Level then
            ...
            self:UpdateLvUpResourceItems({{ResourceId = nil, Count = 0},{ResourceId = nil, Count = 0},{ResourceId = nil, Count = 0},{ResourceId = nil, Count = 0},{ResourceId = nil, Count = 0}})
            -- UIManager:LoadUI(nil,"UpgradePrompt",self:GetZOrder(),"LevelUp",self.Type,Level,NewLevel,self.Attrs,self.ComparedAttrs,self,Callback)
            -- 调用主界面回调更新信息
...
function Component:OnLevelUpAnimFinishedCallback()
    ...
    if(self.Level >=  self.MaxLevel)then
        ...
        --继续升级
        self:InitLvUpView({Target = self.Target,
                                    Type = self.Type,
                                    Tag = self.Tag})
    end
