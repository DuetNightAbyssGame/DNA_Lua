...
function Component:InitBreakLevelUpComp(...)
    ...
    -- self.Btn_Upgrade.Button_Area.OnClicked:Add(self,self.TryToBreakLevelUp)
...
--         self.Owner.RightBtnClickedCallback = self.TryToBreakLevelUp
...
function Component:OnBreakLvUpResourcesChanged(ResourceId)
    if self.CurrentMode ~= "BreakLevelUp" then return end
...
function Component:UpdateBreakLevelUpInfo(ResourcesChanged)
    ...
    if(CurrenModSlotNum > 0)then
        table.insert(Attrs,{Key = "ModSlotNum",Value = CurrenModSlotNum + 1})
        table.insert(ComparedAttrs,{Key = "ModSlotNum",Value = TargetModSlotNum + 1})
    end
    ...
    for i, Attr in ipairs(Attrs) do
        if(Attr.Value ~= ComparedAttrs[i].Value)then
            ...
            table.insert(self.Attrs,{AttrName = GText(Data.Name),
            AttrValue = CommonUtils.AttrValueToString(Data,Attr.Value),
            AttrDesc = GText(Data.AttrDesc)})
            table.insert(self.ComparedAttrs,{AttrName = GText(Data.Name),
            AttrValue = CommonUtils.AttrValueToString(Data, ComparedAttrs[i].Value or 0),Delta=(ComparedAttrs[i].Value or 0)-Attr.Value})
        end
    end
            ...
            if UnlockLevel then
                ...
                if CharDispatchTagData then
                    ...
                    if not CurrentUnlocked and NextUnlocked then
                        table.insert(Attrs,{
                            IsDispatchTag = true,
                            DispatchTagData = CharDispatchTagData,
                        })
...
function Component:OnTargetBreakLevelUp(Ret,Uuid,TargetBreakLevel)
    ...
    if Ret == ErrorCode.RET_SUCCESS then
        ...
        if Avatar and Avatar[self.Type.."s"][Uuid] and TargetBreakLevel == self.BreakLevel + 1 then
            ...
            -- UIManager:LoadUI(nil,"UpgradePrompt",self:GetZOrder(),"BraekLevelUp",self.Type,OldBreakLevel,self.MaxBreakLevel,self.Attrs,self.ComparedAttrs,self,Callback)
            -- 调用主界面回调更新信息
...
-- UIManager(GameInstance):LoadUINew("Setting")
