...
---@class Armory_Main_Base_C:BP_UIState_C
...
function M:Construct()
    ...
    self:UnbindFromAnimationStarted(self.Auto_Out, {self, self.OnOutAnimStarted})
    self:BindToAnimationStarted(self.Auto_Out, {self, self.OnOutAnimStarted})
    self:UnbindFromAnimationFinished(self.Auto_Out, {self, self.OnOutAnimFinished})
    self:BindToAnimationFinished(self.Auto_Out, {self, self.OnOutAnimFinished})
    self:UnbindFromAnimationFinished(self.RoleList_In, {self, self.OnRoleListInAnimFinished})
    self:BindToAnimationFinished(self.RoleList_In, {self, self.OnRoleListInAnimFinished})
    self:UnbindFromAnimationStarted(self.RoleList_In, {self, self.OnRoleListInAnimStarted})
    self:BindToAnimationStarted(self.RoleList_In, {self, self.OnRoleListInAnimStarted})
...
---@alias ParamType {MainTabName:string, SubTabName:string, PreviewCharIds:table, PreviewWeaponIds:table}
...
function M:InitUIInfo(Name, IsInUIMode, EventList, Params)
    ...
    self.Loaded = false
    Params = Params or {
        --MainTabName = ArmoryUtils.ArmoryMainTabNames.Char -- 跳转到主Tab
        --SubTabName = ArmoryUtils.ArmorySubTabNames.Attribute --跳转到子Tab
        --OnCloseDelegate = function -- 关闭回调
        --EPreviewSceneType = CommonConst.EPreviewSceneType.PreviewCommon -- 预览场景类型
        --bNeedStandBtn = false --是否需要角色立绘按钮
        --PreviewCharIds = {1101}, -- 预览角色Id列表（1级或满级）
        --PreviewWeaponIds = {10101,10298,20298,20301} -- 预览武器列表（1级或满级）
        --PreviewPetIds = {}, -- 预览宠物Id列表（1级或满级）
        --PreviewCharInfos = {Char:BattleDump()} --预览角色信息列表(与BattleDump返回值结构一致)
        --PreviewWeaponInfos = {Weapon:BattleDump()} --预览角色信息列表(与BattleDump返回值结构一致)
        --PreviewPetInfos = {Pet:BattleDump()} --预览宠物信息列表(与BattleDump返回值结构一致)
        --CharUuids = {}    --真实Avatar的角色Uuid（非预览模式）
        --WeaponUuids = {}    --真实Avatar的武器Uuid
        --PetUniqueIds = {}    --真实Avatar的宠物UniqueId
        --MeleeWeapon = 10101 -- 预览模式当前近战武器
        --RangedWeapon = 10101 -- 预览模式当前远程武器
        --bFromArchive = false -- 是否来自图鉴
        --bFormPersonalPage = false -- 是否来自个人主页
        --SelectedTargetId = nil -- 默认选中的角色/武器/宠物Id
        --SelectedTargetUuid = nil -- 默认选中的角色/武器/宠物Uuid
        --SelectedTargetIndex = nil --默认选中的角色/武器/宠物序号
        --DoNotSort = false --按传入的顺序显示

        --bHideCharTab = false --隐藏角色Tab
        --bHideMeleeTab = --隐藏近战Tab
        --bHideRangedTab = --隐藏远程Tab
        --bHideUltraTab = --隐藏同律Tab
        --bHideWeaponTab = --隐藏近战远程混合Tab
        --bHidePetTab = --隐藏轮盘Tab
        --bHideBattleWheel = --隐藏轮盘Tab

        --bHideCharAttribute = false --隐藏角色属性
        --bHideCharSkill = false --隐藏角色技能
        --bHideCharGrade = false --隐藏角色溯源
        --bHideCharAppearance = false --隐藏角色外观
        --bHideCharFiles = false --隐藏角色档案
        --bHideWeaponAttribute = false -- 隐藏武器属性
        --bHideWeaponGrade = false --隐藏武器熔炼
        --bHideWeaponAppearance = false --隐藏武器外观
        --bHidePetAttribute = false --隐藏宠物属性
        --bHidePetEntry = false --隐藏宠物词条

        --bHideDeployBtn = false --隐藏出战按钮
        --bHideBoxBtn = false --隐藏左下角列表展开按钮
        --bHideSquadBuildBtn = false --隐藏阵容预设按钮
        --bHideWheelPlan = false --隐藏切换轮盘方案按钮
        --bNoEndCamera = false  --不需要收场镜头
        --Title = string --标题

        --AppearanceIndex = 1 -- 外观套装索引
        --BattleWheelIndex = 1 --轮盘索引
        --EscapeArmoryCharID = 3301 --抽卡后退出军械库的角色ID
    }
    ...
    if(Params.PreviewCharIds or Params.PreviewCharInfos
     or Params.PreviewWeaponIds or Params.PreviewWeaponInfos
     or Params.PreviewPetIds or Params.PreviewPetInfos)then
    --预览模式初始化
        try({
            exec = function()
    ...
    if(self.DoNotSort)then
        --如果不排序，记录下初始的排序,这行需要再预览模式初始化后
        self:RecordInitialOrder()
    end
    --ArmoryUtils:RecreateAllReddotInfos()
    ...
    local PreprocessParams = {  SelectedTargetUuid = Params.SelectedTargetUuid,
    ...
    if(self.ComparedChar and Type ~= ArmoryUtils.ArmoryMainTabNames.Char)then
        --部分Tab初始化依赖角色，跳转到非角色Tab先初始化角色数据
    ...
    self.ActorController = ActorController:New({
        ...
        OnRecorverCameraEnd = {Func = self.OnRecorverCameraEnd,Obj = self},
        OnRecorverCameraStart = {Func = self.OnRecorverCameraStart,Obj = self},
    })
    ...
    self:CreateMainTabs({Char = self.ComparedChar})
...
function M:RecordInitialOrder()
    local Avatar = ArmoryUtils:GetAvatar()
    local InsertInitialOrderUuids = function(InitialOrderTable,InfoArray,CreateInfo2Uuids)
...
function M:JumpToTab(MainTabName,SubTabName)
    ...
    if(#self.MainTabs > 0)then
        if(MainTabName)then
            local TabIdxToSelect = nil
            local TabToSelecte = nil
...
function M:CreateTabInfos(TabsConfig,Params)
    ...
    for Idx, TabName in ipairs(TabIndexes) do
        ...
        if(Tab.CreateFunc)then
            ...
            -- elseif(LockReasonText)then
            --     local TabContent = self:CreateTabContent(Tab,Idx,bCreatedWidthClass)
            --     TabContent.IsLocked = true
            --     TabContent.LockReasonText = LockReasonText
            --     table.insert(ResultTabs,TabContent)
            end
        end
    end
    return ResultTabs
...
function M:UpdateSubTabs(SubTabs)
    ...
    local TabNameToSelect = self.SubTabName_JumpTo or (self.CurSubTab and self.CurSubTab.Name)
    local TabTypeToSelect = self.CurSubTab and self.CurSubTab.Type
    self.SubTabName_JumpTo = nil
    local TabToSelect = nil
    for _, Tab in ipairs(SubTabs) do
        local bShouldSelectTab = TabTypeToSelect==nil or (TabTypeToSelect and Tab.Type == TabTypeToSelect)
...
function M:CreateAndSelectSubTab(Params)
    ...
    local _Params = {Char = self.ComparedChar,bCreatedWidthClass = true,}
...
function M:DefaultInitSubUI(Params)
    ...
    if(SubUI == nil)then
        ...
        --local Data = DataMgr.WidgetUI["Armory"..SubUIName]
        ...
        if(UIManager and WidgetPath)then
            ...
            if(SubUI)then
                ...
                --特殊处理：角色、武器属性用同一控件
                if((self.CurSubTab.Type == CommonConst.ArmoryType.Weapon or self.CurSubTab.Type == CommonConst.ArmoryType.Char)
                    and self.CurSubTab.Name == ArmoryUtils.ArmorySubTabNames.Attribute)then
    ...
    if(self.CurrentSubUI)then
        if(SubUI ~= self.CurrentSubUI)then
...
function M:UpdateUpgradeReddotByResourceChanged(ResourceId)
    if(self.CurMainTab.Name == ArmoryUtils.ArmoryMainTabNames.Char
    or self.CurMainTab.Name == ArmoryUtils.ArmoryMainTabNames.Melee
    or self.CurMainTab.Name == ArmoryUtils.ArmoryMainTabNames.Ranged)then
        if(UpgradeUtils.IsResourceForUpgrade(ResourceId))then
...
function M:Close()
    ...
    if(self.IsPreviewMode)then
        self:ModifyWaitForCloseEventCount(true)
        self:BindToAnimationFinished(self.Auto_Out, {self, function() self:ModifyWaitForCloseEventCount(false) end})
        --self:StopAnimation(self.Auto_Out)
    else
        if(not self.IsSecondaryUI)then
            --如果自身是一级界面才播battlemain动画
...
function M:CreateConstInfos()
    self.MainTabsStyle = {
        ...
        --SoundFunc = self.PlayMainTabSound,
    ...
    self.AlwaysReturnTrue = function()
        return true
    end
    self.WeaponGradeSubTab = {
        ...
        CheckReddot = function(Params)
            ...
            return false,Count > 0 and Weapon.GradeLevel < MaxGradeLevel
        end
    }
                ...
                Tabs = {
                    [ArmoryUtils.ArmorySubTabNames.Attribute] = {
                        ...
                        CheckReddot = function(Params)
                            ...
                            return false,ArmoryUtils:TryAddCharRewardReddot(Char.CharId) 
                                        or ArmoryUtils:TryAddUnlockableCharReddot(Char.CharId)
                                        or UpgradeUtils.CalcCharOrWeaponLevelUp(Char,CommonConst.DataType.Char,Char.Level,Char.Level + 1).CanUpgrade
                                        or UpgradeUtils.CalcBreakLevelUp(Char,CommonConst.DataType.Char,Char.EnhanceLevel + 1).CanUpgrade
                        end
                    },
                    ...
                    [ArmoryUtils.ArmorySubTabNames.Grade] = {
                        -- PCWidgetPath = "WidgetBlueprint'/Game/UI/WBP/Armory/PC/WBP_Armoy_TraceAbility_P.WBP_Armoy_TraceAbility_P'",
                        -- MobileWidgetPath = "WidgetBlueprint'/Game/UI/WBP/Armory/Mobile/WBP_Armoy_TraceAbility_M.WBP_Armoy_TraceAbility_M'",
                        ...
                        CheckReddot = function(Params)
                            ...
                            return not self.IsPreviewMode and self.CheckCharUltraGradeNewReddot and self:CheckCharUltraGradeNewReddot(Char),
                                    not self.IsPreviewMode and self.CheckCharPromoteReddot and self:CheckCharPromoteReddot(Char)
                        end
                    },
                    [ArmoryUtils.ArmorySubTabNames.Appearance] = {
                        PCWidgetPath = "/Game/UI/WBP/Armory/PC/WBP_Armory_Appearance_P.WBP_Armory_Appearance_P",
                    ...
                    [ArmoryUtils.ArmorySubTabNames.Files] = {
                        ...
                        CheckIsUnlocked = function(Avatar,Char)
                            ...
                            if(self.Params.bHideCharFiles
                             or Char.CharId == Gender2RoleIds[0]
                             or Char.CharId == Gender2RoleIds[1])then
                                return false
                            end
                    ...
                    [ArmoryUtils.ArmorySubTabNames.Attribute] = {
                        ...
                        CheckReddot = function(Params)
                            ...
                            return false,ArmoryUtils:TryAddWeaponRewardReddot(Weapon.WeaponId)
                                        or UpgradeUtils.CalcCharOrWeaponLevelUp(Weapon,CommonConst.DataType.Weapon,Weapon.Level,Weapon.Level + 1).CanUpgrade
                                        or UpgradeUtils.CalcBreakLevelUp(Weapon,CommonConst.DataType.Char,Weapon.EnhanceLevel + 1).CanUpgrade
                        end
                    },
            ...
            [ArmoryUtils.ArmoryMainTabNames.UWeapon] = {
                CreateFunc = function(Avatar,Params)
                    ...
                    local UWeaponTabConfig = {
                        --Name = ArmoryUtils.ArmoryMainTabNames.UWeapon,
                        Type = CommonConst.ArmoryType.Weapon,
                    ...
                    local CopyTab = function(TabToBeCopy)
                        local NewTab = {}
                    ...
                    if(Params.Avatar)then
                        --未获取角色的同律武器使用另一个Avatar
                        Avatar = Params.Avatar
                    end
                    if(Char and IsUWeaponUnlocked)then
                        for i, Uuid in ipairs(Char.UWeaponEids) do
                            local bHasUWeapon = Uuid 
                                    and Avatar.UWeapons[Uuid]
                                    and not DataMgr.UWeapon[Avatar.UWeapons[Uuid].WeaponId].IsInvisible
                            if(bHasUWeapon)then
