...
---@type WBP_Armory_SkillDetail_P_C
...
function M:AddReddotListener()
    ...
    -- self.ReddotNodeName = CommonConst.DataType.Char..Const.Skill..CommonUtils.ObjId2Str(self.CharUuid)
    -- ReddotManager.AddListener(self.ReddotNodeName, self, function(self, Count)
    --     if(self.IsPreviewMode)then
    --         return
    --     end
    --     local Len = self.Skill_Tab:GetNumItems()
    --     local Contens = self.Skill_Tab:GetListItems()
    --     local CacheDetail = ReddotManager.GetLeafNodeCacheDetail(self.ReddotNodeName)
    --     for i = 1,Len do
    --         Contens[i].Upgradeable = Count>0 and CacheDetail[tostring(Contens[i].Skill.SkillId)]==1
    --     end
    --     self.Skill_Tab:RegenerateAllEntries()
    -- end)
end
...
function M:RemoveReddotListener()
    ...
    --     ReddotManager.RemoveListener(self.ReddotNodeName, self)
    --     self.ReddotNodeName = nil
    -- end 
end
...
function M:Construct()
    ...
    self.Tab_SkillDetail:Init({
        ...
        Tabs = {
            {Text = GText("UI_Armory_Introduce"),Idx = 1},
            {Text = GText("UI_Armory_Numerical"),Idx = 2},
        }
    })
...
function M:OnLevelUpBtnClicked()
    ...
    if(self.CurrentNodeInfo.NodeIdx == 1)then
...
function M:OnCharSkillLevelUp(Ret, SkillId, Level, NewLevel)
    ...
    UIManager(self):LoadUI(nil,"SkillLevelUp",self:GetZOrder(),
        ...
        ExText = ExText,
        Attrs = self.Attrs,ComparedAttrs = self.ComparedAttrs,
        SkillId = SkillNodeInfo.Skill.SkillId,
        OnClosedCallbackObj = self, OnClosedCallbackFunc = Callback})
...
function M:InitUIInfo(Name, IsInUIMode, EventList, Params)
    ...
    local CharUuid = Params.CharUuid --角色Uuid
    local Target = Params.Target --角色
    local SelectedSkillId = Params.SelectedSkillId --选中的技能id
    local SelectedAttrId = Params.SelectedAttrId --选中的技能id
    self.OutAnimStyle = Params.OutAnimStyle or 1
    self.OnClosedObj = Params.OnClosedObj --关闭回调的对象
    self.OnClosedCallback = Params.OnClosedCallback --关闭回调
    self.OnDestructCallback = Params.OnDestructCallback --销毁回调
    self.OnDestructObj = Params.OnDestructObj --销毁回调的对象
    self.IsPreviewMode = Params.IsPreviewMode   --是否预览模式（不能升级）
    --self.PreviewCharInfos = Params.PreviewCharInfos --Dump出来的角色信息
    --self.PreviewWeaponInfos = Params.PreviewWeaponInfos --Dump出来的角色信息
    if(Params.PreviewCharInfos)then
        if(not ArmoryUtils:GetDummyAvatar())then
            self.bDestroyDummyAvatarWhenClose = true
            ArmoryUtils:CreateDummyAvatarByDumpInfo({CharInfos = Params.PreviewCharInfos,
                                                            WeaponInfos = Params.PreviewWeaponInfos,
                                                            CurrentChar = Params.CurrentChar
                                                                         or (Params.PreviewCharInfos and Params.PreviewCharInfos[1]),
    ...
    if(not CharUuid)then
        CharUuid = Avatar.Chars and next(Avatar.Chars)
    end
    ...
    local _Params = {Parent = self,Target = self.Target,IsPreviewMode = self.IsPreviewMode,
    OnTreeNodeClicked = self.OnTreeNodeClicked,OnTreeNodeStateChanged = self.OnTreeNodeStateChanged}
    ...
    for BranchIdx, BranchInfo in ipairs(SkillTreeInfo) do
        for NodeIdx, NodeInfo in ipairs(BranchInfo) do
            ...
            elseif(NodeInfo.AttrId)then
                if(NodeInfo.AttrId == AttrId)then
                    return BranchInfo,NodeInfo
                end
            end
        end
...
function M:InitSkillDesc(CharSkill,SkillData)
    ...
    local SkillDescInfos = {{Title = GText(SkillData.SkillName) or "",
                            Desc = SkillDescText,
                            Owner = self,
    ...
    for  _,Item in ipairs(ExplainList) do
        ...
        if(CombatTermData)then
            ...
            table.insert(SkillDescInfos,{Title = GText(CombatTerm),
...
function M:InitSkillKeyInfo(SkillData)
    ...
    if(not IsMobile and Key and Key ~= "")then
        ...
        local KeyInfo = {Desc = GText(SkillData.SkillBtnDesc)}
        if(self.IsGamepadInput)then
            ...
            KeyInfo.KeyInfoList = {
                {Type = "Text",Text = CommonUtils:GetKeyText(Key)},
            }
            self.Skill_Key:CreateCommonKey(KeyInfo)
        end
...
function M:UpdateAttrList(CharSkill,SkillData)
        ...
        for GroupIdx, Group in pairs(SkillData.SkillDescGroups) do
            for GroupName, DescIdxs in pairs(Group) do
                if(DescIdxs and #DescIdxs > 0)then
                    local _Group = {Name = GText(GroupName),DescIdxs = {}}
    ...
    self.Preview_Skill:Init({Contents = ComparedContents,ContentInGroup = ContentInGroup, ContentGroups = ContentGroups})
end
...
function M:UpdateLevelInfos(Skill)
    ...
    if(MaxLevel and Skill.Level < MaxLevel)then
        ...
        if(Res.CanLevelUp)then
            self.WidgetSwitcher_Btn:SetActiveWidgetIndex(0)
            --self.Btn_LevelUp:SetReddotVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
            self.Btn_LevelUp:SetText(GText("UI_FUNC_LEVELUP"))
            self.Btn_LevelUp:ForbidBtn(false)
        else
            --self.Btn_LevelUp:SetReddotVisibility(UIConst.VisibilityOp.Collapsed)
            if(not Res.IsBreakLevelEnough)then
                self.WidgetSwitcher_Btn:SetActiveWidgetIndex(1)
                self.Hint_Skill:SetStyle(2)
...
function M:UpdateBtnCheckVisibility()
    if(self.ShouldShowBtnCheck and self.WidgetSwitcher_Page:GetActiveWidgetIndex() == 1)then
...
function M:OnRichTextHyperLinkClicked(Term)
    ...
    -- self.HyperLinkTipWidget:Init({Title = GText(Term),Desc = GText(TermData.CombatProperTermExplaination),Owner = self,OnClosed = self.OnRichTextHyperLinkTipClosed})
...
function M:UpdateAttrNodeDetails(NodeInfo)
    ...
    elseif(Data.AddAttrs["Rate"])then
        IsRate = true
...
function M:PlayInAnim()
    ...
    self:BindToAnimationFinished(self.Detail_In, {self,self.OnInAnimationFinished})
...
function M:PlayOutAnim()
    ...
    if(self.OutAnimStyle == 1)then
        self:UnbindAllFromAnimationFinished(self.Out)
        self:BindToAnimationFinished(self.Out, {self,self.OnOutAnimationFinished})
        self:PlayAnimation(self.Out)
    elseif(self.OutAnimStyle == 2)then
        self:UnbindAllFromAnimationFinished(self.Detail_Out)
        self:BindToAnimationFinished(self.Detail_Out, {self,self.OnOutAnimationFinished})
