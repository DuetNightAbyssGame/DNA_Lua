...
---@type WBP_Story_Review_Main_P_C
...
function M:CreateNormalOption(TalkData)
    ...
    local TalkBtn = TalkWidget.Item_Btn
    if not TalkData.Options or (#TalkData.Options == 1 and TalkData.Options[1] == TalkData.HighLightDialogueId) then
        TalkBtn:SetVisibility(UE4.ESlateVisibility.Collapsed)
    else
        TalkBtn:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
...
function M:HandleScrollViewItemNavigation(NavigationTable)
    ...
    for Index = 1, Length do
        ...
        if Index ~= 1 then
            CurNavigationItem:SetNavigationRuleCustom(EUINavigation.Up, {self, function()
            ...
            PreNavigationItem:SetNavigationRuleCustom(EUINavigation.Down, {self, function()
            ...
            if Index == Length then
                CurNavigationItem:SetNavigationRuleCustom(EUINavigation.Down, {self, function()
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if InKeyName == "Gamepad_FaceButton_Bottom" then
        if self.FocusItem and self.FocusItem.Item_Btn then
...
function M:OnKeyUp(MyGeometry, InKeyEvent)
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if InKeyName == "Gamepad_RightStick_Up" then
        if self.ScrollState == "Up" then
            self.ScrollState = nil
        end
    elseif InKeyName == "Gamepad_RightStick_Down" then
        if self.ScrollState == "Down" then
            self.ScrollState = nil
        end
    elseif InKeyName == "V" or InKeyName == "Escape" or InKeyName == "Gamepad_FaceButton_Right" or InKeyName == "Gamepad_Special_Left" then
        self:Close()
    end
    return UE4.UWidgetBlueprintLibrary.Handled()
end
...
function M:UpdateBottomTips(GamePadTipState)
    ...
    if not IsGamePad then
        ...
        BottomKeyWidget:CreateCommonKey(
            {
                KeyInfoList = {{ Type = "Text", Text = "Esc", ClickCallback = self.CloseForBottom, Owner = self }},
                Desc = GText("UI_BACK"),
            }
        )
    else
        if GamePadTipState and GamePadTipState ~= "" then
            local TextStr = ""
            if GamePadTipState == "Audio" then
...
function M:RefreshChangeFocusItemOnGamePad()
    ...
    for i = 1, #Children do
        ...
        local IsVisible = (ChildTop >= ScrollTop and ChildBottom <= ScrollBottom)
