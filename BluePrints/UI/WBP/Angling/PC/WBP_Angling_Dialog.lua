...
function M:InitLureContent()
    ...
    if Avatar then
        ...
        for LureId, Data in pairs(DataMgr.FishingLure) do
            ...
            table.insert(LureList, {LureId = LureId, Rarity = Rarity, Num = Num, Level = Level, FishingLureSort = FishingLureSort})
        end
    ...
    if Avatar then
        ...
        for RodId, Data in pairs(DataMgr.FishingRod) do
            ...
            if Num >= 1 then
                local Rarity = DataMgr.Resource[Data.ResourceId].Rarity
                local Level = Data.FishingRodLevel
                table.insert(RodList, {RodId = RodId, Rarity = Rarity, Num = Num, Level = Level})
            end
        end
...
function M:OnWearBtnClick()
    ...
    if self.CurrentTabIdx == 1 then
        ...
        if self.WearLureContent == self.SelectLureContent and self.SelectLureContent.Count ~= 0 then
            return
        end
        if self.SelectLureContent.Count == 0 then
