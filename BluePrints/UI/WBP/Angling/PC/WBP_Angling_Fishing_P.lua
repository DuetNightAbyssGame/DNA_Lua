...
function M:Init(RootPage, FishingSpotId)
    ...
    self.Angling_Special:BindToAnimationFinished(self.Angling_Special.Out, {self, self.OnSpecialFishingPanelOut})
    ...
    -- self:BindToAnimationFinished(self.In, {self, self.OnAnimationInEnd})
    ...
    self.ResourceBar:InitResourceBar({DataMgr.FishingLure[self.RootPage.FishingLureId].ResourceId})
    ...
    self.ResourceBar:InitGamePadTip({KeyInfo = {
                KeyInfoList = {
                    {
                        Type = "Img",
                        ImgShortPath = "Menu",
            ...
            },ClickFuncObj = self, ClickFunc = self.OnClickTip})
...
function M:SwitchEndFishing(bInterupt, bSuccess)
    ...
    -- end
    -- self:SwitchWaitStart(false) 
    -- self:AvatarCompleteFish(bSuccess)
end
...
--AnglingButton指WBP_Angling_Fishing_Btn
...
function M:FishIconMove()
    ...
    -- print(_G.LogTag,"LXZ FishIconMove", self.TotalGameTime, CurveY, self.MaxFishY, self.MinFishY)
...
function M:UpdateFishingItem()
    local ItemSize = UIManager(self):GetWidgetRenderSize(self.WBP_Angling_Fishing_Item)
    -- print(_G.LogTag,"LXZ UpdateFishingItem", self.MaxFishY, ItemSize)
    -- self.MaxItemY = self.MaxFishY - ItemSize.Y/2
    -- self.MinItemY = self.MinFishY + ItemSize.Y/2
...
function M:ChangeItemMove(bPress)
    ...
    if bPress then
        ...
        self.handle = self:GetEaseValue(Position.Y, Position.Y - Long, self.ItemUpTime, FuncName, {self, self.ChangeItemMoveUp})
    else
        ...
        self.handle = self:GetEaseValue(Position.Y, self.MaxItemY, Time, FuncName, {self, self.ChangeItemMoveDown})
    end
end
...
function M:CheckFish(InDeltaTime)
    ...
    -- print(_G.LogTag,"LXZ CheckFish", self.EffectGameTime)
...
function M:OnSpaceDown()
    if self.SpacePress or not self.InitSucc or not self.bCanSpace then
        return
    end
...
function M:OnSpaceUp()
    if not self.SpacePress or not self.InitSucc or not self.bCanSpace then
        return
    end
...
function M:Handle_KeyEventOnPC(InKeyName)
    if InKeyName == "SpaceBar" then
        ...
        self:OnSpaceDown()
    elseif InKeyName == "E" and self.Angling_Special:GetVisibility() ~= ESlateVisibility.Collapsed then
...
function M:CheckSkipAngling()
    if not self.LastFishId or self.LastFishId == -1 then
    ...
    if Avatar then
        local function ShowFishMap(Ret, FishId, FishSize, AvatarIsSuccess, RewardReturn)
            ...
            if AvatarIsSuccess then
                ...
                if Avatar:GetFishCountByFishId(FishId) == 1 then
                    UIManager(self):LoadUINew("AnglingNewFish", {FishId = LastFishId, FishingPage = self, IsNew = true, FishSize = FishSize, FishingSpotId = self.RootPage.FishingSpotId, Rewards = RewardReturn})
                    ...
                    UIManager(self):LoadUINew("AnglingNewFish", {FishId = LastFishId, FishingPage = self, IsNew = false, FishSize = FishSize, FishingSpotId = self.RootPage.FishingSpotId, Rewards = RewardReturn})
                    -- UIManager(self):LoadUINew("ExploreToastSuccess","UI_Fishing_Success")
                    -- self:ShowRewardUI(FishId)
                end
                -- local UnLockData = EMCache:Get("FishUnLockData", true)
                -- --如果弹解锁新图鉴的UI，获得奖励UI在解锁UI的关闭回调里弹
                -- if not UnLockData then
                --     UnLockData = {}
                --     UnLockData[LastFishId] = 2
                --     EMCache:Set("FishUnLockData", UnLockData, true)
                --     ReddotManager.IncreaseLeafNodeCount("AnglingMap", 1)
                -- elseif UnLockData[LastFishId] == 1 or UnLockData[LastFishId] == nil then
                --     UnLockData[LastFishId] = 2
                --     EMCache:Set("FishUnLockData", UnLockData, true)
                --     ReddotManager.IncreaseLeafNodeCount("AnglingMap", 1)
                -- end
...
function M:RefreshInfoByInputTypeChange(CurInputDevice, CurGamepadName)
    ...
    elseif CurInputDevice == ECommonInputType.Touch then
        ...
        -- self.WBP_Angling_Fishing_Btn.WidgetSwitcher_Key:SetVisibility(ESlateVisibility.Collapsed)
    end
