...
---@type WBP_Chat_ChatItem_C|TimerMgr
local M = Class({"BluePrints.Common.TimerMgr","BluePrints.UI.BP_EMUserWidget_C"})
M._components = { "BluePrints.UI.WBP.Chat.View.HeadAnchorComp",}
...
function M:OnListItemObjectSet_System(Content) 
    ...
    if (Text == nil and self.MsgWrap.Message ~= nil) then
...
--endregion
--委托结算界面打开个人主页有BUG，临时屏蔽一下，因为自己的头像只有打开个人主页这一个选项，所以直接屏蔽
...
function M:OnGamePadDown(InKeyName)
    ...
    elseif InKeyName == Const.GamepadFaceButtonLeft then 
        if not self.IsOpen and self.Owner.FocusStateType == ChatFocusType.SelectChat and self.MsgWrap.ModSuitInfo then 
...
function M:OnCreateChatItem(Owner, Content)
    if (not Owner) or (not Owner._ChatItemList) then return end
