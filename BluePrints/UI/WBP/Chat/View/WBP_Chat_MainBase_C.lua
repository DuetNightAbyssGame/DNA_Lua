...
function M:_SetUpChatMsgList()
    ...
    if CommonUtils.GetDeviceTypeByPlatformName(self) ~= "Mobile" then
        self._ChatItemList = {} -- 辅助手柄选中聊天记录
    end
end

function M:OpenDisturbWindows()
    local Parms={
        RightCallbackFunction =function () EventManager:FireEvent(EventID.ComfirmDisturbClick)end,
...
function M:BtnEmptyOnClicked()
    ...
    elseif self.CurrChannel == ChatCommon.ChannelDef.Friend then
        ...
        self:Close()
    -- elseif self.CurrChannel == ChatCommon.ChannelDef.League then
    --     --@todo 跳转到公会系统
...
function M:BtnNewMsgOnClicked(bMuteSound)
    -- local NewTipContent = self.List_Dialog:GetItemAt(self._NewTipIndex-1)
    -- NewTipContent.Data.bRead = true
    -- if IsValid(NewTipContent.UI) then
    --     NewTipContent.UI:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- end
    -- self._NewTipIndex = nil
    --self.SB_Dialog:SetScrollOffset(NewTipContent.ScrollOffset)
    self:_AutoScrollToEnd()
    self.Group_NewMessage:SetVisibility(UIConst.VisibilityOp.Collapsed)
...
function M:BtnfaceOnPressed()
    ...
    else self.CurrExtraPanelName = "" end
end
...
function M:BtnSendOnClicked(bSkipCheck)
    ...
    ---@note 由于禁用态要防重入，空文本的判断需要提前
    --if self.WBP_Chat_InputBlock:GetText() == "" then
    if self.Com_Input:GetText() == "" then
        --self.WBP_Chat_InputBlock:ShowTip(true, GText("UI_Chat_NotEmptyText"))
    ...
    if bSkipCheck then
        --self:_SendChatMsg(self.WBP_Chat_InputBlock:GetText())
        self:_SendChatMsg(self.Com_Input:GetText())
        return
    end
    --ChatController:CheckTextValid(self.WBP_Chat_InputBlock:GetText(),
    ChatController:CheckTextValid(self.Com_Input:GetText(),
    function(bValid, MsgText)
        if not bValid then 
    ...
    function(bWarning, TipText)
        --self.WBP_Chat_InputBlock:ShowTip(bWarning, TipText)
...
function M:InitUIInfo(Name,bInUIMode,EventList,...)
    ...
    AudioManager(self):PauseObjectAllEvent(self,false)  ---damn，不知道什么地方把我的音效暂停了
...
function M:ApplyEmoji(EmojiData)
    ...
    -- local Text = self.WBP_Chat_InputBlock:GetText()
    local Text = self.Com_Input:GetText()
    if EmojiData.GroupId == ChatCommon.EmojiGroupId then
        --self.WBP_Chat_InputBlock:SetText(Text..EmojiTextShow)
        self.Com_Input:InsertText(EmojiTextShow)
        --self.Com_Input:SetText(Text..EmojiTextShow)
    else
        --self.WBP_Chat_InputBlock:SetText(EmojiText)
        self.Com_Input:SetText(EmojiText) 
        self:BtnSendOnClicked(true)
        --self.WBP_Chat_InputBlock:SetText(Text)
...
function M:RemoveReddotListen()
    if not self._bAddedReddotListen then return end
    for ChannelName,ChannelType in pairs(ChatCommon.ChannelDef) do
        local NodeName = ChatCommon.ReddotNamePre..ChannelName
        if not ReddotManager.GetTreeNode(NodeName) then return end
...
function M:_SetUpMsgCache()
    ...
    --self.WBP_Chat_InputBlock:SetText(MsgCache)
...
function M:Close()
    if self.IsBeginToClose then return end
    ...
    --self.WBP_Com_Tab_P:PlayOutAnim()
    M.Super.Close(self)
end

---@param MsgWrap MessageWrap
function M:_AddNewMsgToListView(MsgWrap)
    --@note 过滤掉无效消息已经读过的NewTip条
    if not MsgWrap then
        return
    end
    if self.List_Dialog:GetNumItems() == 0 then
        self.WS_Dialoglist:SetActiveWidgetIndex(0)
    end
...
---@param TimeWrap MessageWrap
---@param MsgWrap MessageWrap
function M:HandleChatMsgRecv( TimeWrap, MsgWrap)
    ---@note 好友频道下没有选中好友，不刷新消息
    if self.CurrChannel == ChatCommon.ChannelDef.Friend and not ChatModel:GetCurrentFriendUid() then
        return
    end
    --当显示了新消息跳转按钮时，此时如果又有新消息接收进来，在新接收的消息处标记“新消息”
    -- if self.Group_NewMessage:IsVisible() then
    --     local FakeNewTipWrap = {
    --         MsgType = ChatCommon.MsgType.NewTip,
    --         Message = {
    --             bRead = false,
    --         }
    --     }
    --     self:_AddNewMsgToListView(FakeNewTipWrap)
    -- end
...
function M:HandleEnterChatChannel(ErrCode, ChannelType)
    if ChannelType ~= self.CurrChannel then return end
    if ErrCode == ErrorCode.RET_SUCCESS then
        --@note 进入频道成功，刷新好友列表确保好友信息同步，
        FriendController:SendRequest(FriendCommon.EventId.RefreshFriend)
    else
        if self.CurrChannel == ChatCommon.ChannelDef.TeamUp then
...
function M:HandleRefreshFriend(ErrCode)
    if ErrCode ~= ErrorCode.RET_SUCCESS then return end
...
function M:_HandleRefreshFriendInPrivateChannel()
    ...
    if NowCount==0 then
        local CurrentPlatform = CommonUtils.GetDeviceTypeByPlatformName(self)
        local ShowEmptyText = CurrentPlatform == "PC" and GText("UI_Friend_NoAnyFriend") or GText("UI_Friend_AddBeforeChat")
