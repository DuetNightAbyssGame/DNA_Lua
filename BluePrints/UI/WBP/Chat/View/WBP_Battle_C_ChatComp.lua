...
function Component:InitChat()
    ...
    -- self:AddDispatcher(EventID.OnlineRegionChange, self, function(_, OldState, NewState)
    --     if NewState then
    ...
    if UnlockRuleName then
        ...
        if bUnlocked then 
            ...
            self.ChatUnlockKey = Avatar:BindOnUIFirstTimeUnlock(UnlockRuleName, function()
                if not self.ChatUnlockKey then return end
                self:_EnterChannel()
...
function Component:_InitChatPC(Avatar, ...)
    ...
    ChatController:RegisterEvent(self, function(self,EventId,...)
        if EventId == ChatCommon.EventID.CloseMainView then
            local bBattle = ...
            if not bBattle then return end
        ...
        elseif EventId == ChatCommon.EventID.ChatMsgRecv then
            ...
            if Avatar.ChatChannelMute[MsgWrap.Message.ChannelType]==1 then
        ...
        elseif EventId == ChatCommon.EventID.EnterChatChannel then 
            ...
            if IsValid(ChatController:GetView(self)) then return end
            if ChatModel:GetCurrentChannel() ~= ChannelType then return end
            -- 判断条件显示
...
function Component:_InitChatMobile(Avatar, ...)
    -- 移动端初始化聊天
    local ChatEntryConf = ...
    -- self.Chat_Entry.OnClicked:Add(self, self.ChatEntryOnClicked)
...
function Component:_EnterChannel()
    ...
    if CommonUtils.GetDeviceTypeByPlatformName(self) == "PC" then
        self:ListenForInputAction(DataMgr.KeyboardMap.OpenChat.ActionName, EInputEvent.IE_Pressed, false, {self, self.OpenChat})
    end
...
function Component:_LeaveChannel()
    -- for _,Channel in pairs(ChatCommon.ChannelDef) do
    --     ChatController:SendRequestLeaveChatChannel(Channel)
    -- end
    --ChatController:SendRequestLeaveChatChannel()
end

function Component:OpenChat()
    if IsValid(ChatController:GetView(self)) then return end
    if EMUIAnimationSubsystem:EMAnimationIsPlaying(self, self.Chat_Out) or 
        EMUIAnimationSubsystem:EMAnimationIsPlaying(self, self.Chat_In) then return end
    --local Avatar = GWorld:GetAvatar()
    --if Avatar:IsInHardBoss() or Avatar:IsInDungeon() then return end
    self:OpenSystemByAction(DataMgr.KeyboardMap.OpenChat.ActionName, true, true)
end

function Component:InitChatSimple()
    if not self.bRebuildChatSimple then return end
    local PlatfromType = CommonUtils.GetDeviceTypeByPlatformName(self)
    if (PlatfromType == "Mobile") then
        -- 手机端
        return
    end
    local Avatar = GWorld:GetAvatar()
...
function Component:UpdateChatSimple(MsgWrap)
    ...
    if not self.Pos_ChatSimple:IsVisible() then return end
...
function Component:_ProcessChatSimpleListModify()
    ...
    if ListCount <= MaxCount and ListCount<self.ChatSimpleOpenQueue:Size() then
...
function Component:RemoveChatSimpleProcessTimer()
    if not self:IsExistTimer(self.ProcessTicker) then return end
...
function Component:_SetUpChannelText()
    ...
    if ChannelConf and ChannelConf.SIcon then  
        --self.Text_ChatChannelNormal:SetText(GText(ChannelConf.Name))
