...
function M:OnBtnClicked()
    ...
    local Params = {
        OnDialogCloseCallback = function()
            -- self.Owner:BtnQuickReplyOnPressed()
            -- self.Owner:BtnQuickReplyOnReleased()
        end,
...
function M:Open(Owner, ViewName)
    ...
    for i=0,self.Col-1 do
        for j=0,self.Row-1 do
            ---@type WBP_Chat_QuickAnswerItem_C
    ...
    self:UpdateWidth()
    if CommonUtils.GetDeviceTypeByPlatformName(self) ~= "Mobile" then
        self.Key_Edit:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Menu", Owner = self} }})
        local GameInputModeSubsystem = UGameInputModeSubsystem.GetGameInputModeSubsystem(self)
        if (IsValid(GameInputModeSubsystem)) then
...
function M:OnGamePadDown(InKeyName)
    local IsEventHandled = false
    if InKeyName == Const.GamepadSpecialRight then --Menu
        IsEventHandled = true
        self:OnBtnClicked()
