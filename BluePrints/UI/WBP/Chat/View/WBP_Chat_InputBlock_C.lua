...
---@type WBP_Chat_InputBlock_C|TimerMgr
...
function M:FocusInputField()
	--if self.Text_Input:HasKeyboardFocus() then return end
...
function M:OnInputTextCommitted(Text, CommitType)
    if self._bFocusing then return end
    if Text == "" then
...
function M:OnInputTextChanged(Text)
    ...
    local bOverLength = #TextTable > MaxLen
    if bOverLength then
