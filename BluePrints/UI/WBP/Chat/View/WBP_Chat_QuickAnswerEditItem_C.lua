...
---@type WBP_Chat_QuickAnswerEditItem_C
...
function M:Construct()
    ...
    if CommonUtils.GetDeviceTypeByPlatformName(self) ~= "Mobile" then
        self.Key_Edit:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "A", Owner = self} }})
        self.Key_EditNo:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "B", Owner = self} }})
        self.Key_EditYes:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "A", Owner = self} }})
    ...
    --self.LimitCount:SetText(DataMgr.GlobalConstant.ChatQuickMsgMaxLen.ConstantValue)
end
...
function M:OnYesReleased()
    ...
    --ChatController:CheckTextValid(self.Text_Input:GetText(), 
    ChatController:CheckTextValid(self.Com_Input_Light:GetText(),
    ...
    end, {EmptyHint = GText("UI_Chat_NotSaveEmpty")})
    self:UpdateUIStyleInPlatform()
end

function M:OnEditClicked()
    --self.Group_Limit:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    self.Btn_Edit:SetVisibility(UIConst.VisibilityOp.Collapsed)
...
function M:Init(Owner, Index, Text)
    ---@type WBP_Chat_QuickAnswerEdit_C
    self.Owner = Owner
    self.Index = Index
    self.IsEnter = Index == 1
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    if self.CurInputDeviceType == ECommonInputType.Gamepad then 
        if (self.Owner.IsEditting and self.Owner.CurrItem == self) or 
        (not self.Owner.IsEditting and self.Owner.CurrSelectItem == self) then 
            self.IsEnter = true
...
function M:OnGamePadDown(InKeyName)
    ...
    if InKeyName == Const.GamepadFaceButtonDown then --A
        ...
        IsEventHandled = true
    elseif InKeyName == Const.GamepadFaceButtonRight then --B
        local IsEditting = self.Btn_Edit:GetVisibility() == UIConst.VisibilityOp.Collapsed
