...
---@type WBP_Chat_PlayerlistItem_C
...
function M:RemoveReddotListen()
    if not self.ReddotName then return end
...
function M:BtnAreaOnHovered()
    if self.Content.bSelected then return end
    self:StopAllBtnAnim()
...
function M:OnListItemObjectSet(Content)
    ...
    local Switch = {
        --[ChatCommon.ChannelDef.League] = self.OnListItemObjectSet_League,
...
function M:OnListItemObjectSet_InTeam()
    ...
    --self:AddReddotListen(TeamData.Uid)
    self.Common_Subsize_Reddot_Num:SetVisibility(UIConst.VisibilityOp.Collapsed)
    self:AddEventListen()
...
function M:_SetHeadIcon(HeadIconId)
    local SetHeadOpacity = function(bHasHead)
...
function M:_SetOnlineState(AvatarInfo)
    ...
    if not AvatarInfo.IsOnline then
        ...
        if not AvatarInfo.IsInDungeon and not AvatarInfo.IsInSpecialQuest then
...
function M:OnGamePadDown(InKeyName)
    ...
    elseif InKeyName == Const.GamepadFaceButtonDown then
        if not self.IsOpen and self._bSetUpEvent then 
            self.Head_Anchor:Open(true)
