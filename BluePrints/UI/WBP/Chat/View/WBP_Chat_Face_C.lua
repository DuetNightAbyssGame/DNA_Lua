...
function M:Open(Owner, ViewName)
    ...
    for GroupId, Data in pairs(DataMgr.EmojiGroup) do
        if not ChatModel:HasEmojiGroup(GroupId) then goto continue end
    ...
    self:UpdateWidth()
    if CommonUtils.GetDeviceTypeByPlatformName(self) ~= "Mobile" then
        self.Key_FaceTabLeft:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "LT", Owner = self} }})
        self.Key_FaceTabRight:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "RT", Owner = self} }})
        local GameInputModeSubsystem = UGameInputModeSubsystem.GetGameInputModeSubsystem(self)
        if (IsValid(GameInputModeSubsystem)) then
...
function M:OnGamePadDown(InKeyName)
    local IsEventHandled = false
    if InKeyName == Const.GamepadLeftTrigger then --LT
        IsEventHandled = true
        self:SwitchNextTabGroup(-1)
    elseif InKeyName == Const.GamepadRightTrigger then --RT
        IsEventHandled = true
        self:SwitchNextTabGroup(1)
