...
---@type WBP_Chat_Main_P_C|BP_UIState|WBP_Chat_MainBase_C
local M = Class({
...
function M:Construct()
    ...
    -- self.WBP_Com_Tab_P.Bg_Bottom:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- self.WBP_Com_Tab_P.BackgroundBlur_Bottom:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- self.WBP_Com_Tab_P.Bg_Top:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- self.WBP_Com_Tab_P.BackgroundBlur_Top:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- local HBSlot = UWidgetLayoutLibrary.SlotAsCanvasSlot(self.WBP_Com_Tab_P.HB)
    -- local HBPos = HBSlot:GetPosition()
    -- HBPos.X = 0
    -- HBSlot:SetPosition(HBPos)
    -- local Panel_TabSlot = UWidgetLayoutLibrary.SlotAsHorizontalBoxSlot(self.WBP_Com_Tab_P.Panel_Tab)
    -- Panel_TabSlot.Padding.Left = 0
    -- Panel_TabSlot:SetPadding(Panel_TabSlot.Padding)
...
function M:InitUIInfo(Name,bInUIMode,EventList,...)
    ...
    for Id,ChannelInfo in pairs(DataMgr.Channel) do
        if not ChatModel:IsChannelExclude(Id) then 
            ...
            table.insert(Tabs, {Text=Text,TabId=Index,IconPath=ChannelInfo.Icon,ChannelType=Id})
            ChannelType2Index[Id] = Index
        end
    end
    --self.bInDungeonSettlement = bInDungeonSettlement
    if (not self.bInDungeonSettlement) then
        Tabs[ChannelType2Index[ChatCommon.ChannelDef.SettlementOnline]] = nil
    end
    table.sort(Tabs, function(a,b) return a.TabId < b.TabId end)
    local TabInfo = {
        StyleName = "Text",
        Tabs = Tabs,
        OwnerPanel = self,
    ...
    -- for _, TabWidget in pairs(self.WBP_Com_Tab_P.EMScrollBox_TabItem:GetAllChildren()) do
    --     local TabSlot = UWidgetLayoutLibrary.SlotAsCanvasSlot(TabWidget.Btn:GetParent())
    --     TabSlot:SetSize(self.TabSize)
    -- end
    self:InitFocusWidget()
    self:InitUIStyleInPlatform()
    self:FreshTabDisturbIcon()
    --self.WBP_Com_TabSub01:PlayInAnim()
    ...
    -- self.WBP_Com_Tab_P.Key_Left:GetParent():SetVisibility(UIConst.VisibilityOp.Collapsed)
...
function M:FreshTabDisturbIcon()
    ...
    for index, TabWidget in pairs(self.WBP_Com_TabSub01.List_Tab:GetAllChildren()) do
        ...
        TabWidget:SetDisturbIcon(Avatar.ChatChannelMute[ChannelId]==1)
    end
end
...
function M:BtnResetOnClicked()
    ...
    self.WidgetSlot:SetPosition(ChatModel:GetMainUIPos())
    self.WidgetSlot:SetSize(ChatModel:GetMainUISize())
    --self.List_Dialog:RegenerateAllEntries()
    --尺寸和位置有改变
...
function M:_AddReddotListenInner(ChannelName,ChannelType)
    ...
    if Node and (not Node:HadAddChangeCb(self)) then   
        ReddotManager.AddListener(NodeName,self, function(self,Count)
            ...
            if not Tabs or not Tabs[ChannelType] then return end
            ---@type WBP_Com_TabIcon_P_C
            local TabUI = Tabs[ChannelType].UI
            if not IsValid(TabUI) then return end
            if Count > 0 then
...
function M:RefreshTeamMemberListInPC()
    ...
    for i = 1, TeamCommon.MaxTeamMembers, 1 do         
        ...
        NowCount = NowCount + 1
        ---@type TeamData
        local Member = TeamData ~= nil and TeamData.Members[i] or nil
        Content.Data = Member
...
function M:OnPreviewKeyDown(MyGeo,InKeyEvent)
    ...
    if self.PreViewKeyTable==nil then
        self.PreViewKeyTable = {
            [UIConst.GamePadKey.DPadDown]=function() --Down
            ...
            [Const.GamepadDPadUp] = function() --Up
            ...
            [Const.GamepadDPadLeft] = function() --Left
            ...
            [Const.GamepadDPadRight] = function() --Right
...
function M:OnKeyDown(MyGeo, InKeyEvent)
    if self.IsBeginToClose then return Unhandled end
    local ParentHandled = M.Super.OnKeyDown(self,MyGeo,InKeyEvent)
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        if self:OnGamePadDown(InKeyName) then 
            return Handled
        end
    end
    if InKeyName == DataMgr.KeyboardText.Enter.Key then
        --if self.WBP_Chat_InputBlock:GetText() ~= "" then
    ...
    -- if self.WBP_Com_Tab_P:Handle_KeyEventOnPC(InKeyName) then
    --     return Handled
    -- end
...
function M:OnMouseMove(MyGeometry, InMouseEvent)
    --@note 确保界面能够正常ReleaseMouseCapture
...
function M:InitGamepadKeyTable(InKeyName)
    self.GamepadKeyTable = {
        [Const.GamepadLeftShoulder] = function() --LB
            self.WBP_Com_TabSub01:ClickTab(self.CurrChannel - 1)
            return true
        end,
        [Const.GamepadRightShoulder] = function() --RB
            self.WBP_Com_TabSub01:ClickTab(self.CurrChannel + 1)
            return true
        end,
        [Const.GamepadSpecialLeft] = function() --View
        ...
        [Const.GamepadSpecialRight] = function() --Menu
        ...
        [Const.GamepadLeftThumbstick] = function() --LS
        ...
        [Const.GamepadFaceButtonRight] = function() --B
            local RefreshFocusState = nil
...
function M:OnGamePadDown(InKeyName)
    ...
    if self.Com_Input.IsShowGamPadKey then --input keys
        if InKeyName == Const.GamepadFaceButtonLeft then --X
            ...
            IsEventHandled = true
        elseif InKeyName == Const.GamepadRightThumbstick then --RS
            if self.Com_Input:GetText() ~= "" then
        ...
        elseif InKeyName == Const.GamepadFaceButtonUp then --Y
            self:BtnSendOnClicked()
            IsEventHandled = true
        end
    end
...
function M:RefreshUIReset()
    ...
    if IsShowKeyReset then 
        IsShowKeyReset = (self.FocusStateType ~= ChatFocusType.SelectChat) and (self.FocusStateType ~= ChatFocusType.PlayerList
         or self.CurrChannel ~= ChatCommon.ChannelDef.InTeam)
    end
...
function M:UpdateUIStyleInPlatform()
    ...
    if IsGamepad then 
        ...
        if self.FocusStateType == ChatFocusType.Default then 
            BottomKeyInfo = {{
                KeyInfoList = { { Type = "Img", ImgShortPath = "B", Owner = self} },
                Desc = GText("UI_BACK"),
            }}
        elseif self.FocusStateType == ChatFocusType.PlayerList then 
            table.insert(BottomKeyInfo, {
                KeyInfoList = { {Type = "Img", ImgShortPath = "LS", Owner = self } },
                Desc = GText("UI_CTL_SelectMessages"),
                bLongPress = false,
            })
            if self.MaxScrollOffset > 0 then 
                table.insert(BottomKeyInfo, {
                    KeyInfoList = { { Type = "Img", ImgShortPath = "RV", Owner = self} },
                    Desc = GText("UI_Controller_Slide"),
                })
            end
            table.insert(BottomKeyInfo, {
                KeyInfoList = { {Type = "Img", ImgShortPath = "A", Owner = self } },
        ...
        elseif self.FocusStateType == ChatFocusType.SelectChat then
            ...
            if self.CurrSelectChatItem then 
                ...
                if self.CurrSelectChatItem.MsgWrap.ModSuitInfo then 
                    table.insert(BottomKeyInfo, {
                        KeyInfoList = { { Type = "Img", ImgShortPath = "X", Owner = self} },
    ...
    if BottomKeyVisibilty == UIConst.VisibilityOp.Visible then
        ...
        if self.FocusStateType ~= ChatFocusType.PlayerList
        or self.CurrChannel ~= ChatCommon.ChannelDef.InTeam then
...
function M:RefreshUIWithMenuChanged(bOpen)
    ...
    if not bOpen then 
        ...
        self:UpdateUIStyleInPlatform()
        --点另外一个按钮关闭时，CurrExtraPanelName不准确了
...
function M:InitUIStyleInPlatform()
    self:AddTimer(0.01, function()
        self.Btn_Sent.Key_Text:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Y", Owner = self}}})
    ...
    self.Key_Reset:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Up", Owner = self} }, bAllowForbid = bAllowForbid})
    self.Key_Recruit:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "X", Owner = self} }, bAllowForbid = bAllowForbid})
    self.Key_PlayerListTitle:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Menu", Owner = self}}, bAllowForbid = bAllowForbid})
    self.Key_NewMassage:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Down", Owner = self} }, bAllowForbid = bAllowForbid})
    self.Key_QuickReply:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Left", Owner = self} }, bAllowForbid = bAllowForbid})
    self.Key_Face:CreateCommonKey({ KeyInfoList = { { Type = "Img", ImgShortPath = "Right", Owner = self} }, bAllowForbid = bAllowForbid})
    self.Btn_Sent:SetForbidden()
...
function M:SetQuickReplyFocus()
    if not self.CurrExtraPanel then return end
...
function M:OnAnalogValueChanged(MyGeometry,InAnalogInputEvent)
    ...
    if InKeyName == "Gamepad_RightY" then
        ...
        local IsVisible = Visibility == "SelfHitTestInvisible"
...
function M:FindCurrentDialogItem()
    ...
    local SelectItem, MinDistance = nil, nil
    ...
    for k = Num, 1, -1 do 
        ...
        if Item.SelectMask then
            ...
            if Y0 >= DialogStartY and Y1 <= DialogEndY then 
                return Item, true
            end
            local Distance = 0
            if Y0 < DialogStartY then 
                Distance = Distance + DialogStartY - Y0
            end
            if Y1 > DialogEndY then 
