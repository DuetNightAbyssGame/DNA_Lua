...
---@type WBP_Chat_SimpleChatItem_C|TimerMgr
local M = Class({"BluePrints.Common.TimerMgr","BluePrints.UI.BP_EMUserWidget_C"})

function M:OnListItemObjectSet(Content)
    if not GWorld.NetworkMgr:CheckIsConnected(true) then return end
...
function M:Open(MsgWrap, bSound)
    ...
    if ModSuitContent then Content = ModSuitContent end
    local DyePlanContent = ChatController:ParseDyePlanText(MsgWrap)
    if DyePlanContent then Content = DyePlanContent end
    ...
    local _, Key = self:AddTimer(0.01, function()
        ...
        if HalfLength == 0 then
            ...
            --@note OverflowPolicy未开放走手动缩略，否则走自动缩略
        ...
        elseif TargetLineWidth>NowLineWidth and HalfLength >0 then 
...
function M:RemoveTicker()
    if not self:IsExistTimer(self._TickForTextSize) then return end
...
function M:RemoveCloseTimer()
    if not self:IsExistTimer(self._TimerForAutoClose) then return end
