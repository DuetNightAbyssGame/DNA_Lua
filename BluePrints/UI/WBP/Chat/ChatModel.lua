...
---@field SimpleChatOutAnimCurve UCurveFloat
...
function M:HasMainUIChanged()
    return self._CachedMainUISize.X ~= self._OriginMainUISize.X or self._CachedMainUISize.Y ~= self._OriginMainUISize.Y or
           self._CachedMainUIPos.X ~= self._OriginMainUIPos.X or self._CachedMainUIPos.Y ~= self._OriginMainUIPos.Y
end
...
function M:AddReddotCount(ChannelType, FriendUid)
    ...
    if self:GetAvatar().ChatChannelMute[ChannelType] == 1 then
...
function M:ClearReddotCount(ChannelType, FriendUid)
    if not ChannelType then ChannelType = self._CurrentChannel end
    if ChannelType ~= ChatCommon.ChannelDef.Friend then
        ---@type ReddotTreeNode
        local NodeName = ChatCommon.ReddotNamePre..ChatCommon.ChannelNames[ChannelType]
...
function M:AddFriendReddotNode(Uid, bAppendParent)
    ...
    if bAppendParent == nil then bAppendParent = true end
    ...
    if bAppendParent and not ReddotManager.GetTreeNode(ParentNodeName) then
        ReddotManager.AddNodeEx(ParentNodeName, {[Key] = {}},1)
        ReddotManager.AddNodeEx(ChatCommon.ReddotName, {[ParentNodeName] = {}})
    end
    return Node
end
...
function M:AddMessage(Message, bCalcUnread)
    ...
    --@note 私聊需要对不在场消息的添加做限制，私聊目前仅好友频道，私聊会重刷消息列表
    if self._CurrentFriendUid and Message.Type == CommonConst.MESSAGE_TYPE_PRIVATE then 
        ...
        elseif self._CurrentFriendUid ~= FriendUid then 
            ...
            --@note 即便是好友频道，如果发消息的不是当前好友，也不添加到Model中
            return 
        end
    end
        ...
        if self._CurrentFriendUid then
            ...
            for i=0, DataMgr.GlobalConstant.ChatMsgCountInBattleMain.ConstantValue do
                ...
                for Uid, Chat in pairs(self:GetFriendChatDatas()) do
                    if not self:GetAvatar().Friends[Uid] then goto continue2 end
                    if i==0 then  FinalIndexies[Uid] = Chat.Messages:Length() else
                        local FinalIndex = FinalIndexies[Uid]
                        if FinalIndex ==0 then goto continue2 end
                ...
                if not Newest then goto continue end
...
function M:GetChannelCDTimerKey(ChannelType)
    if ChannelType == nil then ChannelType = self._CurrentChannel end
...
-- function M:ReadNewTip()
--     self._MessageDict[self._CurrentChannel]:ReadNewTip()
-- end
