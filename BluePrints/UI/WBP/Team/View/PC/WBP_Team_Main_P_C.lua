...
---@type WBP_Team_Main_P_C|BP_UIState_C
local M = Class({"BluePrints.UI.WBP.Team.View.WBP_Team_Main_Base"})
...
function M:OnInputDeviceChange()
    ...
    elseif not TeamController:IsMobile() then
        ...
        -- if self.OnTeamMainFocusChanged then
...
function M:DelTeammateUI(Uid)
    if not self.Teammate2UI then return end
    if not self.Teammate2UI[Uid] then return end
    ...
    for _, HeadUI in pairs(self.Teammate2UI) do
        if HeadUI.Index > DelIndex then
            HeadUI:PlayMove()
        end
    end
    if not self.bBattle and self.Head_My.Index > DelIndex then
...
function M:OnInitAddBtn()
    if self.bBattle then return end
...
function M:HideAddBtn(i)
    ...
    if not AddBtn:IsVisible() then return end
...
function M:ShowAddBtn(i)
    if self.bBattle then return end
    local AddBtn = self.EmptyHeadeAnchor:GetChildAt(i-1)
    if AddBtn:IsVisible() then return end
    local CanvasSlot = UWidgetLayoutLibrary.SlotAsCanvasSlot(self.Head_My.Head_Team)
    local BtnSizeX = CanvasSlot:GetSize().X
    local PaddingX = self.WB_Player.InnerSlotPadding.X
...
function M:FocusUIByUid(Uid)
    if self.bIsFocusable and not self.bOpenBtnList or not self:IsVisible() then return end
