...
---@class WBP_Team_PlayerItem_Base:UEMUserWidget
...
---@param Owner WBP_Team_PlayerList_P_C
---@param Member AvatarInfo
function M:Init(Owner,Member,Index, bAnim)
    ...
    local MeleeWId = 0
    local RangedWId = 0
    local MeleeWLevel = nil
    local RangedWLevel = nil
    local Eid = nil
    ...
    if TeamModel:IsYourself(Member.Uid) then
        ...
        CharId = CharId~=0 and CharId or Player.CurrentRoleId
    ...
    if CharId~=0 then
        ...
        -- local HeadIcon = LoadObject(HeadPath..HeadImg.."."..HeadImg.."'")
...
function M:AddPhantomUI(PhantomEid)
    ...
    if TeamModel:GetAvatar():IsInBigWorld() then return end
    local PhantomUI = nil
...
function M:DelPhantomUI(Eid)
    ...
    if not IsValid(PhantomUI) then return end
...
function M:OnGetMenuContent(Anchor)
    if not self.Button_Area:IsChecked() then return end
    local InitTeamKickBtn = function(Content, AvatarInfo) --请离队伍
    ...
    local InitTeamLeaderTransBtn = function(Content, AvatarInfo) --移交队长
    ...
    local InitAddFriendBtn = function(Content, AvatarInfo) --添加好友
    ...
    if TeamModel:IsYourself(self.Member.Uid) then
        ...
        if TeamModel:IsTeamLeader(TeamModel:GetAvatar().Uid) then
            Switch = { InitTeamKickBtn, InitTeamLeaderTransBtn, InitShowRecordBtn}
        else 
            Switch = { InitShowRecordBtn,}
        end
