...
---@type WBP_Team_Head_C|TimerMgr
local Component = {}

---@param MemberInfo AvatarInfo
...
--function Compoment:StopAllStateAnim()
...
function Component:SetHeadState(HeadState)
    ...
    elseif self.HeadState == TeamCommon.HeadState.VoteRefused then
        ...
        --头像拒绝状态播放完之后，以便下次调用HeadState回到正常状态
...
function Component:OnGetMenuContentComp(Anchor)
    if not self.Head_Team.bSelected then return end
    ...
    local InitLeaveTeamBtn = function(Content, AvatarInfo) --离开队伍
