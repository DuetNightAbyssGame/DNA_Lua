---@class DailyTalkController :Controller
...
function M:ShowDailyRewards(TalkTriggerId, RewardInfo)
    if not self.bInited then return end
    -- for RewardType, Rewards in pairs(Reward) do
    --     for RewardId, ReawrdItems in pairs(Rewards) do
    --         for RewardRarity, RewardCount in pairs(ReawrdItems) do
...
function M:CheckNeedShowRewardBubble()
    ...
    for NpcId, NpcTalkInteractiveComponent in pairs(NpcMap) do
        if IsValid(NpcTalkInteractiveComponent) then
            local Npc = NpcTalkInteractiveComponent.Owner
            if (not self.EnterStory) and 
                    NpcTalkInteractiveComponent.DistanceCheck(Npc, Player, DailyTalkCommon.CheckShowBubbleDistance) then
