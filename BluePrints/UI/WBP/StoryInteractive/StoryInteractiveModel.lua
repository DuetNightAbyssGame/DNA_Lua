---@class StoryInteractiveModel : Model
---@field Super Model
...
function M:GetNpcTalkTriggers(NpcId)
    ...
    if not TalkTriggerIds then return end
    local Result = {}
...
local function SimpleTableCount(Table) -- 仅用于判断当前Table内的数量是否大于1, >1时返回>1的值（不是正确数值），否则返回Table数量长度（0或1）
    local Count = 0
    if not Table then return Count end
    local Index, Elem = next(Table)
    if not Index then return Count end
    Count = Count + 1
    if not next(Table, Index) then return Count end
    Count = Count + 1
    return Count
end
