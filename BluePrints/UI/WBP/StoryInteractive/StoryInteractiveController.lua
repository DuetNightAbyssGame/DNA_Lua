---@class StoryInteractiveController : Controller
...
function M:TryStartInteractive(NpcId, NpcCharacter, PlayerActor, EndCallback)
    if UE4.URuntimeCommonFunctionLibrary.IsPlayInEditor(NpcCharacter) then
        if self:GetAvatar() == nil and self.bInited ~= true then --- 编辑器下的在测试场景中运行
            self:Init()
        end
    end
...
function M:DirectStartInteractive(TalkInfo, NpcCharacter, PlayerActor, EndCallback)
    ...
    if TalkInfo.Type == "TalkTrigger" then
        ...
        self:DirectStartTalkTrigger(TalkInfo.TalkTriggerId, 
            NpcCharacter.NpcId, NpcCharacter, PlayerActor, EndCallback)
        return
    end
...
function M:DirectStartTalkTrigger(TalkTriggerId, NpcId, NpcCharacter, PlayerActor, EndCallback)
    ...
    TalkContext:StartTalk(TalkTriggerId, nil, nil, PlayerActor, NpcCharacter, EndCallback,
        (TS and TS:GetNpcPlayDialogueCallback(NpcId)) or nil)
end
...
function M:CreateTalkPawn(bCalculateTrans, bUseProceduralCamera, ProceduralCameraId, InteractiveActor)
    ...
    if(bCalculateTrans and ProceduralParams) then
        TalkContext:FreeSimpleProceduralCamera(Player, self.TalkPawn, Player, InteractiveActor,
...
function M:SwitchToTalkPawn()
    ...
    local GameInstance = GWorld.GameInstance
    local TalkContext = GameInstance:GetTalkContext()
    self.AIControllerForPlayer = TalkContext:GetWorld():SpawnActor(ATalkAIController:StaticClass(),
...
--     self.AIControllerForPlayer:Possess(self.TalkPawn)
--     self.TalkPawn:K2_DestroyActor()
--     DebugPrint("TestPawnClearStep1")
-- end

-- function M:TestPawnClearStep2()
--     -- local GameInstance = GWorld.GameInstance
--     -- local TalkContext = GameInstance:GetTalkContext()
--     -- self.AIControllerForPlayer = TalkContext:GetWorld():SpawnActor(ATalkAIController:StaticClass(),
--     --     UE4.UKismetMathLibrary.MakeTransform(UE4.FVector(), UE4.FRotator(),
...
--     -- self.TalkPawn = TalkContext:GetWorld():SpawnActor(TalkPawnClass, UE4.UKismetMathLibrary.MakeTransform(Loc,Rot, FVector(1)))
--     -- self.AIControllerForPlayer:Possess(self.TalkPawn)
--     DebugPrint("TestPawnClearStep2")
--     self.AIControllerForPlayer:K2_DestroyActor()
-- end

-- _G.StroyInteractiveController = M
