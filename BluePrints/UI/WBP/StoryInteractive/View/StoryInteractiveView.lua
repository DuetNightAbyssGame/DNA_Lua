...
--- @class StoryInteractiveView 
--- @alias StoryInteractiveView StoryInteractiveView |BP_UIState_C | WBP_Story_Interactive_P_C
local M = Class({
    ...
    for Index, InteractiveInfo in ipairs(self.InteractiveInfos) do
        ...
        if not ShowName then
            if InteractiveInfo.Type == "TalkNode" then
                ...
                if QuestChainId == 0 or not QuestChainId then
        ...
        InteractiveItem:InitInteractiveInfo(ShowName, Icon, QuestChainId, SpecialQuestId, DynQuestId,
            InteractiveInfo.bForbidden, InteractiveInfo.ForbiddenMsg)
        ...
        DebugPrint("StoryInteractiveView ShowItem Type ", InteractiveInfo.Type, "ShowName: ", GText(ShowName), 
            "OtherInfo", InteractiveInfo.QuestChainId, InteractiveInfo.SpecialQuestId, InteractiveInfo.TalkTriggerId)
    end
    local InteractiveItem = UIManager(self):_CreateWidgetNew("StoryInteractiveItem")
...
function M:BindClickDelegate(bBind)
    ...
    if (bBind) then
        self:ListenForInputAction("StoryInteractiveClick", EInputEvent.IE_Pressed, false,{self, self.OnTalkClickPressed})
        self:ListenForInputAction("StoryInteractiveClick", EInputEvent.IE_Released, false,{self, self.OnTalkClickReleased})
    end
end
...
function M:BindClickItem(bBind)
    ...
    if bBind then
        self:ListenForInputAction("StoryInteractive", EInputEvent.IE_Pressed, true, {self, self.PressedSelectAction});
        self:ListenForInputAction("StoryInteractive", EInputEvent.IE_Released, true, {self, self.ReleasedSelectAction});
    end
end
...
function M:OnNormalExit(ExcludeItem)
    ...
    Controller:BlendOutCamera(BlendOutTime, function() end)
...
function M:OnItemClicked(Index, Item)
    ...
    if InteractiveInfo then
        ...
        Controller:BlendOutToTalk( 0, function()end)
        self:Close()
        self:RealClose()
        -- 
        Controller:DirectStartInteractive(InteractiveInfo, 
        ANpcCharacter.GetNpc(self, self.NpcId),
        Player, self.EndCallback)
    end
...
function M:PressedSelectAction()
    ...
    if self.bPressSelectAction then return end
...
function M:SetGameInputActionBlocking(bShouldBlock)
    ...
    -- if self.bGameInputActionBlocking ~= bShouldBlock then
    --     self.bGameInputActionBlocking = bShouldBlock

    --     local InputSettings = UInputSettings.GetInputSettings()
    --     local ActionNames = TArray(FName)
    --     InputSettings:GetActionNames(ActionNames)
    
    --     if bShouldBlock then
    --         for i = 1, ActionNames:Num() do
    --             local ActionName = ActionNames:Get(i)
    --             self:ListenForInputAction(ActionName, EInputEvent.IE_Pressed, true, {self, self.OnConsumeInputAction})
    --             self:ListenForInputAction(ActionName, EInputEvent.IE_Released, true, {self, self.OnConsumeInputAction})
    --         end
    --     else
    --         for i = 1, ActionNames:Num() do
    --             local ActionName = ActionNames:Get(i)
    --             self:StopListeningForInputAction(ActionName, EInputEvent.IE_Pressed)
    --             self:StopListeningForInputAction(ActionName, EInputEvent.IE_Released)
    --         end
    --     end
    -- end
end
