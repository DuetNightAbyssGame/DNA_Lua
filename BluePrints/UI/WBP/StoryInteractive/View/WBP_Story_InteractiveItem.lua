---@class WBP_Story_InteractiveItem
---@alias WBP_Story_InteractiveItem WBP_Story_InteractiveItem|WBP_Story_InteractiveItem_C
local M = Class({    "BluePrints.UI.BP_EMUserWidget_C","BluePrints.Common.DelayFrameComponent","BluePrints.UI.BP_EMUserWidgetUtils_C"})
...
function M:UpdateIcon()
    ...
    -- ScreenPrint("WYX UpdateIcon "..InteractiveIconPath .. (bUseMask and " true" or " false"))
    if InteractiveIconPath then
        ...
        rawset(self, "LoadResourceID", nil)
        local Handle = UE.UResourceLibrary.LoadObjectAsyncWithId(self, InteractiveIconPath, {self,M.OnIconLoadFinish})
        if Handle then
...
function M:GetQuestInteractiveIconPath(InQuestChainId, InSpecialQuestId, DynQuestId)
    ...
    if DynQuestId and DynQuestId ~= 0 then
