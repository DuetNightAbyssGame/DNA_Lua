...
function WBP_Bag_Sell_View_C:Initialize(Initializer)
    ...
    self.AllTypeCoinInfo = {}                  -- 所有当前出售后获得的货币信息
    self.AllTypeRewardsInfo = {}               -- 所有当前分解后后获得的奖励Id
    self.AllRewardContentList = {}             -- 当前分解可获得的道具信息
    ...
    self.BagCurState = nil                      -- 当前背包的状态

    self.CurFocusWidget = "DefaultWidget"       -- 当前聚焦的Widget名称
end
...
function WBP_Bag_Sell_View_C:OnUpdateBagItemByAction(OpAction, ErrCode, ...)
    ...
    if (OpAction == "WeaponBulkBreakDown") then
        -- 如果当前正好选中武器Tab，则进行刷新
    ...
    elseif (UKismetStringLibrary.EndsWith(OpAction, "Sale", ESearchCase.CaseSensitive)) then
        ...
        -- GetItemPopup:BindActionOnClosed(self.PlayOutAnim, self)  
        if (OpAction == "ModBulkSale") then
        ...
        elseif (OpAction == "FishResourceBulkSale") then
            ...
            for CoinId, Price in pairs(SaleFishPrice) do
                ...
                if bIsNotShowGetItemPage then
                    bIsNotShowGetItemPage = false
                end
            end
        elseif (OpAction == "DraftBulkSale") then
...
function WBP_Bag_Sell_View_C:PlayInAnim()
    ...
    -- local function PlayInAnimFinished()
    --     self:RefreshBaseInfo()
    --     self:InitListenEvent()
    -- end
    -- self:BindToAnimationFinished(self.In, {self, PlayInAnimFinished})
...
function WBP_Bag_Sell_View_C:RefreshBaseInfo()
    ...
    self.Com_NumInput:Init({
        -- MaxValue = self.NeedDealWithStuffData[self.CurSelectStuffContentInList.Uuid].Count,
    ...
    if (self.ParentWidget and (self.ParentWidget.CurTabId == BagCommon.ItemTypeToTabId.MeleeWeapon or 
            self.ParentWidget.CurTabId == BagCommon.ItemTypeToTabId.RangedWeapon)) then
    ...
    if (self.InitStuffDataInList ~= nil) then
        ...
        if (self.CurrentMode == BagCommon.BagItemSelectOpMode.ResolveMode) then
            -- self.Bg_Num:SetVisibility(UE4.ESlateVisibility.Collapsed)
            self.Panel_KeyCount:SetVisibility(UE4.ESlateVisibility.Collapsed)
        else
            -- self.Bg_Num:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    ...
    -- 按钮音效相关
    self.Button_List.SoundFunc = function(Btn)
...
function WBP_Bag_Sell_View_C:RefreshCoinInfo(CoinId)
        ...
        for k, v in pairs(self.AllTypeCoinInfo) do
            if (v > 0) then
                ...
                if (k == CommonConst.Coins.Coin3) then
                    ...
                    bIsNeedStuffIcon = LimitedCoinNum > CurCoinNumDaily
    ...
    if (TotalRewardTypeNum <= 0) then
        ...
        for i = TotalRewardTypeNum + 1, AllChildren:Length(), 1 do
...
--     local function FillWithRewardData(ItemType, ItemId, Count)
--         local RewardInfo = DataMgr[ItemType][ItemId]
--         if RewardInfo then
--             local RewardObject = {}
--             RewardObject.Id = ItemId
--             RewardObject.Icon = ItemUtils.GetItemIconPath(ItemId, ItemType)
--             RewardObject.ParentWidget = self
--             RewardObject.ItemType = ItemType
--             RewardObject.Rarity = RewardInfo.Rarity or RewardInfo[ItemType.."Rarity"] or 0
--             RewardObject.IsShowDetails = true
--             RewardObject.UIName = BagCommon.BagStuffSelectUIName
...
--     end
--     local AllRewardList = RewardUtils:GetRewards(RewardIds, nil)
--     local FinalRewardList = self:GenerateAllRewardIds(AllRewardList)
--     if (type(FinalRewardList) == "table") then
--         for _, Value in pairs(FinalRewardList) do
--             local ItemId, ItemInfo = Value.ItemId, Value.ItemInfo
--             FillWithRewardData(ItemInfo.TableName, ItemId, ItemInfo.ItemCount)
--         end
--     end
...
function WBP_Bag_Sell_View_C:RefreshRewardInfo()
    ...
    if (#self.AllRewardContentList > 0) then
        ...
        for idx = #self.AllRewardContentList + 1, AllChildren:Length(), 1 do
...
function WBP_Bag_Sell_View_C:SetNavigationRuleForRewardItem(TotalRewardTypeNum)
    if (TotalRewardTypeNum == 1) then
        ...
        for idx = 1, TotalRewardTypeNum, 1 do
...
function WBP_Bag_Sell_View_C:IsCanChangeToGamePadViewMode()
    ...
    if (self.CurFocusWidget == "DefaultWidget") then
        ...
        local AllChildren = self.ItemBox:GetAllChildren()
        for i = 1, AllChildren:Length(), 1 do
...
function WBP_Bag_Sell_View_C:TryToSaleStuff()
    ...
    for ItemUuid, ItemData in pairs(self.NeedDealWithStuffData) do
        -- local CoinConfig = DataMgr.Resource[k]
        ...
        if (not bIsNeedShowWarning) then
            if (ItemData.StuffType == BagCommon.StuffType.Mod) then
                local ModItemNum = math.floor(self.NeedDealWithStuffCount[self:GetStuffObjId(ItemUuid)] + 0.5)
                -- Mod处于已装备状态
                if (StuffServerData ~= nil and (StuffServerData.WeaponUuids:Length() > 0 or StuffServerData.CharUuids:Length() > 0) 
                    and (StuffServerData.Count > 0 and StuffServerData.Count <= ModItemNum)) then
    ...
    if (self.ParentWidget and self.ParentWidget.CurTabId == BagCommon.ItemTypeToTabId.Mod) then
        ...
        if (CurCoinNumDaily >= LimitedCoinNum) then
            ...
            CommonDialogParams.Tips = {GText("UI_Bag_ModExtract_Delete")}
        end
    else
    ...
    for k, v in pairs(self.AllTypeCoinInfo) do
        if (v > 0) then
            ...
            if (k == CommonConst.Coins.Coin3 and bIsNeedCheckLimitType) then
                local LimitedCoinNum, CurCoinNumDaily = self:GetCoin3DataInfo()
                if (CurCoinNumDaily + v > LimitedCoinNum and not CommonDialogParams.IsShowEmptyText) then
                    bIsNeedShowTips = true
                    CommonDialogParams.Tips = {GText("UI_Bag_ModExtract_Warning")}
                end
                ValueCount = math.min(math.floor(v + 0.5), math.floor(LimitedCoinNum - CurCoinNumDaily + 0.5))
            else
                ValueCount = math.floor(v + 0.5)
    ...
    CommonDialogParams.RightCallbackFunction = function() self:ConfirmDealWithItems() end
...
function WBP_Bag_Sell_View_C:TryToResolveStuff()
    ...
    for ItemUuid, ItemData in pairs(self.NeedDealWithStuffData) do
        ...
        if (StuffServerData) then
            ...
            if (AssisterId and AssisterId ~= 0) then
...
function WBP_Bag_Sell_View_C:FillWithStuffSellData(StuffData, AddTypeMode)
    ...
    local AddStuffCount = nil
    if (AddTypeMode == "All") then
        AddStuffCount = StuffData.StuffCount
    elseif (AddTypeMode == "ReserveOne") then
        AddStuffCount = StuffData.StuffCount - 1
    else
        AddStuffCount = 1
    end
    StuffData.StateTagInfo = {Name="InSelectList", ExtraData={AddStuffCount, StuffData.StuffCount, StuffData.Price, StuffData.CoinId, self.RemoveBagItemInList}}
...
function WBP_Bag_Sell_View_C:RemoveFromResolveList(StuffContent)
    local NeedRemoveStuffContent = nil
    local AllItemCount = self.List_Item:GetNumItems()
    for i = 0, AllItemCount - 1, 1 do
        local ItemObj = self.List_Item:GetItemAt(i)
    ...
    if (NeedRemoveStuffContent == nil) then
...
function WBP_Bag_Sell_View_C:AfterRemoveItemFromList(StuffContent, IsRemoveSelectedItem)
    ...
    elseif (IsRemoveSelectedItem) then
        ...
        if (self.GameInputModeSubsystem:GetCurrentInputType() == ECommonInputType.Gamepad) then
            if (SelectItemObj and SelectItemObj.SelfWidget) then
    ...
    for index, StuffContent in ipairs(StuffContentList) do
        ...
        if (StuffContentInList.SelfWidget) then
            StuffContentInList.SelfWidget:SetSelectNum(Utils.FormatNumber(SaleNum, true), 
                                                       Utils.FormatNumber(StuffContent.Count, true))
        end
        if (self.ParentWidget) then
...
function WBP_Bag_Sell_View_C:UpdateItemNumFromList(StuffContent, DeltaNum)
    ...
    if (StuffContentInList.SelfWidget) then
        StuffContentInList.SelfWidget:SetSelectNum(Utils.FormatNumber(SaleNum, true)) --Utils.FormatNumber(StuffContent.Count, true)  SetSelectNum    
    end
    if (self.ParentWidget) then
        self.ParentWidget:OnRefreshSaleSelectNum(StuffContent.Uuid, SaleNum)
    end
...
function WBP_Bag_Sell_View_C:OnListSelectStuffClicked(Content, ClickReason)
    ...
    if (IsValid(self.CurSelectStuffContentInList) and self.CurSelectStuffContentInList.Uuid == StuffUuid) then
        -- -- 选择的同一个
        -- if (self.GameInputModeSubsystem:GetCurrentInputType() == ECommonInputType.Gamepad) then
        --     -- 手柄模式下，默认再次点击的时候，移除当前
        --     if (ClickReason ~= "ChangeHoverInGamepad" and ClickReason ~= "SelectFirstItem" and ClickReason ~= "AddNewItem") then
        --         self:RemoveBagItemInList(StuffUuid)
        --     else
        --         self:RefreshKeyCount()
        --     end
        -- else
...
function WBP_Bag_Sell_View_C:OnGamePadButtonDown(InKeyName)
    ...
    if (not IsEventHandled) then
        ...
        elseif (InKeyName == UIConst.GamePadKey.FaceButtonLeft) then  --按下X移除当前道具
            if (self.BagCurState == BagCommon.AllBagState.ChooseSaleState or BagCommon.AllBagState.WeaponResolveState) then
                if self.CurSelectStuffContentInList and self.CurSelectStuffContentInList.bMinus then
...
function WBP_Bag_Sell_View_C:TryToHoverToItemOnNavigation(SelectItem)
    ...
    if (ToHoverStuffData) then
        ...
        -- if self.ParentWidget.BagCurState == BagCommon.AllBagState.ChooseSaleState then
        --     --DebugPrint("SelectItem.AddNum  ",SelectItem.AddNum)
        --     -- DebugPrint("SelectItem.ExtraData  ",SelectItem.StateTagInfo.ExtraData[1])
        --     if  SelectItem.AddNum and SelectItem.AddNum > 0 then
        --         self.ParentWidget.RemoveBtn = true
        --         self.ParentWidget.HoverItem = SelectItem  --记录下当前的可被取消选中的 Hoveritem 方便清除
