...
function M:PostEnterTalkTask(TalkTask, TaskData, OnPostEnterTalkTaskFinished)
	...
	if self.bBlackScreenSameScreen then
		...
		while true do
			if CurrentDialogueId == nil then
				break
			end
			local DialogueData = DataMgr.Dialogue[CurrentDialogueId]
			if (DialogueData == nil or DialogueData.NextOptions) then
				break
			end
...
function M:PlayDialogue(TalkTask, DialogueData, TaskData)
	...
	local CurrentTextWidget = nil
	if self.bBlackScreenSameScreen then
		self.BlackScreenText:SetText(""); -- 清空黑屏文字
	...
	CurrentTextWidget:BindToAnimationFinished(CurrentTextWidget.TextOpacityChange, { CurrentTextWidget, function()
...
function M:SkipDialogueTyping()
	if (IsValid(self.CurrentTextWidget)) then
		-- ue4 未提供跳过功能，且设置播放位置为结束时间、设置播放速率等操作不会使动画处于完成的表现。
		-- 因此采用设置播放位置为结束时间前0.01秒的方式来模拟跳过。
...
function M:Show(ShowTag)
	if ShowTag == "Review" or ShowTag == "Wiki" then
		return
	end
