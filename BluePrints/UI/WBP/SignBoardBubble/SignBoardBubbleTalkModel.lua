---@class SignBoardBubbleTalkModel :Model
...
function M:AddStartWaitTrigger(TriggerId)
    if not self.WaitTriggerQueue then return end
    if self.WaitTriggerQueue[TriggerId] then return end
...
function M:GetTalkTriggerId(TriggerId)
    ...
    if not TriggerInfo then return end
...
function M:CheckWaitTriggerQueue()
    if not self.WaitTriggerQueue then return end
    if self:GetRunningTalkTrigger() then return end
    ...
    for TriggerId, Time in pairs(self.WaitTriggerQueue) do
        ...
        -- DebugPrint("CheckCondition", self.FinishTrigger[TriggerId])
        if (not self.FinishTrigger[TriggerId]) and 
             FTalkTriggerComponent:CheckCondition(NpcGossipTrigger[TriggerId].RelatedTalks) and
             WaitTime > (NpcGossipTrigger[TriggerId].BubbleWaitTime or InvitateBubbleTime) then
            table.insert(TriggerIds, TriggerId)
        end
    end
    if #TriggerIds <= 0 then return end
...
function M:CheckTriggerDistance(TriggerId)
    ...
    if not Player:CheckCanInteractiveTrigger() then return end
    if not IsValid(Player.InteractiveTriggerComponent) then return end
...
function M:GetNpcCanTrigger(NpcId)
    ...
    if not TriggerIds then return end
    return TriggerIds
end


function M:CheckCanTrigger(TriggerId)
    if not self:GetServerCanTriggerNpc(TriggerId) then return end
    local NpcGossipTrigger = DataMgr.NpcGossipTrigger
    local NpcTriggerConvert = DataMgr.NpcGossipTriggerConvert
...
function M:ResetTalkData()
    ...
    self.FinishTrigger = {} --- 模拟原本行为，在触发范围内，对话只会触发一次，除非离开触发范围
...
function M:ResetTalkWaitTime(RunningTriggerId)
    ...
    for TriggerId, Time in pairs(self.WaitTriggerQueue) do
        -- local TriggerInfo = NpcGossipTrigger[TriggerId]
        -- local WaitTime = NowTime - Time
        -- WaitTime = math.fmod(WaitTime, TriggerInfo.BubbleWaitTime or InvitateBubbleTime)
        -- self.WaitTriggerQueue[TriggerId] = WaitTime
