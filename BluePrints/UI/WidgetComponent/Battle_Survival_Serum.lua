...
function Battle_Survival_Serum:SetUnitParams(MechanismID)
	...
	if BuffTypeImgPathPostFix then
		...
		UE.UResourceLibrary.LoadObjectAsync(self,BuffIconPath,{self,Battle_Survival_Serum.OnBuffIconLoadFinish})
	end
	-- local BuffIcon = LoadObject(BuffIconPath)
	-- self.Img_Buff:SetBrushResourceObject(BuffIcon)
    self:OnEnergyChanged(self.Energy)
	local ResurgenceMat = self.Bar_Agentia_Surge:GetDynamicMaterial()
    if (ResurgenceMat ~= nil) then
