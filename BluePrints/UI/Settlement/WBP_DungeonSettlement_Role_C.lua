...
function WBP_DungeonSettlement_Role_C:Construct()
    ...
    self.UpdateExpKey_Bar = "UpdateExp_Bar_"..self:GetName()     -- 经验条动画key
    self.UpdateExpKey_Audio = "UpdateExp_Audio_"..self:GetName() -- 音效key

    self:SetVisibility(ESlateVisibility.Hidden)
end

-- RoleName: "Char" 角色， "Player" 玩家 "MeleeWeapon" 近战武器 "RangedWeapon" 远程武器
-- IsDisplayCurInfo：为true时，初始化显示CurExp CurLevel, 而非CachedExp CachedLevel
-- CurInfo: {Exp = , Level = ,}, 允许传空。如果传空，直接从Player身上获取（例如给ESC界面复用时
-- IsShowTextWhenZero: 为true时，当IncreasedExp = 0，改为显示对应武器/角色名
-- 返回值：bool. 如果返回false，则初始化失败，外部需要折叠自己
function WBP_DungeonSettlement_Role_C:SetItem(OldBattleInfo, RoleName, IncrsExp, IsDisplayCurInfo, CurInfo, IsShowTextWhenZero)
    ...
    if RoleName == "MeleeWeapon" or RoleName == "RangedWeapon" then
    ...
    -- 客户端自行计算差值  todo：角色和武器也要类似处理
    if self.RoleName == "Player" then
        self.IncreasedExp = self:CalcIncreasedExp()
        --self.Bar_Exp:SetVisibility(ESlateVisibility.Collapsed)      -- 不知道为啥，这东西会挡住真正的进度条增长，干脆折叠算了
    end
    ...
    if self.RoleName == "Player" then   -- 和鸣框PC和移动端长度不一样，单独设置
        -- 不用单独设置了 蓝图用锚点改自适应了
        -- local Player =  UE4.UGameplayStatics.GetPlayerCharacter(self, 0)
        -- if Player.UIModePlatform == "Mobile" then
        --     self.Bar_SizeX = self.Length_M or 356
        -- else
        --     self.Bar_SizeX = self.Length_P or 555
        -- end

        -- local Bg_Slot = UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(self.Bg_Exp)
        -- local Bg_NewSize = FVector2D(self.Bar_SizeX, Bg_Slot:GetSize().Y)
        -- Bg_Slot:SetSize(Bg_NewSize)
    end
    ...
    -- 这里处理文字 (+经验, 已满级)
    if IsShowTextWhenZero and (self.IncreasedExp == 0) then
        if self.RoleName == "Player" then
...
function WBP_DungeonSettlement_Role_C:PlayExpAnim()
    ...
    -- 经验条上涨动效逻辑
    local UpdateExpFun_Bar = function()
        ...
        if IsBreakLevel then
            self:SetPercent_BarExp(0)
            if not (self.RoleName == "Player") then
    ...
    local UpdateExpFun_Audio = function()
...
function WBP_DungeonSettlement_Role_C:GetCharInfo(Player)
    ...
    if CharBreakInfo then
        ...
        if EnhanceLevelInfo then
            ...
            if self.AvatarLevel >= BreakLimitLevel then
                bCanEnhance = true
            end
        end
    end
...
function WBP_DungeonSettlement_Role_C:GetWeaponInfo(Player)
    if not Player[self.RoleName] then                               -- 防御性处理，按理说不该存在没有对应武器却还要显示此控件的情况
        self.CurInfo = {
            Exp = self.CachedExp,
    ...
    if not self.CurInfo then
        self.CurInfo = {
            Exp = Player[self.RoleName]:GetAttr("Exp"),             -- Player.MeleeWeapon or Player.RangedWeapon
...
function WBP_DungeonSettlement_Role_C:SetPercent_BarExp(percent)
    ...
    -- local Exp_Slot = UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(self.Panel_Exp)
    -- local Exp_Slot2 = UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(self.Bar_Exp)
    -- if Exp_Slot and Exp_Slot2 then
    --     local NewSize = FVector2D(Exp_Slot:GetSize().X * percent, Exp_Slot2:GetSize().Y)
    --     Exp_Slot2:SetSize(NewSize)
    -- end
    ...
    -- 已有延时timer 就不用再加了，timer触发时会按self.CachedPercent设置
