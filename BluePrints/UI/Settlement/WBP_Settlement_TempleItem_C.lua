...
---@type WBP_Settlement_TempleItem_C
local WBP_Settlement_TempleItem_C = Class({"BluePrints.Common.TimerMgr","BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_Settlement_TempleItem_C:SetStarRewards(RewardsInfo, CurrentStar, HistoryMaxStar)
    ...
    -- 临时，现在蓝图里特效可交互，会影响物品交互，暂时先用代码关掉
...
function WBP_Settlement_TempleItem_C:StartShowNoStarRewards()
    ...
    if self.IntervalTime ~= nil and self.RewardsInfo ~= nil then
...
function WBP_Settlement_TempleItem_C:ItemMenuAnchorChanged()
    if UIManager(self.ParentUI):IsHaveMenuAnchorOpen() then
        ...
        -- 检查当前输入设备类型
        if self.ParentUI.CurInputDeviceType == ECommonInputType.Gamepad then
