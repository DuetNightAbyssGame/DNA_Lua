...
---@type WBP_Shop_Recommend_Gift4_1_C
...
function M:Construct()
    ...
    if self.ItemId and DataMgr.ShopItem[self.ItemId] then
        self.ShopItemData = setmetatable({}, {__index = DataMgr.ShopItem[self.ItemId]})
    end
...
function M:OnGamePadDown(InKeyName)
    ...
    elseif InKeyName == UIConst.GamePadKey.FaceButtonRight then
        if self.List_Reward:HasFocusedDescendants() or self.List_Reward:HasAnyUserFocus() then
