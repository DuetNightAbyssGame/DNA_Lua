...
---@type WBP_Shop_ExchangePart_C
local M = Class("BluePrints.UI.UI_PC.Common.Common_Dialog.Common_Dialog_ContentBase")

local TipsWidgetName = {
    ["Item_Tip3"] = 1,
    ["Item_Currency"] = 2,
}

function M:Construct()
    self.CurrentCount = self:GetMaxCount() == 0 and 0 or 1
...
function M:InitCommonWidget()
    local ConfigData = {
        ...
        ClickInterval = 1,
        MinValue = self:GetMaxCount() == 0 and 0 or 1,
...
function M:UpdateCost(Cost)
    ...
    local Params = {ResourceId = 99, Denominator = Cost, Numerator = Cost}
