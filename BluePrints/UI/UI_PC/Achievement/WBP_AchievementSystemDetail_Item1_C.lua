...
---@type Achievement_SystemDetail_Item1_PC_C
...
function M:OnListItemObjectSet(Content)
    ...
    -- if not self.AchievementSystem.ContentReddotMap then
    --     self.AchievementSystem.ContentReddotMap = {}
    -- end
    ...
    if self.AchievementSystem.FirstIndex and self.AchievementSystem.FirstIndex==self.Index then
        ...
        self.AchievementSystem.FirstIndex=-1
         ...
         if self.AchievementSystem.CurrentTypeId ==self.ID then
            self.Common_List_Subcell_PC_0:SelectCell()
            self:BP_OnItemSelectionChanged(true)
         else
    ...
    -- self.Pressed:SetVisibility(ESlateVisibility.Collapsed)
...
--     DebugPrint("AYFNavigate check gamepad A")
