...
---@class Shop_Item_PC_C:WBP_Shop_Item_Base_C
...
function M:InitCommonItem(ShopItemData)
    ...
    ItemContent.Rarity = DataMgr[ShopItemData.ItemType][ShopItemData.ItemId].Rarity or 
        DataMgr[ShopItemData.ItemType][ShopItemData.ItemId][ShopItemData.ItemType .. "Rarity"]
...
function M:ResetItem()
    ...
    --self.Group_SoldOutBGTime:SetVisibility(UIConst.VisibilityOp["Collapsed"])
    --self.Bg_Discount_SoldOut:SetVisibility(UIConst.VisibilityOp["Collapsed"])
    --self.Bg_Time_SoldOut:SetVisibility(UIConst.VisibilityOp["Collapsed"])
...
function M:SwitchDifficultyColor(DifficultyId)
    ...
    elseif DifficultyId == 'Middle' then
        return self.HardBG_Level02
    elseif DifficultyId == 'Hard' then
        return self.HardBG_Level03
    elseif DifficultyId == 'Impossible' then
...
function M:HandleCheck()
    ...
    Avatar:PurchaseImpressionShopItem({self, self.OnReceiveServerPurchaseCallback},tonumber(ImpressionShopId),tonumber(DiceNum))
end
...
function M:OnReceiveServerPurchaseCallback(Ret, IsCheckSuccess, Rand1, Rand2, PurchaseRewards)
    ...
    if IsCheckSuccess then
        self.ImpressionResultUI:SetOnCloseDelegate({self,self.OnSelfSold})
...
function M:ShowCheckUI()
    ...
    ImpressionResultUI:SetOnCloseDelegate({self,self.OnPurchaseEnd})
    ImpressionResultUI:SetOnInterruptedExitDelegate({self, self.OnPurchaseEnd})
end
