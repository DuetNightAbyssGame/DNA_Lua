...
function Guide_Image_Main:CloseButtonLogic()
    -- IsClosing 用于标记是否正在关闭,避免重复关闭
    -- HasReachEnd 用于标记是否到达过最后一页,可以按下 Esc 退出
...
function Guide_Image_Main:InitButton()
    if self.isPC then
        -- Gamepad按钮
        self.Key_L:CreateCommonKey({KeyInfoList = {{Type = "Img", ImgShortPath = "LT",}}})
        self.Key_R:CreateCommonKey({KeyInfoList = {{Type = "Img", ImgShortPath = "RT",}}})
    end
    -- PC+Mobile按钮
...
function Guide_Image_Main:SwitchToRight()
    if not self:CheckCanSwitchPage(1) then
        if self.CurInputDeviceType == ECommonInputType.Gamepad then -- 手柄需要屏蔽关闭按钮功能
            return
        end
...
function Guide_Image_Main:Handle_OnPCDown(InKeyName)
    ...
    if InKeyName == "Right" then
        self:SwitchToRight()
        self.Button_Next:SoundFunc()
        return true
    end
    if InKeyName == "Left" then
        self:SwitchToLeft()
