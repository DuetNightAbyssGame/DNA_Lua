...
function WBP_Battle_Blood_Boss_PC_C:InitBossUI(Owner,IsBossInPart,BossUIType)
    ...
    local function DoInit()
        if not self.Owner or not self.Owner.BillboardComponent then return end
...
function WBP_Battle_Blood_Boss_PC_C:InitBossComponent()
    ...
    if self.Owner:IsBossMonster() then
        ...
        if BossData.BossUIValues.ShowES ~= nil then
        ...
        if self.Part_Count == 1 or self.Part_Count > CommonConst.BOSS_BLOOD_PART_MAX or Sum > 1.00000001 or Sum < 0.999999999999 then
            self.IsBossInPart = false
        end
    ...
    --     self.ShieldBar = BloodBarUtils.LoadSubWidget(self,self.SizeBox_Shield,"ShieldBar",self.BloodBarLenght,self.BloodBarHeight,self.Shield/self.MaxShield)
    -- end
...
function WBP_Battle_Blood_Boss_PC_C:InitBossParams()
    ...
    self.BossTickTime = 0.033 -- 扣血的Tick时间
    self.BossAnimTime = Const.BloodBarAnimTime -- 扣血实际时间
    self.BossDelayTime = Const.BloodBarDelayTime -- 扣血的延迟时间
    self.LastAttackTime = 0 -- 上一次受击为Attack的时间
    self.ComboAttackTime = 0.3 -- 连击时间
    self.FixTime = 1.0 -- 单独扣血的修正时间
    self.BossHpPercent = 1.0
    self.BossShieldPercent = 1.0
    self.BossTNPercent = 1.0 -- 实际韧性条百分比
    self.LastTNPercent = 1.0
    self.HelpBossTNPercent = 1.0 -- 辅助韧性条百分比
    self.LastTN = 0.0 -- 上一个韧性值
    self.NowTN = 0.0 -- 下一个韧性值
    self.TNDeductEffectHeight = 0.0
    self.TNEqualZero = false -- TN是否为0
...
function WBP_Battle_Blood_Boss_PC_C:InitBossConfig()
    ...
    local BossName = GText(self.Owner.Data.UnitName) --self.Owner:GetCharacterName()
    ...
    -- if self.ShowES == true then
    --     -- self.PanelShield:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    --     -- self.SizeBox_Shield:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    -- else
    --     -- self.SizeBox_Shield:SetVisibility(UE4.ESlateVisibility.Collapsed)
    --     -- self.PanelShield:SetVisibility(UE4.ESlateVisibility.Collapsed)
    -- end
    if not self.IsShowToughnessBar then
        ...
        -- self.Toughness_Split:SetVisibility(UE4.ESlateVisibility.Collapsed)
    else
        ...
        -- self.Toughness_Split:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
...
function WBP_Battle_Blood_Boss_PC_C:ResetBossToughness()
    ...
    self.DeductTNOriginalPositionX = 0--self.DeductTNOriginalPositionX or UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(self.DeductToughness):GetPosition().X
...
function WBP_Battle_Blood_Boss_PC_C:InitMultiHpBar(BossData)
    ...
    local BossBattleData = nil
    if BossData and BossData.BattleRoleId then
        BossBattleData = DataMgr.BattleMonster[BossData.BattleRoleId]
    end
    if not BossBattleData or not BossBattleData.MultiHpBar or BossBattleData.MultiHpBar <= 1 then
...
function WBP_Battle_Blood_Boss_PC_C:OnMultiHpBarLayerChange(bAdd,ChangeNum,Caller)
    ...
    -- 自行计算ChangeNum，C++传过来的ChangeNum不正确
    ...
    elseif self.CurMultiColorArrayIndex >= ArrayNum then
...
function WBP_Battle_Blood_Boss_PC_C:SetMultiHpBarColor()
    ...
    self.HpBar:OnChangeLayerSetColor(self.MultiHpBarColorArray[Index],
        self.MultiDeductBarColorArray[Index],self.CurHpLayer > 1 and self.MultiHpBarColorArray[BGColorIndex] or FLinearColor(1.0,1.0,1.0,0.3))
end
...
function WBP_Battle_Blood_Boss_PC_C:UpdateToughnessEffect()
    ...
    self.ToughnessEffect:SetPosition(
        FVector2D(self.ToughnessLength * self.BossTNPercent, self.ToughnessEffect:GetPosition().Y)
    )
    -- self:PlayAnimation(self.Deduct_toughness)
...
function WBP_Battle_Blood_Boss_PC_C:RefreshBossInfoByAction(ActionName, DamageEvent)
    ...
    if ActionName == "Attack" then
        ...
        if Info and Info[1] then
            ...
            if SkillType == "Attack" or SkillType == "FallAttack" or SkillType == "HeavyAttack" or SkillType == "SlideAttack" then
        ...
        if NowTime - self.LastAttackTime > self.ComboAttackTime then
            ...
            self.BossDelayTime = Const.BloodBarDelayTime --1.0
        end
    end
    -- if not self.Owner:IsBossMonster() then
    --     self:UpdateBossBlood(ActionName, DamageEvent.EnergyShieldReduce)
    -- end
...
function WBP_Battle_Blood_Boss_PC_C:UpdateBossBlood(ActionName)
    ...
    if (ActionName == "Attack") then
        ...
        if (self.MaxShield <= 0.00000001) then
...
function WBP_Battle_Blood_Boss_PC_C:UpdateBossToughness(bNowBossInDefeated)
    ...
    if not self.MaxTN then self.MaxTN = self.Owner:GetAttr("MaxTN") end
    if not self.NowTN then self.NowTN = self.MaxTN end
    ...
    if self.NowTN == self.MaxTN then
        ...
        if self.TNEqualZero then
            ...
            -- self:PlayAnimation(self.Toughness_Finsh)
        end
    ...
    if self.NowTN < 0.0000001 or bNowBossInDefeated then
        self.TNEqualZero = true
        self.BossTNPercent = 0
        -- self.HelpBossTNPercent = 0
        self:SetToughnessBarPercent(self.BossTNPercent)
    else
        if self.TNReduce < 0 then
            ...
            self:RecoverBossTN()
        elseif self.TNReduce > 0.00001 or self.NowTN == self.MaxTN then
            self.VX_percent:SetVisibility(UE4.ESlateVisibility.Collapsed)
            if (self:IsExistTimer("RealRecoveryTN")) then
                self:RemoveTimer("RealRecoveryTN")
            end
            --self.HelpBossTNPercent = math.max(self.HelpBossTNPercent, self.Toughness_Deduct.Percent)
            self.BossTNPercent = math.min(math.max(self.NowTN / self.MaxTN, 0), 1)
            -- self:UpdateToughnessEffect()
            self:SetToughnessBarPercent(self.BossTNPercent)
        ...
        if (self.NowTN / self.MaxTN) >= 0.7 and self.IsPlayOut then
            ...
            -- EMUIAnimationSubsystem:EMPlayAnimation(self,self.Loop_Out)
...
function WBP_Battle_Blood_Boss_PC_C:UpdateEnergyBar(InEid, NewEnergy, isInit)
    ...
    local UpdateProgress = function()
        ...
        if isInit==true or NewEnergyText>self.EnergyText then
...
function WBP_Battle_Blood_Boss_PC_C:RecoverBossTN()
    ...
    local AnimTime = Const.MonsterTNRecoverTickInterival -- 韧性恢复实际时间
    if self.TNEqualZero then
        AnimTime = Const.BossTNToZeroRecoverTickInterival
    end
    local TickTime = AnimTime / Const.BossTNRecoverTickFrequency -- 韧性恢复的Tick时间
    local TNRecoveryFunc = function()
        local NowTime = UE4.UGameplayStatics.GetRealTimeSeconds(self)
        ...
        local DelayTN =
            math.min(math.floor(self.NowTN + self.TNReduce * (1 - (NowTime - self.NowTNTime) / AnimTime)), self.MaxTN)
        --self:CheckToughnessIndex(DelayTN)
        self.LastTNPercent = self.BossTNPercent
...
function WBP_Battle_Blood_Boss_PC_C:ReduceBossTN()
    ...
    local TickTime = 0.033 --self.BossTickTime
    ...
    -- self.TNDeductEffectHeight = DeductEffectHeight
    if self:IsExistTimer("RealRecoveryTN") then
        self:RemoveTimer("RealRecoveryTN")
    end
    if self:IsExistTimer("RealReduceBossTN") then
        self:RemoveTimer("RealReduceBossTN")
        self.HelpBossTNPercent = self.Toughness_BarDeduct.Percent -- 上一次的平滑还没完成的同时又受到攻击，此时更新平滑的起点
    end
    ...
    local RealReduceBossTN = function()
...
function WBP_Battle_Blood_Boss_PC_C:CheckToughnessPartBroken()
    ...
    for key, value in pairs(self.ToughnessHit) do
        ...
        if self.LastTNPercent >= PartPercent and self.BossTNPercent < PartPercent then
            ...
            -- self:PlayAnimation(self.Toughness_Broken)
...
function WBP_Battle_Blood_Boss_PC_C:CloseBossBlood()
    -- local AnimObj = self:GetAnimationByName("Bar_Out")
