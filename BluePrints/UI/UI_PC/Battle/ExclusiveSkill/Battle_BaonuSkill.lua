...
function M:OnLoaded(OwnerPlayer, Params)
    ...
    self.BuffState = {Layer = 0, LastTime = -1, IsCountDown = false}
...
function M:OnBuffsChanged(Buffs)
    local NewBuffState = {Layer = 0, LastTime = -1, IsCountDown = false}
    local BaonuBuff
...
function M:LightBuffIcon(NewBuffState)
    ...
    if Interval > 0 then
        if self.BuffState.Layer ~= 0 then
