...
---@type SelectRole_Role_Widget_PC_C
...
function M:Init()
    ...
    if self.DeviceInPc then
        ...
        -- self.PC:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
    else
        -- self.Phone:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
        -- self.PC:SetVisibility(ESlateVisibility.Collapsed)

        -- self:CreateMobileKeyAndLogic(self.Btn_ObserveFace, GText("UI_REGISTER_FACE"), self.OnClickButtonObserve)
        -- self:CreateMobileKeyAndLogic(self.Btn_ObserveBody, GText("UI_REGISTER_WHOLE"), self.OnClickButtonObserve)
        -- self:CreateMobileKeyAndLogic(self.Btn_TurnAround, GText("UI_REGISTER_TURN"), self.OnClickButtonTurnAround)
    end
...
function M:ChangeRoleBtnStyle(SelectRole)
    ...
    -- self:StopAnimation(self["Select_"..OtherRole])
    ...
    if SelectRole == "Female" and self.NowRole ~= "Female" then
    ...
    elseif SelectRole == "Male" and self.NowRole == "Female" then
        self:StopAnimation(self.Change_MtoF)
        self:PlayAnimation(self.Change_FtoM)
        AudioManager(self):PlayUISound(self, "event:/ui/common/gender_select_role_exchange", nil, nil)
    end
    -- self["Btn_"..OtherRole].WidgetStyle.Normal = self["Disabled"..OtherRole.."BtnStyle"]
end
...
function M:OnClickButtonNext()
    ...
    -- self.NowState = StateEventIndex.Name
    if self.RootPage.DeviceInPc then
        --从起名界面回来时，默认保持初始状态，所以在转到起名界面前先恢复到初始状态
        self.RootPage.Key_Observe:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
...
function M:OnClickButtonDetail(Tag)
    if Tag == "L" then
        ...
        if self.NowState == StateEventIndex.Female or self.NowState == StateEventIndex.Name then
            self:OnClickButtonMale()
