...
function M:PreConstruct(IsDesignTime)
    ...
    -- local GameInputSubsystem = UGameInputModeSubsystem.GetGameInputModeSubsystem(GWorld.GameInstance:GetGameUIManager())
...
function M:Construct()
    ...
    if not self.DeviceInPc then
        ...
        -- self.Up:SetVisibility(ESlateVisibility.Collapsed)
        -- self.Down:SetVisibility(ESlateVisibility.Collapsed)
    end

    if self.DeviceInPc then
        -- self.Key_TurnAround:SetVisibility(ESlateVisibility.Collapsed)
        -- self:CreateCommonKeyAndLogic(self.SelectRole_Role_Widget_PC, self.Key_TurnAround, "R", GText("UI_REGISTER_TURN"), self.SelectRole_Role_Widget_PC.OnClickButtonTurnAround)
        self:CreateCommonKeyAndLogic(self, self.Key_Observe, "Esc", GText("UI_Unselect"), 
                                        self.OnClickButtonObserve, self)
        -- self:CreateCommonKeyAndLogic(self.Page_Name, self.Key_Back, "Esc", GText("UI_Unselect"), self.Page_Name.OnClickButtonBack, 
        --                                 self.Page_Name)
    ...
    self:BindToAnimationFinished(self.Out, {self.Page_Name, self.Page_Name.FinishNode})
...
function M:ShowWidget(MyGeometry, InKeyEvent)
    --目前设计，sequence切换时UI常驻，因此这段暂时先注释
    self.Page_Role:UnlockCanClick()
    -- if self.SelectRole_Role_Widget_PC.NowState == 3 or self.SelectRole_Role_Widget_PC.NowState == 4 then
    --     return
    -- end
    -- self.SelectRole_Role_Widget_PC:PlayAnimation(self.SelectRole_Role_Widget_PC.In)
    -- self.SelectRole_Role_Widget_PC:SetVisibility(4)
end
...
function M:OnClickBtn()
    ...
    -- self:GetSelectImage({{Btn = self.Btn_Select02_L, Type = "DetailL"},{Btn = self.Btn_Select02_R, Type = "DetailR"}})
    if self.Page_Role.NowState == 5 or self.Page_Role.NowState == 4 then
        self:GetSelectImage({{Btn = self.Btn_Select01_L, Tag = "L"},{Btn = self.Btn_Select01_R, Tag = "R"}}, "FullBody", false)
    elseif self.Page_Role.NowState == 6 or self.Page_Role.NowState == 7 then
        self:GetSelectImage({{Btn = self.Btn_Select02_L, Tag = "L"},{Btn = self.Btn_Select02_R, Tag = "R"}}, "Detail", true)
    end
end
...
function M:RefreshInfoByInputTypeChange(CurInputDevice, CurGamepadName)
    ...
    elseif CurInputDevice == ECommonInputType.Touch then
        -- self.WBox_Key:SetVisibility(ESlateVisibility.Collapsed)
    end
end
