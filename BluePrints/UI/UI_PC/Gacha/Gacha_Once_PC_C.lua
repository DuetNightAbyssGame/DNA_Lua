...
function G:Init(Parent,Result,IsForceSkip)
    ...
    self.IsManyResults = false --是否是多个结果
    self.IsSingleGacha = true  --是否是单抽
    self.CanShare = false --是否可以分享
    self.NotGacha = false
    self.ThisTimeGetChar = {} --本次抽卡获得的角色
...
function G:RefreshInfoByInputTypeChange(CurInputType, CurGamepadName)
    ...
    if self.Parent and self.Parent:CheckScreenShotWidget() then return end
...
function G:HandleResult()
    for key,GachaResult in pairs(self.ResultData) do
        ...
        elseif GachaResult.Sign == CommonConst.GachaWeaponType then
            if self.Parent.OwnedWeapon[GachaResult.ResultId] == true then
                self.ResultData[key].IsNew = false
            else
...
function G:CheckCharGradeCanMax(CharId)
    ...
    local AlreadyGetCount = self.ThisTimeGetChar[CharId] --目前已经抽到的排序
    if (OldGradeCount + AlreadyGetCount) > MaxNum then
        return true
    end
    return false
end
...
function G:RefreshGachaOnceUI()
    ...
    self.IsSpineAnimation = false --是否播放spine动画
    self.CanShare = false
    ...
    -- self.Text_WeaponName:SetVisibility(UE4.ESlateVisibility.Collapsed)
end
...
function G:PlayCharVoiceAndText()
    self.Text_VoiceText:SetVisibility(UE4.ESlateVisibility.Collapsed)
    if not self.VoiceCharId then return end
    if self.NotGacha then return end
    ...
    -- local VoiceResNums = #NeedPlayVoiceInfo
    ...
    --分段播放语音 暂时不需要了
    -- if VoiceResNums > 1 then
    --     local PlaySecondVoice = function()
    ...
    --                 if (self:IsExistTimer("PlaySecondVoice")) then
    --                     self:RemoveTimer("PlaySecondVoice")
    --                 end
    --                 self.VoiceEventInstance = AudioManager(self):PlayCharVoice(Player,CharName, NeedPlayVoiceInfo[2].VoiceRes[1],nil,"GachaVoice")
    --                 self.Text_VoiceText:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
    --                 self.Text_VoiceText:SetText(GText(NeedPlayVoiceInfo[2].VoiceText[1]))
    --                 self:PlayAnimation(self.Text_In)
    --             end
    --         end
    --     end
    --     self:AddTimer(0.01, PlaySecondVoice,true,0,"PlaySecondVoice",true)
    -- end
end
...
function G:SetCharImgRole(TargetId)
    ...
    -- if CharInfo.CampIcon then
    --     local BigIcon = LoadObject(CharInfo.CampIcon)
    --     self.Image_Icon:SetBrushResourceObject(BigIcon)
    -- end
...
function G:SetWeaponImgRole(TargetId)
    ...
    for _,Tag in pairs(WeaponInfo.WeaponTag) do
        ...
        if TagInfo and TagInfo.WeaponTagfilter then
            ...
            if ImageType ~= nil and ReverseMap[ImageType] then
...
function G:ShowExtraItem()
    ...
    if not self.IsNeedChange and Type == "Char" then
        local CharInfo = DataMgr.Char[TargetId]
...
function G:CheckHaveShowResult()
    for i = self.CurGachaResultIndex + 1,self.GachaTimes,1 do
...
function G:PlayInAnim(Rarity,CharId)
    ...
    elseif Rarity == 5 then
        -- if self.IsSpineAnimation then
        --     self:PlayAnimation(self.In_Spine)
        --     AnimTime = self.In_Spine:GetEndTime()
        -- else
            -- self:PlayAnimation(self.In_Yellow)
            -- AnimTime = self.In_Yellow:GetEndTime()
        -- end
...
function G:PlayCharSpine()
    ...
    if self.IsSpineAnimation and self.GachaOnceSpineUI then
        ...
        -- local function ShowCharName()
        --     self.Text_WeaponName:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        --     self.GachaOnceSpineUI.CharTitle:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        -- end
        -- self.GachaOnceSpineUI.Spine_Char.AnimationComplete:Add(self,ShowCharName)
    end
end
...
-- Params = {
--     TargetId = CharId/WeaponId,
--     Sign = CommonConst.GachaCharType/CommonConst.GachaWeaponType,
--     Rarity = 5,
--     TargetInfo = {
--        [1] = {TargetId = CharId/WeaponId,
--               Sign = CommonConst.GachaCharType/CommonConst.GachaWeaponType,
--               Rarity = 5,
--             }
--     },
...
function G:HandleShowResult(ShowResult)
    ...
    elseif ShowResult.Sign == CommonConst.GachaWeaponType then
        if not Avatar:CheckWeaponEnough({[TargetId] = 2}) then
            ShowResult.IsNew = true
        end
    end
end
