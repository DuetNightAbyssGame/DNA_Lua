...
-- WidgetBlueprint'/Game/UI/WBP/Gacha/PC/WBP_Gacha_P.WBP_Gacha_P'
-- WidgetBlueprint'/Game/UI/WBP/Gacha/Mobile/WBP_Gacha_M.WBP_Gacha_M'
...
function G:Construct()
    ...
    -- self.Btn_Gacha_Choose:BindEventOnClicked(self,self.OnClickBtnSelect)
    ...
    self.IsGachaMainInvalid = false  --抽卡是否无效
end
...
function G:OnLoaded(...)
    ...
    self.IsAutoClose = false --单抽结束后自动关闭抽卡系统，序章特供，默认不关闭
    self.InitParams = ... or {}
...
function G:InitValidGachaPool()
        ...
        if Gacha.Usable == 1 then
            ...
            if GachaInfo then
                ...
                if ConditionSucc then
                    ValidGachaPool[GachaId] = {GachaId = GachaId,Sequence = GachaInfo.Sequence or 0,
                    GachaName = GachaInfo.GachaName,GachaType = GachaInfo.GachaType }
                end
            end
        end
    end
    ...
    for TabId,Data in pairs(ValidGachaTabMapPool) do
        ...
        table.insert(self.ValidGachaTab,{TabId = TabId,TabSequence = DataMgr.GachaTab[TabId].Sequence or 0,ValidGachaPools = Data})
    end
...
function G:InitGachaMain(InitGachaTabId)
    self.CommonTabInfo ={}
    self.GachaIdMapBg = {}
    self.GachaRecordsCache = nil  --抽卡记录客户端缓存
    self.GuaranteedDict = nil --各卡池保底
    self.IsPCPlatform = CommonUtils.GetDeviceTypeByPlatformName(self) == "PC"  --是否是PC端
    self.VoiceEventInstance = nil --语音
...
function G:InitCommonTab(InitGachaTabId)
    if self.IsPCPlatform then
        ...
        self.Common_Tab:Init({DynamicNode = {"Back","ResourceBar","BottomKey"},
                                LeftKey = "NotShow",
                                RightKey = "NotShow",
                                bShowBubble = true,
                                TitleName = GText("MAIN_UI_GACHA"),
                                InfoCallback = "NotShow",
                                BottomKeyInfo = {--{KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=self.OnClickBtnClose, Owner=self}}, 
                                                --GamePadInfoList = {{Type="Img", ImgShortPath="B", ClickCallback=self.OnClickBtnClose, Owner=self}},
                                                Desc = GText("UI_BACK")},
                                StyleName = "Text", OwnerPanel = self, BackCallback = self.OnClickBtnClose})
        ...
        self.Common_Tab:Init({DynamicNode = {"Back","ResourceBar"},
                                InfoCallback = "NotShow",
                                bShowBubble = true,
                                TitleName = GText("MAIN_UI_GACHA"),
                                StyleName = "Text", OwnerPanel = self, BackCallback = self.OnClickBtnClose})
...
function G:InitListPool(InitGachaTabId)
    ...
    local AddListItem = function()
        ...
        if Data then
            ...
            -- PoolContent.GachaId = Data.GachaId
            ...
            if InitGachaTabId then
                ...
                if self.NowTabId == TabIndex then
                    PoolContent.IsSelected = true
                end
            end
...
function G:OnGachaPoolUpdate()
    ...
    CommonDialogParams.RightCallbackFunction = function() self:OnClickBtnClose() end
...
function G:OnUpdateUIStyleByInputTypeChange(CurInputType, CurGamepadName)
    ...
    if not self.IsPCPlatform then return end
    if self:CheckScreenShotWidget() then return end
...
function G:InitGamepadView()
    ...
    elseif self.OpenUI =="GachaTen" then
...
function G:HandleKeyReleased(Key)
    ...
    if Key.KeyName == UIConst.GamePadKey.DPadUp then
        if self.WS_BottomBtn:GetActiveWidgetIndex() == 0 and self.Group_Change:IsVisible() then
...
function G:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    elseif (InKeyName == "Escape") then
        self:OnClickBtnClose()
    elseif InKeyName == "Q" and self.NowTabId ~= 1 then
        self:GachaTabToLeft()
    elseif InKeyName == "E" and self.NowTabId ~= self.MaxTabNum then
...
function G:OnGamePadDown(InKeyName)
    local IsEventHandled = false
    if InKeyName == UIConst.GamePadKey.LeftShoulder and self.NowTabId ~= 1 then
        self:GachaTabToLeft()
    ...
    elseif InKeyName == UIConst.GamePadKey.FaceButtonTop then
        if self.WS_BottomBtn:GetActiveWidgetIndex() == 1 then
            self:OnClickBtnSelect()
        else
            if self.Group_TenTimes:IsVisible() then
...
function G:OnTabSelected(Content)
    if self.IsGachaMainInvalid then return end
...
function G:RefreshSelectedGachaPool()
    ...
    if GachaInfo.IsShowSpecialTitle then
        ...
        if self.GachaUpTitleSign and self.GachaUpTitleSignIsIn then
            -- self.GachaUpTitleSign:SetVisibility(UE4.ESlateVisibility.Collapsed)
...
function G:RefreshBackground()
    ...
    for i = 1,AllBgCount,1 do
        ...
        if Child == self.GachaIdMapBg[self.PreGachaId] then
            ...
            Child:BindToAnimationFinished(Child.Out,{Child, function()
...
function G:InitTargetTitle(IsChar,Title,TargetName,TargetRarity,TargetId)
    ...
    if IsChar then
        ...
        if not BattleWeaponInfo then return end
...
function G:RefreshGachaImg()
    ...
    if TimeLimitResourceCount > 0 then
        ...
        if TimeLimitResourceCount >= TenCost then
            ...
            if TimeLimitResourceCount + ShowResourceCount < TenCost then
                if ShowResourceCount >= TenCost then
                    self.Text_TenTimesCostNum:SetText('<Default>'..TenCost..'</>')
                    GachaTentimesUI.Text_TenTimesCostNum:SetText('<Default>'..TenCost..'</>')
                end
    ...
    -- self.Item_OnceIcon:Init(Content)
    -- self.Item_TenTimesIcon:Init(Content)
...
function G:GetHistoryTab()
    ...
    for key, value in pairs(DataMgr.Gacha) do
        ...
        NowTime < value.GachaEndTime then
            if value.GachaHistoryType == 2 and self.NowGachaId ~= key then
                table.insert(HistoryTab,{GachaId = key,GachaType = value.GachaType})
            end
...
function G:CheckIsSelectGacha()
    ...
    if DataMgr.GachaSelect[self.NowGachaId] then
        local GachaPool = Avatar.GachaPool[self.NowGachaId]
        local GachaSelfSelect = GachaPool.SelfSelect
        if GachaPool.Already5StarCount == 0 then
...
function G:OnClickBtnShop()
    ...
    if not self.ShopIsUnlock then return end
    AudioManager(self):PlayUISound(self, "event:/ui/common/click_level_01", nil, nil)
    local GachaInfo = DataMgr.Gacha[self.NowGachaId]
...
function G:OnDrawGacha(Ret,Data)
    if Ret == ErrorCode.RET_SUCCESS then
        ...
        -- local GachaOnceUI = self:GetGachaOnceUI()
        -- GachaOnceUI:Init(self,Data)
        self:RefreshResourceConsume()
        self:RefreshGuaranteedDict()
        self:CheckIsSelectGacha()
    elseif DataMgr.ErrorCode[Ret] then
...
function G:InitGachaList()
    self.GachaCharList = {}    --卡池角色列表 按星级
    self.GachaWeaponList = {}  --卡池武器列表 按星级
    self.GachaUpList = {}      --卡池Up列表 按星级
    self.UpTargetList = {}     --卡池Up列表
    for GachaId,GachaInfo in pairs(DataMgr.Gacha) do
        ...
        if GachaInfo.UPChar then
            for key,value in pairs(GachaInfo.UPChar) do
                ...
                local Info = {Type = "Char",TargetId = key,TargetRarity = Rarity,Probability = value,Type = "Char"}
        ...
        elseif GachaInfo.UPWeapon then
            for key,value in pairs(GachaInfo.UPWeapon) do
                ...
                local Info = {Type = "Weapon",TargetId = key,TargetRarity = Rarity,Probability = value,Type = "Weapon"}
                table.insert(self.UpTargetList[GachaId],Info)
                table.insert(UpList[Rarity],Info)
            end
        end
...
function G:PlayGuideGachaVoice(VoiceName, ExStoryInfo)
    -- ExStoryInfo 如果是 前主角相关，填 "EXPlayer"
    ...
    if EventExist then
        ...
        PlayStruct.EventKey = "GachaVoice" --自定义，Stop用的也是这个Key
        PlayStruct.bStopWhenAttachedToDestoryed = true
        PlayStruct.bPlayAs2D = true --看需要
        PlayStruct.SelectKey = SelectKey
        -- AttachedActor 自定义， Stop也传这个AttachedActor
...
function G:PlayInAnim()
    ...
    self.CantClick = true
    self:BindToAnimationFinished(self.In,{self,self.PlayInAnimFinished})
...
function G:Destruct()
    -- if(self.GachaDetailsUI)then
    --     self.GachaDetailsUI:RemoveFromParent()
    -- end
