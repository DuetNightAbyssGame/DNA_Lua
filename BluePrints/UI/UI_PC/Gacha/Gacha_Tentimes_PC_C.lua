...
function G:Init(Parent,Result,IsSingleGacha)
    ...
    for i = 1,CommonConst.GachaTenResults,1 do
...
function G:GamePadFocusChangeItem()
    if self.CanFocusChangeItem then
        for Index = 1,CommonConst.GachaTenResults,1 do
    ...
    if GachaRebateInfo then
        ...
        if ResourceId and Count then
            if self.ResourceReturn[ResourceId] then
                self.ResourceReturn[ResourceId] = {ResourceId = ResourceId ,Count = self.ResourceReturn[ResourceId].Count + Count}
            else
...
function G:RefreshResourceBar(GachaResourceId)
    local ResourceData = {CommonConst.Coins.Coin4,CommonConst.Coins.Coin1,GachaResourceId}
