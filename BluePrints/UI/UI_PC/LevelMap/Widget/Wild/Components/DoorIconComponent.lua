...
function Component:OnScaleChange_Component(Percent)
    ...
    for Id,Point in pairs(self.DoorIcons) do
        ...
        if ThisVisible then
            if Point:GetVisibility() ~= ESlateVisibility.HitTestInvisible then
                if Point:SetDoorVisibility("Scale",true) then
            ...
            if Point:GetVisibility() ~= ESlateVisibility.Collapsed  and not self.IsMinimap then
