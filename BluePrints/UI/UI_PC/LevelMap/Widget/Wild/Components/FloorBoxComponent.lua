...
function Component:InitFloorBox(CoroutineIndex)
    ...
    -- local array= GWorld.GameInstance:GetSceneManager().FloorBoxArray
    -- if not array or not self.IsInRegion then 
    --     FloorWidgetTable[self.MaxFloorId].Btn.OnClicked:Broadcast()
    --     FloorWidgetTable[self.MaxFloorId]:PlayAnimation(FloorWidgetTable[self.MaxFloorId].Click)
    --     -- self:ShowFloor(self.MaxFloorId)
    --     return
    -- end
    -- for _,box in pairs(array) do
    --     if UKismetMathLibrary.IsPointInBoxWithTransform(self.Player.CurrentLocation,box:GetTransform(),box.RootComponent.BoxExtent) then
    --         FloorWidgetTable[box.BuildingFloor].Btn.OnClicked:Broadcast()
    --         FloorWidgetTable[box.BuildingFloor]:PlayAnimation(FloorWidgetTable[box.BuildingFloor].Click)
    --         FloorWidgetTable[box.BuildingFloor].SizeBox:SetRenderOpacity(1)
    --         -- self:ShowFloor(box.BuildingFloor)
    --         return
    --     end
    -- end
...
function Component:OnFloorBtnClicked(FloorId,NeedClickAni)
    ...
    self:OnScaleChange(self.CurrentPercent)--todo:考虑给点做显示tag
end
