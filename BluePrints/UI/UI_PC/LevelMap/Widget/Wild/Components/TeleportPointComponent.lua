...
function Component:InitTeleportPoint(CoroutineIndex)
    ...
    if TmpFullRegionStoreDatas ~= nil then
        for _, RegionBaseData in pairs(TmpFullRegionStoreDatas) do
            ...
            if data then
                ...
                -- point:Init(self, data, state,self.OnTeleportPointClick,self.OnTeleportHover,self.OnTeleportUnhover)
                ...
                -- transferDatas[RegionBaseData.CreatorId]=nil
    ...
    for _,data in pairs(transferDatas) do 
        ...
        select:SetRenderTranslation(position)
        self.TeleportPoints[data.Id]=point
        -- self.TeleportState[data.Id]=false
        self.TeleportPointLocation[data.Id]=FVector2D(data.MechanismPos[1],data.MechanismPos[2])
        self.SelectWidgetTable[data.Id]=select
    ...
    for _,HardBossData in pairs(DataMgr.HardBossMain) do
        ...
        if TeleportId and not self.TeleportPoints[TeleportId] then
            ...
            if RegionId == self.RegionData.RegionId then
                if Avatar and Avatar:CheckHardBossUnlockCondition(HardBossData.HardBossId) then
                    local point,select=self:NewPointAsync(self.InitCoroutines[CoroutineIndex])
...
function Component:ShowFloor_Component(FloorId)
    ...
    for id,teleportPoint in pairs(self.TeleportPoints) do
        ...
        -- if self.TeleportPoint2FloorId[id]==FloorId then
        --     teleportPoint:SetPointVisibility('Floor',true)
        -- else
        --     teleportPoint:SetPointVisibility('Floor',false,true)
        -- end
    end
end
...
function Component:OnTeleportPointClick(Id)
    ...
    if self.TeleportIdToHardBossId[self.CurrentConveyId] then
        ...
        if self.TeleportState[data.Id] or Const.UnlockRegionTeleport then
            ...
            -- self.LevelMap_Convey_Widget_PC.Btn_Go:ForbidBtn(false)
            self.LevelMap_Convey_Widget_PC.Switch_Button:SetActiveWidgetIndex(0)
            -- self.LevelMap_Convey_Widget_PC.Switch_Button:GetActiveWidget():SetFocus()
            local Dungeons = self:CheckTempleAndPartyInfo(data.Id)
            if Dungeons ~= nil and #Dungeons > 0 then
                self.LevelMap_Convey_Widget_PC.Group_Temple:SetVisibility(self.GameInputModeSubsystem:GetCurrentInputType() == ECommonInputType.Gamepad and 
                ESlateVisibility.HitTestInvisible or ESlateVisibility.SelfHitTestInvisible)
            ...
            -- self.LevelMap_Convey_Widget_PC.Btn_Go:ForbidBtn(true)
...
function Component:CheckTempleAndPartyInfo(Id)
    ...
    -- 后面有单独判断是否处于特殊任务并显示禁用态，神庙解锁前没有单独的剧情模式，先注掉这块判断
    -- if self.Player then
    --     local Controller = self.Player:GetController()
