...
---@type LevelMap_ChangeArea_Widget_PC_C
...
function M:Construct()
    ...
    for _,id in pairs(MapData) do
        ...
        -- worldWidget.Img_Player:SetVisibility(ESlateVisibility.Collapsed)
        ...
        -- worldWidget:PlayAnimationReverse(worldWidget.Spin_Arrow)
...
function M:Init(Parent)
    ...
    for _,world in pairs(self.WorldList) do
        ...
        -- world:PlayAnimation(world.Spin_Arrow,world.Spin_Arrow:GetEndTime(),1,EUMGSequencePlayMode.Reverse)
    ...
    if worldId then
        ...
        -- self.WorldList[worldId].Img_Player:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
        -- self.WorldList[worldId].WS_Type:SetActiveWidgetIndex(1)
    end
    -- self:ShowOrHideRegionMapWidgtItemByTimeLine()
...
function M:OnWorldClick(id)
    if self.LastWorldWidget and self.LastWorldWidget~=self.WorldList[id] and self.LastWorldWidget.IsOpen then
        -- self.LastWorldWidget:StopAllAnimations()
        -- self.LastWorldWidget:PlayAnimationReverse(self.LastWorldWidget.Spin_Arrow)
    ...
    if self.LastWorldWidget~=self.WorldList[id] then
        -- self.WorldList[id]:StopAllAnimations()
        -- self.WorldList[id]:PlayAnimation(self.WorldList[id].Spin_Arrow)
        self.WorldList[id]:PlayAnimation(self.WorldList[id].Click)
        self.WorldList[id].IsOpen=true
    else
        if self.WorldList[id].IsOpen then
            -- self.WorldList[id]:StopAllAnimations()
            -- self.WorldList[id]:PlayAnimationReverse(self.WorldList[id].Spin_Arrow)
    ...
    for _,regionMapId in pairs(DataMgr.WorldMap[id].WorldMapRegion) do
        ...
        if regionData.RegionId==2101 or (DataMgr.RegionMap[regionMapId].RegionMapUnlockCondition and 
        not ConditionUtils.CheckCondition(Avatar,DataMgr.RegionMap[regionMapId].RegionMapUnlockCondition) and not Const.UnlockRegionTeleport)then--据点特殊处理
            goto continue
        end
        ...
        -- widget.Img_Player:SetVisibility(ESlateVisibility.Collapsed)
        ...
        for _,subRegionId in pairs(regionData.IsRandom) do
            if subRegionId==self.CurrentRegionId then
                -- widget.Img_Player:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
                widget.WS_Type:SetActiveWidgetIndex(1)
                WS_Type = true
                break
            end
        end
        if regionData.RegionId==1011 and self.CurrentRegionId==210101 then--据点特殊处理
            -- widget.Img_Player:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
            widget.WS_Type:SetActiveWidgetIndex(1)
            WS_Type = true
        end
        widget.Btn_ShowUp.OnClicked:Add(self,function()
            ...
            if self.LastRegionWidgetId ~= regionMapId then
                ...
                -- self.Parent:ShowMissionIndicatorsInRegionMap()
            end
            self.LastRegionWidgetId = regionMapId
        end)
...
function M:ShowOrHideRegionMapWidgtItemByTimeLine()
    if not self.WorldList then return end
    ...
    for id, worldWidget in pairs(self.WorldList) do
        ...
        local isNeedShowByRegionType = (MapRegionType =="EX" and UIRegionType == MapRegionType) or
        ((MapRegionType == "Now" or MapRegionType == nil) and UIRegionType ~= "EX")
...
function M:CheckIsNeedShowGuideInDoorWidgtItem(RegionId)
    ...
    if not QuestRegionMapId or not DataMgr.RegionMap[QuestRegionMapId] or not DataMgr.RegionMap[QuestRegionMapId].RegionId then
        return false
    end
