...
---@type LevelMap_Point_Widget_PC_C
...
function M:Construct()
    ...
    self:BindToAnimationFinished(self.In,{self,self.OnFadeIn})
    -- self.VisibilityTag={}
...
function M:Init(Parent,Data,IsUnlock,ClickFunction,HoverFunction,UnoverFunction)
    ...
    if ImagePath then--暂时传送点写死，梦魇残声填表
        local image=LoadObject(ImagePath)
        if image then
            self.Img_Point:SetBrushFromTexture(image,true)
        end
    else
        if IsUnlock or Const.UnlockRegionTeleport then
            -- self.NormalAni=self.Normal
            -- self:StopAllAnimations()
            -- self:PlayAnimation(self.Normal)
            local complete=true
            if Data.Temples ~= nil or Data.Parties ~= nil then
                -- 神庙&派对完成情况
                if Data.Temples ~= nil then
                    ...
                    for _,DungeonId in pairs(Data.Temples) do
                        ...
                        if DungeonInfo.RewardId ~= nil and #DungeonInfo.RewardId > 1 then
                            IsStarTemple = true
                        ...
                        if not Avatar or not Avatar.Dungeons[DungeonId] or (IsStarTemple and Avatar.Dungeons[DungeonId].MaxStar < Max) or (not IsStarTemple and not Avatar.Dungeons[DungeonId].IsPass) then
                            complete = false
                            break
                        end
                    end
            ...
            -- self.NormalAni=self.DarkGrey
            -- self:StopAllAnimations()
            -- self:PlayAnimation(self.DarkGrey)
...
function M:OnDeliveryMeshanismOpen(CreatorId)
    ...
    elseif self.Data.TelepointId then
        local Avatar = GWorld:GetAvatar()
        if DataMgr.TeleportPoint[self.Data.TelepointId] and DataMgr.TeleportPoint[self.Data.TelepointId].StaticId == CreatorId
        and (Avatar and ConditionUtils.CheckCondition(Avatar, self.Data.UnlockConditionId)) then
