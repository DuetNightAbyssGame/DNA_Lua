...
---@type WBP_LevelMap_TempleConveyItem_C
...
function M:ShowTempleInfo(TempleInfo)
    ...
    elseif TempleInfo.PassInfo == "PassStar" then
        -- self.HB_Star:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
        ...
        if self.TempleInfo.Star == 3 then
            self.Image_Complete:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
        else
...
--     if not self.TempleInfo then return UE4.UWidgetBlueprintLibrary.Handled() end
--     local GameInstance = GWorld.GameInstance
--     local UIManager = GameInstance:GetGameUIManager()
--     local GameMode = UE4.UGameplayStatics.GetGameMode(GameInstance)
--     if not UIManager or not GameMode then return end
...
--         UIManager:ShowUITip(UIConst.Tip_CommonTop, GText("UI_Temple_Lockedcondition")..GText("Name_"..self.TempleInfo.ConditionId))
--     else
--         -- 弹窗确认
...
function M:OnClickTempleItem()
    if not self.TempleInfo then return end
    local GameInstance = GWorld.GameInstance
    local UIManager = GameInstance:GetGameUIManager()
    local GameMode = UE4.UGameplayStatics.GetGameMode(GameInstance)
    local Avatar = GWorld:GetAvatar()
    if not UIManager or not GameMode or not Avatar then return end
    if self.TempleInfo.PassInfo == "Lock" then
        UIManager:ShowUITip(UIConst.Tip_CommonTop, GText("UI_Tosat_Temple_Locked"))
    elseif self.TempleInfo.PassInfo == "NoCompleteCondition" and self.TempleInfo.ConditionId > 0 then
...
function M:EnterTemple()
    ...
    if bIsInTeam then
        Avatar:EnterDungeon(self.TempleInfo.TempleId, CommonConst.DungeonNetMode.Standalone,
            function(RetCode, ...)
                local bCanEnter = self.HandleEnterDungeonRetCode(RetCode, ...)

                if bCanEnter then
                    UIManager(self):LoadUINew("DungeonMatchTimingBar",
                        self.TempleInfo.TempleId, Const.DUNGEON_MATCH_BAR_STATE.SPONSOR_WAITING_CONFIRM, false)
                end
            end)
    else
