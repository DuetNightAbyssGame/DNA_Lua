...
---@type LevelMap_Indicator_Widget_PC_C
...
function M:Init(WildMap,IdicatorLimitSize,TrackTarget,IsGamer)
    ...
    local size=self:GetDesiredSize()--USlateBlueprintLibrary.GetLocalSize(self.Indicator.Panel_Indicator:GetCachedGeometry())
...
function M:OnPointerMove(CurrentDragOffset,GamerLoc)
    ...
    if self.Location.X>self.IdicatorLimitSize.X or self.Location.X<-self.IdicatorLimitSize.X or self.Location.Y>BottomY or self.Location.Y<-self.IdicatorLimitSize.Y then
