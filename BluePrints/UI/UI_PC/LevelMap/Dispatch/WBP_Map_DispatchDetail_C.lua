...
function M:Initialize(Initializer)
    ...
    self.Dispatch = nil
    self.AbilityList = {}   --能力列表{item,{tag, bool}}
    self.Owner = nil
    self.AgentList = {}     --代理人槽位 {index，id}
    self.SpecialAbility = {}    --特殊能力{id，{item}}
    self.AgentAbility = {}      --代理人能力{id，{tag,item}}
    self.AgentCount = 0     --代理人个数
...
function M:InitDispatchDetail(Dispatch)
    ...
    self.List_Reward:SetNavigationRuleCustom(EUINavigation.Down,{self,function()
        if self.WS_Reward:GetActiveWidgetIndex() == 0 then
...
function M:SetBasicReward()
    ...
    if Rewards == nil then
        ...
        return a.Rarity > b.Rarity -- 按稀有度从高到低排序
    end)
    ...
    for _, RewardContent in ipairs(RewardContents) do
        local Reward = NewObject(UIUtils.GetCommonItemContentClass())
            ...
            Reward.AfterInitCallback = function(Widget)
...
function M:OnListItemInited(Content, EntryUI)
    ...
    if (DispatchState == CommonConst.DispatchState.Unlock or (DispatchState == CommonConst.DispatchState.Doing and self.Dispatch.DispatchCharsList:Length() == 0)
        or DispatchState == CommonConst.DispatchState.CanDispatch) then  --未解锁
    ...
    for Type, Count in pairs(DispatchDemand) do
        for i = 1, Count do          
            ...
            if AbilityItem ~= nil then
                ...
                -- self.AbilityList[AbilityItem] = self.AbilityList[AbilityItem] or {}
                -- self.AbilityList[AbilityItem] [Type] = false
...
--     self.Btn_CheckAbility_S:PlayAnimation(self.Btn_CheckAbility_S.Click)
--     UIManager(self):ShowCommonPopupUI(100139, Params)
-- end
...
--点击槽位
-- function M:OnClickAgent(Index)
--     local DispatchState = self.Dispatch.State
--     if DispatchState == CommonConst.DispatchState.Doing or DispatchState == CommonConst.DispatchState.Perfect or   DispatchState == CommonConst.DispatchState.Success or DispatchState == CommonConst.DispatchState.Qualified
--             or DispatchState == CommonConst.DispatchState.Disqualified then
--         return
--     end
--     if self.AgentList[Index] == 0 then
--         if self.Owner.DispatchAgentList then
--             UIManager(self):ShowUITip(UIConst.Tip_CommonTop, GText("UI_Dispatch_Toast_ChoseAgent"))
--         else
--             self:OpenAgentList() 
--         end         
--     else
--         self:RemoveAgentData(Index, true)
--     end
-- end

--更新代理人槽位UI
-- function M:UpdateAgentUI(IsPlay)
--     for i = 0, 2 do
--         local Agent = self.WBox_Agent:GetChildAt(i)
--         local Id = self.AgentList[i + 1]
        
--         if Id == nil then   --被禁用
--             Agent:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
--             Agent.RemoveAgent:SetVisibility(ESlateVisibility.Collapsed)
--             Agent.Switch_Type:SetActiveWidgetIndex(0)
--             Agent.Panel_AddAgent:SetVisibility(ESlateVisibility.Collapsed)
--             Agent:SetRenderOpacity(0.5)
--         elseif Id == 0 then --槽位代理人被移除或没有代理人
--             Agent:SetRenderOpacity(1)
--             Agent:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
--             Agent.Switch_Type:SetActiveWidgetIndex(0)
--             Agent.Panel_AddAgent:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
--             Agent.RemoveAgent:SetVisibility(ESlateVisibility.Collapsed)
--             Agent.Img_Add:SetVisibility(ESlateVisibility.Visible)
--         else    --有代理人
--             Agent:SetRenderOpacity(1)
--             local CharId = self:GetIdByUuid(Id)
--             local Char = DataMgr.Char[CharId]
--             Agent.Switch_Type:SetActiveWidgetIndex(1)
--             if ((self.Dispatch.State == CommonConst.DispatchState.Doing and self.Dispatch.DispatchCharsList:Length() > 0) or self.Dispatch.State == CommonConst.DispatchState.Perfect
--                 or self.Dispatch.State == CommonConst.DispatchState.Success or self.Dispatch.State == CommonConst.DispatchState.Qualified or self.Dispatch.State == CommonConst.DispatchState.Disqualified) then
--                 Agent:SetVisibility(UIConst.VisibilityOp.HitTestInvisible)
--                 Agent.RemoveAgent:SetVisibility(UE4.ESlateVisibility.Collapsed)
--                 self:UpdateAbilityUI(true, Id, IsPlay)
--             else
--                 Agent.RemoveAgent:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
--             end          
--             Agent.Img_Add:SetVisibility(ESlateVisibility.Collapsed)
--             local Icon = Char.Icon
--             Agent.Item:SetIcon(Icon)
--             Agent.Item:SetRarity(Char.CharRarity)
--         end
--     end 
-- end
...
function M:UpdateRemoveTips()
    if self.MatchTime == false and self.MatchSuccess == false then
        if self.Tip_ExBuff.Main:GetRenderOpacity() ~= 0 then
...
function M:UpdateAddTips()
    if self.MatchTime or self.MatchSuccess then
        -- if self.Tip_ExBuff.Visibility == ESlateVisibility.Collapsed then
        --     self.Tip_ExBuff:SetVisibility(UE4.ESlateVisibility.Visible)
        --     self.Tip_ExBuff:PlayAnimation(self.Tip_ExBuff.In)
        if self.Tip_ExBuff.Main:GetRenderOpacity() == 0  then
            --self.Tip_ExBuff:SetRenderOpacity(1)
            self.Tip_ExBuff:PlayAnimation(self.Tip_ExBuff.In)
        end
...
function M:UpdateRemainingTime()
    ...
    for _, Uuid in pairs(self.Dispatch.DispatchCharsList) do
        ...
        local Tag = Char:GetCurrentUnlockDispatchTag()
        --local Info = DataMgr.Char[CharId].DispatchTag
        for Tag, Number in pairs(Tag) do
            if Tag == "Workaholic" then
                Count = Count + Number
            end
...
function M:OnClickConfirm()
    ...
    elseif DispatchState == CommonConst.DispatchState.Perfect or   DispatchState == CommonConst.DispatchState.Success or DispatchState == CommonConst.DispatchState.Qualified
            or DispatchState == CommonConst.DispatchState.Disqualified then
        self:GetReward()
    end
end
...
function M:CancelDispatch()
    local DispatchState = self.Dispatch.State
    if (DispatchState == CommonConst.DispatchState.Doing and self.Dispatch.DispatchCharsList:Length() > 0) then  --派遣中
        local Parent = nil 
        if self.Owner.DispatchAgentList then
            Parent = self.Owner.DispatchAgentList
        elseif self.Owner.DispatchList then
...
function M:InitBtnInfo()
    local DispatchState = self.Dispatch.State
    if (DispatchState == CommonConst.DispatchState.Unlock) then  --未解锁
    ...
    elseif (DispatchState == CommonConst.DispatchState.Doing and self.Dispatch.DispatchCharsList:Length() == 0) then --进行中
        self:IsShowExtraText(true)
        self:StopAnimation(self.Progessing)
        self:PlayAnimation(self.Initial)
        ...
        self.Buff:SetVisibility(ESlateVisibility.Collapsed)
    elseif (DispatchState == CommonConst.DispatchState.Doing and self.Dispatch.DispatchCharsList:Length() > 0) then  --派遣中
    ...
    elseif DispatchState == CommonConst.DispatchState.CanDispatch then --可派遣
        if self.Owner.DispatchAgentList then
            self:OpenAgentList()
        else
        ...
        self.Buff:SetVisibility(ESlateVisibility.Collapsed)
    elseif DispatchState == CommonConst.DispatchState.Perfect or   DispatchState == CommonConst.DispatchState.Success or DispatchState == CommonConst.DispatchState.Qualified
            or DispatchState == CommonConst.DispatchState.Disqualified then   --已完成
        ...
        self:InitAbility(self.Dispatch.DispatchCharsList)
        --self.Text_NoBuff:SetText(GText("UI_Dispatch_Toast_NoBuff"))
    end
            ...
            for i =1, Count do
                ...
                if TypeInfo.IsBuff == 1 then
                    ...
                    for i, Value in pairs(self.AbilityList) do
                        ...
                        if Tag == Type and Active == false then
                            self:AbilityPlayAnimation(Tag, Value.Widget, IsPlay, true)
                            --Item:PlayAnimation(Item.Active_Feedback)
                            Value.IsActive = true
        ...
        if Info ~= nil then
            for Item, Tag in pairs(Info) do
                ...
                for i, abilityData in ipairs(self.AbilityList) do
                    if abilityData.Widget == Item then  -- 通过Widget字段匹配
                        abilityData.IsActive = false       -- 修改对应Tag的状态
                        break
                    end
                end               
            end
...
function M:GotoCloestTeleportPoint()
	...
	-- if GameMode == nil or GameMode:GetRegionDataMgrSubSystem() == nil then
    --     ScreenPrint("GameMode == nil or GameMode:GetRegionDataMgrSubSystem() == nil, RegionId：" .. tostring(GotoSubRegionId) .. " DispatchId:" .. tostring(self.Dispatch.DispatchId))
    --     return 
    -- end
    ...
    --     ScreenPrint("FullRegionStoreDatas == nil, RegionId：" .. tostring(GotoSubRegionId) .. " DispatchId:" .. tostring(self.Dispatch.DispatchId))
    --     return
    -- end
    -- local HasOpenTeleport = false
    -- for _, RegionBaseData in pairs(TmpFullRegionStoreDatas) do
    --     if RegionBaseData.SubRegionId == GotoSubRegionId and RegionBaseData.State and RegionBaseData.State.OpenState == true then
    --         HasOpenTeleport = true
    --         break
    --     end
    -- end

    -- if HasOpenTeleport == false then
    --     UIManager(self):ShowUITip(UIConst.Tip_CommonTop, GText("UI_Dispatch_Toast_NoTeleport"))
    --     ScreenPrint("该区域没有激活的传送点,RegionId：" .. tostring(GotoSubRegionId) .. " DispatchId:" .. tostring(self.Dispatch.DispatchId))
    --     return
    -- end
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    if (IsUseKeyAndMouse) then
        ...
        if self.Owner.DispatchList == nil and self.Owner.DispatchAgentList == nil then
            self:InitPadKeyInfoByDetail()
            self:SetFocus()
        end
    end
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if self.TipsOpen == true and InKeyName ~= "Gamepad_FaceButton_Right" then
        return UE4.UWidgetBlueprintLibrary.Handled() 
    end
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        elseif (InKeyName == "Gamepad_FaceButton_Right") then
            ...
            elseif self.Owner.DispatchList then
                ...
                if self.List_Reward:HasFocusedDescendants() or self.List_ExReward:HasFocusedDescendants() then
                    self:SetFocus()
                    self:InitPadKeyInfoByDetail()
                    self:ShowPadUI(true)
                    return UE4.UWidgetBlueprintLibrary.Handled()
            ...
            IsHandled = true
        elseif (InKeyName == "Gamepad_FaceButton_Down") then
...
function M:OnPreviewKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        elseif (InKeyName == "Gamepad_DPad_Left") then
            if self.WS_BuffType:GetActiveWidgetIndex() == 0 then
        ...
        elseif (InKeyName == "Gamepad_FaceButton_Bottom") then
            ...
            elseif self.Dispatch.State == CommonConst.DispatchState.Perfect or self.Dispatch.State == CommonConst.DispatchState.Success or self.Dispatch.State == CommonConst.DispatchState.Qualified
                or self.Dispatch.State == CommonConst.DispatchState.Disqualified then
                self:GetReward()
            end  
            IsHandled = true
        end
