...
---@type WBP_Map_DispatchAgentList_C
...
function M:InitAgentList(Owner)
    ...
    -- self.Owner.Tab:UpdateBottomKeyInfo({
    --         { 
    --             KeyInfoList =  {{
    --                 Type="Text", 
    --                 Text="Esc", 
...
function M:GetAgentState(Uuid)
    ...
    --local CharUuid = CommonUtils.ObjId2Str(Avatar.CurrentChar)
...
function M:OnClickClose()
    ...
    if(self.Owner.DispatchList ~= nil) then
        ...
        --self.Owner.DispatchList:RefreshBaseInfo()
...
function M:OnListItemClicked(Content, IsByAuto)
    ...
    if Content.IsChoose then
        if not (self.Owner.DispatchDetail.Dispatch.State == CommonConst.DispatchState.CanDispatch) then
            Content.IsChoose = not Content.IsChoose
            return
        end
...
function M:InitDispatchingAgentList()
    ...
    for Id, Dispatch in pairs(Avatar.Dispatches) do
        if (Dispatch.State == CommonConst.DispatchState.Success or Dispatch.State == CommonConst.DispatchState.Perfect or Dispatch.State == CommonConst.DispatchState.Qualified
            or Dispatch.State == CommonConst.DispatchState.Disqualified or (Dispatch.DispatchCharsList:Length() > 0)) then
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        -- if (InKeyName == "Gamepad_Special_Left") then
        --     self.Owner.DispatchDetail:OpenSpecailAbility()
        -- end
    end
