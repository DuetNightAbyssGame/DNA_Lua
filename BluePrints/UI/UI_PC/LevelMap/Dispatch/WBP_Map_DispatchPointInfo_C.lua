...
---@type WBP_Map_DispatchPointInfo_C
...
function M:SetState(Dispatch)
    ...
    if (self.DispatchInfo.State == CommonConst.DispatchState.Success or self.DispatchInfo.State == CommonConst.DispatchState.Perfect or self.DispatchInfo.State == CommonConst.DispatchState.Qualified
        or self.DispatchInfo.State == CommonConst.DispatchState.Disqualified) then
    ...
    elseif self.DispatchInfo.State == CommonConst.DispatchState.Doing then
        if self.DispatchInfo.DispatchCharsList:Length() > 0 then --派遣中
...
function M:InitAgent()
    self.WBox_MiniHead:ClearChildren()
    for i = 1, self.DispatchInfo.DispatchPerson, 1 do
...
function M:RefreshAgent(Id)
    ...
    --self.WBox_MiniHead:ClearChildren()
    -- self.HeadList = {}
    -- for _, Id in pairs(Agent) do
    --     local IsPlay = not CommonUtils.HasValue(self.AgentList, Id)
    --     self:AddAgent(Id, IsPlay)
    -- end
...
function M:OnItemSelect(IsPlayAni)
    if  (self.DispatchInfo.State == CommonConst.DispatchState.Doing and self.DispatchInfo.DispatchCharsList:Length() > 0) or self.DispatchInfo.State == CommonConst.DispatchState.Perfect or   self.DispatchInfo.State == CommonConst.DispatchState.Success or self.DispatchInfo.State == CommonConst.DispatchState.Qualified
            or self.DispatchInfo.State == CommonConst.DispatchState.Disqualified or self.DispatchInfo.State == CommonConst.DispatchState.CanDispatch then
        self.AgentTips:SetVisibility(ESlateVisibility.Visible)
        if IsPlayAni then
...
function M:AddAgent(Id, Index)
    ...
    --local NextHead = self.WBox_MiniHead:GetChildAt(Index + 1)
    if NextHead and NextHead.IsChoose == false and NextHead.IsForid == false then
...
function M:FindFirstHead()
    local Childs = self.WBox_MiniHead:GetAllChildren()
    for key, value in pairs(Childs) do
        if value.IsChoose == false and value.IsForid == false then
            return value, key
        end
    end
    return nil
...
function M:PlaySelectAnimation()
    ...
    if self.DispatchInfo.State == CommonConst.DispatchState.CanDispatch then
        ...
        --local NextHead = self.self.WBox_MiniHead:GetChildAt(Index)
...
function M:ShowMiniHead(Id)
    if (self.DispatchInfo.State == CommonConst.DispatchState.Doing and self.DispatchInfo.DispatchCharsList:Length() <= 0) then --手动处理
        self.AgentTips:SetVisibility(ESlateVisibility.Collapsed)
        self:IsShowTime(false)
        return
    end
    ...
    elseif self.DispatchInfo.State == CommonConst.DispatchState.Perfect or self.DispatchInfo.State == CommonConst.DispatchState.Success or self.DispatchInfo.State == CommonConst.DispatchState.Qualified
        or self.DispatchInfo.State == CommonConst.DispatchState.Disqualified then
            self.AgentTips:SetVisibility(ESlateVisibility.Visible)
            if Id == self.DispatchInfo.DispatchId then
                self:PlayAnimation(self.In)
