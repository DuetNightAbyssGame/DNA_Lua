...
local WBP_TrueHardBoss_Main_PC_C = Class({"BluePrints.Common.TimerMgr", "BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_TrueHardBoss_Main_PC_C:FillWrapBox(Index)
    ...
    local function FillWrapBoxFunc()
        ...
        if ScrollBoxSize.Y ==0 or EntrySize.Y == 0 then
            return false
        end
...
function WBP_TrueHardBoss_Main_PC_C:RefreshListBossInfo(Index)
    ...
    --暂时隐藏地点名
    --self.Text_Position:SetText(GText(DataMgr.SubRegion[BossInfo.SubRegionId].SubRegionName))
