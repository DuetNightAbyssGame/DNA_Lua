...
function WBP_Archive_Reward_C:InitListTabInfo()
    ...
    self.List_Reward:SetNavigationRuleCustom(EUINavigation.Left,{self,self.OnUINavigation})
...
function WBP_Archive_Reward_C:AddListReward(ArchiveType, SumNum)
    ...
    --     else
    --         self.Key_02:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    --     end
...
function WBP_Archive_Reward_C:EnterSelectMode(Entry)
    ...
    if not Entry then
        Entry = self:NavigateToFirstDisplayedItem(self.List_Reward)
    end
...
-- function WBP_Archive_Reward_C:OnAnalogValueChanged(MyGeometry,InAnalogInputEvent)
--     local InKey = UE4.UKismetInputLibrary.GetKey(InAnalogInputEvent)
--     local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
--     if (InKeyName == "Gamepad_RightY") then
--         local a = UKismetInputLibrary.GetAnalogValue(InAnalogInputEvent) * 0.5
--         local CurScrollOffset = self.List_Reward:GetScrollOffset()
--         local ScrollOffset = math.clamp(CurScrollOffset - a,0, self.List_Reward:GetScrollOffsetOfEnd())
--         self.List_Reward:SetScrollOffset(ScrollOffset)
--     end
--     return UE4.UWidgetBlueprintLibrary.UnHandled()
-- end
