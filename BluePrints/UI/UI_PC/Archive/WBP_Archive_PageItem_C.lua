...
local WBP_Archive_PageItem_C = Class({"BluePrints.UI.BP_UIState_C"})
...
function WBP_Archive_PageItem_C:Construct()
    ...
    --self.List_Item.BP_OnItemClicked:Add(self,self.ClickListItem)
...
function WBP_Archive_PageItem_C:InitCommonTab()
    ...
    for _, Data in pairs(DataMgr.ArchiveTab) do
        if Data.ArchiveID == self.Type then
            table.insert(self.AllTabInfo, {Text=GText(Data.TabName), IconPath=Data.Icon, 
                            TabId=Data.TabId, TabType=Data.TabPara, ArchiveID=Data.ArchiveID})
        end
    end
    self.TabId2TabType = {}
    self.TabType2TabId = {}
    ...
    for _,Info in pairs(self.AllTabInfo) do
        ...
        if Info.ArchiveID == 1006 then
            if Info.TabType ~= "AllMonster" and Info.TabType ~= "OtherMonster" then
    ...
    self.TabConfigData = {
        ...
        LeftKey="Q", RightKey="E", Tabs=self.AllTabInfo, 
        DynamicNode={"Back", "ResourceBar", "BottomKey"}, 
        StyleName="Text", OwnerPanel=self, BackCallback=self.OnClickBack,
        BottomKeyInfo = {
            {
...
function WBP_Archive_PageItem_C:TabItemClick(TabWidget)
    ...
    if not self.SortFunction or self.SortFunction ~= self.DefaultSortFunction then
        ...
        self.Sort:Init(self, {"UI_Select_Unique"}, CommonConst.DESC)
    end
...
function WBP_Archive_PageItem_C:RefreshList(bAnimation)
    self.Panel_ItemInfo:SetVisibility(UE4.ESlateVisibility.Collapsed)
    if bAnimation == nil then bAnimation = true end
    self:GetListData()
    self:MarkDirty()
    self.CurSelectContent = nil
...
function WBP_Archive_PageItem_C:FillReddotInfo()
    if self.NodeName then
        ...
        if self.Type == 1005 then
            for Index, ItemInfo in ipairs(self.ListDatas) do
                if ItemInfo.BookSeriesId then
                    ...
                    for _,ResourceId in pairs(ResourceIdTable) do
                        if CacheDetail[ResourceId]==1 then
                            ItemInfo.IsNew = true
                        end
                    end
                end
            ...
            for Index, ItemInfo in ipairs(self.ListDatas) do
                if ItemInfo.Id and CacheDetail[ItemInfo.Id]==1 then
                    ItemInfo.IsNew = true
                end
            end
        end
...
function WBP_Archive_PageItem_C:GetAllResourceData()
    ...
    for ResourceId,Data in pairs(DataMgr.Resource) do
        if Data.ItemArchiveType == 1 or Data.ItemArchiveType == 2 then
    ...
    --         local Data = DataMgr.Resource[Id]
    --         self:FillListDatasWithResourceInfo(Data)
    --     end
    -- end
    -- self.SortFunction(self)
...
function WBP_Archive_PageItem_C:GetNormalResourceData()
    ...
    for ResourceId,Data in pairs(DataMgr.Resource) do
        if Data.ItemArchiveType and Data.ItemArchiveType == 1 then
            self.Num_Total = self.Num_Total + 1
            if ArchiveList[ResourceId] then
                self.Num_Now = self.Num_Now + 1
                self:FillListDatasWithResourceInfo(Data)
    ...
    --     if DataMgr.Resource[Id] then
    --         local Data = DataMgr.Resource[Id]
    --         if Data.ItemArchiveType == 1 then
    --             self:FillListDatasWithResourceInfo(Data)
    --         end
    --     end
    -- end
...
function WBP_Archive_PageItem_C:GetTaskResourceData()
    ...
    for ResourceId,Data in pairs(DataMgr.Resource) do
        if Data.ItemArchiveType and Data.ItemArchiveType == 2 then
            self.Num_Total = self.Num_Total + 1
            if ArchiveList[ResourceId] then
                self.Num_Now = self.Num_Now + 1
                self:FillListDatasWithResourceInfo(Data)
    ...
    --     if DataMgr.Resource[Id] then
    --         local Data = DataMgr.Resource[Id]
    --         if Data.ItemArchiveType == 2 then
    --             self:FillListDatasWithResourceInfo(Data)
    --         end
    --     end
    -- end
...
function WBP_Archive_PageItem_C:GetMonsterData(TabType)
    -- if not self.AllMonsterGalleryId then
    --     self.AllMonsterGalleryId = {}
    --     for Id,Data in pairs(DataMgr.GalleryRule) do
    --         if not Data.IsNotOpen then
    --             table.insert(self.AllMonsterGalleryId, Data.CharId)
    --         end
    --     end
    -- end
    self.Num_Now = 0
    ...
    -- for GalleryRuleId,_ in pairs(ArchiveList) do
    --     local GalleryData = DataMgr.GalleryRule[GalleryRuleId]
    --     local MonsterData = DataMgr.Monster[GalleryData.PreferredMonsterId]
    --     if (not GalleryData.DisableArchive) and self:CheckMonsterInCamp(GalleryData.PreferredMonsterId,TabType) then
    --         local FunctionDes = DataMgr.ArchiveTab[self.CurTabId].TabName
    --         if TabType == "AllMonster" then
    --             local Camp = self:GetMonsterCamp(GalleryData.PreferredMonsterId)
    --             for TabId,TabType in pairs(self.TabId2TabType) do
...
function WBP_Archive_PageItem_C:SetupListContent(ItemIndex, ItemInfo, Content, Type)
    ...
    -- Content.OnInFinished = function (EntryS,ContentS)
    --     if ContentS.ItemIndex == 1 and not self.CurSelectContent then
...
function WBP_Archive_PageItem_C:TryOpenCollect()
    if self.WS_Empty:GetActiveWidgetIndex() == 1 then
        return
    end
...
function WBP_Archive_PageItem_C:ClickListItem(Content)
    if Content and not Content.IsEmpty then
        ...
        --self.CurSelectContent.Entry:SetFocus()
        --self.CurSelectContent.Entry.bIsDealWithVirtualAccept = true
        self:RefreshDetail()
        self:RefreshBtn()
...
function WBP_Archive_PageItem_C:ClickListItemWhenSelectItemChanged(Content)
    if Content and Content.Entry then
        if not self.CurSelectContent or self.CurSelectContent.ItemIndex ~= Content.ItemIndex then
        ...
        -- self.Panel_Info:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        -- if (IsValid(self.CurSelectContent)) then
        --     if Content.Id == self.CurSelectContent.Id then
        --         return
        --     end
        --     self.CurSelectContent.Entry:SetSelected(false)
        -- end
...
function WBP_Archive_PageItem_C:RefreshBtn()
    if self.Name == "Read" then
        if self.CurSelectContent.IsBookSeriesUnlock then
            ...
            -- self.Btn_Read:ForbidBtn(false)
            self.Btn_Read:SetText(GText("UI_BookArchive_Read"))
            self.WS_Type:SetActiveWidgetIndex(0)
        else
            self.ReadBtnForbid = true
            -- self.Btn_Read:ForbidBtn(true)
            -- self.Btn_Read:SetText(GText("UI_BookArchive_SeriesLockedEntrance"))
...
function WBP_Archive_PageItem_C:ShowBookLockedTip()
    --UIManager(self):ShowUITip(UIConst.Tip_CommonToast, GText("UI_BookArchive_SeriesLockedEntrance"))
end

function WBP_Archive_PageItem_C:OnObjectSetFinished(Content,Widget)
    if Content.ItemIndex == 1 and not self.CurSelectContent and Content.Entry then
        Content.Entry:SetDoNotPlaySound(true)
        Content.Entry:OnMouseButtonDown()
        Content.Entry:OnMouseButtonUp()
        Content.Entry:SetDoNotPlaySound(false)
...
function WBP_Archive_PageItem_C:InitKeyboardView()
    ...
    -- end
    -- self.Btn_Collect:SetVisibility(UIConst.VisibilityOp["Visible"])
end
...
function WBP_Archive_PageItem_C:RefreshReddot()
    ...
    if self.Type == 1005 then
        ...
        for _, Content in pairs(Contents) do
            if not Content.IsEmpty then
                Content.IsNew = false
                if Content.Id and CacheDetail[Content.Id]==1 then
                    Content.IsNew = true
                end
            end
        end
...
function WBP_Archive_PageItem_C:SetArchiveReddotRead(Content)
    if self.NodeName and not Content.BookSeriesId then
        ...
        local CacheDetail = ReddotManager.GetLeafNodeCacheDetail(self.NodeName)
        if(CacheDetail[Content.Id] == 1)then
...
function WBP_Archive_PageItem_C:CheckResourceIsBelongNormalResource(ResourceId)
    ...
    if ResourceData and ResourceData.ItemArchiveType == 1 then
        return true
    else
        return false
    end
...
function WBP_Archive_PageItem_C:CheckResourceIsBelongTaskResource(ResourceId)
    local ResourceData = DataMgr.Resource[ResourceId]
    if ResourceData and ResourceData.ItemArchiveType == 2 then
        return true
    else
        return false
    end
...
-- function WBP_Archive_PageItem_C:GetNewBookSeriesNum()
--     local CacheDetail = ReddotManager.GetLeafNodeCacheDetail(DataMgr.ReddotNode.ArchiveNewReadBook.Name)
--     if CacheDetail then
--         local BookSeries = {}
--         local NewBookSeriesNum = 0
--         for ResourceId, State in pairs(CacheDetail) do
--             local BookSeriesId = DataMgr.Resource[ResourceId].BookSeriesId
--             if not BookSeries[BookSeriesId] then
--                 BookSeries[BookSeriesId] = 1
--                 NewBookSeriesNum = NewBookSeriesNum + 1
--             end
--         end
--         return NewBookSeriesNum
--     end
--     return 0
-- end
