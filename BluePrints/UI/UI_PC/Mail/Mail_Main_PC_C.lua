...
function M:OnLoaded(...)
    ...
    if self.Platform == "PC" then
        ...
        table.insert(BottomKeyInfo,  {
            ...
            Desc = GText("UI_BACK")})
        self.Common_Tab:Init({DynamicNode = {"Back","BottomKey"},
                                LeftKey = "NotShow",
                                RightKey = "NotShow",
                                TitleName = GText("MAIN_UI_MAIL"),
                                BottomKeyInfo = BottomKeyInfo,
                                StyleName = "Text", OwnerPanel = self, BackCallback = self.OnPressESC})
    else
        self.Common_Tab:Init({DynamicNode = {"Back"},
                                TitleName = GText("MAIN_UI_MAIL"),
                                StyleName = "Text", OwnerPanel = self, BackCallback = self.OnPressESC})
    end
...
function M:RefreshTabBottomKey()
    ...
    if self.Common_Tab and self.Common_Tab.UpdateBottomKeyInfo then
        ...
        if self.WBP_Mail_Root:CheckHaveRewardToReceiveAll() then
            table.insert(BottomKeyInfo,  {
                KeyInfoList = {{Type="Text", Text="Space", ClickCallback=self.OnPressSPACE, Owner=self}}, 
