...
---@type WBP_MiniGame_Mima_Btn_C
local WBP_MiniGame_Mima_Btn_C = Class({"BluePrints.UI.BP_EMUserWidget_C"})
...
function WBP_MiniGame_Mima_Btn_C:InitButtonEvent()
    self.Btn.OnClicked:Add(self, function() self:OnPagePointClicked() end)
    self.Btn.OnPressed:Add(self, function() self:OnPagePointPressed() end)
    self.Btn.OnHovered:Add(self, function() self:OnPagePointHovered() end)
    self.Btn.OnUnhovered:Add(self, function() self:OnPagePointUnhovered() end)
end
...
function WBP_MiniGame_Mima_Btn_C:OnPagePointClicked()
    --点击后调用GetIndex方法，再调用主界面方法 把值传过去
    DebugPrint("thy   Click ", self.Index)
    self.Owner:InputPassword(self.Index)
    self:BindToAnimationFinished(self.Click, {self, self.AfterClick})
