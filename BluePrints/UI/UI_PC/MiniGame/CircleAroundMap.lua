...
function CircleAroundMap:InitUIWithPlatform()
    if self.isPC then
        ...
        -- self.MiniGame_Time.WidgetSwitcher_MP
...
function CircleAroundMap:InitAfterBeginPlay()
    ...
    if not self.bMapIndexIsLocked then
        ...
        local MinMapIndex = nil
        if self.MapIndex % 4 == 0 then
...
function CircleAroundMap:InitPointerAndBlock()
    ...
    -- Init Pointer
    if TableInfo["PointerNum"] == 1 then
        self.PointerArray[1]:InitPointer(self, 0, 10, self.PointerSpeed)
    elseif TableInfo["PointerNum"] == 2 then
        if #self.SwitchPosition == 6 then
            self.PointerArray[1]:InitPointer(self, 0, 10, self.PointerSpeed)
            self.PointerArray[2]:InitPointer(self, 60, 10, self.PointerSpeed)
        elseif #self.SwitchPosition == 8 then
            self.PointerArray[1]:InitPointer(self, 0, 10, self.PointerSpeed)
...
function CircleAroundMap:InitTextTimerAndButton()
    ...
    -- self.Text_Success:SetText(GText("UI_MiniGame_Success"))
    -- self.Text_Fail:SetText(GText("UI_MiniGame_Fail"))
...
function CircleAroundMap:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if InKeyName == "SpaceBar" or InKeyName == UIConst.GamePadKey.FaceButtonBottom then
        EventManager:FireEvent(EventID.CircleAroundTryToTriggerPointer)
        self.KeyDown = true
    end
    if InKeyName == "Escape" and self.bCanEscape then
        self:GameFailed()
    end
    if InKeyName == "LeftAlt" and not self.IsShowingCursor then
        self.IsShowingCursor = true
        self:ResetCursor()
    end
    if InKeyName == "F" or InKeyName == "Gamepad_FaceButton_Top" then
...
function CircleAroundMap:OnKeyUp(MyGeometry, InKeyEvent)
    ...
    if InKeyName == "LeftAlt" and self.IsShowingCursor then
        self.IsShowingCursor = false
        self:SetCursor(0)
    end
    return UE4.UWidgetBlueprintLibrary.Handled()
...
function CircleAroundMap:CheckIfInputRegionHasSwitch(LowIndex, HighIndex)
    for i=1,#self.SwitchPosition do
        if self.SwitchPosition[i] >= LowIndex and self.SwitchPosition[i] <= HighIndex then
...
function CircleAroundMap:CountDown()
    ...
    if self.Time <= 3 then
        ...
        -- self.MiniGame_Time.Text_Time:SetColorAndOpacity(UE4.UUIFunctionLibrary.GetSlateColorByRGBA(1,0,0,0.7))
    end
    if self.Time <= 0 then
        self:GameFailed()
        return
...
function CircleAroundMap:CheckIfGameSuccess()
    if self.ActiveSwitch == #self.SwitchPosition then
        -- print(_G.LogTag, "Error: Game Success")
        self:GameSuccess()
    end
end
