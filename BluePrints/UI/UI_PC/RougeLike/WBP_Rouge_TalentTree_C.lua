...
function WBP_Rouge_TalentTree_P_C:OnTeleportPointClick(Id, DoNotPlayAnimation)
    ...
    self.SelectedId = Id
    --self.Parent:ScrollToPoint(self.Points[self.SelectedId])
...
function WBP_Rouge_TalentTree_P_C:ChangeBranch(Branch)
    ...
    for TalentId, RougeTalentInfo in pairs(self.RougeTalent) do
        if RougeTalentInfo.Branch == self.Branch then
            ...
            Point:SaveBtnSizeXY()
            if (not FarthestX) or (Position.X > FarthestX) then
                self.FarthestTalentId = TalentId
                FarthestPoint = Point
                FarthestX = Position.X
                self.FarthestX = FarthestX
            end
            if (not ShortestX) or (Position.X < ShortestX) then
        ...
        if RougeTalentInfo.Branch == self.Branch then
            ...
            if RougeTalentInfo.UnLock then
                for _, PostTalentId in pairs(RougeTalentInfo.UnLock) do
                    ...
                    -- 之前需要根据前后天赋的类型来调节线段左右两端的形态
                    -- if RougeTalentInfo.Type == 1 then
                    --     Line.WidgetSwitcher_L:SetActiveWidgetIndex(0)
                    -- else
                    --     Line.WidgetSwitcher_L:SetActiveWidgetIndex(1)
                    -- end
                    -- if self.RougeTalent[PostTalentId].Type == 1 then
                    --     Line.WidgetSwitcher_R:SetActiveWidgetIndex(0)
                    -- else
                    --     Line.WidgetSwitcher_R:SetActiveWidgetIndex(1)
                    -- end
                    if AngleSin > 0 then
...
function WBP_Rouge_TalentTree_P_C:SelectDefaultTalentPoint(Point)
    ...
    elseif Point.Priority == self.DefaultTalentPoint.Priority then
        if Point.Position.X < self.DefaultTalentPoint.Position.X then
            self.DefaultTalentPoint = Point
        elseif Point.Position.X == self.DefaultTalentPoint.Position.X then
            if Point.Position.Y < self.DefaultTalentPoint.Position.Y then
                self.DefaultTalentPoint = Point
            end
        end
    end
...
function WBP_Rouge_TalentTree_P_C:CheckState(Id)
    if self.BranchActiveTalent[Id] then
        ...
        if self:CheckCanActive(Id) then
            return "CanActive"
        else
            return "CantActive"
        end
...
function WBP_Rouge_TalentTree_P_C:ActiveTalent(Id, Level)
    ...
    --     Talent = CommonUtils.Copy(Avatar.RougeLike.Talent)
    -- end
    -- if Talent[Id] and (Talent[Id] >= Level) then
    --     return
    -- end
    -- Talent[Id] = Level
	...
	-- if Avatar then
	-- 	Avatar:SaveTalent(Talent, Callback)
	-- end
    if Avatar then
    ...
    for FirstTalentId,FirstPoint in pairs(self.Points) do
        ...
        for SecondTalentId,SecondPoint in pairs(self.Points) do
            ...
            if FirstTalentId ~= SecondTalentId then
                ...
                --         if (FirstLocationTable.Top <= SecondLocationTable.Bottom and FirstLocationTable.Bottom >= SecondLocationTable.Top)
                --         or (FirstLocationTable.Bottom >= SecondLocationTable.Top and FirstLocationTable.Top <= SecondLocationTable.Bottom) then
                --             LeftFlag = true
                --         end
                --     end
                -- elseif FirstLocation.X < SecondLocation.X then
                ...
                if FirstLocation.Y > SecondLocation.Y then
                    if not UpFlag then
                        if (FirstLocationTable.Left <= SecondLocationTable.Right and FirstLocationTable.Right >= SecondLocationTable.Left)
                        or (FirstLocationTable.Right >= SecondLocationTable.Left and FirstLocationTable.Left <= SecondLocationTable.Right) then
                            UpFlag = true
                        end
                    end
                elseif FirstLocation.Y < SecondLocation.Y then
...
function WBP_Rouge_TalentTree_P_C:TrySetNavigation(TalentId, Direction)
    ...
    if FinalTalentId then
        ...
        Point:SetNavigationRuleCustom(Direction,{Point,Point.OnUINavigation})
    end
end
...
function WBP_Rouge_TalentTree_P_C:TryGetBestTalentId(TalentId, Direction, TargetTalentIds)
    ...
    local FinalTalentId = nil
    local FinalPowLength = nil
    for _,Id in pairs(TargetTalentIds) do
        ...
        if Direction == EUINavigation.Left then
            if FirstLocation.X > SecondLocation.X then
                ...
                elseif PowLength < FinalPowLength then
                    ...
                    FinalPowLength = PowLength
                elseif PowLength == FinalPowLength then
                    if self.PointLocation[FinalTalentId].Y < SecondLocation.Y then
                        FinalTalentId = Id
                        FinalPowLength = PowLength
                    end
                end
        ...
        elseif Direction == EUINavigation.Up then
            if FirstLocation.Y > SecondLocation.Y then
                local TwoTalentVec = FirstLocation - SecondLocation
                local PowLength = TwoTalentVec.X * TwoTalentVec.X + TwoTalentVec.Y * TwoTalentVec.Y
                if not FinalPowLength or PowLength < FinalPowLength then
                    FinalTalentId = Id
                    FinalPowLength = PowLength
                end
            end
