...
local WBP_Rouge_DifficultySelection_P_C = Class({"BluePrints.UI.BP_UIState_C"})
...
function WBP_Rouge_DifficultySelection_P_C:UpdateRougeLikeScore()
    ...
    if Avatar then
		...
		if RewardInfo and RewardInfo.Reward and RewardInfo.EachGradePoints then
			...
			if self.CurWeeklyMaxScore > MaxScore then
...
function WBP_Rouge_DifficultySelection_P_C:UpdateRougeLikeInfoAbstract()
    local Avatar = GWorld:GetAvatar()
    if Avatar then
        local Callback = function(Abstract)
    ...
    if Avatar then
        for i=1,#AllDiffculty do
            ...
            if SelectedIndex then
                ...
                if UnlockTable[AllDiffculty[i]] then
                    SelectedIndex = i
                end
            end
        end
...
function WBP_Rouge_DifficultySelection_P_C:RefreshRewardsList(DifficultyId)
    ...
    local IsGot = false
    if Avatar then
        if Avatar.RougeLike.PassCount and Avatar.RougeLike.PassCount[DifficultyId] and Avatar.RougeLike.PassCount[DifficultyId] >= 1 then
            IsGot = true
        end
    end
    local RougeLikeDifficulty = DataMgr.RougeLikeDifficulty
    local Rewards = {RougeLikeDifficulty[DifficultyId].FirstTimeReward}
...
function WBP_Rouge_DifficultySelection_P_C:OnBtnClicked()
    ...
    if Avatar then
		local function Callback (Ret)
			-- local UIManager = GWorld.GameInstance:GetGameUIManager()
			-- local StyleOfPlay = UIManager:GetUI("StyleOfPlay")
			-- if not StyleOfPlay then
			-- 	return
			-- end
			-- StyleOfPlay:PlayOutAnim()
			-- if StyleOfPlay.CurTabId == "RougeMain" then
			-- 	StyleOfPlay.CurSubUI:PlayAnimation(StyleOfPlay.CurSubUI.Click)
			-- end
		...
		-- local DifficultyIndex = self.Difficulty_Select:GetSortInfos()
		-- local DifficultyId = DataMgr.RougeLikeSeason[self.SeasonId].DifficultyId[DifficultyIndex]
		local RougeLikeDifficulty = DataMgr.RougeLikeDifficulty
...
function WBP_Rouge_DifficultySelection_P_C:UpdateTabDifficultySelection()
    local BottomKeyInfo = {
        {
            KeyInfoList = {{Type = "Text", Text = "Esc", ClickCallback = self.OnClickEsc, Owner = self}},
