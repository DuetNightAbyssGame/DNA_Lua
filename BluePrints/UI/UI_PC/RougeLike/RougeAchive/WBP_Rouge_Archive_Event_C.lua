...
-- function M:OnListItemEntryGenerated(EntryWidget)
--     if self._bListEntryInitialized then
--         return
--     end
--     self._bListEntryInitialized = true
--     local Entries = self.List_Item:GetDisplayedEntryWidgets()
--     -- ULowEntryExtendedStandardLibrary.SortObjectArrayDirectly(Entries,{
--     --     self, function(self, Entry1, Entry2)
--     --         return Entry1.Content.Index < Entry2.Content.Index
--     --     end
--     -- },false)
    
-- end

-- function M:OnUpdateOtherItemIndex(CallerIndex)
--     CallerIndex
-- end

-- function M:OnMouseButtonUp(MyGeometry, MouseEvent)
--     self:OnListItemReleased()
...
function M:_CalcEntryScale(Entry, ItemOffset)
    ...
    -- if math.abs(EntryCoord-mid)>1 then
    --     Scale = Scale*0.95
    -- end
...
function M:_ApplyEntryScale(ItemOffset)
    if not ItemOffset then 
        ItemOffset= self.List_Item:GetScrollOffset()
    end
    ---@param Entry UEMUserWidget
...
function M:_OnListItemScrolled(ItemOffset, DistanceRemaining)
    ...
    --self:_ApplyEntryScale(ItemOffset)
    if self.bSelecting then return end
    if not self.CenterPadding then return end
    local FullFillCount = self.List_Item:GetFullFillItemCount()
    local RealIndex =  self.CurSelectIndex + FullFillCount
    local RealOffset = ItemOffset+self.CenterPadding
    if math.abs(RealOffset - RealIndex) >=1 then
        if RealOffset > RealIndex then
...
function M:ResortEventData(EventInfo)
    ...
    for _, Data in pairs(EventInfo.Data) do
        ...
        table.sort(OrderedList, function(a, b) return a.RLArchiveSubIdSort < b.RLArchiveSubIdSort end)
    ...
    table.sort(OrderedList, function(a, b) return a.Id < b.Id end)
...
function M:InitBtns()
    ...
    self.Btn_Click.Key_GamePad:CreateCommonKey({ KeyInfoList = {{Type = "Img", ImgShortPath = "A"}} })
...
function M:OnAnalogValueChanged(MyGeometry,InAnalogInputEvent)
    if not self.bGamepadReady then return UIUtils.Unhandle end
...
-- function M:_CalcRawFullFillCount()
--     ---@type UWidget
--     local OneItem = self.List_Item:GetDisplayedEntryWidgets():GetRef(1)
--     local ItemWidth = OneItem:GetDesiredSize().X
--     local ListRealWidth = UIManager():GetWidgetRenderSize(self.List_Item).X
--     for _, Item in pairs(self.List_Item:GetDisplayedEntryWidgets()) do
--         ListRealWidth = ListRealWidth - Item:GetDesiredSize().X+ItemWidth
--     end
--     return ListRealWidth/ItemWidth
-- end

function M:SelectItem(Index, bAdjustScale)
    --Traceback("WBP_Rougelike_Archive_Event_P_C,,SelectItem Index: ",Index)
    self.bSelecting = true
    local FullFillCount = self.List_Item:GetFullFillItemCount()
    local RealIndex =  Index + FullFillCount
    if math.abs(self.CurSelectIndex-Index)>FullFillCount then
        ...
        elseif self.CurSelectIndex < Index then
            self.List_Item:SetCurrentScrollOffset(self.CurSelectIndex+self.MaxNum+FullFillCount-self.CenterPadding)
        end
    ...
    -- if bAdjustScale then
    --     self:TimerToAdjustItemScale()
    -- end
    -- self:AddTimer(0.3, function()
    --     self:_ApplyEntryScale(self.CurSelectIndex + FullFillCount-self.CenterPadding)
    -- end)
end


-- function M:TimerToAdjustItemScale()
--     local FullFillCount = self.List_Item:GetFullFillItemCount()
--     local Interval = 0
--     if(self:IsExistTimer("BeginTicker")) then
--         self:RemoveTimer("BeginTicker")
--     end
...
function M:OpenItem()
    ...
    for _, Data in ipairs(self.CurSelectItem.SubItems) do
        ...
        --todo 更新红点
        --self.DataModel:MarkArchiveItemSeened(RougeConst.ArchiveType.Event, Data.EventId)
    end
    
    --self.DataModel:UpdateArchiveReddot(RougeConst.ArchiveType.Event)
    UIManager(self):LoadUINew("Rouge_Event_Main", {FirstUnlockId, nil, self.CurSelectItem.SubItems})
end
...
function M:OpenReward()
   --UIManager(self):LoadUINew("RougeArchiveReward", RougeConst.ArchiveType.Event, self.DataModel)
...
function M:MakeRewardData()
    ...
    for Index,ArchiveInfo in ipairs(SortedArchiveInfo) do
        ...
        RewardData.ReceiveAllParam={DataModel=self.DataModel}
        ...
        while (CurIndex*Step<=RewardData.NumMax) do
            ...
            Item.ReceiveParm={DataModel=self.DataModel}
            local Rewards={}
