...
---@type WBP_RougeLike_Archive_StoryLine_Item_C
...
function M:OnCellClicked()
    ...
    --    if self.Parent.CurSelectIndex == self.Index and self.IsUnlocked then
    --        self.Parent:OpenItem()
    --        return
    --    end
    --end
...
function M:OnCellHovered()
    ...
    --self.List_SubItem:SetNavigationRuleExplicit(EUINavigation.Down, self.Parent.List_Item:GetItemAt(self.Index+1).UI.Btn_Click)

    if self.IsSelected and self.IsUnlocked then
        self.Btn_Click:SetNavigationRuleExplicit(EUINavigation.Down, self.List_SubItem) -- 如果已经解锁并选中（代表List已经展开），向下导航到SubList
    else
        if self.Parent.List_Item:GetItemAt(self.Index + 1) then -- 否则向下导航到List的下一个Item
            self.Btn_Click:SetNavigationRuleExplicit(EUINavigation.Down, self.Parent.List_Item:GetItemAt(self.Index + 1).UI.Btn_Click)
        end
    end
    if self.Parent.List_Item:GetItemAt(self.Index - 1) then 
        ...
        local FocusItem
        if UpItem.UI.IsSelected and UpItem.UI.IsUnlocked then -- 去找上一个Item的List_SubItem的最后一个
