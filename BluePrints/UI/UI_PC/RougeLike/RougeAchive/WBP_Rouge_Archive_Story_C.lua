...
---@type WBP_RougeLike_Archive_StoryLine_P_C
local M = Class({"BluePrints.UI.BP_UIState_C", "BluePrints.UI.BP_EMUserWidget_C"})
...
function M:ResortStoryData(StoryInfo)
    ...
    --    table.insert(self.StoriesList, Data)
    --end
...
function M:InitStoryItems()
    ...
    --     self:ChooseSeries(self.CurSelectIndex)
    -- end
...
function M:SetAllNavigationRule()
    ...
    for Index = 0, ItemNum - 1 do
        ...
        --Item.Btn_Click:SetNavigationRuleExplicit(EUINavigation.Down)
    end
end
...
function M:HoverSeries(Item) -- 专门给手柄做的
    ...
    if Item.IsUnlocked then
        self.Root:UpdateOtherPageTab(
                {
                    {
                        GamePadInfoList = {{Type="Img", ImgShortPath="A", ClickCallback=self.OnReturnKeyDown, Owner=self}},
...
function M:ChangeBackGround(Item, IsUnlocked)
    if IsUnlocked then
        ...
        if self.Group_Empty:GetVisibility() == ESlateVisibility.Collapsed then
...
function M:OpenItem()
    ...
    if self.CurSelectSubItem.IsUnlocked then
        ...
        UIManager(self):LoadUINew("Rouge_Event_Main", {nil, StoryId, true})
...
function M:OpenReward()
    --UIManager(self):LoadUINew("RougeArchiveReward",RougeConst.ArchiveType.Story, self.DataModel)
