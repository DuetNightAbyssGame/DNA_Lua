...
function WBP_Rouge_EnterToast_C:InitEnterToast(RoomIndex, RoomId, SeasonId)
    ...
    self:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)   -- 切场景的ui可能会把此ui Hide掉，但是没Show，导致之后ui一直没显示。强制显示一下
    ...
    self:AddTimer(self.ToastExistTime, function() self:Close() end)
end
