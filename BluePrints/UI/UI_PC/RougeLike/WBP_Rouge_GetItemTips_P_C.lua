...
function WBP_Rouge_GetItemTips_P_C:OnLoaded(InfoDatas,AwardList)
	...
	for _, InfoData in pairs(InfoDataList) do
		if InfoData.BlessingId then
			...
			MenuContent.AwardInfo={AwardType="Blessing",AwardId=InfoData.BlessingId,NotHit=true,not Islose}
			...
			MenuContent.AwardInfo={AwardType="Treasure",AwardId=InfoData.TreasureId,NotHit=true,not Islose}
			MenuContent.Parent=self
			MenuContent.NotHit=true
	...
	-- 				Item.SelfWidget:SetNavigationRuleCustom(UE4.EUINavigation.Left, {self, self.OnNavigateLeft})
	-- 				Item.SelfWidget:SetNavigationRuleCustom(UE4.EUINavigation.Right, {self, self.OnNavigateRight})
	-- 			end
	-- 		end
   	-- 	end
	-- ,false,0,nil,true)
	if not Islose then
		...
		UIUtils.PlayListViewFramingInAnimation(self, self.UsedList, {
			AnimName="In_get"})
...
function WBP_Rouge_GetItemTips_P_C:UpdateSelectedWidget(SelectedIndex)
    ...
    if NewFocusItem and NewFocusItem.SelfWidget then
		-- self.GameInputModeSubsystem:SetTargetUIFocusWidget(NewFocusItem.SelfWidget)
...
function  WBP_Rouge_GetItemTips_P_C:OnKeyDown(MyGeometry, InKeyEvent)
	...
	elseif InKeyName == Const.GamepadLeftThumbstick then
		if self.CurrentSelectItem and self.CurrentSelectItem.SelfWidget and self.CurrentSelectItem.SelfWidget.Rouge_SuitSign 
		and self.CurrentSelectItem.SelfWidget.Rouge_SuitSign:GetVisibility() ~= UIConst.VisibilityOp.Collapsed then
			self.CurrentSelectItem.SelfWidget.Rouge_SuitSign.Com_BtnQa:OnViewInfoClick()
			self:ShowBottomGamePadKey(false)
		elseif self.CurrentSelectItem and self.CurrentSelectItem.Rouge_SuitSign
		and self.CurrentSelectItem.Rouge_SuitSign:GetVisibility() ~= UIConst.VisibilityOp.Collapsed then
...
function WBP_Rouge_GetItemTips_P_C:ShowBottomGamePadKey(bShow)
	if self.ItemNum ~= 1 then return end
	if bShow then
		self:UpdateKeyGamePad()
	else
...
function WBP_Rouge_GetItemTips_P_C:UpdateKeyGamePad()
	if UIUtils.UtilsGetCurrentInputType()==ECommonInputType.Gamepad and self.CurrentSelectItem and self.CurrentSelectItem.ExplanationId ~= nil and #self.CurrentSelectItem.ExplanationId > 0 then
		...
		if self.Key_GamePad03:GetVisibility() ~= ESlateVisibility.Collapsed then
			self.Key_GamePad03:SetVisibility(ESlateVisibility.Collapsed)
		end
	end
	if UIUtils.UtilsGetCurrentInputType()==ECommonInputType.Gamepad and self.CurrentSelectItem and self.ItemNum == 1 and self.CurrentSelectItem.bShowSuitSign then
		self.Key_GamePad01:SetVisibility(ESlateVisibility.SelfHitTestInvisible)
	else
		if self.Key_GamePad01:GetVisibility() ~= ESlateVisibility.Collapsed then
