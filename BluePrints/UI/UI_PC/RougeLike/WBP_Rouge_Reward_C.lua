...
local WBP_Rouge_Reward_C = Class({"BluePrints.UI.BP_UIState_C"})
...
function WBP_Rouge_Reward_C:SetInfo()
    ...
    self:AddTimer(0.001,function()
        ...
        -- local RestCount = UIUtils.GetListViewContentMaxCount(self.List_Reward, ListItemUIs) - ListItemUIs:Length()
        -- if RestCount <= 0 then
        --     return
        -- end
        -- local ClassPath = '/Game/UI/UI_PC/Common/Common_Item_subsize_PC_Content.Common_Item_subsize_PC_Content_C'
...
function WBP_Rouge_Reward_C:Refresh()
    ...
    if self.RemainRefreshTime and self.RemainRefreshTime < RemainRefreshTime then
        self:SetInfo()
    end
    ...
    if RemainRefreshTime > 24 * 60 * 60 then
        ...
        RemainTimeStr = RemainTimeStr .. string.format(GText("UI_Rougelike_Reward_Day"), math.floor(RemainRefreshTime / (24 * 60 * 60)))
        RemainRefreshTime = RemainRefreshTime % (24 * 60 * 60)
    end
    if RemainRefreshTime > 60 * 60 or TimeCount == 1 then
        TimeCount = TimeCount + 1
        RemainTimeStr = RemainTimeStr .. string.format(GText("UI_Rougelike_Reward_Hour"), math.floor(RemainRefreshTime / (60 * 60)))
...
function WBP_Rouge_Reward_C:InitKeyboardCommonTab()
    ...
    local BottomKeyInfo = {
        {
            KeyInfoList = {{Type = "Text", Text = "Esc", ClickCallback = self.PlayOutAnim, Owner = self}},
            Desc = GText("UI_BACK"),
        }
    }
    if self:CheckHaveAnyRewardCanGet() then
        table.insert(BottomKeyInfo, 1, {
            KeyInfoList = { { Type = "Text", Text = "Space", ClickCallback = self.GetRougeLikeReward, Owner = self } },
    ...
    if bIsOpen then
        ...
        if (self.GameInputModeSubsystem:GetCurrentInputType() == ECommonInputType.Gamepad) then
            if self.IsInSelectState then
                ...
                if self.CurFocusedItem and self.CurFocusedItem:CheckIsCompleteState() then
                    self:InitCommonTab(true)
                else
                    self:InitCommonTab(false)
                end
