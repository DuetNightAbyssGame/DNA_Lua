...
function WBP_Rouge_ItemUpgrade_P_C:OnLoaded(Params)
    ...
    self.Item_Before:OnLoaded({AwardType="Blessing",AwardId=Params.ItemId,AwardLevel=Params.OldLevel-1},self,true)
    self.Item_Now:OnLoaded({AwardType="Blessing",AwardId=Params.ItemId,AwardLevel=Params.NewLevel-1},self,true)
    self.Text_Tip:SetText(GText("UI_RougeLike_End__ClickEmpty"))
    self.Item_Before:SetNavigationRuleCustom(EUINavigation.Right,{self,self.SetRightNavigation})
    self.Item_Now:SetNavigationRuleCustom(EUINavigation.Left,{self,self.SetLeftNavigation})
...
function WBP_Rouge_ItemUpgrade_P_C:OnItemSelected()
    if UIUtils.UtilsGetCurrentInputType()==ECommonInputType.Gamepad then
        ...
        -- self.CurrentSelectItem.ScrollBox_Desc:SetNavigatePosAngle(0)
        -- self.CurrentSelectItem.ScrollBox_Desc:SetNavigatePosOffsetPercent(UE4.FVector2D(0.5, -0.5))
        -- self.CurrentSelectItem.ScrollBox_Desc:SetNavigatePosOffsetAlignment(UE4.FVector2D(0.5, 0.5))
