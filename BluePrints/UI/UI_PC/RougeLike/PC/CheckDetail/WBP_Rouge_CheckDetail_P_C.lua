...
---@type WBP_Rouge_CheckDetail_P_C
...
function M:OnLoaded(...)
    ...
    -- self.RougeGuideName nil 为背包 非空为图鉴
...
function M:RefreshOpInfoByInputDevice(CurInputDevice, CurGamepadName)
    ...
    -- self.CurInputDeviceTyp 为了判定是不是刚打开界面
...
function M:OnMainTabChanged(TabWidget)
    ...
    for i = 1, #SubTabList do
        ...
        CurrentTab:SetNavigationRuleCustom(UE4.EUINavigation.Right, {self, function()
...
function M:UpdateRougeBagDetail(CurGroupId)
    ...
    for Id, Level in pairs(ItemList) do
        ...
        local ItemData = setmetatable({}, {__index = Data[Id]})
        if not CurGroupId or CurGroupId == 0 or CurGroupId == GroupId then
            table.insert(ItemDataList, {ItemId = ItemData[IdName], ItemType = self.CurItemType, Rarity = ItemData[RarityName]})
        end
    end
...
function M:InitTreasureSuitList(ItemDataList)
    ...
    for i = 1, #ItemDataList do
        ...
        -- 确保分组存在并添加元素
        SuitList[GroupKey] = SuitList[GroupKey] or {}  -- 如果不存在则创建空数组
...
function M:InitBlessingList(ItemDataList)
    ...
    -- self:AddTimer(0.01, function()
        -- local ListItemUIs = self.List_Item:GetDisplayedEntryWidgets()
        -- local EmptyCount = UIUtils.GetListViewContentMaxCount(self.List_Item, ListItemUIs) - ListItemUIs:Length()
...
--     UIUtils.StopListViewFramingInAnimation(self.List_Item, {UIState=self, TimerKeys=self._ListAttrAnimTimerKeys})
-- end
...
function M:InitGuideTabInfo()
    ...
    self.Common_Tab:Init(
    {
        LeftKey = "NotShow", RightKey = "NotShow", DynamicNode = { "Back", "BottomKey" },
        BottomKeyInfo ={
            {KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=self.CloseSelf, Owner=self}},
            GamePadInfoList = {{Type="Img", ImgShortPath="B", ClickCallback=self.CloseSelf, Owner=self}}, Desc=GText("UI_BACK")} },
        OwnerPanel = self,
...
function M:OpenReward()
    ...
    --UIManager(self):LoadUINew("RougeArchiveReward",Type, self.DataModel)
end
...
function M:UpdateRougeGuideDetail(CurGroupId)
    ...
    for Id, Value in pairs(Data) do
        ...
        local ItemData = setmetatable({}, {__index = Data[Id]})
        if (not CurGroupId or CurGroupId == 0 or CurGroupId == GroupId) and RLArchiveId ~= nil then
            table.insert(ItemDataList, {ItemId = ItemData[IdName], ItemType = self.RougeGuideName, RLArchiveId = RLArchiveId})
        end
    end
...
function M:RougeItemFocusChanged(bFocusOnItem)
	...
	-- ScreenPrint("bFocusOnItem: "..(bFocusOnItem and "true" or "false"))
end
