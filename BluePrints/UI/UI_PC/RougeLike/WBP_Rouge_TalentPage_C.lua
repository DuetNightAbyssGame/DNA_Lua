...
function WBP_Rouge_TalentPage_C:SetInfo()
    ...
    for i=1,4 do
        if BranchInfoTable[i] then
            self["Entrance_0"..i]:Init(self, self.ReddotCondition[BranchInfoTable[i].Branch],BranchInfoTable[i].Branch,BranchInfoTable[i].BranchTalentNum,BranchInfoTable[i].ActivedBranchTalentNum,i==1)
        end
...
function WBP_Rouge_TalentPage_C:CheckReachable(TalentId)
    ...
    --     return false
    -- end
    -- if not self.Lines[TalentId] then
    --     self.ReachableTable[TalentId] = 1
    --     return true
    -- end
    -- for FrontTalentId, _ in pairs(self.Lines[TalentId]) do
    --     if self:CheckReachable(FrontTalentId) then
    --         self.ReachableTable[TalentId] = 1
    --         return true
    --     end
    -- end
