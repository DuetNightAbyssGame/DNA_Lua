...
function WBP_Rouge_Main_C:Init()
	self.SelectModeBtn = {self.Entrance_Store, self.Entrance_TalentTree, self.Entrance_Illustration}
...
function WBP_Rouge_Main_C:EnterSelectMode()
    ...
    if (IsValid(self.GameInputModeSubsystem)) then
        for index, BtnName in ipairs(self.SelectModeBtn) do
            if (BtnName ~= nil and BtnName:IsVisible()) then
...
function WBP_Rouge_Main_C:InitSelectTab()
	self.TabConfigData = {
        ...
        StyleName="Text", OwnerPanel=self, BackCallback=self.OnReturnKeyDown,
		BottomKeyInfo = {
			{
...
function WBP_Rouge_Main_C:SetInfo()
    ...
    for TalentId, RougeTalentInfo in pairs(self.RougeTalent) do
        if not self.ReddotCondition then
...
function WBP_Rouge_Main_C:InitTable(JumpType)
    ...
    self.TabConfigData = {
        ...
        DynamicNode={"Back", "ResourceBar", "BottomKey"}, 
        StyleName="Text", OwnerPanel=self, BackCallback=self.OnReturnKeyDown,
        BottomKeyInfo = { { KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=self.OnReturnKeyDown, Owner=self,}},
...
function WBP_Rouge_Main_C:StopAllAnimationsExceptInOut()
    if not (self:IsAnimationPlaying(self.In) or self:IsAnimationPlaying(self.Out) or self:IsAnimationPlaying(self.Next) or self:IsAnimationPlaying(self.Back)) then
...
function WBP_Rouge_Main_C:SwitchIn()
	--self.Difficulty_Select:SwitchClose()
end
...
function WBP_Rouge_Main_C:ShowSeasonFirstStory()
	...
	UIManager(self):LoadUINew("Rouge_Event_Main", {nil, SeasonStoryId, nil, true, true})
...
function WBP_Rouge_Main_C:InitTableDifficultySelection()
    ...
    self.TabConfigData = {
        ...
        DynamicNode={"Back", "ResourceBar", "BottomKey"}, 
        StyleName="Text", OwnerPanel=self.DifficultySelection, BackCallback=self.DifficultySelection.OnClickEsc,
        BottomKeyInfo = { { KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=self.DifficultySelection.OnClickEsc, Owner=self.DifficultySelection,}},
							GamePadInfoList = {{Type="Img", ImgShortPath="B"}},
							Desc = GText("UI_BACK"), bLongPress = false}},
    }
	local ResoucesTab = {}
