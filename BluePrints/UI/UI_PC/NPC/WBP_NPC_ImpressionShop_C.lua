...
function WBP_NPC_ImpressionShop_C:OnEnabled(Npc)
    --DebugPrint("@@@ OnEnabled ImpressionShop",self.bIsEnabled_Impression)
...
function WBP_NPC_ImpressionShop_C:TryChangeIconPath()
    --DebugPrint("WBP_NPC_ImpressionShop_C:TryChangeIconPath", self.IconPath)
    if not self.IconPath then
        return
    end

    if self.CurrentIconPath ~= self.IconPath then
        UE.UResourceLibrary.LoadObjectAsync(self, self.IconPath, {self, WBP_NPC_ImpressionShop_C.OnHeadIconLoaded})
...
function WBP_NPC_ImpressionShop_C:OnDisabled()
    --DebugPrint("@@@ On Disabled ImpressionShop",self.bIsEnabled_Impression)
...
function WBP_NPC_ImpressionShop_C:OnOutAnimationFinished()
    --DebugPrint("@@@ On ImpressionShop Animation Finished")
    self:SetVisibility(UE4.ESlateVisibility.Collapsed)
end
