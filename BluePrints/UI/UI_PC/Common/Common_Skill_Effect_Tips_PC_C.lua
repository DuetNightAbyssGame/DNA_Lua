...
---@type Common_Skill_Effect_Tips_PC_C
local M = Class("BluePrints.UI.BP_UIState_C")

function M:Construct()
    self.TabUI = {self.First,self.Second}
    ...
    if(WeaponId)then
        ...
        if SkillData and SkillData[1] and SkillData[1][0] then 
            ...
            if SkillLevelUpInfo then
                if Level >= #SkillLevelUpInfo + 1 then
            ...
            if(Key)then
                self.Key:CreateCommonKey({KeyInfoList = {
                                            {
                                                Type = "Text",
                                                Text = Key
                                            },
