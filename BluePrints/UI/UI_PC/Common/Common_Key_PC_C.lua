...
---@class Common_Key_PC
---@field Button_Key UButton|nil
---@field Img UImage|nil
---@field Text_Key UTextBlock|nil
---@field Text_Desc UTextBlock|nil
---@field Key_1 Common_Key_PC|nil
---@field Key_2 Common_Key_PC|nil
---@field Key Common_Key_PC|nil
---@field Text_Add Common_Key_PC|nil
---@field bLongPress boolean|nil
---@field Desc string|nil
---@field bEnable boolean
---@field bHasButtonPressed boolean
---@field PressTimer number
---@field bHasTriggerLongPress boolean
---@field bHasTriggerLongRelease boolean
---@field bIsHovered boolean
---@field SubWidgetList table<number, UWidget>
---@field SoundFunc function
---@field KeyInfoList table<number, table>
---@field bButton boolean
---@field IsButton boolean
---@field Spacer_L USpacer|nil
---@field Spacer_R USpacer|nil
---
---！！！！！！新接入的动效要注意，这个类被很多蓝图使用了，有些蓝图不一定有同名动效，播放动效前需要对动效判空
---
...
-- 简单创建一个手柄按键提示图标
-- 输入手柄按键的ImgShortPath, 参见UIConst.GamePadImgKey
function Common_Key_PC:CreateGamepadKey(GamepadImgKey)
    self:CreateCommonKey({KeyInfoList = {{Type = "Img", ImgShortPath = GamepadImgKey}}})
end

--- The format of ImgLongPath is like "Texture2D'/Game/UI/UI_PNG/Common/Key/Icon_Mouse_Button.Icon_Mouse_Button'"
--- The format of ImgShortPath is like "RightMouseButton"
function Common_Key_PC:CreateCommonKey(CreateInfo) -- TODO deprecate this constructor
    ...
    -- rawset(self,"_SetImageRequestID",false)
    ...
    if self.KeyInfoList and CreateInfo.KeyInfoList and #self.KeyInfoList == #CreateInfo.KeyInfoList and #self.KeyInfoList ~= 0 then
        for i = 1, #self.KeyInfoList do
            if (self.KeyInfoList[i].Type ~= CreateInfo.KeyInfoList[i].Type) or 
            (self.KeyInfoList[i].ImgShortPath ~= CreateInfo.KeyInfoList[i].ImgShortPath) or 
            (self.KeyInfoList[i].ImgLongPath ~= CreateInfo.KeyInfoList[i].ImgLongPath) or
            (self.KeyInfoList[i].Text ~= CreateInfo.KeyInfoList[i].Text) or
            (self.KeyInfoList[i].ClickCallback ~= CreateInfo.KeyInfoList[i].ClickCallback) or
            (self.KeyInfoList[i].Owner ~= CreateInfo.KeyInfoList[i].Owner) then
                break
            end
    ...
    if (CreateInfo.bButton ~= nil) then
...
function Common_Key_PC:_InitInternal(CreateInfo)
    ...
    if (self:_IsCompound()) then
        if CreateInfo.bIsSubKeyDesc then
            -- 如果是通过 CreateSubKeyDesc 创建的组合键，调用相应的方法
        ...
        if (self.Desc) then self:SetDescription(self.Desc) end
    else
    ...
    if (self:_CanHandleButtonEvent(CreateInfo)) and not self:IsDisableMouseKeyEvent(CreateInfo.KeyInfoList[1]) then
        ...
        if (self.bButton) then self.Button_Key:SetVisibility(UIConst.VisibilityOp.Visible) end
    else
        if (self.bButton) then self.Button_Key:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible) end
    end
...
function Common_Key_PC:_ResetWhenNotNeedUpdate()
    if not self.SubWidgetList then return end
...
function Common_Key_PC:_CreateSubKey(KeyInfo, KeyName)
    ...
    --table.insert(self.SubWidgetList, Widget)
...
function Common_Key_PC:CreateSubKeyDesc(KeyInfo)
    ...
    self.KeyInfoList = KeyInfo.KeyInfoList
    if (KeyInfo.Desc) then self:SetDescription(KeyInfo.Desc) end
    ---@type Common_Key_PC
    ...
    -- 将KeyWidget组件加载到当前组件的Key上
    -- self.Button_Key:SetVisibility(UE4.ESlateVisibility.Visible)
...
function Common_Key_PC:_InitSingleKeyContent(KeyInfo)
    if (KeyInfo.Type == "Text") then
        if self.bIsSubKeyDesc then return end  --防止手柄端切换时被重新刷新为pc
        -- local text = GText(KeyInfo.Text)
        -- if self.Text_Key then
        --     self.Text_Key:SetText(text)
        --     -- 根据文本长度调整间隔
        --     self:_AdjustSpacers(text)
        -- end
...
function Common_Key_PC:_CheckAndRegisterCallback(KeyInfo)
    if (KeyInfo.ClickCallback and type(KeyInfo.ClickCallback) == "function") then self:AddExecuteLogic(KeyInfo.Owner, KeyInfo.ClickCallback) end
    if (KeyInfo.PressCallback and type(KeyInfo.PressCallback) == "function") then self:AddPressLogic(KeyInfo.Owner, KeyInfo.PressCallback) end
    if (KeyInfo.ReleaseCallback and type(KeyInfo.ReleaseCallback) == "function") then self:AddReleaseLogic(KeyInfo.Owner, KeyInfo.ReleaseCallback) end
end
...
function Common_Key_PC:OnListItemObjectSet(ListItemObject)
    if ListItemObject.Owner ~= nil and ListItemObject.Owner.InitCommonKey ~= nil then
...
function Common_Key_PC:_SetBattleShortCutKeyHidden()
    ...
    local CurrentHidden
    if Avatar then CurrentHidden = EMCache:Get("BattleShortcutHudKeysHidden", true) end
    if (CurrentHidden) then self:SetVisibility(ESlateVisibility.Hidden) end
end

-- function Common_Key_PC:GetBlueprintClass(Type, bButton)
--     if (Type == "Text") then
--         if (bButton == true) then
--             return UE4.UClass.Load(
--                 "WidgetBlueprint'/Game/UI/UI_PC/Common/Common_Key/Common_Key_Btn_Text_PC.Common_Key_Btn_Text_PC_C'")
--         else
--             return UE4.UClass.Load(
--                 "WidgetBlueprint'/Game/UI/UI_PC/Common/Common_Key/Common_Key_Show_Text_PC.Common_Key_Show_Text_PC_C'")
--         end

--     else
--         if (bButton == true) then
--             return UE4.UClass.Load(
--                 "WidgetBlueprint'/Game/UI/UI_PC/Common/Common_Key/Common_Key_Btn_Img_PC.Common_Key_Btn_Img_PC_C'")
--         else
--             return UE4.UClass.Load(
--                 "WidgetBlueprint'/Game/UI/UI_PC/Common/Common_Key/Common_Key_Show_Img_PC.Common_Key_Show_Img_PC_C'")
--         end
--     end
-- end
...
function Common_Key_PC:_SetImage(Type, ImgShortPath, ImgLongPath, bLargeSize)
    ...
    if (ImgShortPath) then
        ...
        if Type == "Img" then
            ...
            --重定向一下Space和Esc
            if ImgShortPath == "Space" then
                ImgShortPath = "SpaceBar"
            elseif ImgShortPath == "Esc" then
                ImgShortPath = "Escape"
            end
        end
        ...
        Img = UE4.UResourceLibrary.LoadObjectAsync(self, ImgPath, {self,function(self,Object)
        ...
        Img = UE4.UResourceLibrary.LoadObjectAsync(self, ImgLongPath, {self, function(self, Object)
            self:_AsyncLoadImgCallBack(Object, currentRequestID)
        end})
    end
    self.ImgPath = ImgPath
...
function Common_Key_PC:_SetImageByInstruction(ImgShortPath, ImgLongPath)
    ...
    if self.Img then
        -- local Material = self.Img:GetDynamicMaterial()
        -- if Material then
        --     Material:SetTextureParameterValue("MainTex", Img)
        -- end
...
function Common_Key_PC:_GetCurrentGamepadName()
    ...
    if (IsValid(self.GameInputModeSubsystem)) then return self.GameInputModeSubsystem:GetCurrentGamepadName() end
    return "Generic"
end
...
function Common_Key_PC:GetCurrentInputType()
    ...
    if (IsValid(self.GameInputModeSubsystem)) then return self.GameInputModeSubsystem:GetCurrentInputType() end
...
function Common_Key_PC:_ExecuteLogic()
    if (type(self.ExecuteLogicObjectFunc) == "function") then self.ExecuteLogicObjectFunc(self.ExecuteLogicObject) end
end

function Common_Key_PC:_PressLogic()
    if (type(self.PressLogicObjectFunc) == "function") then self.PressLogicObjectFunc(self.PressLogicObject) end
end

function Common_Key_PC:_ReleaseLogic()
    if (type(self.ReleaseLogicObjectFunc) == "function") then self.ReleaseLogicObjectFunc(self.ReleaseLogicObject) end
end
...
function Common_Key_PC:OnButtonPressed(bChild,bSetTimeRange,StartAtTime,EndAtTime)
    ...
    if (not self.bEnable) then return end
    DebugPrint("OnButtonPressed", self.bEnable, self.bLongPress,self.bSpecialLongPress)
    if bSetTimeRange then self.LongPressDuration = EndAtTime - StartAtTime + Const.ShortPressThreshold end

    if bSetTimeRange then
        self:_ForeachChildren(function(v) v:OnButtonPressed(true,bSetTimeRange,StartAtTime,EndAtTime) end)
    else
        self:_ForeachChildren(function(v) v:OnButtonPressed(true) end)
    end

    if (not bChild and self.SoundFunc and not self.bLongPress) then self.SoundFunc() end
    ...
    if (not self.bLongPress) then
        ...
        self.UpdateTimer = URuntimeCommonFunctionLibrary.K2_SetTimerDelegate({self, function() self:_Update(bSetTimeRange,StartAtTime,EndAtTime) end}, self.LongPressTimerInterval, true, 0)
    end
end
...
function Common_Key_PC:OnButtonReleased()
    ...
    if (not self.bEnable) then return end
    self:_ForeachChildren(function(v) v:OnButtonReleased() end)
...
function Common_Key_PC:OnButtonReleasedInsideButtonFrame()
    if (not self.bEnable) then return end
    self:_ForeachChildren(function(v) v:OnButtonReleasedInsideButtonFrame() end)
    if (not self.bLongPress) then
        self:_ExecuteLogic()
    else
    end
...
-- function Common_Key_PC:OnButtonClicked()
--     if (not self.bEnable) then
--         return
--     end
--     if (#self.SubWidgetList > 0) then
--         for i, v in ipairs(self.SubWidgetList) do
--             v:OnButtonClicked()
--         end
--     end
--     self:PlayCommonKeyAnimation(self.Click)
-- end
...
function Common_Key_PC:_OnButtonRealShortPressed()
    ...
    if (not self.bIsHovered) and self.Hover then self:PlayAnimation(self.Hover) end
    self:_PressLogic()
end

function Common_Key_PC:_OnButtonRealShortReleased()
    -- if self.Press then self:PlayAnimationReverse(self.Press) end
    if self.Normal then self:PlayAnimation(self.Normal) end -- 短按release改为播放Normal动画
    if (not self.bIsHovered) and self.UnHover then self:PlayAnimation(self.UnHover) end
    self:_ReleaseLogic()
end
...
function Common_Key_PC:_OnButtonRealLongPressed(bSetTimeRange,StartAtTime,EndAtTime)
    ...
    if self.SoundFunc then self.SoundFunc() end
    if (self.bIsHovered) and self.UnHover then self:PlayAnimation(self.UnHover) end
end

-- function Common_Key_PC:OnAnimationStarted(Anim)
--     -- DebugPrint(TXTTag,"OnAnimationStarted",Anim)
--     if Anim == self.Press  then
--         DebugPrint(TXTTag,"PressAnim",self)
--     end
--     if Anim == self.LongPress  then
--         DebugPrint(TXTTag,"LongPressAnim",self)
--     end
--     if Anim == self.Normal  then
--         DebugPrint(TXTTag,"NormalAnim",self)
--     end
-- end

function Common_Key_PC:_OnButtonRealLongReleased()
    if (not self.bHasTriggerLongPress) then return end
    if self.LongPress then
        self:_ResetState(self.LongPress)
        self:PlayAnimation(self.Normal)
        AudioManager(self):StopSound(self, "LongPress")
    elseif self.bSpecialLongPress then
        if self.Press then 
            -- self:_ResetState(self.Press)
            self:PlayAnimation(self.Normal)
        end
    end
    if (self.bIsHovered) and self.Hover then self:PlayAnimation(self.Hover) end
end

function Common_Key_PC:OnButtonHovered()
    if self.LongPress and self:IsAnimationPlaying(self.LongPress) then return end
    self.bIsHovered = true
    if (not self.bEnable) then return end
    self:_ForeachChildren(function(v) v:OnButtonHovered() end)
    if (self.bLongPress and self.bHasTriggerLongPress) then return end
    if (not self.bLongPress and self.bHasButtonPressed) then return end
...
function Common_Key_PC:OnButtonUnhovered()
    ...
    if (not self.bEnable) then return end
    self:_ForeachChildren(function(v) v:OnButtonUnhovered() end)
    if (self.bLongPress and self.bHasTriggerLongPress) then return end
    if (not self.bLongPress and self.bHasButtonPressed) then return end
    if self.Hover then self:PlayAnimationReverse(self.Hover) end
end

function Common_Key_PC:DisableKey()
    self:_ForeachChildren(function(v) v:DisableKey() end)
    -- self.bDisableResetWhenChangeDevice = true
    if (self.bHasButtonPressed) then self:OnButtonReleased() end
    self.bisForbid = true
    self.bEnable = false
    self.PressTimer = 0
    self.bHasButtonPressed = false
    if self.Hover then self:_ResetState(self.Hover) end
    self:StopAllAnimations()
    if self.Forbidden then self:PlayAnimation(self.Forbidden) end
end

function Common_Key_PC:EnableKey()
    self:_ForeachChildren(function(v) v:EnableKey() end)
    self.bisForbid = nil
    self.bEnable = true
    self.PressTimer = 0
    self.bHasButtonPressed = false
    self:StopAllAnimations()
    if self.Normal then self:PlayAnimation(self.Normal) end
end

function Common_Key_PC:ResetHasButtonPressed()
    self:_ForeachChildren(function(v) v.bHasButtonPressed = false end)
    self.bHasButtonPressed = false
end

function Common_Key_PC:PlayLoopAnim()
    self:_ForeachChildren(function(v) v:PlayLoopAnim() end)
    if (self.Loop) then self:PlayAnimation(self.Loop) end
end

function Common_Key_PC:StopLoopAnim()
    self:_ForeachChildren(function(v) v:StopLoopAnim() end)
    if (self.Loop) then
...
--- Use "play and stop anim" to reset to first frame state.
...
-- function Common_Key_PC:OnClick()
--     if self.OnClickCb then
--         self.OnClickCb(self.OnClickObj)
--     end
-- end

-- function Common_Key_PC:BindCallbackOnClick(obj, cb)
--     self.OnClickObj = obj
--     self.OnClickCb = cb
-- end
