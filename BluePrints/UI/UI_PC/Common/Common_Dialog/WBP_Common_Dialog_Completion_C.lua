...
---@type Common_Dialog_Completion_C
...
function M:HasFocusDetail()
    if self.HighLevel:HasFocusedDescendants() or self.HighLevel:HasAnyUserFocus() 
        or self.LowLevel:HasFocusedDescendants() or self.LowLevel:HasAnyUserFocus() then
        return true
    end
    return false
end
