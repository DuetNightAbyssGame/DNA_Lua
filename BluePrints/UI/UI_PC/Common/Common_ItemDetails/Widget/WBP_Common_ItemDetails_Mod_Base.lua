...
---所有Modtips子蓝图的基类
---!!!!!! ModServerData不一定始终有效！！！！！
---@class Common_ItemDetails_Mod_Base
...
function M:InitItemInfo(ItemType, ItemId, UnitId)
    ...
    local ModLevel, ModCost
    ---@type Mod
    local ModServerData
    local ModCardLvel = 0
    local Count = 0
    local PlayerAvatar = ModController:GetAvatar() ---注意,有可能是DummyAvatar
    ...
    if self.ParentWidget.Img_Aura then
        if ModInfo.ApplySlot and #ModInfo.ApplySlot == 1 and ModInfo.ApplySlot[1] == 9 then
...
-- 背包内初始化Mod信息（为了支持铸造图纸进背包，传统接口里有很多操作ParentWidget的逻辑，背包里没法用）
