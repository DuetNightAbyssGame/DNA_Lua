...
---@class Common_ItemDetails_Armory_Dialog_PC:Common_ItemDetails_Mod_Base,Common_ItemDetails_Armory_Dialog_PC_C
local M = Class({"BluePrints.UI.UI_PC.Common.Common_ItemDetails.Widget.WBP_Common_ItemDetails_Mod_Base"})
M._components = {
...
function M:OnInitItemInfo(ModInfo, ModServerData)
    ...
    if not self.ParentWidget.Content.IsArmoryMod then return end
    --- 如果不存在ModServerData，该mod非军械库mod
    ...
    --移动端要重置按钮动画..
    if Platfrom == "Mobile" then
        Btn:SwitchNormalAnimation()
        BtnUnEquip:SwitchNormalAnimation()
        -- self.ParentWidget.Btn01_Mod.WS_Icon_L:SetVisibility(UIConst.VisibilityOp.Collapsed)
        -- self.ParentWidget.Btn02_Mod.WS_Icon_R:SetVisibility(UIConst.VisibilityOp.Collapsed)
...
function M:UpdateAttrList()
    if self.ParentWidget.Content.bIsHoverState then return end
    ...
    self.ComparedTarget, self.ComparedAttrs, self.Attrs = 
        ModModel:GenerateAttrList(PreAttrs, NowAttrs, self.Index2AttrKey, true, PureTargetAttrs,nil, ExtraVolume)
...
function M:Destruct()
    ...
    if not self.EnableBtn then return end
...
function M:OnBtnUnEquipMod()
    ...
    --local Content = self.ParentWidget.Content
