...
---@class WBP_Common_ItemDetails_Forge_C: Common_ItemDetails_Forging_C
...
function WBP_Common_ItemDetails_Forge_C:SetDraftNotEnough()
    ...
    if self.Switch_Type:GetActiveWidgetIndex() == 1 then 
        return 
    end
...
function WBP_Common_ItemDetails_Forge_C:InitNotStartedView(DraftInfo)
    ...
    if self.CurrentDraftInfo.CanProduce then 
        ...
        -- 更新材料不足提示文本
        if not self.CurrentDraftInfo.IsFoundryEnough and self.CurrentDraftInfo.IsResourceEnough then 
            -- 金币不足
...
function WBP_Common_ItemDetails_Forge_C:UpdateAccerateTickets()
    ...
    if FoundryData then 
        ...
        -- self.Text_TicketsCost:SetText(Cost)

        local Avatar = GWorld:GetAvatar()
        local Count = Avatar:GetResourceNum(AccelerateCostType)
        -- self.Text_Hold:SetText(Count)
