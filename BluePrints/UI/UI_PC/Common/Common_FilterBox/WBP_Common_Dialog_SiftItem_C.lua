...
function M:OnBtnSelectAllClicked()
    ...
    for i = 1, numChildren do
        ...
        if child and child.CheckBox_Selection then
            if child.CheckBox_Selection:IsChecked() ~= isChecked then
...
function M:OnSelectionItemChanged(CheckState, selectionUI)
    ...
    for i = 1, numChildren do
        local child = children:Get(i)
        if child and child.CheckBox_Selection then
            if not child.CheckBox_Selection:IsChecked() then
                allSelected = false
                break
            end
        end
...
function M:InitNavigationRules()
    -- local firstCheckBox = self:SetFirstCheckBoxFocusTarget()
    self.Btn_SelectAll:SetNavigationRuleCustom(EUINavigation.Down, {self,self.SetFirstCheckBoxFocusTarget})
end
