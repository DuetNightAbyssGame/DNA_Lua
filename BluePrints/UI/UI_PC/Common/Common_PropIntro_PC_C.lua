...
function M:Construct()
    ...
    self.Common_Key_Show_Desc_PC:CreateCommonKey({KeyInfoList={{Type = "Text",Text = "Tab"}},Desc = GText("UI_DETAILS")})
end
...
function M:CheckAndStartNextPropAnim(PropInfo)
    if PropInfo then
        ...
        if TableName == "Resource" and Data.FunctionDes then
        ...
        if(Rarity > 4)then
            ...
            --self.Text_Name:SetColorAndOpacity(SlateColor)
        ...
        -- self.Text_Intro_omit:SetVisibility(ESlateVisibility.Collapsed)
        -- self:AddTimer(0.05, function()
        --     local TextSize = self.Text_Intro:GetDesiredSize()
        --     local Geometry = self.Text_Intro.GetParent():GetPaintSpaceGeometry()
        --     local BlockSize = UUIFunctionLibrary.GetGeometrySize(Geometry)
        --     if TextSize.Y ~= 0 then
        --         if TextSize.Y > BlockSize.Y then
        --             self.Text_Intro_omit:SetVisibility(ESlateVisibility.Visible)
        --         end
        --         self:RemoveTimer("IsClipBounds")
        --     end
        -- end, true, 0, "IsClipBounds")
