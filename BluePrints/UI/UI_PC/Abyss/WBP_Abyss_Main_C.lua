...
function WBP_Abyss_Main_C:SetRewardParams()
    ...
    local ConfigData={
        Items = {},
         ...
         ReceiveAllParam={SelectAbyssId=self.SelectAbyssId},
         SortType=2,
         Rewards={},
         NowNum=0,
         NumMax=0,
...
function WBP_Abyss_Main_C:InitInfo(FirstSelectedAbyssId)
    ...
    --self.List_Level.OnListViewScrolled:Add(self, self.OnListViewScrolled)
    self.Text_BacktoTop:SetText(GText("Abyss_Level_Back"))
    self.Store:SetText(GText("Abyss_ExchangeStore"))
    self.Title:SetInfo({MainTitle = "Abyss_entry"})
    ...
    self.List_Level:SetNavigationRuleCustom(EUINavigation.Left,{self,self.OnUINavigation})
        ...
        if self.IsEndless then
            ...
            self.FirstTimer = self:AddTimer(0.001,function()
                ...
                self:AddTimer(0.001,function()
                    ...
                    -- self:RemoveTimer(self.ScrollTimer)
                    -- self.ScrollTimer = nil
                    -- self:ScrollToOffest(0, self.BottomOffset, true)
                    -- self.List_Level:SetEmptyGridItemCount(0)
                end)
                self:RemoveTimer(self.FirstTimer)
                self.FirstTimer = nil
            end)
    ...
    self.ScrollTimer = self:AddTimer(0.033,function(_,DeltaTime)
        ...
        if(self.LerpAlpha >= 1)then
            ...
            if self.CurFocusedLevel then
                ...
                --     local TargetContent = self.List_Level:GetItemAt(self.Target-1)
                --     self.List_Level:BP_NavigateToItem(TargetContent)
                end
            end
            return
        end
...
function WBP_Abyss_Main_C:RefreshTimerInfo(AbyssId)
    ...
    -- if (AbyssInfo.AbyssType == 2 or AbyssInfo.AbyssType == 3) and AbyssInfo.AbyssEndTime then
    --     self.Panel_RefreshTime:SetVisibility(UIConst.VisibilityOp.SelfHitTestInvisible)
    --     self:SetTimer(AbyssInfo.AbyssEndTime)
    -- else
    --     self.Panel_RefreshTime:SetVisibility(UIConst.VisibilityOp.Collapsed)
    -- end
end
...
function WBP_Abyss_Main_C:Refresh(EndTime)
    ...
    if RemainRefreshTime > 24 * 60 * 60 then
        ...
        RemainTimeStr = RemainTimeStr .. string.format(GText("UI_Time_Day_NotHighlight"), math.floor(RemainRefreshTime / (24 * 60 * 60)))
        RemainRefreshTime = RemainRefreshTime % (24 * 60 * 60)
    end
    if RemainRefreshTime > 60 * 60 or TimeCount == 1 then
        TimeCount = TimeCount + 1
        RemainTimeStr = RemainTimeStr .. string.format(GText("UI_Time_Hour_NotHighlight"), math.floor(RemainRefreshTime / (60 * 60)))
...
function WBP_Abyss_Main_C:GetGotLevelStar(AbyssId, AbyssLevelId ,LevelIndex)
    ...
    if Avatar then
        ...
        -- if LevelIndex < Abyss.MaxAbyssProgress[1] then
        --     GotStar = self:GetSumLevelStar(AbyssLevelId)
        -- elseif LevelIndex == Abyss.MaxAbyssProgress[1] then
        --     GotStar = Abyss.MaxAbyssProgress[2]
        -- end
    end
    return GotStar
end
...
function WBP_Abyss_Main_C:OnClickedAbyssLevelCell(AbyssLevelCell)
    ...
    self:OpenSubUI({Idx = "AbyssSelect"}, false, self.SelectAbyssId, AbyssLevelCell.Index)
end
...
function WBP_Abyss_Main_C:InitTable()
    self.TabConfigData = {
        ...
        BottomKeyInfo = { { KeyInfoList = {{Type="Text", Text="Esc", ClickCallback=self.Root.OnClickBack, Owner=self.Root,}}, GamePadInfoList = {{Type="Img", ImgShortPath="B"}}, Desc = GText("UI_BACK"), bLongPress = false}},
...
-- function WBP_Abyss_Main_C:OnListViewScrolled(CurrentOffset)
--     if not self.IsEndless then
--         if self.IsEndless and self.TargetOffset and self.Mid and self.TargetOffset - CurrentOffset > self:GetOffsetByIndex(self.Mid + 3) then
...
--    --region 打开肉鸽界面时需要隐藏组队
--     if self.TeamHeadUI then
--         if TabId == "RougeMain" or TabId == "AbyssMain" or TabId == "AbyssLineup" or TabId == "AbyssSelect" then
...
function WBP_Abyss_Main_C:NavigateToFirstDisplayedItem(List)
    ...
    if ItemUIs:Length() > 0 then
        ...
        for i=1,ItemUIs:Length() do
            ...
            if Index then
                -- if (not TargetWidget) or (Index < TargetWidget.Content.Index) then
                --     TargetWidget = Widget
                -- end
...
function WBP_Abyss_Main_C:OnEntryGenerated(Entry)
    if self.IsEndless then
        if self.CanJump then
            ...
            if Entry.Content.IsLastLevel then
                self.BacktoTop:SetVisibility(UIConst.VisibilityOp.Collapsed)
            end
        end
    end
