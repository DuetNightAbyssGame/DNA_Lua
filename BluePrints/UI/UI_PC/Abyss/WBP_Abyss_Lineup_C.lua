...
local WBP_Abyss_Lineup_C = Class({"BluePrints.UI.BP_EMUserWidget_C","BluePrints.Common.TimerMgr"})
...
function WBP_Abyss_Lineup_C:InitTable()
	--AudioManager(self):PlayUISound(self, "", "AbyssMainOpenSound", nil)
    self.BottomKeyInfo = { 
        { 
            GamePadInfoList =  {{
                Type="Img", 
    ...
    self.TabConfigData = {
        ...
        StyleName="Text", OwnerPanel=self.Root, BackCallback=self.Root.OnClickBack,
    ...
    for SlotName, EName in pairs(self.ESlotName) do
        for DungeonIndex, DungeonPanel in pairs(self.Dungeons) do
            ...
            if Uuid and self:CheckUnit(Uuid) then
                ...
                if DataType == "Weapon" then
                    local Tags = {CommonConst.ArmoryTag.Melee,CommonConst.ArmoryTag.Ranged}
...
function WBP_Abyss_Lineup_C:InitSelectiveList()
    self.OrderByDisplayNames = {"UI_LEVEL_SELECT","UI_RARITY_NAME"}
...
function WBP_Abyss_Lineup_C:SetDetailPanelLocation(SelectedDungeon, IsInit)
    ...
    if SelectedDungeon == 1 then
        SetLocationAnimName = "Expand"
    elseif SelectedDungeon == 2 then
...
function WBP_Abyss_Lineup_C:SlotSelectionChanged(SlotName, DungeonIndex, bToList)
    ...
    if DungeonIndex ~= PreDungeonIndex then
        --DebugPrint("lhr@SlotSelectionChanged", DungeonIndex, SlotName)
    ...
    for DungeonIndex, DungeonPanel in pairs(self.Dungeons) do
        for SlotName, Slot in pairs(DungeonPanel.Slots) do
            if Slot and not Slot.IsEmpty then
                ...
                if Type == "Weapon" then
                    Type = DungeonPanel:GetWeaponType(SlotName)
                    if Type ~= self.CurSlotType then goto continue end
...
function WBP_Abyss_Lineup_C:FillSelectiveList()
    ...
    --@note WBP_Armory_Selective_Listing Init 用例
...
function WBP_Abyss_Lineup_C:OnItemIsHoverChanged(ItemContent, bHovered)
    if not ItemContent.Uuid then return end
    if not self.UsingGamepad then
        return
    end
    --self:ShowItemDetails(false, nil, true)
...
function WBP_Abyss_Lineup_C:OnListItemClicked(Content)
    ...
    if Content.TeamIdx then
        --如果是当前槽位选中的该物品，则取消装备；否则，与原本装备该物品的槽位进行交换
        ...
        if Content.Uuid == CurrentUuid then
            ...
            --Toast提示
            UIManager(GWorld.GameInstance):ShowUITip(UIConst.Tip_CommonToast, GText(Content.UnitName)..GText("Abyss_Party_Replaced")..
                "<Highlight>"..self.CurDungeonPanel.Name.."·"..self.SlotName2TextMap[self.CurSlotName].."</>")
            return
        end
    end
    self:CallFunctionByName(self.CurSlotType .. "Main_OnListItemClicked",Content)
...
function WBP_Abyss_Lineup_C:SaveLineup()
    ...
    local TeamTable = {
        [1] = {Char = NullUUid,MeleeWeapon = NullUUid,RangedWeapon = NullUUid,Phantom1 = NullUUid,PhantomWeapon1 = NullUUid,Phantom2 = NullUUid,PhantomWeapon2 = NullUUid,Pet = NullUnitId},
        [2] = {Char = NullUUid,MeleeWeapon = NullUUid,RangedWeapon = NullUUid,Phantom1 = NullUUid,PhantomWeapon1 = NullUUid,Phantom2 = NullUUid,PhantomWeapon2 = NullUUid,Pet = NullUnitId},
    }
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        if (InKeyName == "Gamepad_DPad_Up") then
            if not self.bInList then
                if self.Dungeons and #self.Dungeons > 1 then
...
function WBP_Abyss_Lineup_C:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        elseif (InKeyName == "Gamepad_Special_Right") then
            if not self.bInList then
                if not self.CurDungeonPanel.Btn_MonsterInfo.IsPressing then
...
function WBP_Abyss_Lineup_C:OnKeyUp(MyGeometry, InKeyEvent)
    -- DebugPrint("lhr1@WBP_Abyss_Lineup_C:OnKeyUp")
    local InKey = UE4.UKismetInputLibrary.GetKey(InKeyEvent)
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if self.bInList then
        if self.List_Lineup:OnListKeyUp(MyGeometry, InKeyEvent) then
            return UE4.UWidgetBlueprintLibrary.Handled()
        end
    end
    local IsHandled = true
...
function WBP_Abyss_Lineup_C:OnAnalogValueChanged(MyGeometry,InAnalogInputEvent)
    ...
    local InKeyName = UE4.UFormulaFunctionLibrary.Key_GetFName(InKey)
    if(InKeyName == "Gamepad_LeftX" or InKeyName == "Gamepad_LeftY")then
        local LeftDelta = UKismetInputLibrary.GetAnalogValue(InAnalogInputEvent)
        if LeftDelta and LeftDelta > 0.5 or LeftDelta < -0.5 then
            if (IsValid(self.GameInputModeSubsystem)) then
                if not self.UsingGamepad then
...
-- 清除子控件监听设备切换事件，防止子控件自行更改手柄图标可见性
...
function WBP_Abyss_Lineup_C:ShowReturnConfirmWindow()
    ...
    CommonDialogParams.OnCloseCallbackFunction = function() 
        self:SetFocus()
