...
function WBP_Experience_Main_C:PlayBaiAnim()
    ...
    if self.IsHome then
        ...
        UIManager(self):SwitchUINpcCamera(true, "ExperienceMain", self.NpcId, {IsHaveInOutAnim=self.IsNeedPlayNpcAnim})
    end
end
...
function WBP_Experience_Main_C:Construct()
    ...
    --self.WBP_Com_BtnLockCondition:BindEventOnClicked(self, self.TipsIn)
...
-- function WBP_Experience_Main_C:OnMouseWheel(MyGeometry,Event)
--     UE.UKismetSystemLibrary.PrintString(self,"OnMouseWheelOnMouseWheel")
--     self.ScrollBox:OnMouseWheel(MyGeometry,Event)
-- end
...
function WBP_Experience_Main_C:InitNormalInfo(...)
    ...
    if Avatar then
        ...
        if CurrentExp then self.Text_EXNum:SetText(tostring(CurrentExp)) end
        if MaxExp then self.Text_EXNumMax:SetText(tostring(MaxExp)) end
    end
    local NumMax = 0
...
function WBP_Experience_Main_C:InitScrollBox()
    ...
    local LevelInfos = {}
    for Index,LevelInfo in pairs(DataMgr.PlayerLevelUp) do
        table.insert(LevelInfos,{PlayerLevel = LevelInfo.PlayerLevel})
    end
...
function WBP_Experience_Main_C:GetDistance(Index, CurrentOffset)
    if not CurrentOffset then
        CurrentOffset = self.ScrollBox:GetScrollOffset()
    end
...
function WBP_Experience_Main_C:SelectFirstItem()
    ...
    if not FirstItem then
        FirstItem = AllChildren:GetRef(TargetIndex+1)
    end
...
function WBP_Experience_Main_C:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        elseif (InKeyName == UIConst.GamePadKey.FaceButtonTop) then
            ...
            if Index == 0 and not self.IsInSelectState and not self.IsInTipState then
                IsEventHandled = true
...
function WBP_Experience_Main_C:PlayOutAnim()
    ...
    self:SwitchCamera(true)--需要提前恢复相机
...
function WBP_Experience_Main_C:UpdateSelectedInfo(ExperienceItem, Speed, isDragSelect, IsFirstTime)
	...
	-- 滚动选中的时候，不要走这里，不然会出现抢动效
    if isDragSelect ~= true then
...
function WBP_Experience_Main_C:UpdateRewardInfo()
    self:UpdateBtnState()
    local Rewards = {DataMgr.PlayerLevelUp[self.SelectedItem.Level].PlayerLevelReward}
        ...
        if RewardInfo then
            ...
            for i = 1, #Ids do
                ...
                Content.OnFocusReceivedEvent = {
                    Obj = self,
                    Callback = function() self:OnAddedToFocusPathEvent() end
                }
...
function WBP_Experience_Main_C:UpdateNodeInfo()
    ...
    if LevelInfo and LevelInfo.NodeType then
        ...
        for Index, Type in ipairs(LevelInfo.NodeType) do
            local SortedItemInfos = nil
            if Type == "Walnut" then
                if LevelInfo.NodeContent[Index] and LevelInfo.NodeContent[Index].Id then
                    ...
                    for ItemId,Info in pairs(ShopItem) do
                        if Info.ItemType == "Walnut" and Info.UnlockLevel == self.SelectedItem.Level then
                            if Walnut[Info.TypeId] and IncludIds[Walnut[Info.TypeId].WalnutType] then
                                if (not Info.StartTime or NowTime >= Info.StartTime) and (not Info.EndTime or NowTime <= Info.StartTime) then
                                    if not SortedItemInfos then
                                        SortedItemInfos = {}
                                    end
            ...
            if Type ~= "Walnut" or (Type == "Walnut" and SortedItemInfos) then
                HaveItem = true
...
function WBP_Experience_Main_C:UpdateArrow()
    if self.SelectedItem.Index == 1 then
...
function WBP_Experience_Main_C:RefreshNextRewardInfo()
    ...
    for i=1,AllChildren:Length() do
        local ChildItem = AllChildren:GetRef(i)
        if ChildItem.Index and ChildItem.Index > StartIndex and ChildItem.CanGet and not ChildItem.IsGot then
...
-- function WBP_Experience_Main_C:TipsIn_C()
--     if not self.EMMenuAnchor_Item:IsOpen() then
--         self.WBP_Com_BtnLockCondition:OnBtnClicked()
--     end
-- end

-- function WBP_Experience_Main_C:TipsIn()
--     if not self.EMMenuAnchor_Item:IsOpen() then
--         self.EMMenuAnchor_Item:Open(true)
--     end
-- end
...
function WBP_Experience_Main_C:OnMenuOpenChanged(IsOpened)
    if(self.EMMenuAnchor_Item.bAllowHover) then return end
...
function WBP_Experience_Main_C:InitWidgetInfoInGamePad()
    -- self.WBP_Com_BtnLockCondition:SetDefaultGamePadImg("Y")
...
function WBP_Experience_Main_C:EnterSelectMode(Name)
    ...
    if (IsValid(self.GameInputModeSubsystem)) then
        ...
        if Name == "Reward" then
            ...
            Widget:SetNavigationRuleCustom(EUINavigation.Left,{self,self.OnUINavigation})
            Widget:SetNavigationRuleCustom(EUINavigation.Right,{self,self.OnUINavigation})
            EnterSuccess = true
        elseif Name == "Walnut" then
            if self.Switch_promote:GetActiveWidgetIndex() == 1 then 
                local Contents = self.ListView_Promote:GetListItems()
                local TargetContent = nil
                for _, Content in pairs(Contents) do
                    if Content.Type == "Walnut" and Content.Entry then
...
-- function WBP_Experience_Main_C:EnterOrLeaveTipMode()
--     if not self.IsInTipState then
--         self:EnterTipMode()
--     else
--         self:LeaveTipMode()
--     end
...
-- function WBP_Experience_Main_C:EnterTipMode()
--     if (self.IsInTipState) then
--         return
--     end
--     self:TipsIn_C()
...
-- function WBP_Experience_Main_C:LeaveTipMode()
--     if (not self.IsInTipState) then
--         return
--     end
--     self:TipsOut()
