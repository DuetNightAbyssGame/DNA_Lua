...
function M:Construct()
    ...
    self:SetCurrentWave()
    self:StartCountDown()
	-- if GameState.DungeonVoteInterval > 0 then
	-- 	self.CountDown.Text_CountDown:SetText(GameState.DungeonVoteInterval)
	-- else
    ...
    -- UIManager(self):CloseResidentUI("Vote")
    UIManager(self):HideAllUI_EX({self:GetName()}, true, "Vote", true)
...
function M:StartCountDown(Info)
    ...
    if not IsDedicatedServer(GameState) and IsAuthority(GameState) then
        ...
        self.Switch_Show:SetActiveWidgetIndex(1)
        -- self.Text_Tips:SetText(string.format(GText("UI_Vote_Finish_Choice"), self.CurrentWave))
...
function M:OnRepDungeonVoteInterval()
    ...
    UIManager(self):HideAllUI_EX({self:GetName()}, false, "Vote", true)
    self.bInClose = true
    self:SetFocus()
    self:Close()
end
...
function M:OnClickFaceButtonRight()
    ...
    elseif self.FocusState == "OpenTeamInfo" then
        -- self.MultiPlayer:CloseAllAnchor()
...
function M:OnKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        ...
        --     self.Box_Continue.Cost.Common_Item_Icon:OnMouseButtonUp()
