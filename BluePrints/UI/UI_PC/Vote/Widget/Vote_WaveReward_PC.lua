...
function M:InitLeave(DefenceWave, CurrentDungeonProgress)
    ...
    if DefenceWave == CurrentDungeonProgress then
        ...
        VoteMain:BindToAnimationFinished(VoteMain.Auto_In, {self, self.PlayAnimationPanelIn})
...
function M:ReadRewardInfo(Reward, ListItemNums)
    for i, ResourceId in ipairs(Reward.Id) do
        ...
        if Avatar then
            if Avatar.bDungeonDoubleCost and not DataMgr.Dungeon[DungeonId].IsWalnutDungeon then
                DungeonDoubleCost = 2
            end
        end
        local Content
...
function M:NewItemContent(ItemType, ItemId, Count, BonusType, NeedAdd, IsExtra)
    ...
    Obj.UIName = "Vote"
    Obj.OnMenuOpenChangedEvents = {Obj = self, Callback = self.OnItemMenuOpenChanged}
    Obj.OnAddedToFocusPathEvent = {Obj = self, Callback = self.OnAddedToFocusPathEvent}
    Obj.OnRemovedFromFocusPathEvent = {Obj = self, Callback = self.OnRemovedFromFocusPathEvent}
...
-- function M:OnItemSelectionChanged(Content)
--     print(_G.LogTag,"LXZ OnItemSelectionChanged", Content:GetName(), Content.CurrentDungeonProgress, Content.RewardId, Content.Type)
--     self.RewardList:ClearListItems()
--     self:InitRewardList(Content.CurrentDungeonProgress, Content.RewardId, Content.Type == "Continue")
-- end
...
function M:OnPreviewKeyDown(MyGeometry, InKeyEvent)
    ...
    if (UE4.UKismetInputLibrary.Key_IsGamepadKey(InKey)) then
        if InKeyName == "Gamepad_DPad_Right" then
            ...
            if Content then
                if not self.IsContinue and (self.ResourceNum == 1 or self.RewardContentList[self.ResourceNum] == Content) then
                    local VoteMain = UIManager(self):GetUIObj("Vote")
        ...
        elseif InKeyName == "Gamepad_DPad_Left" then
            local Content = self.RewardList:BP_GetSelectedItem()
            if Content then
                if self.IsContinue and self.RewardContentList[1] == Content then
                    local VoteMain = UIManager(self):GetUIObj("Vote")
