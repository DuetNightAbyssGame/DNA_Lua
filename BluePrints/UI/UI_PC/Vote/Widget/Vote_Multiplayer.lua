...
function M:Init(RootPage)
    ...
    for i, PlayerState in pairs(GameState.PlayerArray) do
        ...
        self.TeamHeadList[Eid] = {TeamHead = TeamHead, PlayerState = PlayerState, Index = i}
    end
...
function M:OnPlayerVote(VoteValue, Eid, bMainPlayer, bFirstMainPlayer)
    if self.WB_Player_L:GetChildrenCount() == 0 and self.WB_Player_R:GetChildrenCount() == 0 then
...
function M:OnPlayerVoteContinue(Eid, bMainPlayer, bFirstMainPlayer)
    ...
    if (self.bSelectContinue or self.bSelectLeave) and self.WB_Player_L:GetChildrenCount() == 0 then
        self.bSelectLeave = false
    end
...
function M:OnPlayerVoteLeave(Eid, bMainPlayer, bFirstMainPlayer)
    ...
    if (self.bSelectContinue or self.bSelectLeave) and self.WB_Player_R:GetChildrenCount() == 0 then
        self.bSelectContinue = false
    end
...
function M:BP_GetDesiredFocusTarget()
    if self.WB_Player_L:GetChildrenCount() ~= 0 then
        return self.WB_Player_L:GetChildAt(0).Head_Team.Button_Area
    elseif self.WB_Player:GetChildrenCount() ~= 0 then
