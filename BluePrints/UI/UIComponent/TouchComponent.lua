...
function TouchComponent:Initialize(Initializer)
    self.WidgetStartPos = nil           -- 交互对象的开始位置（Local）
    self.WidgetCurPos = nil             -- 交互对象的当前位置（Local）
    self.NowTouchPos = {}               -- 当前触控点位置（Key是PointerIndex）
    self.m_LastPosTable = {}            -- 上一次停留的Pos位置（Key是PointerIndex）
    ...
    self.SubTouchItemsStartPos = {}     -- Widget在当前控件之中的起始位置   
    ...
    self.Offset = FVector2D(0, 0)           -- 暂时没有用
...
function TouchComponent:MouseOrTouchButtonDown(InGeometry, InGestureEvent)
    ...
    local SubTouchItem, SubTouchItemName, SubTouchItemStartPos, SubTouchParentWidget = nil, nil, nil, nil
    for k, v in pairs(self.AllTouchItems) do
        local WidgetWorldPos, WidgetWorldSize = self:GetWorldPos(v)
        -- local CanvasSize = UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(v):GetSize()
        ...
        if ((ScreenSpacePosition.X >= StartCamparePosX and ScreenSpacePosition.X <= EndCamparePosX) and 
            ...
            if (SubTouchItem == nil) then
                ...
                SubTouchItem = v
                SubTouchItemName = k
                SubTouchItemStartPos = FVector2D(WidgetLocalPos.X + WidgetWorldSize.X * 0.5 * Scale * WidgetLocalScale,
                            WidgetLocalPos.Y + WidgetWorldSize.Y * 0.5 * Scale * WidgetLocalScale)
    ...
    if (self.WidgetStartPos == nil) then
    ...
    if TotalTouchCount <= 2 then
        ...
        if TotalTouchCount <= 1 then
            ...
            self.Offset = UE4.USlateBlueprintLibrary.AbsoluteToLocal(InGeometry, ScreenSpacePosition) * Scale --+FVector2D(120,120)
...
function TouchComponent:MouseOrTouchButtonMove(InGeometry, InGestureEvent)
    ...
    if self.Touching_Flag[PointerIndex + 1] then
        ...
        local ScreenSpacePosition = UE4.UKismetInputLibrary.PointerEvent_GetScreenSpacePosition(InGestureEvent)
        local thisPos = UE4.USlateBlueprintLibrary.AbsoluteToLocal(InGeometry, ScreenSpacePosition) * Scale
        -- local changepos = (thisPos - self.Offset)
...
function TouchComponent:MouseOrTouchButtonUp(InGeometry, InGestureEvent, TargetPointerIndex)
    ...
    if self.NowTouchPos[PointerIndex + 1] ~= nil then
        ...
        elseif TotalTouchCount == 1 then
            ...
            -- self.m_DragStartPos = self.NowTouchPos[1]
        end
        local Handled = UE4.UWidgetBlueprintLibrary.Handled()
        if self.UpdatePosFlag[self.SubTouchItemsName[PointerIndex + 1]] == true then
...
--更新回弹的位置
-- function TouchComponent:UpdateEdgeLerp(PointerIndex)
--     local CanvasSlot = UE4.UWidgetLayoutLibrary.SlotAsCanvasSlot(self.SubTouchItems[PointerIndex + 1])
--     CanvasSlot:SetPosition(FVector2D(self.WidgetCurPos.X, self.WidgetCurPos.Y))
-- end
