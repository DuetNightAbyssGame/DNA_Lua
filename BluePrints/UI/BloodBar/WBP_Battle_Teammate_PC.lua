...
function WBP_Teammate_PC_C:SetNameMaterial(MaterialRef)
    UResourceLibrary.LoadObjectAsync(self, tostring(MaterialRef), {
        ...
        function(obj, Material)
            ...
            if Owner:IsPhantom() and MaterialRef ~= self.NameMaterial_Phantom then return end
            if Owner:IsPlayer() and MaterialRef ~= self.NameMaterial_Player then return end
...
function WBP_Teammate_PC_C:UpdateBar()
    ...
    local IsHeal = self.LastHp < self.CurHp --or self.LastShield < self.CurShield
    ...
    if self.HpBar then
        ...
        -- local IsLowPercent = self.HpBar:CheckIsLowPercent(self.LowHpPercent)
        -- self.HpBar:CheckAndPlayLowHealth(IsLowPercent)
    end
    ------------------------------------------------------------------------------
    ...
    if self.ShieldBar and self.MaxShield > 0 then
        ...
        elseif self.CurOverShield > self.LastOverShield then
            self.ShieldBar:SetOverShieldPercent(CurPercent)
            if self.LastOverShield <= 0.00001 then
...
function WBP_Teammate_PC_C:SetName(PlayerState)
    ...
    elseif PlayerState then
        if PlayerState:IsA(AEMPlayerState) then
            ...
            --self.Text_Name:SetColorAndOpacity(self.NameColor_Normal)
            self:SetNameMaterial(self.NameMaterial_Player)
...
function WBP_Teammate_PC_C:OnDead()
    ...
    --self:AddBloodState(self.Eid, BloodBarUtils.AllBloodState.Dead)
end
...
function WBP_Teammate_PC_C:OnRecovery(Eid)
    ...
    -- self:PlayAnimationReverse(self.LifeSwitch)
    if not IsValid(self.Owner) then
        self:SetImage(false)
    end
...
function WBP_Teammate_PC_C:SetInvincible(IsInvincibility)
    ...
    if IsInvincibility then
        -- self.Group_Invincibility:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
        ...
        -- self.Group_Invincibility:SetVisibility(UE4.ESlateVisibility.Collapsed)
    ...
    if not IsInvincibility and self.WaitingToPlayGreyAnim ~= nil then
        self:PlayHpBarGreyAnimation(self.WaitingToPlayGreyAnim == 0)
    end
end
...
function WBP_Teammate_PC_C:PlayHealAnimation()
    ...
    --     self.PlayerBloodReturn:PlayAnimation(self.PlayerBloodReturn.Player_BloodReturn)
    -- end
end

function WBP_Teammate_PC_C:OpenTeamInfo()
    local BattleMain = UIManager(self):GetUIObj("BattleMain")
    if (BattleMain ~= nil and type(BattleMain.OpenTeamInfo) == "function") then
...
function WBP_Teammate_PC_C:SetImage(IsDead)
    ...
    UE4.UResourceLibrary.LoadObjectAsync(self,MiniIconPath..NormalIconName.."."..NormalIconName.."'",{self,WBP_Teammate_PC_C.LoadImageFinish})
    -- local IconImage = LoadObject(MiniIconPath..NormalIconName.."."..NormalIconName.."'")
    -- if IconImage then
    --     self.Img_Head:SetBrushResourceObject(IconImage)
    -- end
end
...
function WBP_Teammate_PC_C:AddBloodState(TargetEid, StateName)
    -- 设置血条状态
    if (StateName == nil or TargetEid ~= self.Eid) then
        return
    end
    if (StateName == BloodBarUtils.AllBloodState.OverReach and self.BloodState[BloodBarUtils.AllBloodState.Dead] ~= nil) then
        -- 死亡状态与距离过远互斥，依然显示死亡状态
        return
    end
...
function WBP_Teammate_PC_C:_SetBloodInOverReachState(TargetEid)
    ...
    if (not self.WidgetInOverReach) then
        ...
        if (self.WidgetInOverReach ~= nil) then
            ...
            -- self.TimerHandleForAnim = self:AddTimer(5.0, function() EMUIAnimationSubsystem:EMPlayAnimation(self.WidgetInOverReach, self.WidgetInOverReach.Loop)
            --                                                     end, true, 0, "BloodOverReachLoopAnim")
            self.TimerHandleForAnim = self:AddTimer(5.0, function() self.WidgetInOverReach:PlayAnimation(self.WidgetInOverReach.Loop)
                                                            end, true, 0, "BloodOverReachLoopAnim")
        end
    end
...
function WBP_Teammate_PC_C:InitShortageUI()
    ...
    self.ShortageItems[901] = ShortageItem  -- 保存
    ...
    self.ShortageItems[902] = ShortageItem2  -- 保存
end
