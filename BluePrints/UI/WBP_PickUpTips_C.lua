...
---@class WBP_PickUpTips_C : WBP_ButtomPickUpTips
...
function WBP_PickUpTips_C:Tick(MyGeometry, DeltaTime)
    if #self.WaitingItemList == 0 then
        return
    end
    local TipsItem = self:GetValidTips() 
    if TipsItem then
        TipsItem:UpdateTips(self.WaitingItemList[1][1], self.WaitingItemList[1][2],
        self.WaitingItemList[1][3],self.WaitingItemList[1][4])
...
function WBP_PickUpTips_C:DoShowPickUpTips(ItemId, ItemCount,TableName)
    ...
    if TipsItem then
        ...
        table.insert(self.WaitingItemList, { ItemId, ItemCount, nil, TableName })
    end
end
...
---@param TipsItem WBP_PickUpTipsItem_C
