...
---@type WBP_Set_MultipleLangContent_Item_C
...
function M:Tick(DeltaTime)
    if self.Content and self.Content.DownloadProcess then
        ...
        if TotalBytes > 0 then
            local SizeText = string.format("%.0fMB/%.0fMB", BytesSoFar / (1024 * 1024), TotalBytes / (1024 * 1024))
...
function M:UpdateVoiceDownloadState()
    ...
    if self.Content.DownloadedVoiceState == EOptionalPatchAssetState.Downloaded then
        ...
        self.Text_DownloadSize:SetText(string.format("%.0fMB", (self.Content.DownloadProcess and self.Content.DownloadProcess.TotalBytes or 0) / (1024 * 1024)))
    ...
    elseif self.Content.DownloadedVoiceState == EOptionalPatchAssetState.Downloading then
        ...
        self.Text_DownloadSize:SetText(string.format("%.0fMB/%.0fMB", (self.Content.DownloadProcess and self.Content.DownloadProcess.BytesSoFar or 0) / (1024 * 1024), (self.Content.DownloadProcess and self.Content.DownloadProcess.TotalBytes or 0) / (1024 * 1024)))
