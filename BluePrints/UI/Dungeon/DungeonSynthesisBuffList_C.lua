...
---@type WBP_Play_DeputeWeekly_Hud_Buff_C
...
function M:Construct()
    self.BuffIds = {5000301, 5000302, 5000303, 5000304}
    self.BuffIconPath = {
...
function M:InitListenEvent()
    ...
    for _, BuffId in pairs(self.BuffIds) do
        Player.BuffManager:BP_BindOnBuffAdded(BuffId, {self, self.OnBuffAdded})
        Player.BuffManager:BP_BindOnBuffRemoved(BuffId, {self, self.OnBuffRemoved})
        Player.BuffManager:BP_BindOnBuffRefreshed(BuffId, {self, self.OnBuffRefreshed})
    end
end
...
function M:RemoveListenEvent()
    local Player = UE4.UGameplayStatics.GetPlayerCharacter(self, 0)
    for _, BuffId in pairs(self.BuffIds) do
        Player.BuffManager:BP_UnbindOnBuffAdded(BuffId, {self, self.OnBuffAdded})
        Player.BuffManager:BP_UnbindOnBuffRemoved(BuffId, {self, self.OnBuffRemoved})
        Player.BuffManager:BP_UnbindOnBuffRefreshed(BuffId, {self, self.OnBuffRefreshed})
    end
end
...
function M:OnBuffAdded(Buff)
    ...
    if self.BuffItemContents[BuffId] then
        -- 如果已存在 说明无需处理 等OnBuffRefreshed再处理
        return
    end
