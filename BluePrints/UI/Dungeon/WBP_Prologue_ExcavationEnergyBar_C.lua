...
function WBP_Prologue_ExcavationEnergyBar_C:SetDataFromEntity(Ent)
    if Ent == nil or Ent.NowEnergy == nil or Ent.Progress == nil then
        return
    end
    -----------------HP-------------------------
    ...
    if self.LastHp - Hp > 0.00001 then
    ...
    if not self.MechEnergyFull then
        ...
        else self.NextState = "Normal" end
        ...
        if self.CurrentState ~= self.NextState then
            if self.NextState == "Lack" then
                ...
                --self["VX_Warning0"..tostring(Index)]:SetVisibility(UE4.ESlateVisibility.Visible)
                ...
                -- for i = 1, 5 do
                --     self["VX_Warning0"..tostring(i)]:SetVisibility(UE4.ESlateVisibility.Collapsed)
                -- end
                local Material = self.Img_VX_Warning:GetDynamicMaterial()
                if Material then
                    for i = 1, 5 do
    ...
    if Energy > 0 then
        ...
        local Offset = 0.07 - (0.07 + 0.93) * (Ent.Progress / 100)
    ...
    elseif self.UsingEnergy == true then
...
function WBP_Prologue_ExcavationEnergyBar_C:OnFinish()
    ...
    UIManager(self):ShowUITip(UIConst.Tip_ExcavationToast, GText("DUNGEON_EXCAVATION_101"), 5, false, {OrderText = self.Order_Text, Level = self.Level_str})
end
...
function WBP_Prologue_ExcavationEnergyBar_C:OnAttractBattery(BatteryEnergy)
    ...
    UIManager(self):ShowUITip(UIConst.Tip_ExcavationToast, GText("DUNGEON_EXCAVATION_104").." +"..BatteryEnergy, 5, false, {OrderText = self.Order_Text, Level = self.Level_str})
