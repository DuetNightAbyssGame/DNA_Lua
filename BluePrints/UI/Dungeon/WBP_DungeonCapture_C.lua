...
function WBP_DungeonCapture_C:ShowMessage()
	...
	-- UE4.UUIStateAsyncActionBase.ShowGuideToast(GameInstance, 100009, 15)
...
function WBP_DungeonCapture_C:OnLoaded(...)
    ...
    self:ListenForInputAction("ActiveGuide", EInputEvent.IE_Pressed, false, {self, self.ReActiveCurGuideAnim})
    ...
    if not self.RemainingTime then self.RemainingTime = 30 end
...
function WBP_DungeonCapture_C:InitListenEvent()
    -- self:AddDispatcher(EventID.OnGameStateTimerAdded, self, self.OnGameStateTimerAdded)
    -- self:AddDispatcher(EventID.OnGameStateTimerEnded, self, self.OnGameStateTimerEnded)
...
function WBP_DungeonCapture_C:CloseDungeonUI()
    if self.AlreadyClose then return end
...
function WBP_DungeonCapture_C:InitTempleDelayTimeUI(Title)
    --DebugPrint("zwk InitTempleDelayTimeUI")
...
--region 通用，兼容BpAddTimer和ClientTimerStruct，用来显示倒计时的结构
...
function WBP_DungeonCapture_C:UpdateRemainingTimeInHostage(HostageDyingDuration)
    ...
    self.RemainingTime = 15 - (HostageDyingDuration or 0)       -- 15为人质最大濒死时间，战斗那边self.MaxDyingTime写死，暂时写死吧
...
-- function WBP_DungeonCapture_C:StartHostageDeadCountDown(TimeInfo)
--     self.HostageDeadTimeInfo = TimeInfo
-- end
...
function WBP_DungeonCapture_C:UpdateRemainingTimeInTemple()
    -- 这个倒计时只给显示用，Delay节点的计时器在GameMode上
...
function WBP_DungeonCapture_C:RemainingDistance(SelfActor, AnotherActor)  
    ...
    local RefreshDistance = function()
