...
function WBP_DungeonDefenseFloatPro_C:Initialize(Initializer)
    ...
    self.TotalCountDownTime = 180   -- 防御核心总倒计时
...
function WBP_DungeonDefenseFloatPro_C:UpdateMechanismInfo()
    ...
    if not self.LastHp then self.LastHp = CurTargetActor:GetAttr("Hp") end
    if not self.LastShield then self.LastShield = CurTargetActor:GetAttr("ES") end

    --护盾值变化
    local NewShieldPercent = NewShield / MaxShield
    if(self.LastShield > NewShield)then
        ...
        if(NewShield < MaxShield)then
            -- self:PlayBarAnim(self.Anim_Deduct_ShieldBar, LastShieldPercent, NewShieldPercent)
            self.ShieldBar:PlayDeduct(true)
        end
    elseif (self.LastShield < NewShield) then
    ...
    if(self.LastHp > NewHp)then
        ...
        if(NewHp < MaxHp)then
            ...
            -- self:PlayBarAnim(self.Anim_Deduct_BloodBar,LastHpPercent,NewHpPercent)
        end
    end
	if NewHpPercent < 0.5 then
		...
		-- self.BloodProgressBar:SetFillColorAndOpacity(self.CriticalColor)
	else
        self.HpBar:SetProgressBarColorInner(self.NormalColor)
		-- self.BloodProgressBar:SetFillColorAndOpacity(self.NormalColor)
	end
...
-- function WBP_DungeonDefenseFloatPro_C:PlayBarAnim(Animation,LastPercent,NewPercent)
--     --播放进度条减少动画
--     local TotalTime = Animation:GetEndTime()
--     local StartTime = TotalTime - TotalTime * LastPercent
--     local EndTime = TotalTime - TotalTime * NewPercent
--     local PlayShieldAnim = function()
--         local Speed = (EndTime - StartTime) / self.AnimTime
--         self:PlayAnimationTimeRange(Animation,StartTime,EndTime,1,UE4.EUMGSequencePlayMode.Forward,Speed,false)
--     end
    
--     local AnimName =  UE4.UKismetSystemLibrary.GetObjectName(Animation)
--     if(self:IsExistTimer(AnimName))then
--         StartTime = self:GetAnimationCurrentTime(Animation)
--         self:RemoveTimer(AnimName, false)
--         self:AddTimer(self.AnimDelayTime, PlayShieldAnim, false, 0, AnimName, false)
--     elseif(self:IsAnimationPlaying(Animation))then
--         StartTime = self:GetAnimationCurrentTime(Animation)
--         PlayShieldAnim()
--     else
--         PlayShieldAnim()
...
-- function WBP_DungeonDefenseFloatPro_C:OnDefenceVoteBegin()
--     local GameInstance = UE4.UGameplayStatics.GetGameInstance(self)
--     local UIManger = GameInstance:GetGameUIManager()
--     if (UIManger == nil) then
-- 		return
-- 	end
-- 	UIManger:LoadUI(UIConst.DUNGEONERETREAT, "DungenonRetreat", UIConst.ZORDER_FOR_COMMON_TIP)
-- end
