...
function M:UIStateChange_OnTarget()
	...
	-- self:AddTimer(1, self.UpdateTime, true)
...
function M:UpdateSpeed(Speed, Acc, MaxSpeed, MinSpeed,TrollyStateId,Trolly)
	...
	if Speed <= MinSpeed then
		-- if self.SpeedState ~= self.SpeedStates.Slowest then
		...
		if TrollyStateId == SpecialStateId then
			...
			for _,Player in pairs(AllPlayer) do
				if UKismetMathLibrary.Vector_Distance(Trolly:K2_GetActorLocation(),Player:K2_GetActorLocation()) < Radius then
...
function M:UpdateUILength(Length)
	if not self.SplineLength then return end
	local Percent = Length / self.SplineLength
	-- DebugPrint("111111111111111111111111111111111111111 UpdateUILength",Length,self.SplineLength)
...
function M:InitAllMainPathSpline(SplineActor)
	...
	-- DebugPrint("M:InitAllMainPathSpline 111111111111111111111111111111",NowPathId,HijackPathInfo[NowPathId])
	...
	-- 		if PathPoint.NextPathId ~= 0 then
...
function M:InitSplineInfo()
	if self.AlreadInitSplineInfo then return end
	if not self.UISpline then return end
	local GameState = UE4.UGameplayStatics.GetGameState(self)
	local HijackPathInfo = GameState.HijackPathInfo
	local NowPathId = GameState.NowPathId
	local bShouldContinue = true
...
function M:InitAllTargetPoint()
	...
	if not GameState or not GameState.HijackPathInfo or not GameState.HijackPathInfo[GameState.NowPathId] then
		return
	end
	...
	table.sort(
		TriggerBoxTable, 
		function(FirstBoxLoc, SecondBoxLoc)
	...
	if self.InformationPanel.Panel_StrongHold then
		...
		-- self.InformationPanel.Panel_StrongHold:ClearChildren()
