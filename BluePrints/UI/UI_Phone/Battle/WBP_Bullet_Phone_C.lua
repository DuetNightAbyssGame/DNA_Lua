...
function M:PlayButtonStateAnimation()
    --DebugPrint("当前状态", self.CurButtonState)
    ...
    elseif self.CurButtonState == "NoReloadSkill" then
        if not self:IsAnimationPlaying(self.NoReloadSkill) then
...
function M:HandleCurButtonState(CurButtonState)
    if CurButtonState == "ChangeMagazine" then
        if (self.OwnerPlayer.CurrentSkillId ~= self.OwnerPlayer:GetSkillByType(UE.ESkillType.Reload) or
                not IsValid(self.OwnerPlayer:GetCurrentWeapon())) then
            self.CurButtonState = "Normal"
            self:PlayButtonStateAnimation()
        end
    end
...
function M:UpdateBulletType()
    ...
    if BulletIcon then
        ...
        -- self.Icon_Bullet:SetBrushResourceObject(BulletIconObj)
    else
        Path = "/Game/UI/Texture/Dynamic/Atlas/Battle/T_Battle_Bullet"
        -- self.Icon_Bullet:SetBrushResourceObject(BulletIconObj)
    end
    UE.UResourceLibrary.LoadObjectAsync(self,Path,{self,M.OnBulletIconLoadFinish})
