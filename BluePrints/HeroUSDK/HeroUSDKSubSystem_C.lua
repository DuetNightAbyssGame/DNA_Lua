...
function HeroUSDKSubSystem_C:OnMarketConversionDataSuccess(Data)
    ...
    if MarketConversionDataSuccessData == nil or MarketConversionDataSuccessData ~= Data then
        self:UploadTrackLog_Lua("attribution",
        {
            data_info = Data,
        })
...
function HeroUSDKSubSystem_C:PayCallBack(Result, PaymentOrder, Msg)
    ...
    if not bHasError then
        if PayCallBackPaymentOrder.code == 20002 then
            bHasError = true
            local CommonDialog = UIManager(self):ShowCommonPopupUI(100235, {
...
function HeroUSDKSubSystem_C:TryToGoToLoginScene(bAnti, KickOffType, bHasNecessoryPatch)
    ...
    if UGameplayStatics.GetCurrentLevelName(self) == "Login" then
        ...
        if bAnti and self:IsAntiAddctionType(KickOffType) then
            ...
            -- b渠 在登录界面触发防沉迷 直接退出游戏
            if bIsBilibili then
                QuitGame()
            else
                --- 正常情况下直接弹防沉迷弹窗，点击确认后直接退出游戏
    ...
    local GoLoginFunc = function()
        ...
        if HeroUSDKSubsystem():IsHeroSDKEnable() and not UUCloudGameInstanceSubsystem.IsCloudGame() then
            ...
            if self:IsAntiAddctionType(KickOffType) then
                ...
                -- 非IOS系统点确认后退出游戏
                elseif bAnti and PlatformName ~= "IOS" then
                    QuitGame()
                    return
                end
            end
...
function HeroUSDKSubSystem_C:NeedShowAntiAddctionDialog(KickOffType)
    ...
    if bIsWeGame then
        if KickOffType == UE4.EUsdkKickOffType.WeGame_kSystemStatePlatformOffline
        or KickOffType == UE4.EUsdkKickOffType.WeGame_kSystemStatePlatformExit
        or KickOffType == UE4.EUsdkKickOffType.WeGame_kSystemStatePlayerOwnershipExpired
        or KickOffType == UE4.EUsdkKickOffType.WeGame_kSystemStatePlayerOwnershipBanned then
            return false
        end
    end
    return true
...
function HeroUSDKSubSystem_C:OnLoginSuccess()
    ...
    if PlayerAvatar then
        ...
        -- self:SetNewBDCPublicAttriubute(CommonUtils.ObjId2Str(PlayerAvatar.Eid), PlayerAvatar.AccountId)
...
function HeroUSDKSubSystem_C:UploadBDCTrackInfoWhenChangeScene()
    ...
    -- self:UploadBigWorldPathInfo(PlayerCharacter)
    -- self:UploadWeaponUseCountInfo(PlayerCharacter)
    -- self:UploadSkillUseCountInfo(PlayerCharacter)
    -- self:UploadPlayerHurtInfo(PlayerCharacter)
    -- self:UploadPlayerDoDamageInfo(PlayerCharacter)
    -- self:UploadBulletJumpCountInfo(PlayerCharacter)
    -- self:UploadSlideCountInfo(PlayerCharacter)
    -- self:UploadJumpCountInfo(PlayerCharacter)
    -- self:UploadSecondJumpCountInfo(PlayerCharacter)
    -- self:UploadWallJumpCountInfo(PlayerCharacter)
    -- self:UploadFlyShootCountInfo(PlayerCharacter)
    -- self:UploadDeadInfo(PlayerCharacter)
    -- self:UploadRecoveryInfo(PlayerCharacter)
    -- self:UploadSkipTalkInfo(PlayerCharacter)
end

---@param PlayerCharacter BP_PlayerCharacter_C
...
function HeroUSDKSubSystem_C:UploadSkipTalkInfo(PlayerCharacter)
    -- if not PlayerCharacter.UploadBDCTrackInfo.SkipTalkInfo then
    --     return
    -- end
    -- for _, Info in ipairs(PlayerCharacter.UploadBDCTrackInfo.SkipTalkInfo) do
    --     self:UploadTrackLog_Lua("skip_talk", Info)
    -- end
end
...
function HeroUSDKSubSystem_C:EMHeroSDKSwitchAccount()
    ...
    if Platform == "Android" and not self:IsGlobalSDK() then
