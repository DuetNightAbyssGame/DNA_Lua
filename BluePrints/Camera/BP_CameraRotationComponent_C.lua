...
---@type BP_CameraRotationComponent_C
local M = Class()
...
function M:CameraKeepSightOnActorInTick(DeltaTime)
    if(self.bKeepingSightOnActor)then
        local X,Y
        if(self.KeepSightPitchInputDelta ~= 0 or self.KeepSightYawInputDelta ~=0 )then
       ...
       -- UE4.UKismetSystemLibrary.LineTraceMultiByProfile(self,CameraLoc,EndLoc,"PlayerCameraSight",false,nil,EDrawDebugTrace.None,HitResults,true)
...
function M:CameraLookToTarget(Actor,BoneName,Duration,EasingFunc,bDisableUserInput,MaxAngle,MaxDistance,ObscuredMaxTime,bPauseWhenShooting,bLookAndLockOnTarget,OnEnd,OnNoneFound,OnInterrupt)
    ...
    if(IsValid(Actor))then
        ...
        if(self.OnLookToTargetNoneFound)then
...
function M:SetCameraPitchBySkill(ParametersTable)
    --DebugPrint("@zyh调试", ParametersTable.Pitch, ParametersTable.InterpTime, ParametersTable.InterpSpeed, ParametersTable.DisableInputTag)
...
function M:EnablePitchRotInputBySkill(ParametersTable)
    --DebugPrint("@@zyh调试", ParametersTable.EnableInputTag)
    if ParametersTable.InputTag then
