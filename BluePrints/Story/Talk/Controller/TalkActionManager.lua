---@class TalkActionManager_C
require "Const"
local TalkActionManager_C = Class({"BluePrints.Common.TimerMgr"})
local ActionLogType = UE.EStoryLogType.TalkAction

TalkActionManager_C.New = function()
    ---@type TalkActionManager_C
    local Obj = setmetatable({}, {
        __index = TalkActionManager_C
    })
    ---@type table<TalkTaskBase_C,boolean>
    Obj.NpcPlayActionRecordMap = {}
    ---@type table<TalkTaskBase_C,boolean>
    Obj.NpcLookAtRecordMap = {}
    ---@type table<TalkTaskBase_C,table<AActor,number>>
    Obj.NpcRotateToRecordMap = {}
    ---@type table<AActor,number>
...
---@param TalkActionData TalkActionData_C
...
function TalkActionManager_C:StartLookAtImmediate(TalkActor, TargetTalkActor, NotUsingSocket)
    if (TalkActor.Mesh and TalkActor.Mesh:GetAnimInstance()) then
        local AnimInstance = TalkActor.Mesh:GetAnimInstance()
        if not AnimInstance.SetLookAtActorImmediate then return end --- 先屏蔽下
...
--- @return TalkActor, TargetYaw, LookAtedActor 返回转向角色、目标角度、被看向的角色
...
function TalkActionManager_C:FinishRotateTo(TalkActor, TargetYaw, LookAtedActor)
    ...
    if TalkActor.AbortRotation then--可能玩家调过来
...
function TalkActionManager_C:FreeTalkNpcRotateToPlayer(TalkTask, Npc, Callback, bImmediate)
    if(TalkTask.TalkTaskData.BlendInTime == -1 or not TalkTask.TalkTaskData.IsNPCTurnToPlayer or Npc:IsA(ATalkInteractiveItem)) then
        if(Callback) then
            Callback.Func(Callback.Obj)
        end
        return
...
function TalkActionManager_C:FreeTalkPlayerRotateToNpc(TalkTask, Npc, Callback, bImmediate)
    if(TalkTask.TalkTaskData.BlendInTime == -1 or not TalkTask.TalkTaskData.bIsPlayerTurnToNPC) then
        if(Callback) then
            Callback.Func(Callback.Obj)
        end
    else
...
function TalkActionManager_C:RotateOffsetNew(Key, Npc, RotDegree,Callback)
    ...
    StoryPlayable:RotateOffset(RotDegree, {StoryPlayable,
    function()
        RealCallback()
    end})
...
function TalkActionManager_C:FreeTalkNpcRotateRecover(Npc,TalkTask,TalkTaskData,Callback)
    ...
    if (TalkTaskData.BlendOutTime ~= -1) then
        if(TalkTaskData.BlendInTime == -1) then
            -- 临时处理, 从全局恢复
    ...
    if(RecordMap) then
        ...
        if(TargetRot) then
            ...
            self:RotateToTargetRotation(Npc,TargetRot,{Obj = Npc, Func = function()
                Npc:PlayDefaultAnimStartAnimation()
                if (Callback) then
                    Callback()
                end
            end},TalkTask,TalkTaskData)
        else
            if(Callback) then
                Callback.Func(Callback.Obj)
            end
