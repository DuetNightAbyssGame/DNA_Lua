...
function M:PlayRemindAnimation()
    ...
    self:BindToAnimationFinished(self.Remind,{self,function()
...
function M:GetClickButtonText()
    if (not self.QTEConfig) or (not self.QTEConfig.InputActionName) or (not self.QTEConfig.InputGamePadActionName) then
        return
    end
...
function M:OnKeyUp(MyGeometry, InKeyEvent)
    ...
    if TargetKeyName then
        if InKey.KeyName == TargetKeyName then
            self:OnInputPress()
            return UE4.UWidgetBlueprintLibrary.Unhandled()
        end
    end
...
function M:OnAnimationFinished(Animation)
    ...
    elseif Animation == self.Fail then
        if self.OnFailCallbackObj and self.OnFailCallbackFunc then
            self:SetVisibility(ESlateVisibility.Collapsed)
...
function M:OnTimeOut(bForceSuccess ,CB)
    ...
    if not self.bQteResponded then
        if bForceSuccess then
            ...
            self:BindToAnimationFinished(self.Fail,{self,function()
                CB()
            end})
            self:PlayAnimation(self.Fail)
            self:PlayQTEFailAudio()
