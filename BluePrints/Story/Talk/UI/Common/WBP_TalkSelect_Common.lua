...
function M:AddItem(InItem)
    ...
    ItemUI:Init(self, InItem, InItem.Index, ListIdx) -- 需要在Item被添加到ScrollBox，触发UseWidget的生命周期函数后，再调用Init
...
function M:ListenUpDownEvents()
    self.ParentWidget:ListenForInputAction("TalkUpSelect", EInputEvent.IE_Pressed, true, {self.ParentWidget, function()
        self:UpSelectAction()
        end})
    self.ParentWidget:ListenForInputAction("TalkDownSelect", EInputEvent.IE_Pressed, true, {self.ParentWidget, 
    function()
