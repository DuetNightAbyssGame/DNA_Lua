...
---@class WBP_Impression_Common : BP_TalkBaseUINew_C
    ...
    for i = 1, ImpressionItemNum do
        ...
        if not ImpressionDone then
            ...
            if Option then
                ...
                ImpressionItem:SetOnBeginShowCheckOrPlusUI({self, self.OnBeginShowCheckOrPlusUI})
                ImpressionItem:SetOnEndShowCheckOrPlusUI({self, self.OnEndShowCheckOrPlusUI})
                ImpressionItem:SetOnInterruptedExit({self, self.StopStoryLine})
                ImpressionItem:SetOnHovered({self, self.OnImpressionItemHovered})
                ImpressionItem:SetOnUnhovered({self, self.OnImpressionItemUnhovered})
                ImpressionItem:SetOnClicked({self, self.OnImpressionItemClicked})
            else
                ImpressionItem:OnNotWorking()
            end
        else
            --当已有印象选项(加值或检定选项)被选择过时，隐藏其他所有选项，仅显示上次选择的选项
            if i <= #SavedOptions  then
...
function WBP_Impression_Common:SwitchBindExitAnimationEvents(bBind)
    if bBind then
        self:BindToAnimationFinished(self.BtnQuit_Click, {self, self.OnExitButtonClickAnimFinished})
    else
        self:UnbindFromAnimationFinished(self.BtnQuit_Click, {self, self.OnExitButtonClickAnimFinished})
    end
end
...
function WBP_Impression_Common:SwitchBindDimensionAnimationEvents(bBind)
    ...
    if bBind then
        self.DimensionGraph:BindToAnimationStarted(self.DimensionGraph.In, {self, self.OnDetails_InStarted})
        self.DimensionGraph:BindToAnimationFinished(self.DimensionGraph.Out, {self, self.OnDetails_OutFinished})
    else
        self.DimensionGraph:UnbindFromAnimationStarted(self.DimensionGraph.In, {self, self.OnDetails_InStarted})
        self.DimensionGraph:UnbindFromAnimationFinished(self.DimensionGraph.Out, {self, self.OnDetails_OutFinished})
    end
end

function WBP_Impression_Common:SwitchBindOptionAnimationEvents(bBind)
    if bBind then
        self:BindToAnimationFinished(self.Option_In, {self, self.OnOptionInAnimationFinished})
        self:BindToAnimationFinished(self.Option_Out, {self, self.OnOptionOutAnimationFinished})
        self:BindToAnimationStarted(self.Option_Out, {self, self.OnOptionOutAnimationStarted})
        self:BindToAnimationStarted(self.Option_In, {self, self.OnOptionInAnimationStarted})
    else
        self:UnbindFromAnimationFinished(self.Option_In, {self, self.OnOptionInAnimationFinished})
        self:UnbindFromAnimationFinished(self.Option_Out, {self, self.OnOptionOutAnimationFinished})
        self:UnbindFromAnimationStarted(self.Option_In, {self, self.OnOptionInAnimationStarted})
        self:UnbindFromAnimationStarted(self.Option_Out, {self, self.OnOptionOutAnimationStarted})
    end
end
...
function WBP_Impression_Common:SwitchEnableItemEvents(Enable)
    ...
    if Enable then
        self:ListenForInputAction("TalkOption", EInputEvent.IE_Pressed, true, {self, self.OnImpressionItemPressed});
        self:ListenForInputAction("TalkOption", EInputEvent.IE_Released, true, {self, self.OnImpressionItemReleased});
    end
end
...
function WBP_Impression_Common:SwitchEnableMouseWheelEvents(Enable)
    ...
    if Enable then
        self:ListenForInputAction("TalkUpSelect", EInputEvent.IE_Pressed, true, {self, self.OnUpSelectItem})
        self:ListenForInputAction("TalkDownSelect", EInputEvent.IE_Pressed, true, {self, self.OnDownSelectItem})
    end
end
    ...
    local Callback = function()
        if (DialogueData.DialoguePanelType == "Voiceover") then
            ...
            if (DialogueData.DialoguePanelType == "AllHide") then
                self:SetVisibility(ESlateVisibility.Collapsed) -- 在 CommonTalkTask:OnTaskPlayDialogueFinished 中恢复
                self:SetTipImageHidden(true)
                self:SetTextBorderHidden(true)
                self:SwitchEnableTalkClick(false)
                self.WholeDialogueTypingFinished_Delegate:Fire(true)
...
function WBP_Impression_Common:OnOptionItemEnd(ItemIdx, SpecifyFinishType)
    ...
    self.OnOptionOutAnimationFinishedCallback = function()
...
function WBP_Impression_Common:OnUpSelectItem()
    --DebugPrint("WBP_Impression_Common:OnUpSelectItem")
...
function WBP_Impression_Common:OnDownSelectItem()
    --DebugPrint("WBP_Impression_Common:OnDownSelectItem")
...
function WBP_Impression_Common:SwitchEnableTalkClick(bEnable)
    ...
    if bEnable then
        self:ListenForInputAction("TalkClick", EInputEvent.IE_Pressed, false, {self, self.OnTalkClickPressed})
        self:ListenForInputAction("TalkClick", EInputEvent.IE_Released, false, {self, self.OnTalkClickReleased})
        self:ListenForInputAction("TalkClick", EInputEvent.IE_Pressed, false, {self, self.ActiveUI})
        self:ListenForInputAction("TalkClick", EInputEvent.IE_Released, false, {self, self.ActiveUI})
    end
end
...
function WBP_Impression_Common:OnOptionInAnimationFinished()
    ...
    -- local ImpressionAreaId = self:GetOptionImpressionAreaId(self.OptionData)
