...
function M:ShowSkipButton(bShow)
	if (bShow) then
		...
		self.Key_Skip:_ForeachChildren(function(v) v:_Reset2InitState() end)
	else
...
function M:Destruct()
	if self.AutoPlayTimer then self:RemoveTimer(self.AutoPlayTimer) end
	if self.ConfirmTimer then self:RemoveTimer(self.ConfirmTimer) end
end
