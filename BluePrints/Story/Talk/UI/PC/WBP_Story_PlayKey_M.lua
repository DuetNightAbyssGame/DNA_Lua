...
function Component:OnAutoPlayClicked()
    if not self.AutoPlayCallback then return end
    self.AutoPlayCallback.Func(self.AutoPlayCallback.Obj, not self.bAutoPlay)
end

function Component:OnSkipClicked()
    if not self.SkipCallback then return end
    -- 延迟执行，防止发生加卸载问题（https://herogames.feishu.cn/wiki/TVLjws1GDi3X04k2dSWcoJKintf）
