...
function M:ChangeToDiceThrowMode(bUseGamePad)
	...
	-- self.Key_Auto:RemoveExecuteLogic()
	...
	-- self.Key_Skip:RemoveExecuteLogic()
...
function M:BindAutoPlay_Internal(InObj, InFunc, bPress)
	if bPress then
		self.AutoPlayPressCallback = {
			Func = InFunc,
			Obj = InObj
		}
	else
		self.AutoPlayReleaseCallback = {
			Func = InFunc,
			Obj = InObj
		}
	end
...
function M:BindSkip_Internal(InObj, InFunc, bPress)
	if bPress then
		self.SkipPressCallback = {
			Func = InFunc,
			Obj = InObj
		}
	else
		self.SkipReleaseCallback = {
			Func = InFunc,
			Obj = InObj
		}
	end
