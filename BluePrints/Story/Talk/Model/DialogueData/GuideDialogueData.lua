...
GuideDialogueData_C.New = function(TalkTask, DialogueId)
    local DialogueData = DataMgr.Dialogue[DialogueId]
    if not DialogueData then
        local Message = "DialogueId在Dialogue表中不存在"..
        "\nDialogueId:"..DialogueId
        UStoryLogUtils.PrintToFeiShu(GWorld.GameInstance, UE.EStoryLogType.Talk, "GuideDialogueData创建失败：DialogueId不存在", Message)
        return 
    end
