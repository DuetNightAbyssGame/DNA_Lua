...
function M:New(Book)
    local TypingPage = {}
    for k, v in pairs(self) do TypingPage[k] = v end
...
function M:GetMaxSize() return self.Book:GetSize() end
function M:GetRemainHeight() return self:GetMaxSize().Y - self.Size.Y end
...
---从末尾开始移除N行空行，防止RetainerBox报错
function M:RemoveEmptyLinesAtLast()
    local Lenth = #self.Lines
    for i = Lenth ,1,-1 do
        local Line = self.Lines[i]
        if(Line:GetSize().Y == 0 or Line:GetSize().X == 0)then
            self.Lines[i] = nil
        end
    end
end
