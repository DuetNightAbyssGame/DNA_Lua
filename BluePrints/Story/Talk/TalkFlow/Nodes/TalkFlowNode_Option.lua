...
function M:CreateNodeData(DialogueId)
	...
	for Index, OptionId in ipairs(NextOptions) do
		local OptionDialogue = Dialogues[OptionId]
...
function M:GenerateNextNodes()
	for Index, OptionId in ipairs(self.Options) do
		...
		if self.NextDialogueMap[OptionId] then
			local NextNode = self:CreateNextNode("Dialogue", self.NextDialogueMap[OptionId])
		...
		if self.FailDialogueMap[OptionId] then
			local NextNode = self:CreateNextNode("Dialogue", self.FailDialogueMap[OptionId])
...
function M:Iterate(...)
	...
	M.Super.Iterate(self, OutPortName)
end
...
function M:CalcOutPortName(Index, FinishType)
	Index = self.OptionId2Index[Index] or Index
...
function M:Record(OptionId)
	M.Super.Record(self, OptionId)
	--防止传入的是选项序号而不是Id
