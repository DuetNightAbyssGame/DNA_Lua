-- 用于在解锁或更新词条时播放按钮动效，详情见文档https://herogames.feishu.cn/wiki/B23SwmwfYizpXtkTVbDcSAbMnmh#share-X1gzd5BGGoD2thxgkDmcnWTtntb
...
function FDialogueWikiComponent:New(TalkTask)
    local DialogueWikiComponent = setmetatable({}, {
        __index = FDialogueWikiComponent
    })
...
function FDialogueWikiComponent:OnWikiAllConditionsMet(WikiId)
	...
	if next(self.ListenedWikiList) == nil then
		self:UnRegister()
	end
end
