...
function BP_CaptureInteractiveComponent_C:IsCanInteractive(PlayerActor)
    ...
    if LuaConst.OpenComputeInteractive then
        ...
        self:CheckPlayerTag(PlayerActor) and
        CanOpen and not Owner.OpenState
    else
        return self:DistanceCheckComponent(PlayerActor, self.InteractiveDistance, false) and
        self.CFaceToACheckComponent(self, PlayerActor, self.InteractiveFaceAngle) and
        self.AFaceToCCheckComponent(PlayerActor, self, self.InteractiveAngle) and
        self:CheckPlayerTag(PlayerActor) and
        CanOpen and not Owner.OpenState
