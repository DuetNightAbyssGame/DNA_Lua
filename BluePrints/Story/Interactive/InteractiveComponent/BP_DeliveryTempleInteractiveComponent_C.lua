...
function BP_DeliveryTempleInteractiveComponent_C:IsCanInteractive(PlayerActor)
    ...
    if LuaConst.OpenComputeInteractive then
		return self:GetInteractiveName() ~= "" and
        	...
        	self.AFaceToCCheckComponent(PlayerActor, self, self.InteractiveAngle, false)
	else
		return self:GetInteractiveName() ~= "" and
...
function BP_DeliveryTempleInteractiveComponent_C:UpdateDisplayInteractiveBtn(PlayerActor)
	...
	elseif bCanInteractive == false and self:IsBtnDisplayed(PlayerActor) then
...
function BP_DeliveryTempleInteractiveComponent_C:DisplayInteractiveBtn(PlayerActor)
	...
	if not InteractiveUI then
		InteractiveUI = UIManager:LoadUINew(UIConst.InteractiveUIName)
	end
	if not InteractiveUI then
		return
	end
...
function BP_DeliveryTempleInteractiveComponent_C:EnterTemple()
	...
	if Avatar then
		...
		if bIsInTeam then
			Avatar:EnterDungeon(self.TempleId, CommonConst.DungeonNetMode.Standalone,
				function(RetCode, ...)
					local bCanEnter = self.HandleEnterDungeonRetCode(RetCode, ...)

					if bCanEnter then
						UIManager(self):LoadUINew("DungeonMatchTimingBar",
							self.TempleId, Const.DUNGEON_MATCH_BAR_STATE.SPONSOR_WAITING_CONFIRM, false)
					end
				end)
		else
...
function BP_DeliveryTempleInteractiveComponent_C:IsLocked()
	...
	if PreTempleId ~= nil and Avatar.Dungeons and not Avatar.Dungeons[PreTempleId] then
		return true
	end
	if PreTempleId ~= nil and Avatar.Dungeons and Avatar.Dungeons[PreTempleId] and not Avatar.Dungeons[PreTempleId].IsPass then
		return true
	end
	local DungeonInfo = DataMgr.Dungeon[self.TempleId]
...
function BP_DeliveryTempleInteractiveComponent_C:GetInteractiveIcon(PlayerActor)
	...
	-- if #DataMgr.Temple[self.TempleId].RewardId > 1 then
	-- 	return nil
	-- end
	-- return "Texture2D'/Game/UI/Texture/Dynamic/Atlas/Interactive/T_Interactive_Temple.T_Interactive_Temple'"
	return nil
end
...
function BP_DeliveryTempleInteractiveComponent_C:GetStars()
	...
	if self.TempleId <= 0 then return nil end
	...
	if Avatar.Dungeons[self.TempleId] then
		...
		if not TempleInfo.RewardId then
			return nil
		elseif #TempleInfo.RewardId == 1 and Avatar.Dungeons[self.TempleId].IsPass then
			-- -1表示无星级关卡通关
			return -1
		elseif #TempleInfo.RewardId == 1 and not Avatar.Dungeons[self.TempleId].IsPass then
			return -2
		else
		...
		-- 当前神庙没进过，服务器没有保存数据
		if #TempleInfo.RewardId <= 0 then
			return nil
		elseif #TempleInfo.RewardId == 1 then
			-- -2表示无星级关卡灰态
			return -2
		else
			return 0
