...
---@type BP_RegionOnlineInterComponent_C 
...
function BP_RegionOnlineInterComponent_C:CheckAndDisplayAddFriendBtn(PlayerActor, RegionInterAddFriendComp)
	...
	if Avatar then
		...
		if AvatarInfo then
			...
			local friendDict = FriendModel:GetFriendDict() -- 使用 GetFriendDict
			if not friendDict[AvatarInfo.Uid] then -- 如果目标玩家不是好友，才显示添加好友UI
...
function BP_RegionOnlineInterComponent_C:GetInteractiveName() local Avatar = GWorld:GetAvatar()
    local Name = ""
