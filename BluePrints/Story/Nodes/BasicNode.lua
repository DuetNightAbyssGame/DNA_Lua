...
-- 通过editor_config 和node_config 来生成实际的 引脚信息
...
local IsID=function(id)
    return type(id)=="number"
end

local IsName=function(id)
...
function BasicNode:IsAllExecsActivate()
    for _, exec in pairs(self.in_execs) do
        if (exec.active == false) then return false end
    end
    return true
end
