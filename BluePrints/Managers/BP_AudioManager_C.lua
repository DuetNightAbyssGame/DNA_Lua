...
-- Find Sound with Object and Key and Set Parameter
function BP_AudioManager_C:SetEventSoundParam(Object, Key, KeyValueGroups)
    ...
    for key,value in pairs(KeyValueGroups) do
        ...
        -- DebugPrint("LogEMAudioManager SetEventSoundParam", ObjName, Key, key, value)
    end
...
function BP_AudioManager_C:GetGender(IsEXGender)
    ...
    if IsEXGender == true then
        if not Avatar or Avatar.WeitaSex == 0 then
            return "_m"
        elseif Avatar.WeitaSex == 1 then
            return "_f"
        end
    else
        if not Avatar or Avatar.Sex == 1 then
            return "_f"
        elseif Avatar.Sex == 0 then
            return "_m"
        end
    end
end
...
---@param Behavior string Click|Drag|Equip|Unequip
