...
--function BP_CircularElevator:UserConstructionScript()
--end

function BP_CircularElevator:ReceiveBeginPlay()
    self.Overridden.ReceiveBeginPlay(self)
    --self.Diban.ChildActor = Actor
    self:AddTimer(1,self.Init)

    -- self.ElevatorQueue = TArray(UObject)
    ...
    -- self.EndDown = FVector(self.BenchMark.X,self.BenchMark.Y,self.BenchMark.Z)
    -- self.TopDown = FVector(self.BenchMark.X,self.BenchMark.Y,self.BenchMark.Z+7650)
...
--function BP_CircularElevator:ReceiveEndPlay()
--end

 function BP_CircularElevator:ReceiveTick(DeltaSeconds)
    self.Overridden.ReceiveTick(self,DeltaSeconds)
    -- print(_G.LogTag, "LXZ ReceiveTick", self.OnElevatorMove, self.OnDoorOpen, self.OnDoorClose, self:GetName())
    ...
    if self.RecoverCollisionFrameNum == 1 then
    ...
    if self.OnElevatorMove and not self.OnDoorOpen and not self.OnDoorClose and self.RecoverCollisionFrameNum == -1 then
        -- print(_G.LogTag, "LXZ ", self:GetName(), self.ElevatorQueue:Length(), self:GetLocalRole())
...
function BP_CircularElevator:RealMoveLua(ElevatorId)
    --print(_G.LogTag, self.Diban.ChildActor.ElevatorInteractiveComponent:GetName())
    if (IsAuthority(self) ) then
...
function BP_CircularElevator:ClientCloseDoor(Door, Direction, DeltaSeconds)
    ...
    if self.OnElevatorMove == true then
        if not IsAuthority(self) or IsStandAlone(self) then
            self:OnStartMove()
            -- self.OtherElevator:OnStartMove()
        end
        if self.Direction == 1 then
            self:OnElevatorStartDown()
            -- self.OtherElevator:OnElevatorStartDown()
        else
            self:OnElevatorStartUp()
            -- self.OtherElevator:OnElevatorStartUp()
        end
        --起点和终点不一定在一个平面，整体移动
        -- self:ChangeElevatorLoc(1 - self.Direction, true)
...
function BP_CircularElevator:ChangeElevatorLoc(MovePlayer)
    -- if OverLapActor and not OverLapActor:IsPlayer() then
    --     return
    -- end
    ...
    if MovePlayer then
        ...
        -- UE4.UKismetSystemLibrary.DrawDebugBox(self, Transform.Translation, Extent, UE4.FLinearColor(1,0,0,1), Transform.Rotation:ToRotator(), 100)
        ...
        local function RealMovePlayer()
            ...
            -- Player.CharacterMovement:SetMovementMode(3, 0)
...
function BP_CircularElevator:RealMoveElevator(DeltaSeconds, Direction)
    if Direction == 0 then
        ...
        -- self.OtherElevator:MoveElevatorUp(self.OtherElevator.ElevatorQueue[1], DeltaSeconds, false)
    else
        self:MoveElevatorDown(self.ElevatorQueue[self.ElevatorQueue:Length()], DeltaSeconds, self.MovePlayer)
        -- self.OtherElevator:MoveElevatorDown(self.OtherElevator.ElevatorQueue[self.OtherElevator.ElevatorQueue:Length()], DeltaSeconds, false)
    end
end
...
function BP_CircularElevator:MoveElevatorUp(Elevator, DeltaSeconds, MovePlayer)
    ...
    -- print(_G.LogTag,"LXZ MoveElevatorUp000", Current, Elevator:GetName(), self.CurrentDis, self.stage1, self.stage2)
    --保持循环运行，以最下方一节的坐标为基准，超过容许范围则移到最上面
    ...
    elseif self.stage1 == true and self.stage2 == false then
        --匀速状态，仅两节运行，门在最上方
        local Target = UE4.UKismetMathLibrary.VInterpTo_Constant(Current, self.EndUp, DeltaSeconds, self.CurrentSpeed)
        local Target2 = FVector(Target.X,Target.Y,Target.Z+self.SingleHeight)
        self.CurrentDis = self.CurrentDis + Current.Z - Target.Z
    ...
    elseif self.stage2 == true then
        --减速状态，理想情况是门从上往下减速到0，刚好停在要求位置，合理状态从下往上应该是墙壁-门-墙壁
        --self.FixFinish==true 位置修复完成，应直接进入开门流程
        local Tmp = self.DiantiA:K2_GetComponentLocation()
        local Target,Target2,Target3
        --帧率过低时，最下面一节有可能会是门本身，此时坐标计算会出问题，直接强制设置位置并进入开门流程
        if Elevator == self.DiantiA then
            self.CurrentSpeed = 0
            self.FixFinish = true
        ...
        if self.CurrentSpeed > 0 then
            ...
            --结束位置不合法，用一个很慢的速度逼近
            if Tmp.Z >= self.BenchMark.Z-self.UnderFloor+1 or Tmp.Z <= self.BenchMark.Z-self.UnderFloor-1 and not self.FixFinish then
                --self.FixDirection   1:往上  2：往下  0：初始状态
                if Tmp.Z <= self.BenchMark.Z-self.UnderFloor-1 then
                    if self.FixDirection == 0 then
                        self.FixDirection = 1
                    end
                    if self.FixDirection == 2 then
                        self.FixFinish = true
                ...
                -- self.OtherElevator:OpenDoorOnEndMove(MovePlayer)
            end
        end
    end
end
...
function BP_CircularElevator:MoveElevatorDown(Elevator, DeltaSeconds, MovePlayer)
    ...
    if self.stage1 == false then
        ...
        -- print(_G.LogTag,self.stage1,  self.stage2,Elevator:GetName(), self.TopUp)
    elseif self.stage1 == true and self.stage2 == false then
        local Target = UE4.UKismetMathLibrary.VInterpTo_Constant(Current, self.TopUp, DeltaSeconds, self.CurrentSpeed)
        local Target2 = FVector(Target.X,Target.Y,Target.Z-self.SingleHeight)
        self.CurrentDis = self.CurrentDis - Current.Z + Target.Z
...
function BP_CircularElevator:BoxOverLap(Player)
    --玩家离开时，会先触发此函数，再触发GoOut，因此需要PlayerIn来处理
    if self.NowOpenDoorNum == 0 and not (self.OnElevatorMove and not self.OnDoorOpen and not self.OnDoorClose) then
...
function BP_CircularElevator:BoxEndOverLap(Player)
    ...
    if self.NowOpenDoorNum == 0 and self.NowMoveElevatorNum == 0 then
...
function BP_CircularElevator:OpenPlayerPosSync()
    ...
    if not Avatar or self.OnElevatorMove then return end
...
function BP_CircularElevator:RegisterComponent(CompArray)
    -- print(_G.LogTag,"LXZ RegisterComponent",self:GetName())
