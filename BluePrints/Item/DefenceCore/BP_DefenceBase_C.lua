...
-- function BP_DefenceBase_C:ShowDamage(DamageEvent)
--     BP_DefenceBase_C.Super.ShowDamage(self, DamageEvent)
...
--     if AnimInstance:Montage_GetCurrentSection(Montage) == "Idle" then
--         AnimInstance:Montage_JumpToSection("Hit", Montage)
--         -- self:PlayMontage(self.Mesh, self.MontagePath, "Hit", {OnNotifyEnd = self.SetCanPlayHitMontage})
--     end
-- end
...
function BP_DefenceBase_C:ActiveDefence()
    ...
    if self.Mesh then
        --self:PlayMontage(self.Mesh, self.MontagePath, "Start", {OnCompleted = function() self:EMActorDestroy(EDestroyReason.MechanismDead) end}, true)
        self:PlayActiveAnim()
    end
    self.CombatClientEffectComponent:OnActiveEffect()
...
function BP_DefenceBase_C:ShowDeath()
    ...
    -- end
    -- local Montage = AnimInstance:GetCurrentActiveMontage()
    --AnimInstance:Montage_JumpToSection("Dead", Montage)
    self:PlayDeadAnim()
end
...
function BP_DefenceBase_C:OnDead(KillMineRoleEid, KillMineSkillId, DeathReason)
    ...
    if IsStandAlone(self) or IsClient(self) then
        ...
        if DefenceUIName == "DungenonDefenseFloat" or DefenceUIName == "DungenonDefenseProFloat" then
