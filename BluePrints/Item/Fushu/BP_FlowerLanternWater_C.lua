...
-- function BP_FlowerLanternWater_C:CombatPropActive(Info)
--     BP_FlowerLanternWater_C.Super.CombatPropActive(self, Info)
-- end

-- function BP_FlowerLanternWater_C:CombatPropDeActive(Info)
--     BP_FlowerLanternWater_C.Super.CombatPropDeActive(self, Info)
-- end
...
function BP_FlowerLanternWater_C:OnStartTough(Component, OtherActor, OtherComp)
    if OtherActor.IsPlayer and OtherActor:IsPlayer() then
        -- DebugPrint("zwkk 进入灯花范围", OtherActor:GetName())
        ...
        -- if self.IsActive and not self.HasRestore then
        --     if self.IsRedLight and self.SkillEffectHp and self.SkillEffectHp > 0 then
        --         self.Super.PropUseSkill(self, self.SkillEffectHp, OtherActor)
        --     elseif not self.IsRedLight and self.SkillEffectSp and self.SkillEffectSp > 0 then
        --         self.Super.PropUseSkill(self, self.SkillEffectSp, OtherActor)
        --     end
