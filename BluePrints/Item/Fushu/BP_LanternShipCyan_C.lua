...
function M:CommonInitInfo(Info)
    ...
    -- self.RelatedCreatorId = self.UnitParams["RelatedCreatorId"]
...
function M:DeActiveCombat()
    ...
    self:ResetColor()
    -- 因特殊暂停导致的DeActive，只改变颜色相关，不触发buff检查，对抗buff效果依然有效，所以把IsActive设成true，不影响其他判断
...
function M:OnEnterState(NowStateId)
    ...
    elseif NowStateId == self.MovingStateId then
        ...
        if not Player:IsDead() then
            ...
            -- 另外从开始无由生扣血开始，持续检测无由生的血量变化，当低于阈值再高于时，判断为回血了，把圈扩回去
