...
-- function BP_PrologueDoor:ReceiveBeginPlay()
--     BP_PrologueDoor.Super.ReceiveBeginPlay(self)
...
function BP_PrologueDoor:InitComponent(Info)
    ...
    if self:GetCurrentDoorType() == 0 then
...
function BP_PrologueDoor:OnActorReady(Info)
    ...
    if self.CurrentDoorTypeStateId ~= 0 and not StateTable and not DungeonStateTable and IsAuthority(self) then
        self:ChangeState("Manual", 0, self.CurrentDoorTypeStateId)
    end
    if self:GetCurrentDoorType() == 0 or self:GetCurrentDoorType() == 3 then
        --可能有玩家直接出生在开门范围的情况，初始化完成后检测一下
...
function BP_PrologueDoor:StartWait(Actor)
    ...
    if CurrentDoorType == 0 or CurrentDoorType == 3 then
        --等价于self.DoorOpenState = true
...
function BP_PrologueDoor:TriggerByChild(SourceEid)
    ...
    if not MiniGame then
        local GameState = UE4.UGameplayStatics.GetGameState(self)
        local StaticCreator = GameState.StaticCreatorMap:Find(self.GameId)
        -- if StaticCreator and StaticCreator.ChildEids:Length() > 0 then
        --     self.MiniGameEid = GameState.StaticCreatorMap:Find(self.GameId).ChildEids:GetRef(1)
        -- end
...
function BP_PrologueDoor:GetGuidePos()
    if self.DoorTypeStateId:Find(self.StateId)==1 then
        local GameState = UE4.UGameplayStatics.GetGameState(self)
...
-- function BP_PrologueDoor:GetMiniGamePos()
--     local MiniGame = Battle(self):GetEntity(self.MiniGameEid)
--     if self.CreateGame then
--         if IsValid(MiniGame) then
--             return MiniGame:K2_GetActorLocation()+FVector(0,0,160)
--         else
--             local GameState = UE4.UGameplayStatics.GetGameState(self)
--             return GameState.StaticCreatorMap:Find(self.GameId):K2_GetActorLocation()+FVector(0,0,160)
--         end
--     else
--         return self:K2_GetActorLocation()+self.GuidePos.RelativeLocation
--     end
...
function BP_PrologueDoor:CreateRegionData()
    self.RegionData = {
        -- CurrentDoorType = self.CurrentDoorType,
...
function BP_PrologueDoor:InitMiniGameState(NowStateId)
    ...
    --print(_G.LogTag,"LXZ InitMiniGameState", NowStateId, MiniGame.StateId, MiniGame.FiniStateId, MiniGame.DeActiveStateId)
