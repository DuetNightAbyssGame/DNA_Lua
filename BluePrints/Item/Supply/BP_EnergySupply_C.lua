...
-- 生存长按提示Toast相关变量
local bIsToastShowed = false -- bool 本轮内toast有没有显示过，有的话就不再显示了
local WaitToShowToastTime = DataMgr.GlobalConstant["SurvivalProToastTime"]["ConstantValue"] -- 进入范围内后多久显示toast
local ToastTime = 15 -- Toast持续时间
local ShowToastDistance = DataMgr.GlobalConstant["SurvivalProRadius"]["ConstantValue"] -- 这个距离内开启定时器
...
function BP_EnergySupply_C:AuthorityInitInfo(Info)
    ...
    self:AddBuffManager()
    -- self.BuffManager:AddAllowBuffType(50)
end
...
function BP_EnergySupply_C:CommonInitInfo(Info)
    ...
    self.BuffNum = 0
    self.BuffFXNum = 0
    self.BuffFXIds = {900016, 900019, 900017, 900018}
    self.BuffFX = {}
    ...
    -- self.TutorialEnergy = DataMgr.GlobalConstant["SurvivalProTutorialEnergy"]["ConstantValue"]
...
function BP_EnergySupply_C:ClientInitInfo(Info)
    ...
    -- self:AddTimer(0.1, BP_EnergySupply_C.FindTarget_Client, true)
...
function BP_EnergySupply_C:PlayAnim(PlayerId, InteractiveState, MechanismEid)
    ...
    if InteractiveState == 2 then
        --客户端的交互组件可用性也要打开，懒得写新的rpc了，放这用用
        self.DefaultInteractiveComponent.bCanUsed = false
        if PlayerCharacter:IsPlayer() then
...
function BP_EnergySupply_C:FindTarget()
	...
	for i = 0 ,GameMode:GetPlayerNum() - 1 do
        ...
        if Dis<=self.Radius and self:CheckInSameLevel(Player) then
            FindPlayer = true
            --在self.Radius和self.GuideRadius之间时，只显示自身指引；小于self.GuideRadius时，不显示所有指引
            if not self.HasAllEnergyGuideCloseLua[Player.Eid] then
                ...
                -- print(_G.LogTag,"LXZ FindTarget111", Player:GetName(), Player.Eid, self.GuideRadius)
            ...
            elseif Dis>=self.GuideRadius and (self.HasEnergyGuideCloseLua[Player.Eid] or self.HasEnergyGuideCloseLua[Player.Eid] == nil) then
                -- print(_G.LogTag,"LXZ FindTarget222", Player:GetName(), Player.Eid)
            ...
            elseif Dis<self.GuideRadius and not self.HasEnergyGuideCloseLua[Player.Eid] then
                -- print(_G.LogTag,"LXZ FindTarget333")
                self:TryDeactiveGuide(Player.Eid)
                self.HasEnergyGuideCloseLua[Player.Eid] = true
            end
        else
            if self.HasAllEnergyGuideCloseLua[Player.Eid] then
                -- print(_G.LogTag,"LXZ FindTarget", Player:GetName(), Player.Eid)
...
-- function BP_EnergySupply_C:FindTarget_Client()
--     local Player = UE4.UGameplayStatics.GetPlayerCharacter(self,0)
--     if Player and self.BuffId then
--         local Dis = self:CalDistance(Player)
--         -- DebugPrint("FindTarget_Client Dis = ",Dis,self:GetName(),self.Target[Player.Eid],self.Radius,self:CheckInSameLevel(Player))
--         if Dis < self.Radius and self:CheckInSameLevel(Player)then
--             if self.Target[Player.Eid] == nil then
--                 if self:OnPlayerIn(Player.Eid, self.BuffId) then
--                     self.Target[Player.Eid] = Player
--                 end
--             end
--         else
--             if self.Target[Player.Eid] then
--                 self:OnPlayerLeft(Player.Eid)
--                 self.Target[Player.Eid] = nil
--             end
--         end
--         self:CheckPlayerInShowToastDis(Dis,Player.Eid)
--     end
...
function BP_EnergySupply_C:StartRecover()
	...
	for i = 0 ,GameMode:GetPlayerNum() - 1 do
        ...
        if AllGuideClose ~= false then
            self:TryActiveAllGuide(nil, Player.Eid)
            self.HasAllEnergyGuideCloseLua[Player.Eid] = false
        end
    end
    -- self.BuffHandle = self:AddTimer(0.1, self.FindTarget, true)
...
function BP_EnergySupply_C:OutRecover_Lua(Monster)
    ...
    elseif Monster.EnergySupplyCheck ~= nil and Monster.EnergySupplyCheck ~= -1 then
    ...
    if Dis < self.Radius then
        ...
        if self:CheckCanChangeEnergy() and (not IsAuthority(self) or IsStandAlone(self)) then
            ...
            local FXObject = Player.FXComponent:PlayEffectByIDParams(FXID, {
                ...
                Location = {MonsterLoc.X,MonsterLoc.Y,MonsterLoc.Z + Monster.OriginHalfHeight},
                Rotation = {MonsterRot.Pitch, MonsterRot.Yaw, MonsterRot.Roll}
            })
            if FXObject then
            ...
            -- Player.FXComponent:AddFxObjectTag(FXObject, tostring(FXID))
        end
        Monster.EnergySupplyCheck = -1
    end
...
function BP_EnergySupply_C:EnergyToSurvival()
    ...
    if GameMode then
        ...
        if self.NowEnergy == 0 or GameMode.EMGameState.SurvivalValue >= self.MaxSurvivalValue then
            ...
            --self:TriggerBluePrintEvent("OnForceEndInteractive")
...
function BP_EnergySupply_C:OnForceEndInteractive_Lua(SurvivalValue,MaxSurvivalValue)
    ...
    if not IsAuthority(self) or IsStandAlone(self) then
        if SurvivalValue >= MaxSurvivalValue then
...
-- function BP_EnergySupply_C:PlayActiveMontage()
...
-- function BP_EnergySupply_C:PlayStopMontage()
--     local AnimInstance = self.Mesh:GetAnimInstance()
--     local Montage = AnimInstance:GetCurrentActiveMontage()
--     local SectionName = AnimInstance:Montage_GetCurrentSection(Montage)
--     if Montage and SectionName ~= "Stop" and SectionName ~= "StopIdle" then
...
function BP_EnergySupply_C:CheckInSameLevel(Actor)
    ...
    return ActorLevelId~=-1 and ActorLevelId == SupplyLevelId
end
...
function BP_EnergySupply_C:CheckActorLevel(Actor)
    ...
    -- local LevelNum = 0
    -- local LevelId = -1
    -- for _,id in pairs(LevelLoader.homeLevel2ID) do
    --     if LevelLoader:CheckLocationInGridframeByLevelId(id,Actor:K2_GetActorLocation()) then
    --         LevelNum = LevelNum + 1
    --         LevelId = id
    --     end
    -- end
...
-- function BP_EnergySupply_C:OnForceEndInteractive()
-- 	self.EnergySupplyBarComponent:OnForceEndInteractive()
-- end
...
function BP_EnergySupply_C:OnLiquidChange(Liquid)
    ...
    if Sub < 0 then
        -- if IsValid(self.BuffFX[self.LiquidFXNum]) then
        --     self.BuffFX[self.LiquidFXNum]:K2_DestroyComponent(self.BuffFX[self.LiquidFXNum])
        -- end
        self["BottleFull"..self.LiquidFXNum](self, false)
        self.BuffFX[self.LiquidFXNum] = nil
    elseif Sub > 0 then
        -- self.BuffFX[NewNum] = self.FXComponent:PlayEffectByID(self.BuffFXIds[NewNum] or 900018)
        self["BottleFull"..NewNum](self, true)
    end
	if Liquid == self.MaxEnergy and not self.MaxEnergyFX then
        self.MaxEnergyFX = {}
        -- self.MaxEnergyFX[1] = self.FXComponent:PlayEffectByID(900014)
        -- self.MaxEnergyFX[2] = self.FXComponent:PlayEffectByID(900015)
        self["BottleAllFull"](self, true)
    elseif Liquid < self.MaxEnergy and self.MaxEnergyFX then
        -- if IsValid(self.MaxEnergyFX[1]) then
        --     self.MaxEnergyFX[1]:K2_DestroyComponent(self.MaxEnergyFX[1])
        -- end
        -- if IsValid(self.MaxEnergyFX[2]) then
        --     self.MaxEnergyFX[2]:K2_DestroyComponent(self.MaxEnergyFX[1])
        -- end
...
function BP_EnergySupply_C:RealShowToast()
    ...
    if UIManager then
        -- UE4.UUIStateAsyncActionBase.ShowGuideToast(GameInstance, 100011, ToastTime)
        UResourceLibrary.LoadClassAsync(GameInstance, UIConst.GuideTextFloat,
            {
                GameInstance, function(_, ClassObject)
...
function BP_EnergySupply_C:PhantomInteractive(PhantomEid)
    ...
    if not Phantom:IsPhantom() or self.NowPlayerEid ~= 0 then
        return
    end
...
-- --因为lua添加了EffectSourceInterface，需要重写这个方法，不然EffectSourceInterface会覆盖父类方法
-- function BP_EnergySupply_C:GetObjType()
