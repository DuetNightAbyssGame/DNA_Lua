...
function M:InitClanCreator()
    ...
    -- 读取群落数据表ClanManager
    -- local ClanManagerInfo = DataMgr.ClanManager[self.ClanManagerId]
    -- if ClanManagerInfo == nil then
    --     DebugPrint("群落数据表读取失败，Id", self.ClanManagerId)
    --     return
    -- end
    -- self.MonsterSpawnIds = TArray(0)
    -- for k, MSId in pairs(ClanManagerInfo.MonsterSpawnIds) do
    --     self.MonsterSpawnIds:Add(MSId)
    -- end
    -- self.CommonAlertDisable = ClanManagerInfo.CommonAlertDisable or false
...
function M:GetAlertMechanism(Monster)
    ...
    for Eid, MiniGame in pairs(self.AlertMiniGameMap) do
        ...
        if Dis ~= -1 then
            if Dis<MinDis or Result == nil then
                MinDis = Dis
                Result = MiniGame
            end
        end
