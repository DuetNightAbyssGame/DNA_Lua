...
function BP_CrackChest_C:AuthorityInitInfo(Info)
    ...
    self:CreateMain(self.MainBody, {UnitType = "Mechanism", UnitId = self.MainId})
    self:CreateCrack({UnitType = "Mechanism", UnitId = self.MechId})
    self.CanOpen = false
...
function BP_CrackChest_C:TriggerByChild(ChildEid)
    if ChildEid == self.Main.Eid then
        ...
        if self.CrackMap:Length() == 0 then
            --等价于self.Main.CanOpen = true
