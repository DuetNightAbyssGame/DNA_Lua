...
function BP_ElectricGrid_C:OnActorReady(Info)
    ...
    if self.CurrentTypeStateId ~= 0 and IsAuthority(self) then
        self:ChangeState("Manual", 0, self.CurrentTypeStateId)
    end
end

-- function BP_ElectricGrid_C:ReceiveBeginPlay()
--     BP_ElectricGrid_C.Super.ReceiveBeginPlay(self)
-- end

function BP_ElectricGrid_C:ReceiveTick(DeltaSeconds)
    if not self.IsActive then return end
    if not self.HitedArray then return end
...
function BP_ElectricGrid_C:TrySetVariable()
    if not self.Start then
        ...
        -- self.Rays = {}  -- 粒子组件集合
        self.Lasers = {}  -- Laser集合
        self.HitedArray = {}  -- {激光:{}} 每根激光单独计算
        self:GetComponents()
    end
end

-- function BP_ElectricGrid_C:ActiveCombat()
--     self.IsActive = true
-- end

-- function BP_ElectricGrid_C:DeActiveCombat()
--     self.IsActive = false
-- end
...
function BP_ElectricGrid_C:UpdateHitedArray(DeltaSeconds)
    for i, v in pairs(self.HitedArray) do
        for Id, Value in pairs(v) do
            -- DebugPrint("zwk ", i:GetName(), Id, Value)
...
function BP_ElectricGrid_C:OnHitTarget(Port, HitResult)
    -- DebugPrint("zwkkk 电网机关OnHitTarget ",  IsAuthority(self), IsClient(self), IsStandAlone(self))
    if(HitResult.Actor) then
        if HitResult.Actor:IsDead() ~= true and not self.HitedArray[Port][HitResult.Actor.Eid] then
