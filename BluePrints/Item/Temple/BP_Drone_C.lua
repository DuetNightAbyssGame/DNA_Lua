...
function M:TryFindPlayer(Player)
    ...
    --local bHitHead = UE4.UKismetSystemLibrary.LineTraceSingle(self,EyeLcoation,EndPositionHead + (EndPositionHead - EyeLcoation)*5,ETraceTypeQuery.TraceScene,false,nil,2,HitResultHead,true)
    ...
    -- if bHitHead then
    --     DebugPrint("zwkk bHitHead", HitResultHead.Actor:GetName())
    -- end
    local HitResultFoot = FHitResult()
    --local bHitFoot = UE4.UKismetSystemLibrary.LineTraceSingle(self,EyeLcoation,EndPositionFoot + (EndPositionFoot - EyeLcoation)*5,ETraceTypeQuery.TraceScene,false,nil,2,HitResultFoot,true)
    ...
    -- if bHitFoot then
    --     DebugPrint("zwkk bHitFoot", HitResultFoot.Actor:GetName())
    -- end
    ...
    -- end
    --local OverlappingPlayers = self.Cone:GetOverlappingActors()
    if (not bHitHead or not bHitFoot) and not self:CheckPlayerInBox(Player) then
        -- 说明有一根射线打到了玩家，并且玩家不在箱子状态，表明发现玩家
...
function M:ReceiveTick(DeltaSeconds)
    ...
    if not self.FoundPlayer or not self.OverlappingPlayer then
        -- 未发现玩家且玩家离开了触发盒，先检测玩家在不在保底范围内，在的话要继续转向玩家，但警戒值会持续减少
    ...
    -- 以下是自身发现了玩家会执行的部分，会逐渐增加警戒值并转向玩家
    -- 增加警戒值
...
function M:RotateToAim(DeltaSeconds, Player)
    -- local SelfLocation = self.Mesh:K2_GetComponentLocation()
