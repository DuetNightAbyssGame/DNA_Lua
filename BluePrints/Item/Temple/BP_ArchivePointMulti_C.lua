...
function M:OnPlayerEnter(Player)
    ...
    if IsAuthority(self) or IsStandAlone(self) then
        ...
        if GameModeComp ~= nil then
            ...
            true, 0, "OnPlayerTriggerArchivePointMulti", true)
        end
    end
...
function M:CheckShouldToast()
    ...
    if GameState then
        ...
        if not GameState.ArchivePointMap or GameState.ArchivePointMap:Length() < 1 then return true end
        local MinArchiveID = 99
        local MaxArchiveID = 1
        for i, v in pairs(GameState.ArchivePointMap) do
            if v.ArchiveID > MaxArchiveID then
                MaxArchiveID = v.ArchiveID
            end
            if v.ArchiveID < MinArchiveID then
