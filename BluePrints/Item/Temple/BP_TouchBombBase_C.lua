...
function M:TriggerEnter(PlayerActor)
    ...
    if not self.ShouldDirection then return end
...
function M:ShowArrowDirection()
    ...
    if self.ShouldShowDirection and self.Scene and self.Player then
        -- local LookAtRotation = self:CalcRotation()
        -- self.Scene:K2_SetWorldRotation(LookAtRotation, false, nil, false)
...
function M:OnInteractived()
    -- 交互后触发
    -- DebugPrint("zwkk OnInteractived", self.Owner, self.Owner.OnBombInteractived)
