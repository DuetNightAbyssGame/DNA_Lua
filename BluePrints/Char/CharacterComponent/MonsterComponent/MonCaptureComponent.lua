...
function Component:InitInteractiveComponentImpl()
    ...
    if self:HasAnyTags_Table(self, {"Mon.Capture",}, false) then
        self:AddInteractive()
...
function Component:OnTimerBegin(TimerHandleName)
	if TimerHandleName ~= "CaptureLimitTimer" then
		return
	end
...
function Component:CaptureSuccess(PlayerId)
    ...
    --     if self.FinishFXParam.Param and self.FinishFXParam.Param.IsAttached == false then
	...
	if self:GetLocalRole() == 2 then
		...
		if (DungeonCaptureFloat == nil) then
			return
		end
...
function Component:WaitForCapture()
    ...
    -- 联机判断isClient或者isSimulatedProxy都可以
    ...
    if InteractiveState == 0 then
        ...
        if not IsAuthority(self) or IsStandAlone(self) then
            ...
            if self.MonsterFXParam and self.MonsterFXParam.Id then
                if self.MonsterFXParam.Param and self.MonsterFXParam.Param.IsAttached == false then
...
function Component:CaptureMonsterRecover()
    if IsAuthority(self) then
        ...
        --if IsStandAlone(self) and self:GetAttributesSet() then
        --    self:GetAttributesSet():OnRep_Hp()
        --end
    end
    self:SetDead(false, nil, nil, nil)
end

-- 给音效做的，第一次遇到捕获怪，给GameMode发事件
