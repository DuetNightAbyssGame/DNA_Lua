...
function Component:SyncLocation_Lua(ActorLoc, ActorRot, CurVel, Acceleration, MovementMode)
    local SyncInfo = {}
    SyncInfo.Type = "Move"
    SyncInfo.Location = {X = ActorLoc.X, Y = ActorLoc.Y, Z = ActorLoc.Z}
    SyncInfo.Rotation = {Pitch = ActorRot.Pitch, Yaw = ActorRot.Yaw, Roll = ActorRot.Roll}
    SyncInfo.Velocity = {X = CurVel.X, Y = CurVel.Y, Z = CurVel.Z}
    SyncInfo.Acceleration = {X = Acceleration.X, Y = Acceleration.Y, Z = Acceleration.Z}
    ...
    -- SyncInfo.ClientTime = UE4.UGameplayStatics.GetTimeSeconds(self)
    -- PrintTable({ZJYRegionSendSyncInfo = SyncInfo}, 10)
...
function Component:PackSyncInfo(MoveInfo, MainPlayer)
    ...
    --print(_G.LogTag, "[RegionOnline] DoRegionCacheAction  ReceiveMoveInfo",Acceleration, CurVel, TimeUtils:NowTime(), TimeStamp, TimeUtils:NowTime() - TimeStamp)
...
function Component:UpdateCharacterMoveInfo(MoveInfo)
    ...
    local Acceleration = FVector(MoveInfo.Acceleration.X, MoveInfo.Acceleration.Y, MoveInfo.Acceleration.Z)
    local MovementMode = MoveInfo.MovementMode
    -- PrintTable({ZJYRegionReceivesyncInfo = MoveInfo}, 4)
    ...
    -- PrintTable({ZJYRegionReceiveSyncInfo = MoveInfo}, 10)
...
function Component:SendPrepareInfoNew_Lua(FeatureObj)
    ...
    Avatar:SendSyncInfo(SyncInfo,ActionBaseInfo)
    -- local ClassName = SyncInfo.ClassName
    -- local FeatureClass = nil
    -- -- PrintTable({ZJYRegionReceivesyncInfo = MoveInfo}, 4)
    -- if(UE4["U"..ClassName]) then
    --     FeatureClass = UE4["U"..ClassName]:StaticClass()
    -- end
    -- self:ReceivePrepareInfoNew(FeatureClass, SyncInfo)
end
...
function Component:SendPrepareInfo_Lua(ClassName, IntMap, FloatMap, VectorMap, RotatorMap, EnumMap)
    ...
    PrintTable({ZJYRegionReceivesyncInfo = SyncInfo.EnumMap}, 4)
    SyncInfo.VectorMap = {}
    for k, v in pairs(VectorMap) do
        SyncInfo.VectorMap[k] = {X = v.X, Y = v.Y, Z = v.Z}
    end
    SyncInfo.RotatorMap = {}
    for k, v in pairs(RotatorMap) do
        SyncInfo.RotatorMap[k] = {Pitch = v.Pitch, Yaw = v.Yaw, Roll = v.Roll}
    end
    local Avatar = GWorld:GetAvatar()
    if not Avatar then
        return
...
function Component:ReceiveHideInfo_Lua(MoveInfo)
    ...
    if not (self:CharacterInTag("Slide")) and MoveInfo.ActionBaseInfo then
        ...
        local IsCrouching = (CrouchInt ~= nil and CrouchInt > 0.1)
...
function Component:ReceivePrepareInfo_Lua(MoveInfo)
    ...
    -- PrintTable({ZJYRegionReceivesyncInfo = MoveInfo}, 3)
