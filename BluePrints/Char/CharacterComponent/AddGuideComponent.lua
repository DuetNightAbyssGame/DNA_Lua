...
function Component:TryToAddGuide()
    ...
    for i, Player in pairs(GameMode:GetAllPlayer()) do
        ...
        if not self.ShowGuideDistance or Distance < self.ShowGuideDistance then
            ...
            GameMode:TriggerDungeonComponentFun("OnMonsterGuideAdded", self.Eid)  -- 注意保序，必须在AddGuideEid之前
...
function Component:TryToAddRangeGuide(Type)
	...
	for i, Player in pairs(GameMode:GetAllPlayer()) do
        ...
        if self.ShowGuideMinDistance < Distance and  Distance < self.ShowGuideMaxDistance then
            if Type == "in" and self.GuideType ~= "inAdd" then
                self:AddGuideEid(self.Eid, SceneManager, GameState, Type)
            elseif Type == "out" and self.GuideType ~= "outRemove" then
                self:RemoveGuideEid(self.Eid, SceneManager, GameState, Type)
            end
        else
            if Type == "in" and self.GuideType ~= "inRemove" then
                self:RemoveGuideEid(self.Eid, SceneManager, GameState, Type)
            elseif Type == "out" and self.GuideType ~= "outAdd" then
                self:AddGuideEid(self.Eid, SceneManager, GameState, Type)
            end
        end
        return
