...
function Component:GM_PlayerWin(Args)
    ...
    GameMode:TriggerPlayerWin({self.AvatarEidStr}, {Eid})
end
...
function Component:GM_PlayerFailed(Args)
    ...
    GameMode:TriggerPlayerFailed({self.AvatarEidStr})
end
...
function Component:PrintGameModeInfo()
    ...
    GameMode:TriggerDungeonComponentFun("Print"..GameMode.EMGameState.GameModeType.."DebugInfo")   -- 打印各个玩法组件的信息，以后有需要再补

    local Player = UE4.UGameplayStatics.GetPlayerCharacter(GWorld.GameInstance, 0)
    local LevelName = GameMode:GetActorLevelName(Player)
    DebugPrint("Player_LevelName", LevelName)
end
-- GM_PrintLevelDebugInfo End --------------
