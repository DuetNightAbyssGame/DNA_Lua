...
--function BP_EMPlayerState_C:UserConstructionScript()
--end
...
function BP_EMPlayerState_C:DungeonOtherPlayerChange(bIsIn, EndReason)
	...
	if IsClient(self) and not bIsTeammate then 
        if EndReason == EEndPlayReason.RemovedFromWorld or EndReason == EEndPlayReason.Quit then
...
-- function BP_EMPlayerState_C:ReceiveTick(DeltaSeconds)
-- end

--function BP_EMPlayerState_C:ReceiveActorBeginOverlap(OtherActor)
--end

--function BP_EMPlayerState_C:ReceiveActorEndOverlap(OtherActor)
--end
...
function BP_EMPlayerState_C:OnRep_TeamRecoveryState(PrevRecoveryState)
    ...
    -- elseif self.TeamRecoveryState == UE4.ETeamRecoveryState.IsWaitingRecover and PrevRecoveryState == UE4.ETeamRecoveryState.Alive then 
    --     EventManager:FireEvent(EventID.OnTeamRecoveryStateChange, self.Eid, self.TeamRecoveryState, PrevRecoveryState)
    ...
    -- EventManager:FireEvent(EventID.PlayerRecoveryStateChange,self.TeamRecoveryState,self.Eid)
end
...
-- function BP_EMPlayerState_C:OnRep_RecoveryValue()
    -- DebugPrint("PlayerState OnRep_RecoveryValue", self.Eid)
-- end
...
function BP_EMPlayerState_C:OnRep_ActivatedDungeonDeliveryPointId()
    ...
    if self.ActivatedDungeonDeliveryPointId ~= -1 then
        EventManager:FireEvent(EventID.OnTeleportReady)
        DebugPrint("ayff test set TeleportReady open")
    elseif self.ActivatedDungeonDeliveryPointId == -1 then
