...
local function InitLocalAttr()
    ClassRegistry = {}
    ReplicationGraph = {}
    NetGuidMap = {} -- NetGuid->ReplicationGraphNode
    NetGuid = 0
end
...
local function GetReplicatedProperties(Prop)
    ...
    if Prop.__Class__ then
        ...
        if Node then
            -- 如果已经注册到ReplicationGraph中，那么只需要同步引用关系即可
            t._NetGuid = Node.NetGuid
        else
            -- 如果这个Class没有注册到ReplicationGraph中，那么需要用Props生成一个对象出来
...
--- 声明同步属性
--- @Owner table 属性所属对象
--- @PropName string 属性名，可不填，当不传属性名时，调用的意义在于将Owner这个对象同步下去
--- @DefaultValue any 属性默认值，可以不传即为当前值
--- @ReplicatedUsing string 客户端同步到这个属性时调用的方法名，可不传
function DungeonReplicatedProperty:InitReplicatedProperties(Owner, PropName, DefaultValue, ReplicatedUsing)
    ...
    if PropName then
        ...
        rawset(Owner, "Get" .. PropName, function(self) return ReplicationGraph[Owner].PropValues[PropName] end)
    end
