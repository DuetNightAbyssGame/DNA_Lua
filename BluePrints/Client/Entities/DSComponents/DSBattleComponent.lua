...
function Component:HandleAvatarBattleInfo(Callback, AvatarBattleInfos)
    ...
    -- PrintTable({AvatarInfos = GameMode.AvatarInfos}, 10)
    self:CallSkynetServerCallback(Callback)
end

-- 处理情况：GameMode拿到AvatarInfo然后切World导致AvatarInfo丢失
...
function Component:OnAvatarDestroy(AvatarEid)
    ...
    if PlayerController then
        ...
        -- 如果没有创建出来Controller，那么还是要把这个玩家从DS中结算掉
        self:HandleNotValidForReconnect(AvatarEidStr)
    end
    return false
end

-- 玩家已经无法重连到这个场景中，强制断开与DS服务器的连接
function Component:HandleNotValidForReconnect(AvatarEidStr)
    ...
    GameMode:OnAvatarLogout(AvatarEidStr) -- 手动调用DS登出
end

-- 用于客户端crash时，DS检测不到客户端断开连接
...
function Component:BattleFinish(IsWin, AvatarEids, PlayerEndReason)
    ...
    if AvatarArrLen == 0 then
        ServerPrint("BattleFinish with no valid avatar")
        return
    end

    -- ExtraInfo.UnknownRewards = self.CacheUnknownRewardsInDungeon
    -- ExtraInfo.FixedRewards = self.CacheFixedRewards
    ...
    self:CallSkynetServerMethod("BattleFinish", IsWin, GameMode.EMGameState.DungeonProgress - 1, GameTime, ExtraInfo) -- @SnowMoon GameMode的轮次从1开始，完成的轮次需要-1
...
function Component:AddFinishAvatar(ExtraInfo, AvatarEid, RealFinishAvatar, AvatarArr, IsWin)
    ...
    if rawget(self.HasLeaveAvatars, AvatarStr) ~= nil then -- 避免重复结算
        ServerPrint("AddFinishAvatar duplicated Avatar", AvatarStr)
        return
    end

    ServerPrint("AddFinishAvatar", AvatarStr)
    rawset(ExtraInfo, AvatarEid, { PersistenceRewards = {}, CacheRewards = {}, ImmediateResources = {}})
...
-- @SnowMoon 更新副本进度，将Cache合并到Persistence中并清空Cache
