...
-- 以下掉落物相关的接口对应PlayerCharacter中的同名函数
function Component:PickUpToSave(FunctionName, PickUpCount, ItemInfo, UnitId, Transform, CharacterEid, bExtra)
    --print(_G.LogTag, "PickUpToSave", PickUpCount, UnitId, Transform, CharacterEid)
...
function Component:AddImmediateRes(Rewards, Eid)
    --Rewards = RewardUtils:GetResources(Rewards, DropId, ResourceId)
    local ImmediateRes = {}
    ...
    if CommonUtils.Size(ImmediateRes) ~= 0 then
...
function Component:TriggerRewardEvent(LogicRewards)
    ...
    for AvatarEid, RewardsArr in pairs(LogicRewards) do
        ...
        self:ServerConditionalMulticast({[AvatarEid] = false}, "DSGetRewardInDungeon", RewardsArr.Rewards, false, self.RewardCallbackId)
