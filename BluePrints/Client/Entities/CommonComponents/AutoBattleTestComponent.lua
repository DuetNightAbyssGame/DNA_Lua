...
function Component:TestAutoBattleEnd(Callback, bClientMode)
    ...
    if TaskInfo then
        --Data= GText("TaskId:"..tostring(TaskInfo.TaskId).."\n".."DungeonId:"..tostring(TaskInfo.DungeonId).."\n")
        Data= GText("TaskId,DungeonId,")
     end
     --local PlayerRoleId=DataMgr.CharTemplate[CharGroupId]
     --local CharTemplate=DataMgr.CharTemplate[CharGroup]
     local Index=1
     local playerKey = nil
     local AvatarBattleInfo=nil
     local GameMode = GWorld.GameInstance:GetCurrentGameMode()
     if GameMode then
        ...
        if Avatars then
            ...
            if CharId then
                ...
                -- TempData[CharId]=TempData[CharId] or {}
                -- TempData[CharId]=CharId
    ...
    if GameMode then
        ...
        Data=Data..','..tostring(TempData[playerKey]["CharId"])..','..tostring(TempData[playerKey]["AddEnergyShield"])..','
        ..tostring(TempData[playerKey]["Damage"])..','..tostring(TempData[playerKey]["Heal"])
        for key, value in pairs(TempData) do
            if key ~=playerKey then
            -- for _, DetailValue in pairs( Table) do
            --     Data=Data..','..tostring(DetailValue)
            --     Data=Data..','..tostring(value["CharId"])..','..tostring(value[])
            -- end
            Data=Data..','..tostring(value["CharId"])..','..tostring(value["AddEnergyShield"])..','..tostring(value["Damage"])
            ..','..tostring(value["Heal"])
        end
        end
    ...
    --Battle(GWorld.GameInstance):DestoryBattleEvent("AutoTestCount")
