...
function Component:DisconnectRecoverCheck()
	...
	if not IsValid(GameMode)  then return true end
	-- 如果这个时候Loading已经出来了，那么需要强制切重新切一次场景
	local UI = GWorld.GameInstance:GetLoadingUI()
	if UI then
		return true
	end
	local Avatar = GWorld:GetAvatar()
	if not Avatar then return true end
	if not GameMode.GetLevelLoader then return true end
	local WorldLoader = GameMode:GetLevelLoader()
	local PlayerCharacter = UE4.UGameplayStatics.GetPlayerCharacter(GameMode, 0)
	if not IsValid(WorldLoader) or not IsValid(PlayerCharacter) then return true end
	if not WorldLoader.IsWorldLoader then return true end
	local SubRegionId = WorldLoader:GetRegionIdByLocation(PlayerCharacter:K2_GetActorLocation())
	if SubRegionId ~= -1 and SubRegionId == self:GetCurrentRegionId() then
...
-- 发送信息给服务器，服务器进行保存， 当收到服务器的回调时，将QuestDatas和客户端的RegionData数据进行更新
...
--------------- 区域的恢复的两个必要 ---------------
--------------- 1. 进入关卡准备完毕  ---------------
--------------- 2. Avatar状态为InBigWorld ---------
--------------- 两者缺一不可 -----------------------
function Component:HandleTryInitRegionInfo()
	-- if not self.IsInRegion or not self.IsEnterLevelReady then 
	-- wuzhijun IsInRegion服务器目前设置的时机太晚，需要在服务器切关卡流程的最后一步才设置，客户端初始化流程大概是不需要等这个变量的。只有关Loading的判断需要。
	-- 和江涛对过这个变量之前加上的原因不明，先去掉看看。
...
function Component:SetEnterLevelStateReady()
	...
	-- 此句移到OnInit内部
	-- self:HandleTryInitRegionInfo()
end

---------------------------------------------------
function Component:InitRecoverCheck()
	local CheckRegionId = self:GetLastRegionId()
	return self:IsInEnterBigWorld() and
	self:CheckSubRegionType(CheckRegionId,CommonConst.SubRegionType.Field) and CheckRegionId == self.CurrentRegionId
end
...
-- 如果有子区域类型过滤的需求，建议改成用CheckSubRegionType/CheckSubRegionType_NotEqual
...
function Component:CheckCurrentSubRegion(TargetSubRegionId)
	...
	return self:IsInBigWorld() and self:CheckRegionId(CheckSubRegionId) ~= nil -- self:CheckCurrentRegionType(CheckRegionId)
end
...
----- 区域Eid是使用 ActorName和Eid进行组合而成的 因此要保证ActorName和Eid存在不是默认值
-- @fanyuxiao 区域Eid生成时机提前到Actor之前了, 直接拿UUID做区域Eid
function Component:GetWorldRegionEid(Context)
	...
	-- self:AddActorDataToCache(TargetActor)
...
function Component:GetCretorWorldRegionEid(Creator, Eid)
	...
	-- DebugPrint("ZJT_ GetCretorWorldRegionEid ", Creator:GetName(), WorldRegionEid, Eid)
...
function Component:RemoveRegionSkipCallback(RegionId, Obj, Func)
	...
	PrintTable({SkipRegionCallback = self.SkipRegionCallback} , 10)
...
function Component:RemoveSubRegionSkipCallback(SubRegionId, Obj, Func)
    ...
    if (self.SkipSubRegionCallback[SubRegionId] == nil) then
		return
	end
	...
	elseif not Obj and not Func then--2026.2.10smh:看上去是有都为空的情况下清空回调的调用(StopJumpRegion)，所以先这么写
		DebugPrint("Obj,Func为空")
...
function Component:GetSubRegionSkipCallback(SubRegionId, Obj)
    if not self.SkipSubRegionCallback then return end
    if not self.SkipSubRegionCallback[SubRegionId] then return end
    if not Obj then return self.SkipSubRegionCallback[SubRegionId] end
	return self.SkipSubRegionCallback[SubRegionId][Obj]
end

function Component:GetRegionSkipCallback(RegionId, Obj)
	if not self.SkipRegionCallback then return end
	if not self.SkipRegionCallback[RegionId] then return end
	if not Obj then return self.SkipRegionCallback[RegionId] end
...
function Component:HandleExeRegionSkipCallbck(RegionId)
    ...
    if (SkipCallbcks == nil) then return end
...
-- function Component:DeleteRerelyRegionData(RarelyId, RegionDataType) ----- 待优化
-- 	local DeleteTable = {}
-- 	local RarelyRegionDatas = self:GetRarelyDataByRarelyId(RarelyId)
-- 	if not RarelyRegionDatas then return DeleteTable end
-- 	for WorldRegionEid, RegionBaseData in pairs(RarelyRegionDatas) do
-- 		repeat
-- 			self:RemoveCretorIdContollerByCache(RegionBaseData.CreatorId)
-- 			self:RemoveRarelyDataToCache(RegionBaseData)
-- 			self:RemoveActorDataByWorldEid(RegionBaseData, "DeleteRerelyRegionData")
-- 			local LevelName = RegionBaseData.LevelName
-- 			local RegionId = RegionBaseData.RegionId
-- 			local Ret, RegionDatas, RegionIdDatas, RegionLevelDatas = self:GetLevelRegionDatasByLevelName(RegionId, RegionDataType, LevelName)
-- 			if not self:CheckRegionErrorCode(Ret) then break end
-- 			RegionLevelDatas[WorldRegionEid] = nil
-- 			table.insert(DeleteTable, WorldRegionEid)
-- 		until true
-- 	end
-- 	for _, WorldRegionEid in pairs(DeleteTable) do
-- 		RarelyRegionDatas[WorldRegionEid] = nil
-- 		self:RemoveRegionBaseDataByCache(WorldRegionEid, RegionDataType)
-- 	end	
-- 	return DeleteTable
-- end
