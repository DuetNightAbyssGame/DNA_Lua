...
function Component:CheckPetEnough(Pets)
    ...
    for Id, Count in pairs(Pets) do
        if not PetCount[Id] or PetCount[Id] < Count then
            return false
        end
    end
    return true
...
function Component:GetCurEquipPet()
	if not self.CurrentPet or self.CurrentPet == 0 then
		return
	end
...
function Component:GMPetAutoAddExp(AddExpPetUniqueID,CostPetId)
    ...
    for uniqueId, pet in pairs(self.Pets) do
        if uniqueId ~= AddExpPetUniqueID and pet.PetId == CostPetId and pet.Level == 1 then
...
-- 替换词条
---@param EntryReplacePetUniqueId number @替换词条的宠物唯一ID
---@param EntryReplacePetEntryIndex number @替换词条的词条索引
---@param ConsumePetUniqueId number @消耗的宠物唯一ID
---@param ConsumePetEntryIndex number @消耗的词条索引
...
function Component:TestPetEntryUp(EntryUpPetUniqueId,EntryIndex)
	...
	for uniqueId, pet in pairs(self.Pets) do
		for _, v in pairs(pet.Entry) do
			if v == EntryId and #ConsumePetUniqueIds < Count then
...
-- 词条升阶
---@param EntryUpPetUniqueId number @词条升阶的宠物唯一ID
---@param EntryIndex number @词条升阶的词条索引
---@param ConsumePetUniqueIds table<number> @消耗的宠物唯一ID
...
-- 词条锁定
---@param EntryId number @词条锁定的词条ID
---@param IsLock boolean @是否锁定
---@param LockBagPet boolean @是否锁定背包宠物
