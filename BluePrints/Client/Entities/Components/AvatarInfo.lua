...
function Component:InitEscPortraitReddotNode()
	-- local Node = ReddotManager.GetTreeNode("EscPortrait")
    -- if not Node then
    --     Node = ReddotManager.AddNode("EscPortrait",nil,1)
    -- end
    -- ReddotManager.ClearLeafNodeCount("EscPortrait")
	-- local CacheDetail = ReddotManager.GetLeafNodeCacheDetail("EscPortrait")
	-- local Avatar = GWorld:GetAvatar()
	-- for UniqueId, CacheData in pairs(CacheDetail) do
	-- 	if CacheData then
	-- 		ReddotManager.IncreaseLeafNodeCount("EscPortrait")
	-- 	end
	-- end
...
function Component:InitEditBtnReddotNode()
    ...
    if not Node then
        Node = ReddotManager.AddNode("EditBtn",nil,1)
    end
end
...
function Component:InitPortraitReddotNode()
    ...
    if not Node then
        Node = ReddotManager.AddNode("Portrait",nil,1)
    end
...
function Component:InitPortraitFrameReddotNode()
    ...
    if not Node then
        Node = ReddotManager.AddNode("PortraitFrame",nil,1)
    end
...
function Component:_OnPropChangeHeadIconList()
	for _, Value in ipairs(self.HeadIconList) do
		...
		local CacheDetail = ReddotManager.GetLeafNodeCacheDetail("Portrait")
		if CacheDetail[CacheKey] == nil and CacheKey~=10001 then 
			UIUtils.TryAddReddotCacheDetailNumber(CacheKey,"Portrait")
		end
	end
	-- local CacheDetail = ReddotManager.GetLeafNodeCacheDetail("EscPortrait")
	-- if not CacheDetail["EscPortrait"] then
	-- 	UIUtils.TryAddReddotCacheDetail("EscPortrait","EscPortrait")
	-- end
end
...
function Component:SetAvatarSignature(NewSignature, CallBackFunction)
	...
	local SensitiveCallBack = function(ReplaceSignature, Signature, Words)
		ErrorCode:Check(Const.PlayerSignatureIllegal)
	end
	local NotSensitiveCallBack = function(Signature)
		local function Callback(Ret)
...
-- function Component:TestWeita()
-- 	self:SetWeitaInfo("WeitaName", 1, function() end)
-- end
...
function Component:NotifyAvatarLevelUpdate(LevelInfo)
	...
	-- 经验值提升的情况
	if OldLevel == CurLevel and CurExp > OldExp then 
	...
	-- 	local UIManager = GWorld.GameInstance:GetGameUIManager()
	-- 	if UIManager then
	-- 		UIManager:CacheLevelUpInfo(newLevel, "Player")
	-- 		-- 如果不再副本里，则立即显示
	-- 		if not self:IsInDungeon() then  
	-- 			UIManager:ShowPlayerLevelUpToast()
	-- 		end
	-- 	end
	-- end
end

-- function Component:TestFilterStringSensitiveWord()
-- 	local Str = "HHH?傻@!!逼dsgfgdfhrrewerrtwedddfeeeeee"
-- 	local Avatar = GWorld:GetAvatar()
-- 	if Avatar then
-- 		Avatar:FilterStringSensitiveWord(Str)
-- 	end
-- end
...
function Component:OnDailyRefresh(ServerTime)
	...
	self:OnDailyRefreshGachaCache() --每日刷新抽卡卡池Spine动画缓存
