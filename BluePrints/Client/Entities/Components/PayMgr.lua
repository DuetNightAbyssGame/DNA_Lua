...
function Component:OnPayFinished(Result, GoodsId, OrderId, ShopItems)
    ...
    -- FBParameters:Add('user_id', tostring(GameRoleInfo.roleId))
    -- FBParameters:Add('item_id', tostring(GoodsId))
    -- FBParameters:Add('server_id', tostring(GameRoleInfo.serverId))
...
function Component:ListenPayFinish(OrderId, EventFunc)
    ...
    if not EventFunc then return end
...
function Component:UnListenPayFinish(OrderId)
    if not OrderId then return end
    if not self.WaitPayDict then return end
