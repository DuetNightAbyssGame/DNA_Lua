...
function Component:OnDungeonFinish(DungeonId, IsWin, Rewards, DungeonRewards, PlayerTime, GameTime,  ClientRes)
	...
	local RealOnDungeonFinish = function ()
		...
		GWorld.GameInstance:PushLogicServerCallbackInfo(IsWin, DungeonId, Rewards, DungeonRewards, PlayerTime, GameTime, ClientRes)	-- IsWin放到第一个参数，方便后面单独拿
	end
...
function Component:ExitDungeon()
	...
	if GWorld:IsStandAlone() or GWorld:IsClient() then
        -- 退出副本时，客户端清除预加载资源
		if PlayerController then
...
-- @SnowMoon 副本成功则进入临时结算场景后再播放结算动画和结算界面
function Component:CheckMoveToTempScene(CurrentDungeonId, IsWin)
	...
	if LevelLoader then
		...
		-- if DungeonInfo and DungeonInfo.ExitLevel and IsWin then
		if DungeonInfo and DungeonInfo.ExitLevel then
			...
			-- 根据这个场景的坐标和当前玩家的坐标计算玩家在这个场景中的相对坐标
			local LevelTransform = UE4.UKismetMathLibrary.MakeTransform(Position,Rotation,UE4.UKismetMathLibrary.Vector_One()) --圆心
			PlayerCharacterLocation = UE4.UKismetMathLibrary.InverseTransformLocation(LevelTransform, PlayerCharacterLocation) -- 绕圆心往回旋转得到最初关卡的相对位置
			...
			-- if GameState.GameModeType == "Temple" then
			-- 	if DungeonInfo.ExitLevel then
			-- 		MapFile = DungeonInfo.ExitLevel
			-- 	end
			-- 	PlayerCharacterLocation = FVector(DungeonInfo.ExitPlayerLocation[1],DungeonInfo.ExitPlayerLocation[2],DungeonInfo.ExitPlayerLocation[3])
			-- end
...
function Component:SetIronRareDrop(Callback, DungeonId, DropId)
	DebugPrint("SetIronRareDrop", DungeonId, DropId)
	assert(DungeonId)
	assert(DropId)
	local function cb (ret)
		DebugPrint("SetIronRareDrop callback", ret)
		if Callback then
			Callback(ret)
		end
	end
