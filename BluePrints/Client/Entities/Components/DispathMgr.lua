...
function Component:EnterWorld()
    ...
    for Id, Info in pairs(DataMgr.Dispatch) do
        ...
        if CacheDetail and Dispatch then
            if Dispatch.State == CommonConst.DispatchState.Perfect or Dispatch.State == CommonConst.DispatchState.Success or
            Dispatch.State == CommonConst.DispatchState.Qualified or Dispatch.State == CommonConst.DispatchState.Disqualified then
                CacheDetail[Id] = true
...
function Component:_TryAddReddotCacheDetail(Id)
    local CacheDetail = ReddotManager.GetLeafNodeCacheDetail(DataMgr.ReddotNode.Dispatch.Name)
    if CacheDetail and CacheDetail[Id] == nil then
        CacheDetail[Id] = true
    end
