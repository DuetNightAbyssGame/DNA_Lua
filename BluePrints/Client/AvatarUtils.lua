...
function AvatarUtils:GetModCostInSuit_SwitchMod(Avatar, Tag, Uuid, ModSuitIndex, OldMod, NewMod, InitializeFunc)
	if not InitializeFunc then InitializeFunc = self.CheckGetModCostInSuit end
...
function AvatarUtils:CheckChangeModRepeat(Avatar, Tag, Uuid, ModSuitIndex, TargetModSlotId, ModUuid, ModSuit)
	...
	if not ModSuit or not TargetMod then return false end
	local ConflictMods = not skynet and {} or nil
	local SuitModIds = {}
	for ModSlotId, ModSlotEid in pairs(ModSuit) do--记录已装备的且有可能冲突的mod到SuitModIds
		local Mod = Avatar.Mods[ModSlotEid]
		if Mod then
			if self:IsHaveModRepeatGroup(Mod.RepeatGroup, TargetMod.RepeatGroup) then --如果RepeatGroup相同，就有可能冲突，否则一定不冲突
				if TargetModSlotId ~= ModSlotId then
					if Mod.IsOriginal and TargetMod.IsOriginal then
						if skynet then
							return false
						else
	...
	for ModSlotId, Mod in pairs(SuitModIds) do --处理可能冲突的mod数组SuitModIds
		if ModSlotId ~= TargetModSlotId then
			--【【11/11】【系统】【客户端】魔之楔系统-允许重复装备逻辑迭代】 https://www.tapd.cn/tapd_fe/31626021/story/detail/1131626021001282082
			if Mod.ModId==TargetMod.ModId or (Mod.ModId ~= TargetMod.ModId and self:IsHaveModRepeatGroup(Mod.RepeatGroup, TargetMod.RepeatGroup)) then
				if TargetMod.RepeatModLevel == nil or TargetMod.RepeatModLevel <= 0 then--如果没有RepeatModLevel，一定冲突
					if skynet then
						return false
					else
						table.insert(ConflictMods, Mod)
				...
				if Mod.IsOriginal then
					if TargetLevel < (TargetMod.RepeatModLevel or 0) then --如果未装备的mod等级小于RepeatModLevel，那么一定冲突
						if skynet then 
							return false
						else
							table.insert(ConflictMods, Mod)
					...
					if ModLevel < (TargetMod.RepeatModLevel or 0) then
						if TargetLevel < (TargetMod.RepeatModLevel or 0) then --如果未装备的mod等级小于RepeatModLevel，那么一定冲突
							if skynet then 
								return false
							else
	...
	if skynet then return true else
...
function AvatarUtils:GetAttrNameFromAttrData(AttrData, UniqueName)
	...
	if AttrName == "DamageRate" or AttrName == "DamagedRate" then
...
function AvatarUtils:GetDefaultAvatarInfo()
	...
	if Const.DeafaultMeleeWeapon then
		DefaulAvatarInfo.MeleeWeapon = {WeaponId = Const.DeafaultMeleeWeapon}
	end
	if Const.DeafaultRangedWeapon then
		DefaulAvatarInfo.RangedWeapon = {WeaponId = Const.DeafaultRangedWeapon}
	end
...
-- @SnowMoon MeleeWeaponModSuit为一级方案，MeleeWeaponModSuitSecondary为二级方案，
-- 一级方案存在时以一级方案为准，在一级方案不存在的时候才读二级方案
function AvatarUtils:ReShapeSquadInfo(Avatar, Squad)
	local Info = {
		...
		WheelIndex = Squad.WheelIndex, -- 暂时没有什么用
...
function AvatarUtils:GetTargetModSuit(Target, ModSuitIndex)
	...
	-- 这里应该在对Target[ParamsName]类型进行检验
...
-- 获取mod槽解锁状态,true-已解锁,false-未解锁
function AvatarUtils:ModSlotIsUnLock(Target, ModSlotId)
	if Target ~= nil and Target.ModSlotStatus ~= nil and ModSlotId ~= nil and Target.ModSlotStatus[ModSlotId] ~= nil then
...
function AvatarUtils:GenerateModSuitInfo(Avatar, Tag, Eid, ModSuitIndex)
	...
	if Tag == "Char" then
		...
		if Char then
			Target = Char
			TargetInfo = {Tag, Char.CharId, Char.Level, Char.EnhanceLevel, Char.GradeLevel}
		end
	elseif Tag == "Weapon" then
		...
		TargetInfo = {Tag, Weapon.WeaponId, Weapon.Level, Weapon.EnhanceLevel, Weapon.GradeLevel}
	elseif Tag == "UWeapon" then
		local UWeapon = Avatar.UWeapons[Eid]
		if UWeapon then
			Target = UWeapon
		end
		TargetInfo = {Tag, UWeapon.WeaponId, UWeapon.Level, UWeapon.EnhanceLevel, UWeapon.GradeLevel, }
	end
	...
	table.insert(TargetInfo, SuitName )--6
	if not Target then
		return
	end
	...
	for ModSlotId, ModSlotEid in pairs(ModSuit) do
		ModsInfo[ModSlotId] = {}
		if ModSlotEid ~= -1  then
			local Mod = Avatar.Mods[ModSlotEid]
			if Mod then
				ModsInfo[ModSlotId] = { Target.ModSlotPolarity[ModSlotId], Mod.ModId, Mod.Level, Mod.CurrentModCardLevel}
			else
				ModsInfo[ModSlotId] = {Target.ModSlotPolarity[ModSlotId]}
			end
		else
			ModsInfo[ModSlotId] = {Target.ModSlotPolarity[ModSlotId]}
		end
...
--根据鱼的重量计算鱼的售价，鱼对应的ResourceId,鱼的重量（放大10倍后的值）
function AvatarUtils:CalculateFishPrice(FishResourceId,FishSize)
	if not FishResourceId then return 0 end
	if not FishSize then return 0 end
	local FishId = DataMgr.ResourceId2FishId[FishResourceId]
	if not FishId then return 0 end
	local FishInfo = DataMgr.Fish[FishId]
	if not FishInfo.PriceOnWeight then return 0 end
	local PriceOnWeight = FishInfo.PriceOnWeight
	local Price = PriceOnWeight[1]
	FishSize = math.min(FishSize,FishInfo.FishLength[2]*CommonConst.FishSizeScale)--最大重量校正
	local Diff = FishInfo.FishLength[2] - FishInfo.FishLength[1]
	local Weight = Diff * CommonConst.FishSizeScale / PriceOnWeight[2]	--权重
	local Stage = (FishSize - FishInfo.FishLength[1]*CommonConst.FishSizeScale) // Weight --整除
	local Remain = (FishSize - FishInfo.FishLength[1]*CommonConst.FishSizeScale) % Weight --取余
	if Remain == 0 then --边界情况
...
--获取目标数据统计即累计获得数量,传类型+id
--目前支持Char、Weapon、Skin、WeaponSkin、CharAccessory、WeaponAccessory、Resource(仅ResourceSType为GestureItem)
function AvatarUtils:GetTargetDataStatistics(Avatar,TargetType,TargetId)
	if not Avatar or not TargetId or not TargetType then return 0 end
...
--个人主页获取展示的角色信息，FromDb为true表示来自数据库
	...
	if Avatar.PersonalInfo and Avatar.PersonalInfo.CharDisplay then
        for key,value in pairs(Avatar.PersonalInfo.CharDisplay) do
            ...
            if Avatar.Chars and Avatar.Chars[CharUuid] then
				...
				local SkillTree = {}
				for i = 1, CommonConst.CharSkillTreeCount, 1 do
					if not SkillTree[i] then
						SkillTree[i] = {}
					end
					for j = 1, CommonConst.CharSkillTreeCount, 1 do
                ...
                if value.AppearancePlan and value.AppearancePlan ~= -1 then
				...
				if CharHair then
					...
					for i = 1,DataMgr.GlobalConstant.HairColorPart.ConstantValue,1 do
						if not Appearance.HairColors then
							Appearance.HairColors = {}
						end
...
function AvatarUtils:GetAbyssSeasonMaxProgress(Avatar,FromDb)
	...
	for AbyssId ,Info in pairs(DataMgr.AbyssSeason) do
		if not Info.AbyssSeasonId and Info.AbyssType == 1 then
			...
			if Abyss then
				...
				if MaxAbyssProgress[1] < MaxLevel then
					IsAbyssNotPass = true
				else
				...
				if IsAbyssNotPass or not SeasonId or not DataMgr.AbyssSeasonList[SeasonId] then
    ...
    if AbyssInfinite and AbyssInfinite.MaxAbyssProgress and
    (AbyssInfinite.MaxAbyssProgress[1] > 1 or
    (AbyssInfinite.MaxAbyssProgress[1] == 1 and AbyssInfinite.MaxAbyssProgress[2] > 0)) then
...
function AvatarUtils:CalculateAbyssAllRoomCount(Abyss)
	...
	local MaxAbyssProgress = Abyss.MaxAbyssProgress or {1,0}
	for LevelIndex = 1,MaxAbyssProgress[1] - 1,1 do
...
function AvatarUtils:GetPlayerPersonalInfoAchievementCount(Avatar,FromDb)
	...
	for key, Achv in pairs(Avatar.Achvs) do
		...
		if AchvInfo then
			...
			if AvatarUtils:IsAchvFinished(Achv,FromDb)
			and not AvatarUtils:IsAchvLocked(Avatar,Achv.AchvId,FromDb) then
...
function AvatarUtils:IsAchvLocked(Avatar,AchvId,FromDb)
	...
	for _,preId in pairs(AchvInfo.AchievementRequire) do 
		...
		if (not achievePre) or (not AvatarUtils:IsAchvFinished(achievePre,FromDb))
		or AvatarUtils:IsAchvLocked(Avatar,preId,FromDb) then
			return true
		end
	end
	return false
...
--获取钓鱼昼夜系统当前所处的时期,早晨：4:00-12:00;中午：12:00-20:00;夜晚：20:00-4:00
function AvatarUtils:GetFishCurrentPeriod(Avatar)
	local CurPeriod
	if not Avatar.TimeOfDay then return 0 end
...
function AvatarUtils:GetRaidSeasonsRecord(Avatar, FromDb)
	...
	local NowTime = TimeUtils.NowTime()
	for k, v in pairs(RaidSeasons) do repeat
		local RaidSeasonId = k
		if FromDb then
			RaidSeasonId = tonumber(RaidSeasonId)
		end
		local RaidSeasonConf = DataMgr.RaidSeason[RaidSeasonId]
		if not RaidSeasonConf then
			break
		end
		local RaidSeasonEventConf = DataMgr.EventId2RaidSeason[RaidSeasonConf.EventId]
		if not RaidSeasonEventConf then
			break
		end
		-- 只获取历史赛季
		if NowTime <= TimeUtils.EastEightToLocalTimestamp(RaidSeasonEventConf.EventEndTime) then
			break
		end
		-- 只获取参加过正式赛的
		if not v.PreRaidGroupId or not v.RaidGroupId then
			break
		end
		table.insert(Ret, {
