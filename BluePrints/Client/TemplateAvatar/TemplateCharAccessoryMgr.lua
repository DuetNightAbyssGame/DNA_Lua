...
function Component:AddAccessory(AccessoryId, Reason, GiveNotOpen)
    ...
    if not AccessoryId or not Info then return end
    GiveNotOpen = GiveNotOpen or false
    if Info.IsNotOpen and not GiveNotOpen then return end
    if self:CheckRepeatPressup(AccessoryId) then return end
