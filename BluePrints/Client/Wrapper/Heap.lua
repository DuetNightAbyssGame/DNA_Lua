...
function Heap.AdjustDown(Source, ParentIndex, EndIndex)
	...
	if Left <= EndIndex and Heap.GetSortValue(Source[Left]) < Heap.GetSortValue(Source[MinIndex]) then
		MinIndex = Left
	end
	if Right <= EndIndex and Heap.GetSortValue(Source[Right]) < Heap.GetSortValue(Source[MinIndex]) then
		MinIndex = Right
	end
...
function Heap.AdjustDownWithComp(Source, ParentIndex, EndIndex, CompFunc)
	...
	local Right = 2 * ParentIndex + 1
	local MinIndex = ParentIndex
	if Left <= EndIndex and CompFunc(Source[MinIndex], Source[Left]) then
		MinIndex = Left
	end
	if Right <= EndIndex and CompFunc(Source[MinIndex], Source[Right]) then
		MinIndex = Right
	end
	if MinIndex == ParentIndex then
		return
