...
local Explore = Class("Explore", CustomTypes.CustomAttr)
    Explore.__Props__ = {
        RarelyId = prop.prop("Int", "client save"), ----- 探索组ID
        ExploreState = prop.prop("Int", "client save", 0), ---- 探索组状态
        ExploreData = prop.prop("ExploreBaseAttr", "client save"), ----- 探索组数据
    ...
    function Explore:RestingState()
        if self.ExploreState == CommonConst.ExploreState.InActive then return end
        self.ExploreState = CommonConst.ExploreState.InActive
