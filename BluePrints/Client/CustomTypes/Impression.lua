...
ImpressionAttr.__Props__ = {
    ...
    function ImpressionAttr:Init(Param)
        if not Param or type(Param) ~= table then return end
...
Impression.__Props__ = {
        RegionId = prop.prop("Int", "client save", 0),   ------ 唯一标识的ID  必要
...
function Impression:InitImpression(RegionId)
    ...
    if not RegionId or RegionId <= 0 then return end
...
return {Impression = Impression, ImpressionDict = ImpressionDict, ImpressionAttrDict = ImpressionAttrDict,
