...
AbyssLevel.__Props__ = {
	...
	--当前锁定阵容最佳进度，关卡A房间数，关卡B房间数，eg:2,3 已通过关卡A的第2个房间，关卡B的第3个房间
	MaxLockedTeamProgress = prop.prop("IntList", "client save",{0,0}),
	--当前Level进度，关卡A房间数，关卡B房间数，eg:2,3 已通过关卡A的第2个房间，关卡B的第3个房间
	...
	-- 阵容快照1
	PlayerInfo1 = prop.prop("Str", "save"), -- 角色主体信息
	-- 阵容快照2
	PlayerInfo2 = prop.prop("Str", "save"), -- 角色主体信息
...
function AbyssLevel:UpdatePassTime(PassTime,DungeonIndex)
	if not PassTime or PassTime <= 0 then return end
	if self.PassTime[DungeonIndex] == -1 then
...
function AbyssLevel:UpdateFastestPassTime(IsLoop)
	...
	if self.FastestPassTime == -1 then
...
Abyss.__Props__ = {
	...
	-- --赛季状态，0未开始，1已开始，2已结束
	-- AbyssState = prop.prop("Int", "client save",0),
	--解锁状态，true-未解锁，false-已解锁
	...
	--当前进度，层数，关卡A房间数，关卡B房间数，eg:3,2,3 已通过第三层，关卡A的第二个房间，关卡B的第3个房间
	AbyssProgress = prop.prop("IntList", "client save",{1,0,0}),
	--已经跳层的次数
...
function Abyss:InitRewardGotList()
	...
	for _,Id in ipairs(self.AbyssRewardList) do
		...
		if Info and (not Info.RewardAddOn) and Info.Level and Info.Level <= RoomCount then
			self.RewardGotList[Info.Level] = {Info.Reward,0}
		end
	end
end
...
function Abyss:CheckRewardIsGot(RoomCount)
	if self.RewardGotList[RoomCount] then
		return self.RewardGotList[RoomCount][2] == 1
	end
	return false
end
...
function Abyss:CheckRewardCanGet(RoomCount)
	...
	if AllRoomCount < RoomCount then
		return false
	end
	if self.RewardGotList[RoomCount][2] == 0 then
		return true
	end
	return false
end
...
function Abyss:CheckHaveRewardToGet()
	...
	for key,value in pairs(self.RewardGotList) do
		if key <= AllRoomCount and value[2] == 0 then
			return true
		end
 	end
	return false
...
function Abyss:IsLoopAbyss()
	if self.AbyssType and self.AbyssType == 3 then
		return true
	end
	return false
end
...
function Abyss:IsAbyssPass()
	...
	if self.MaxAbyssProgress[1] < AbyssLevelCount then
		return false
	else
...
function Abyss:GetAllPassRoomCount()
	...
	for LevelIndex = 1,self.MaxAbyssProgress[1] - 1,1 do
