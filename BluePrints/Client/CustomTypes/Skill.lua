...
local Skill = Class("Skill", CustomTypes.CustomAttr)
	...
	function Skill:InitLockState()
		-- local Data = DataMgr.Skill[self.SkillId][self.Level][self.GradeLevel]
		-- if Data then
		-- 	local SkillUnLockWithCharEnhanceLevel = Data.SkillUnLockWithCharEnhanceLevel
		-- 	if SkillUnLockWithCharEnhanceLevel ~= nil and SkillUnLockWithCharEnhanceLevel > self.EnhanceLevel then
		-- 		self:Lock()
		-- 	else
		-- 		self:UnLock()
		-- 	end
...
return {Skill = Skill, SkillList = SkillList,
		SkillTreeNode = SkillTreeNode,SkillTreeNodes = SkillTreeNodes,SkillTree = SkillTree}
