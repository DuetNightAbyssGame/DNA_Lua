...
function  FishingSpot:InitLastRefreshTime()
    local StartTime = TimeUtils.DataToTimestamp(CommonConst.ShopRefreshBeginTime[1],
...
function FishSize:ReduceCount(FishSize,Count)
    if not self.FishSize2Count[FishSize] then return false end
    if self.FishSize2Count[FishSize] < Count then return false end
