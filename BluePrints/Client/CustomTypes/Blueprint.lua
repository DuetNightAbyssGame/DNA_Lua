...
local Draft = Class("Draft", CustomTypes.CustomAttr)
    Draft.__Props__ = {
        ...
        -- DraftStageStartTime = prop.prop("Int"), ----- 该变量不需要Save 也不需要 Client 临时的数据变量 在此标记
        ...
        State = prop.prop("Int", "client save", 0), --- 0代表未制作， 1 代表制作中 2 代表制作完成
    ...
    function Draft:RedirectResourceData()
        ...
        local Resource = {}
        if not DraftInfo.Resource then return Resource end
    ...
    function Draft:RedirectWeaponData()
        local DraftInfo = DataMgr.Draft[self.DraftId]
        local Weapon = {}
        if not DraftInfo.Resource then return Weapon end 
        for _, ResourceInfo in ipairs(DraftInfo.Resource) do
    ...
    function Draft:RedirectModData()
        local DraftInfo = DataMgr.Draft[self.DraftId]
        local Mod = {}
        if not DraftInfo.Resource then return Mod end
        for _, ResourceInfo in ipairs(DraftInfo.Resource) do
	...
	function Draft:ReduceCount(count)
		if type(count) == "number" and count > 0 and self.Count >= count and not self.IsInfinity then
			self.Count = self.Count - count
			return true
		end
		return false
    ...
    function Draft:StartProduct( DoingNum)
        if DoingNum <= 0 then return false end
