...
local QuestData = Class("QuestData", CustomTypes.CustomAttr)
	...
	function QuestData:SetTargetId(TargetId)
		if TargetId and self.TargetId == TargetId then return end
		self.TargetId = TargetId
	end
	...
	function QuestData:SetTargetProgress(TargetCount)
		if not TargetCount or TargetCount <= 0 then return end
	...
	function QuestData:IsComplete()
		return self.CurrentProgress ~= 0 and self.TargetProgress ~= 0 and self.TargetProgress <= self.CurrentProgress
	end
	FormatProperties(QuestData)

----- 这里一直没有想好是放到任务链中作为任务链的一部分数据还是解耦合单独拎出来
...
local QuestChain = Class("QuestChain", CustomTypes.CustomAttr)
	QuestChain.__Props__ = {
		...
		State = prop.prop("Int", "client save", 0), -- 0：未解锁，1：可接取，2：进行中，3：完成，4：执行前置任务链
	...
	function QuestChain:Init(QuestChainId)
		if not QuestChainId then return end
	...
	function QuestChain:AddCompleteQuestId(QuestId)
		if not QuestId or type(QuestId) ~= 'number' then return end
		if tonumber(QuestId) <= 0 then return end
...
return {QuestChain = QuestChain,
	QuestTracking = QuestTracking, QuestChains = QuestChains,
	QuestData = QuestData, QuestDataDict = QuestDataDict,
