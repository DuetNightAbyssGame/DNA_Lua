...
local function make_sandbox()
    ...
    local env_mt = { __index = proxy, __newindex = function(t,k,v)
    ...
    local function load_str(str)
        local chunk = function() return _G.load(str)() end
        local env = {}
        setmetatable(env, env_mt)
        debug.setupvalue(chunk, 1, env)
        return chunk, env
...
local function match_upvalues(value_info_map, old_upvalues)
    --[[
    value_info_map {
        {function_name:{old_function:new_function}},
        {function_name:{old_function:new_function}},
        ...,
        {function_name:{old_function:new_function}}
    }
...
local function update_modules(old_modules, new_modules, new_envs)
    -- old_modules, {old_module,old_module,...,old_module}
    -- new_modules, {new_module,new_module,...,new_module}
    -- new_envs, 忽略吧，看起来没什么用
    ...
    for i, old_module in ipairs(old_modules) do
        ...
        -- HOT_RELOAD_MARK的标记，只是用于简版的reload，用处不大，默认全走下面的流程就好了
        ...
        -- else
            ...
            -- 将新module里的数据覆盖旧module里，但是导致旧module和新module同时存在，第二次热更就会热更失败
            ...
            --     end
            -- end
            -- 所以用替换整个table引用的方式，即用HOT_RELOAD_MARK的方式
            table.insert(moduleres.values, { [new_module] = { [old_module] = new_module } } )

            print("--------------Print ValueMap--------------")
            print(dump(moduleres.values))

            -- 根据替换表，和旧的upvalue列表，得到新的函数的upvalue列表
...
-- reload，重新加载lua文件，因为无法识别哪个函数被修改了，因此需要替换lua module和module内的所有函数
