...
local IsNilOrEmpty = function(InStr)
...
function ItemUtils.GetItemName(ItemId, TableName)
	...
	if not ItemData.Name then 
		if TableName == "Weapon" then 
			return ItemData.WeaponName 
		elseif TableName == "Resource" then 
			return ItemData.ResourceName
		elseif TableName == "Skin" then
...
function ItemUtils.GetItemIcon(ItemId, TableName)
	...
	if IconPath then
		if not string.find(IconPath, "/Game/") then
...
function ItemUtils.GetItemLimitedInfo(ResourceId)
	...
	for _, LimitedInfo in ipairs(LimitedInfoLst) do
		if LimitedInfo.EndTime > CurrentTime then
			Res = LimitedInfo
			break
		end
	end
...
--检查Avatar中的资源是否在本地配表中存在，如果不存在说明是旧版失效道具
--@param ResourceId number 资源ID
--@return boolean 如果是失效道具返回true，正常道具返回false
...
function ItemUtils.CheckGestureItemResourceNeedDisplay(ResourceId)
	...
	if ResourceInfo and ResourceInfo.ResourceSType and
	ResourceInfo.ResourceSType == "GestureItem" and ResourceInfo.DisplayPath then
		return true
	end
	return false
end
