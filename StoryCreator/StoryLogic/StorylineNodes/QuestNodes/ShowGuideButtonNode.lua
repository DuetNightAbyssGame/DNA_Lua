...
function ShowGuideButtonNode:ShowMessage(Context)
    ...
    if self.ShowEnable then
        ...
        self.UIStateAsyncActionBase = UE4.UUIStateAsyncActionBase.ShowGuideButton(GameInstance, self.MessageId, self.DelayTime, self.IsForceClick,self.IsControlPlayer, self.IsTimePause, self.IsShowCursor,
        self.HighLightUIPath, self.UICompName, self.UIShape, true, UICompSizeOffsetVector, UICompLocOffsetVector,
        self.MessageParentLoc, self.MessageLoc, MessageLocOffsetVector, self.IsResetPlayer, self.IsForbidInAnim, self.IsForbidOutAnim , self.GamePadWidgetName, self.IsAutoClick, self.IsAutoClickByGamepad,self.IsFindByMainUI)
        self.UIStateAsyncActionBase.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
    ...
    elseif GameMode.GameState then
        -- GameMode.GameState:MulticastHideGuideMessage(self.MessageId)
        if CommonUtils.GetDeviceTypeByPlatformName(GameInstance) == "Mobile" then
            ...
            self.Guide_Touch.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
        ...
        elseif CommonUtils.GetDeviceTypeByPlatformName(GameInstance) == "PC" then
            ...
            self.GuideTextPanel.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
