...
function UpdateTaskBarAndTaskMainNode:OnChooseTrack()
    ...
    if Avatar.TrackingQuestChainId == self.CurQuestChainId and Avatar.TrackingQuestChainId ~= 0 and self.CurQuestChainId ~= 0  then
        IsTracking = true
    end
    ...
    if IsTracking then
        -- self:ChangeMainTaskBarInfoBySTLNode(self.QuestChainId, self.QuestData.QuestId, self.Key)
...
function UpdateTaskBarAndTaskMainNode:ChangeMainTaskBarInfo()
    ...
    local NewDescrible = ""
    if self.Key ~= nil and QuestExtraInfo[self.Key] then
