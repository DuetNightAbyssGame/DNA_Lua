...
function ShowQuestHintNode:Start(Context)
    ...
    TaskUtils:SetQuestExtraInfo(self.QuestChainId, self.QuestData.QuestId, {NodeKey = self.Key, Node = self})
    local Avatar = GWorld:GetAvatar()
    if not Avatar then
        return
    end
    if Avatar.QuestChains[Avatar.TrackingQuestChainId] then
        if self.QuestData.QuestId == Avatar.QuestChains[Avatar.TrackingQuestChainId].DoingQuestId then
...
function ShowQuestHintNode:StartListen()
    -- self.Context:AddCallback(self, 1, self.LoopFunction, true, self.NodeKey, self.ListenHandleName)
...
function ShowQuestHintNode:LoopFunction()
    if self.ShowTime == -1 or self.ListenCount == self.ShowTime then
        self:FinishAction()
    end
...
function ShowQuestHintNode:FinishAction()
    ...
    if self.IsShow and self.ShowTime ~= -1 then
