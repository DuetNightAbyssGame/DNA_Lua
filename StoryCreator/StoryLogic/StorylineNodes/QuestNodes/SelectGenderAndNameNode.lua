...
function SelectGenderAndNameNode:Start(Context)
	...
	if RealGameMode and RealGameMode.PlayCastingSequence then
		-- self.NowGameMode = RealGameMode
	elseif GameMode.PlayCastingSequence then
		-- self.NowGameMode = GameMode
	else
		self:FinishAction()
	end
...
function SelectGenderAndNameNode:NodePlayCastingSequence(GameMode, Target, UI)
	...
	--序号4为最后一段写字的动画，此间显隐逻辑特殊，单独处理
	--写动画太僵硬，暂时取消，改为确认名字就结束
	-- if GameMode.Sequence and Target == 4 then
	-- 	GameMode.Sequence.OnFinished:Add(UI, UI.FinishNode)
	-- end
end
