...
function M:Execute(Callback)
	...
	-- self.ExecuteTimer = GWorld.GameInstance:AddTimer(self.TickInterval, function()
	-- 	self.LastTime=self.LastTime+self.TickInterval
	-- end, true)
...
function M:StopRecord()
	...
	for Index, Range in pairs(self.TimerRange) do
		if LastTime >=Range.StartTime then
			--  -1表示正无穷
			if Range.EndTime==-1 or LastTime<Range.EndTime then
