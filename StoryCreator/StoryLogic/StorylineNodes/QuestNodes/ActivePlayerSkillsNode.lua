...
function ActivePlayerSkillsNode:Execute()
    ...
    for index,skill in pairs(self.EnumSkillNameList) do
        if PlayerController:CheckSkillInActive(skill) then
            ...
            if not self.bActiveEnable then
                self.EnumSkillNameArray:Add(skill)
            end
        end
    end
...
-- function ActivePlayerSkillsNode:OnQuestlineSuccess()
--     DebugPrint("ZJT_ ActiveSkills ", self.EnumSkillNameList, self.bActiveEnable)
--     self.Context:SaveSuitUpdateData("UpdateSuitTable2Value", CommonConst.SuitType.PlayerCharacterSuit, CommonConst.PlayerCharacterSuit.DisableSkill, self.EnumSkillNameArray:ToTable(), self.bActiveEnable)
-- end
