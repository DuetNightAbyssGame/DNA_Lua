...
function ShowOrHideUINode:Start(Context)
    ...
    elseif self.Function == "SetContinuedPCGuideVisibility" then
        if GameMode.SetContinuedPCGuideVisibility then
...
function ShowOrHideUINode:OnQuestlineFail()
    ...
    elseif self.Function == "SetContinuedPCGuideVisibility" then
		...
		if BattleMainUI then
			...
			if Instruction then
				if GameMode.SetContinuedPCGuideVisibility and Instruction["RealSet" .. self.ActionParam] == true then
					DebugPrint("ShowOrHideUINode: OnQuestlineFail SetContinuedPCGuideVisibility RealSet")
					BattleMainUI:AddTimer(1, function() GameMode:SetContinuedPCGuideVisibility(self.ActionParam, not self.ShowOrHide) end)
				end
			end
		end
    end
