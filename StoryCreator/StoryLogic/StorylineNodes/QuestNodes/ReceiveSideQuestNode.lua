...
function ReceiveSideQuestNode:CheckIsNeedShowDialog()
	...
	if DataMgr.QuestChain[self.SideQuestChainId].IsShowWindow == nil or
	DataMgr.QuestChain[self.SideQuestChainId].IsShowWindow == false then
		return false
	end
...
function ReceiveSideQuestNode:TrySwitchCamera()
	...
	if BranchTaskNpc then
		...
		self.CurrentCamera = GameInstance:GetWorld():SpawnActor(ACameraActor:StaticClass(),
												FTransform(CurrentCameraRot:ToQuat(), CurrentCameraLoc),
		...
		self.TargetCamera = GameInstance:GetWorld():SpawnActor(ACameraActor:StaticClass(),
												CameraTansrorm,
												UE4.ESpawnActorCollisionHandlingMethod.AdjustIfPossibleButAlwaysSpawn,
												nil, nil, nil)
		...
		-- PlayerController:SetViewTargetWithBlend(self.TargetCamera, 0.8, EViewTargetBlendFunction.VTBlend_Linear, 0, false)
...
function ReceiveSideQuestNode:FinishAction(OptionStr)
	...
	if OptionStr == "ApproveOut" then
		-- 这里不需要取消追踪了，因为目前没有接取自动追踪了
		...
		if MissionIndicatorManager.TrackingSpecialSideQuestChainId ~= nil and MissionIndicatorManager.TrackingSpecialSideQuestChainId == self.SideQuestChainId then
