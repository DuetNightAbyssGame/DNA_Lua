...
function WaitingMechanismEnterStateNode:Execute(Callback)
    ...
    --     local Creator = GameState.StaticCreatorMap:Find(self.CreateId)
	-- 	if Creator and Creator.ChildEids:Length() > 0 then
	-- 		Mechanism = Battle(GameState):GetEntity(Creator.ChildEids[1])
    --     end
    -- else
    ...
    --     return
    -- end
    -- Mechanism.CombatStateChangeComponent:AddSTLCallback(self.StateId, Callback)
    local GameState = UE4.UGameplayStatics.GetGameState(GWorld.GameInstance)
