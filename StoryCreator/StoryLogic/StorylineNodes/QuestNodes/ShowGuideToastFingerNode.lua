...
function ShowGuideToastFingerNode:ShowMessage(Context)
    ...
    if self.ShowEnable then
        ...
        self.UIStateAsyncActionBase = UE4.UUIStateAsyncActionBase.ShowGuideToastFingerNode(GameInstance, self.MessageId,self.MessageSustainTime,
                self.Gesture, self.MessageLoc, MessageLocOffset)
        self.UIStateAsyncActionBase.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
        else
            -- local UIStateAsyncActionBase = UE4.UUIStateAsyncActionBase.ShowGuideToast(GameInstance, self.MessageId, self.MessageSustainTime)
            -- UIStateAsyncActionBase.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
    ...
    elseif GameMode.GameState then
        ...
        if CommonUtils.GetDeviceTypeByPlatformName(GameInstance) == "Mobile" then
            ...
            self.GuideGesture.OnGuideEnd:Add(GameInstance, function () self:FinishAction() end)
...
function ShowGuideToastFingerNode:FinishAction()
    --self.GameMode:HideMessage(self.MessageId)
    --self:Finish()
    self:Finish()
end
