...
function CheckBranchQuestFinishedNode:Start(Context, InportInfo)--BranchQuestFinishOptions数量最好还是和BranchQuestStartNode对应
...
function CheckBranchQuestFinishedNode:OnCancelTrack()
	...
	if not TaskUIObj then
        -- TaskUIObj.VBox_SubTasks:SetVisibility(UE4.ESlateVisibility.Collapsed)'
		return
    end
end
...
function CheckBranchQuestFinishedNode:OnChooseTrack()
	...
	-- local QuestExportData = TaskUtils:GetQuestExtraInfo(self.QuestChainId, self.QuestData.QuestId)
	-- local AllBranchQuestInfo = nil
	-- if QuestExportData ~= nil then
	-- 	for _, Data in pairs(QuestExportData) do
	-- 		if Data.Node and Data.Node.Type == "BranchQuestStartNode" and Data.BranchQuestOptions then
	-- 			AllBranchQuestInfo = Data.BranchQuestOptions
	-- 			break
	-- 		end
	-- 	end
	-- end
	-- if not AllBranchQuestInfo then
	-- 	return
	-- end
	
	-- if TaskUIObj.VBox_SubTasks:GetChildrenCount() == 0 or TaskUIObj.SubTaskWidgetsTable == nil or IsEmptyTable(TaskUIObj.SubTaskWidgetsTable) then
	-- 	if AllBranchQuestInfo then
	-- 		TaskUIObj.SubTaskWidgetsTable = {}
	-- 		local SubTaskIndex = 1
	-- 		for _, BranchQuestInfo in pairs(AllBranchQuestInfo) do
	...
	if self.InportInfos then
		for _, InportInfo in pairs(self.InportInfos) do
			...
			local Avatar = GWorld:GetAvatar()
			if Avatar and NeedFinishCount == 0 and Avatar.TrackingQuestChainId == self.CurQuestChainId then
...
function CheckBranchQuestFinishedNode:UpdateWorldTaskPanel(InportInfo)
	...
	if TaskUIObj.VBox_SubTasks:GetChildrenCount() == 0 then
	...
	if TaskUIObj.SubTaskWidgetsTable and not IsEmptyTable(TaskUIObj.SubTaskWidgetsTable) and IsValid(TaskUIObj.SubTaskWidgetsTable[InportInfo]) then
		self.BlackScreenUIDurationTimer = GWorld.GameInstance:AddTimer(0.5,
		function()
			if IsValid(TaskUIObj.SubTaskWidgetsTable[InportInfo]) then
				TaskUIObj.SubTaskWidgetsTable[InportInfo]:PlayAnimation(TaskUIObj.SubTaskWidgetsTable[InportInfo].CompletionTask)
				...
				TaskUIObj.SubTaskWidgetsTable[InportInfo]:BindToAnimationFinished(TaskUIObj.SubTaskWidgetsTable[InportInfo].CompletionTask, {TaskUIObj.SubTaskWidgetsTable[InportInfo], 
				function()
...
function CheckBranchQuestFinishedNode:TryFinishCurrentNode()
	...
	if Avatar and NeedFinishCount == 0 and Avatar.TrackingQuestChainId == self.CurQuestChainId then
		...
		GWorld.GameInstance:AddTimer(1,
		function()
			local TaskUIObj = TaskUtils:GetTaskBarWidget()
			if not IsEmptyTable(TaskUIObj.SubTaskWidgetsTable) then
				for Name, Widget in pairs(TaskUIObj.SubTaskWidgetsTable) do
	...
	elseif Avatar and NeedFinishCount == 0 and Avatar.TrackingQuestChainId ~= self.CurQuestChainId then
		if not self.HasFinished then
            self:FinishAction()
        end
	end
