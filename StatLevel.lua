...
function ToString (tab, cnt)
    ...
    if cnt >= 10 then -- 这里的4代表嵌套层数，比如：{--1{--2{--3{}}}}
        return tostring(tab)
    end
...
function M:TraceNext(ActorSize, ActorLocation, XBias)
    ...
    -- local StartPoint = self.Player:K2_GetActorLocation()--ActorLocation - ActorSize +  FVector(0,0,1) * ActorSize.Z * 2 + FVector(1,0,0) * 500 * XBias + FVector(0,1,0) * 500 * YBias
    
    -- StartPoint.X = self.Player:K2_GetActorLocation().X 
    -- StartPoint.Y = self.Player:K2_GetActorLocation().Y
    ...
    while(UE4.UKismetMathLibrary.IsPointInBox(StartPoint, ActorLocation ,ActorSize))
    do
        ...
        for k, v in pairs(OutHits) do
            ...
            if v.ImpactNormal.Z == 1 and ( IsStatciMeshActor or IsLandScapeActor )then 
                if not LastImpactPointZ then 
                    LastImpactPointZ = v.ImpactPoint.Z - (self.Player.CapsuleComponent:GetUnscaledCapsuleHalfHeight() * 2 + 4)
                end
...
function M:TravelNext()
    ...
    -- print('111111111', ToString(HitPosTable))
...
function M:StatEnd(GM)
    ...
    for idx, Record in pairs(AllStat) do 
        if Record.FrameTime > MaxFrameTime then 
            MaxFrameTime = Record.FrameTime
            MaxFrameTimeRecord = Record
        end
        if Record.NumDrawCalls > MaxDrawCalls then 
