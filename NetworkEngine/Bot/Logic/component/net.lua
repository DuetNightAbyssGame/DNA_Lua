...
function net:HandleMessage(Message)
    ...
    if Pak == nil or Pak.Cmd == nil then
        self:log("Robot HandleMessage unpack failed Pak is nil")
        return
    end

    if self[Pak.Cmd] == nil then
    ...
    -- self:log("Robot S2C CMD:".. Pak.Cmd ..":".. EntityFunc .. " ByteSize:".. tostring(#Message:GetBytes()))
...
function net:RawRpc(Cmd, ...)
	local PackStr = msgpack.pack({Cmd=Cmd, Args={...}})
	if not PackStr then
		return
	end
    ...
    local RpcRecorder = require "NetworkEngine.Rpc.RpcRecorder"
    RpcRecorder:Send(Cmd, #PackStr, {...},self.printCtx)
