...
function RpcRecorder:Restart()
    UE.UNetworkManager.PrintNetMsgC2S(
        "======================== RpcRecorder:Restart ==============================================="
    )
    self.C2S = {}
    self.S2C = {}
...
function RpcRecorder:Send(Cmd,ByteSize, Args,Ctx)
    if self.RecordMsgs then
        ...
        table.insert(self.RecordMsgs, {Cmd = Cmd,time = time, Args = Args})
    end

    local EntityFunc = ""
    if Cmd == "EntityMessage"then
        EntityFunc = Args[1] 
        Cmd = "EntityMessage:"..EntityFunc
    end
    Ctx = "["..(Ctx or "RPC").."]"
	UE.UNetworkManager.PrintNetMsgC2S(
        Ctx .. "CMD:".. Cmd .. " ByteSize:".. tostring(ByteSize) .. " EntityFunc:" .. EntityFunc
            -- ..self.TableToStr(Args, 10)
    )
...
function RpcRecorder:Recv(Cmd,ByteSize, Args,Ctx)
    ...
    Ctx = "["..(Ctx or "RPC").."]"
    if EntityFunc ~= "SHB" then
        UE.UNetworkManager.PrintNetMsgS2C(
            Ctx .. "CMD:".. Cmd .. " ByteSize:".. tostring(ByteSize) .. " EntityFunc:" .. EntityFunc
                -- .. self.TableToStr(Args, 10)
        )
    end
...
function RpcRecorder:Print(Deep)
	...
	UE.UNetworkManager.PrintNetMsgC2S(
        "[C2S] NetSpeed(B/S):".. tostring(C2SSpeed) .." Delay:"..tostring(delay).."s ".. " Bytes(MB):"..tostring(self.C2S.TotalByteSize/1024/1024)
        ..self.TableToStr(self.C2S, Deep)
    )
    UE.UNetworkManager.PrintNetMsgC2S(
        "======================== C2S Print END ==============================================="
    )
    ...
    UE.UNetworkManager.PrintNetMsgS2C(
        "[S2C] NetSpeed(B/S):".. tostring(S2CSpeed) .." Delay:"..tostring(delay).."s " .. " Bytes(MB):"..tostring(self.S2C.TotalByteSize/1024/1024)
        ..self.TableToStr(self.S2C, Deep)
    )
    UE.UNetworkManager.PrintNetMsgS2C(
        "======================== C2S Print END ==============================================="
    )
